...
void tpd_get_dts_info(void)
{
	...
	if (node1) {
		...
		of_property_read_u32_array(node1, "tpd-resolution", tpd_dts_data.tpd_resolution,
		...
		if (tpd_dts_data.use_tpd_button) {
			...
			of_property_read_u32_array(node1, "tpd-key-dim-local", key_dim_local,
						   ARRAY_SIZE(key_dim_local));
			memcpy(tpd_dts_data.tpd_key_dim_local, key_dim_local,
			       sizeof(key_dim_local));

			for (i = 0; i < 4; i++) {
				pr_debug("[tpd]tpd_dts_data.tpd_key_dim_local[%d].key_x = %d\n", i,
				       tpd_dts_data.tpd_key_dim_local[i].key_x);
				pr_debug("[tpd]tpd_dts_data.tpd_key_dim_local[%d].key_y = %d\n", i,
				       tpd_dts_data.tpd_key_dim_local[i].key_y);
				pr_debug("[tpd]tpd_dts_data.tpd_key_dim_local[%d].key_W = %d\n", i,
				       tpd_dts_data.tpd_key_dim_local[i].key_width);
				pr_debug("[tpd]tpd_dts_data.tpd_key_dim_local[%d].key_H = %d\n", i,
				       tpd_dts_data.tpd_key_dim_local[i].key_height);
			}
		}
...
{
	pr_err("[tpd]tpd_gpio_output pin = %d, level = %d\n", pin, level);
	/* printk("[tpd] fwq before  chage   gpio eint pin mode=%d\n",mt_get_gpio_mode(GPIO_CTP_EINT_PIN)); */
	/* printk("[tpd] fwq before  chage   gpio rst pin mode=%d\n",mt_get_gpio_mode(GPIO_CTP_RST_PIN)); */
	if (pin == 1)
...
static long tpd_compat_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case COMPAT_TPD_GET_FILTER_PARA:
		...
		ret = file->f_op->unlocked_ioctl(file, TPD_GET_FILTER_PARA, (unsigned long)arg32);
		if (ret) {
...
/*----------------------------------------------------------------------------*/
static const struct miscdevice tpd_misc_device = {
...
static int tpd_probe(struct platform_device *pdev)
{
	...
	pr_debug("mtk_tpd: TPD_RES_X = %d, TPD_RES_Y = %d\n", TPD_RES_X, TPD_RES_Y);
...
static int __init tpd_device_init(void)
{
	pr_err("MediaTek touch panel driver init\n");
#if defined(CONFIG_MTK_LEGACY)
#ifdef CONFIG_OF
