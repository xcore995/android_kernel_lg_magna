...
int AbsRawRef[16] = { 77, 11919, 14023, 15163, 16192, 18319, 19337, 21491, 22633
	, 24692, 26853, 27993, 30147, 32253, 34411, 37605 };
...
bool switchPage(int page)
{
	...
	if (count >= DefaultTimeout) {
		TPD_ERR("[s3528]Timeout -- Page switch fail !\n");
		return false;
	}
	return true;
}
		...
		for (i = 0; i < (int)F12_2DTxCount; i++) {
			...
			if ((ImagepF[i][j] < LowerImageLimit[i][j])
				...
				if (f54_window_crack_check_mode) {
					...
					TOUCH_INFO_MSG("[Touch] Tx [%d] Rx [%d]", i, j);
					TOUCH_INFO_MSG
					    ("node_crack_count %d, row_crack_count %d, raw cap %d\n",
					     node_crack_count, row_crack_count, ImagepF[i][j]);
				} else {
					result = false;
					break;
				}
...
int CompareNoiseReport(void)
{
	...
	for (i = 0; i < TxChannelCount; i++) {
		TOUCH_INFO_MSG("[s3528][Touch] Tx[%2d]: ", i);
		out_buf += sprintf(wlog_buf + out_buf, "   %5d : ", i);
...
bool TestPreparation(void)
{
	...
	/* Apply ForceUpdate. */
	Read8BitRegisters(F54CommandBase, &data, 1);
	TOUCH_INFO_MSG("s3528 Apply ForceUpdate => F54CommandBase\n");
	data = data | 0x04;
	Write8BitRegisters(F54CommandBase, &data, 1);
	TOUCH_INFO_MSG("s3528 Apply ForceUpdate=>F54CommandBase\n");
	/* Wait complete */
...
error:
	sprintf(rst, "%d", 0);
	TPD_ERR("[s3528]winodw crack check fail\n");
}
...
int F54Test(int input, int mode, char *buf)
{
	...
	TOUCH_INFO_MSG("[s3528][F54TEST] input = %d, mode = %d\n", input, mode);
