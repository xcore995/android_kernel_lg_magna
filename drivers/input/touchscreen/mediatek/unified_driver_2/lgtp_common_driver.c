...
extern int check_first_fw_update;
#endif
...
static ssize_t store_lpwg_notify(struct i2c_client *client, const char *buf, size_t count)
{
	...
	TOUCH_LOG("type:%d, a:%d, b:%d, c:%d, d:%d\n", type, value[0], value[1], value[2],
		  value[3]);
...
static ssize_t store_ic_rw(struct i2c_client *client, const char *buf, size_t count)
{
	...
	retv = sscanf(buf, "%s %d %d", cmd, &reg, &data);
...
#if defined(TOUCH_DEVICE_S3320)
			/* WORK_AROUND_1 : skip first set of suspend and */
			/* resume pair to avoid black screen on C70(S3320) */
...
static int touch_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (pDriverData->bootMode == BOOT_OFF_CHARGING) {
		/* there is no CFW, but we need to save power */
	...
	TOUCH_LOG("check_first_fw_update: %d\n",check_first_fw_update);
	/* Trigger firmware upgrade if needed */
	if( (pDriverData->binFwInfo.isOfficial || !pDriverData->binFwInfo.isOfficial || check_first_fw_update == 1) && g_boot_mode!=RECOVERY_BOOT)
	{
		if( pDriverData->binFwInfo.isOfficial != pDriverData->icFwInfo.isOfficial ||pDriverData->binFwInfo.version != pDriverData->icFwInfo.version || check_first_fw_update == 1)
		{
			pDriverData->useDefaultFirmware = TOUCH_TRUE;
			queue_delayed_work(touch_wq, &pDriverData->work_upgrade, 0);
		}
...
static int touch_shutdown(struct i2c_client *client)
{
	TOUCH_FUNC();
