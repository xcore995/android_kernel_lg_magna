...
/* extern bool hwPowerSetVoltage(unsigned int powerId,int powerVOlt,char*mode_name); */
#ifndef CONFIG_MTK_LEGACY
enum MSDC_LDO_POWER {
	POWER_LDO_VMCH,
	POWER_LDO_VMC,
...
static u32 msdc_ldo_power(u32 on, unsigned int powerId, int voltage_uv, u32 *status)
{
...
#ifndef CONFIG_MTK_LEGACY
static int msdc_get_ccf_clk_pointer(struct platform_device *pdev,struct msdc_host *host)
{
	if (pdev->id == 0) {
	...
	if(host->id==0) {
		if(IS_ERR(g_msdc0_pll_sel) || IS_ERR(g_msdc0_pll_800m) || 
			...
			pr_err("msdc0 get pll error,g_msdc0_pll_sel=%p,g_msdc0_pll_800m=%p, \
				g_msdc0_pll_400m=%p,g_msdc0_pll_200m=%p\n", \
				g_msdc0_pll_sel,g_msdc0_pll_800m,g_msdc0_pll_400m,g_msdc0_pll_200m);
			return 1;
		} else {
...
static int msdc_drv_probe(struct platform_device *pdev)
{
	...
	pdev->dev.of_node = of_find_compatible_node(NULL, NULL, "mediatek,mt_pmic_regulator_supply");
	...
	if (strcmp(pdev->dev.of_node->name, "msdc0") == 0) {
		...
		msdc_cust_node = of_find_compatible_node(NULL, NULL, "mediatek,MSDC0_custom");
	}
#endif
#ifdef CFG_DEV_MSDC1
	if (strcmp(pdev->dev.of_node->name, "msdc1") == 0) {
		...
		hw = &msdc1_hw;
		pdev->id = 1;
		msdc_cust_node = of_find_compatible_node(NULL, NULL, "mediatek,MSDC1_custom");
	}
#endif
	if (NULL == msdc_cust_node) {
		pr_err("No custom DTS node, Use default parameter on %s\n", pdev->dev.of_node->name);
	} else {
		/* If use custom parameter, custom = <1> should be setted in device tree */
		ret = of_property_read_u32_array(msdc_cust_node, "custom", msdc_custom, ARRAY_SIZE(msdc_custom));
		if (ret ==0 && msdc_custom[0] == 1) {
			/* Get HW parameter from Device tree */
			if (!of_property_read_u32_array(msdc_cust_node, "hw", msdc_hw_parameter, ARRAY_SIZE(msdc_hw_parameter))) {
				if (host_id == 0) {
		...
		if (host_id == 0) {
			/* If use custom parameter, custom_ett = <1> should be setted in device tree */
			ret = of_property_read_u32_array(msdc_cust_node, "custom_ett", msdc_custom, ARRAY_SIZE(msdc_custom));
			if (ret == 0 && msdc_custom[0] == 1) {
				if (of_property_read_u32_array(msdc_cust_node, "ett_settings", (unsigned int *)msdc0_ett_settings, MSDC0_ETT_COUNTS * 4)) {
...
static int msdc_drv_suspend(struct platform_device *pdev, pm_message_t state)
{
	...
	struct msdc_host *host = mmc_priv(mmc);
	void __iomem *base = host->base;
	if ((host->hw->host_function == MSDC_SD) && (host->card_inserted == 1) && (g_msdc1_io == 0)) {
