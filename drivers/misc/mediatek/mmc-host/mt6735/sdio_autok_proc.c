    ...
    for (i=0; i<HOST_MAX_NUM; i++) {
        if (p_autok_thread_data->p_autok_progress[i].host_id == host->id) {
            if (p_autok_thread_data->p_autok_progress[i].done == 1 &&
                ...
                u32 vcore_uv = 0;
                vcore_uv = autok_get_current_vcore_offset();
		pr_debug("init_tune_sdio@msdc_autok_apply_param, id:%d vcore_uv:%d\n",host->id, vcore_uv);
    ...
    switch(select){
        ...
        case PARAMS:
            ...
            for(i=0; i<p_single_autok[id].vol_count; i++){
                pr_debug("[AUTOKD] p_single_autok[%d].vol_list[%d]=%d", id, i, p_single_autok[id].vol_list[i]);
            }
            for(i=0; i<p_single_autok[id].vol_count; i++){
                for(j=0; j<p_single_autok[id].param_count; j++)
                    pr_debug("[AUTOKD] p_single_autok[%d].ai_data[%d][%d]=%d", id, i, j, p_single_autok[id].ai_data[i][j].data.sel);
            }
