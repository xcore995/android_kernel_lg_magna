...
static u64 ebr1_start_addr = 0;
static int parsing_xbr(int xbr_idx, u64 *xbr_addr, u64 *off2mbr)
{
	int err;
	int slot;
    u8 xbr_buf[512];
	...
	if( xbr_idx == 1 ) /* if  it is ebr1 , record its value*/
		ebr1_start_addr = *xbr_addr;
	...
	for (slot = 0; slot < 4; slot++, p++) {
		//offset = (u64)p->start_sect * 512 + this_addr;
		offset = (u64)p->start_sect * 512 + ebr1_start_addr; /*when it's mbr, ebr1_s tart_addr is 0 , else , it's really address*/
...
static void construct_xbr_pmt_mapping(void)
{
    ...
    for (xbr_idx = 0; xbr_idx < XBR_COUNT; xbr_idx++) {
        next = parsing_xbr(xbr_idx, &xbr_addr, off2mbr[xbr_idx]);
        if (!next) {
            break;
        }
    }
