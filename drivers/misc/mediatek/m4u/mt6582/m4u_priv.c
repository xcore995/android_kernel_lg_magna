...
#define M4ULOG(string, args...) pr_debug("M4U [pid=%d] "string, current->tgid, ##args)
...
#define M4ULOG(string, args...) do { \
	if (gM4uLogFlag) { \
		pr_err("M4U [pid=%d] "string, current->tgid, ##args); } \
} while (0)
...
#define M4UTMP(string, args...)  pr_debug("M4U"string, ##args)
			   ...
			   unsigned int mva_end)
{
	...
	if (L2_en) {
		if (isInvAll) {
			...
			while (
			!(event = m4uHw_get_field_by_mask(0, REG_L2_GDC_STATE, F_L2_GDC_ST_EVENT_MSK)))
				;
			m4uHw_set_field_by_mask(0, REG_L2_GDC_STATE, F_L2_GDC_ST_EVENT_MSK, 0);
		} else {
			while (
			!(m4uHw_get_field_by_mask(0, REG_L2_GPE_STATUS, F_L2_GPE_ST_RANGE_INV_DONE)))
				;
					 ...
					 int L2_en, unsigned int mva_start, unsigned int mva_end)
{
	...
	if (L2_en) {
		while (
		!(m4uHw_get_field_by_mask(0, REG_L2_GPE_STATUS_SEC, F_L2_GPE_ST_RANGE_INV_DONE_SEC)))
			;
...
static int MTK_M4U_release(struct inode *a_pstInode, struct file *a_pstFile)
{
	...
	if (pNode->isM4uDrvConstruct == 0
	    || pNode->isM4uDrvDeconstruct == 0 || !list_empty(&pNode->mvaList)) {
	...
	while (pListHead != &(pNode->mvaList)) {
		...
		M4UMSG
		    ("warnning: clean garbage at m4u close: module=%s,va=0x%x,mva=0x%x,size=%d, inode=%p, file=%p\n",
		     m4u_get_module_name(pList->eModuleId), pList->bufAddr, pList->mvaStart,
		     pList->size, a_pstInode, a_pstFile);
...
static long MTK_M4U_ioctl(struct file *a_pstFile, unsigned int a_Command, unsigned long a_Param)
{
	...
	case MTK_M4U_T_CACHE_SYNC:
		...
		M4UDBG
		    ("moduleID=%d, eCacheSync=%d, buf_addr=0x%x, buf_length=0x%x\n",
		...
		/* search invalid main tlb */
		{
			...
			for (i = 0; i < M4U_MAIN_TLB_NR; i++) {
				...
				if ((mva & (F_MAIN_TLB_VALID_BIT | F_MAIN_TLB_INV_DES_BIT))
					...
					M4UMSG("%d:0x%x, 0x%x", i, mva & F_MAIN_TLB_VA_MSK, des);
				}
			}
		}
...
static int m4u_dump_pfh_tlb_des(int m4u_id)
{
	...
	M4UINFO("dump main tlb=======>\n");
	for (i = 0; i < M4U_PRE_TLB_NR; i++) {
		M4UMSG("%d:0x%x:0x%x,0x%x,0x%x,0x%x  ", i,
...
static void m4u_dump_pagetable_range(unsigned int mvaStart, unsigned int nr)
{
	...
	M4UMSG("\n");
	/* M4UMSG("m4u dump pagetable done : start=0x%x, nr=%d ==============<\n", vaStart, nr); */
}
...
int m4u_do_check_port_va_or_pa(M4U_PORT_STRUCT *pM4uPort)
{
	...
	if (PortID == MDP_ROTVO) {
		...
		if ((regVal & 0x8) == 0x0) {
			M4UMSG("m4u_check_port_va_or_pa port = %s, 0x5C4 = 0x%x",
			       m4u_get_port_name(PortID), regVal);
			ret = 1;
		}
	}
	if (PortID == MDP_ROTCO) {
		...
		if ((regVal & 0x80000000) == 0x0) {
			M4UMSG("m4u_check_port_va_or_pa port = %s, 0x5C0 = 0x%x",
			       m4u_get_port_name(PortID), regVal);
			ret = 1;
		}
	}
...
{				/* native */
	...
	M4UINFO
	    ("m4u_config_port(), port=%s, Virtuality=%d, Security=%d.\n",
	     m4u_get_port_name(pM4uPort->ePortID), pM4uPort->Virtuality, pM4uPort->Security);

	if ((PortID == MDP_ROTO || PortID == MDP_ROTCO || PortID == MDP_ROTVO)
	    && pM4uPort->Virtuality == 0) {
		m4u_aee_print
		    ("m4u_config_port(), port=%s, Virtuality=%d, Security=%d\n",
		     m4u_get_port_name(pM4uPort->ePortID), pM4uPort->Virtuality, pM4uPort->Security);
	}
	...
	m4uHw_set_field_by_mask(m4u_base, REG_MMU_PFH_DIST(PortID),
				...
				F_MMU_PFH_DIST_VAL(PortID, pM4uPort->Distance));
	...
	for (mapped_page = 0; mapped_page < page_num; mapped_page++) {
		...
		if (va_tmp >= vma->vm_end || va_tmp < vma->vm_start) {
			...
			if (!vma || vma->vm_start > va_tmp) {
				m4u_aee_print
				    ("cannot find vma, va=0x%x, size=0x%x, fail_va=0x%x, pa=0x%x\n",
				     va, size, va_tmp, pa);
				m4u_dump_maps(va);
				goto err;
			}
	...
	if (ret < page_num) {
		...
		if (unlikely(fatal_signal_pending(current))) {
			...
			if (ret > 0) {	/* return value bigger than 0 but smaller than expected, trigger red screen */
				M4UMSG
				    ("error: page_num=%d, get_user_pages return=%d, module=%s, current_process:%s\n",
				     page_num, ret, m4u_get_module_name(eModuleID), current->comm);
				M4UMSG
				    ("error hint: maybe the allocated VA size is smaller!");
			} else {
				/* return vaule is smaller than 0, maybe the buffer is not exist,
				...
				M4UMSG
				    ("error: page_num=%d, get_user_pages return=%d, module=%s, current_process:%s\n",
				     page_num, ret, m4u_get_module_name(eModuleID), current->comm);
				M4UMSG
				    ("error hint: maybe the VA is deallocated before call m4u_alloc_mva, ");
				M4UMSG
				    ("or no VA has be ever allocated!");
			}
	...
	if ((vma->vm_flags) & VM_PFNMAP) {
		...
		if (bufEnd > vma->vm_end) {
			M4UMSG("error: page_num=%d,module=%s, va=0x%x, size=0x%x, vm_flag=0x%lu.\n",
			       page_num, m4u_get_module_name(eModuleID), BufAddr, BufSize,
			       vma->vm_flags);
			...
			if ((start_pa & 0x02) == 0) {
				...
				if (page_count(page) > 0) {
					if ((m4u_put_unlock_page(page))) {
						M4UMSG("warning: put_unlock_page fail!");
						M4UMSG
						    ("module=%s, va=0x%x, size=0x%x,mva=0x%x\n",
						     m4u_get_module_name(eModuleID), BufAddr,
						     BufSize, MVA);
						M4UMSG("(page is unlocked before put page)\n");
						M4UMSG("i=%d (%d)\n", i, page_num);
					}
				} else {
...
/* check wether input buffer is valid (has physical pages allocated) */
	...
	if (size < BUFFER_SIZE_FOR_FLUSH_ALL) {
		...
		if ((unsigned int)start < PAGE_OFFSET) {	/* from user space */
			...
			(unsigned int)start <= VMALLOC_END) {	/* vmalloc */
			...
			for (i = 0; i < page_num; i++, page_start += DEFAULT_PAGE_SIZE) {
				...
				if (ppage == NULL) {
					ret = -1;
					M4UMSG
					    ("error: ppage is 0 in cache_maint of vmalloc!");
					M4UMSG
					    ("module=%s, start=0x%x, pagestart=0x%x\n",
					     m4u_get_module_name(eModuleID), (unsigned int)start,
					     page_start);
					goto out;
				}
...
static bool m4u_struct_init(void)
{
	...
	if ((pt_pa_sec & M4U_PAGE_TABLE_ALIGN) != 0) {
		...
		dma_free_coherent(NULL, PT_TOTAL_ENTRY_NUM * sizeof(unsigned int), pPT_sec,
				  pt_pa_sec);
		tmp = (unsigned int)dma_alloc_coherent(NULL,
						       PT_TOTAL_ENTRY_NUM * sizeof(unsigned int) +
						       M4U_PAGE_TABLE_ALIGN, &pt_pa_sec,
						       GFP_KERNEL);
		if (!tmp) {
			M4UMSG("dma_alloc_coherent error!  dma memory not available.\n");
			return false;
	...
	pProtectVA = kmalloc(TF_PROTECT_BUFFER_SIZE * TOTAL_M4U_NUM + M4U_PROTECT_BUF_OFFSET,
