	...
	while (pListHead != &(pNode->mvaList)) {
		...
		if (pList->mvaStart != 0) {
			...
			if (!(pList->flags & MVA_REGION_REGISTER)) {
				M4UERR
				    ("warning: in garbage reclaim: mva==0, ");
				M4UERR
				    ("but MVA_REGION_REGISTER is not set!! flag=0x%x\n",
				     pList->flags);
			}
		}
	}
		      ...
		      int security, int cache_coherent, unsigned int *pRetMVABuf)
{
	...
	pMvaInfo =
	    m4u_alloc_garbage_list(0, BufSize, eModuleID, BufAddr, M4U_FLAGS_FIX_MVA, security,
				   cache_coherent);
	ret = __m4u_alloc_mva(pMvaInfo, NULL);
...
int __m4u_alloc_mva(mva_info_t *pMvaInfo, struct sg_table *sg_table)
{
	...
	if (!(flags & M4U_FLAGS_FIX_MVA)) {
		mvaStart =
		    m4u_do_mva_alloc(eModuleID, BufAddr, BufSize + align_page_num * 0x1000,
				     pMvaInfo);
	} else {
...
/* ======================================== */
	...
	if (RangeReg_ID == M4U_INVALID_ID) {
		...
		mutex_unlock(&gM4uMutex);
		/* do not have to return erro to up-layer, nothing will happen even insert tlb range fails */
		return 0;
	}
		...
		if (is_pmem_range((unsigned long *)start_pa, BufSize)) {
			...
			if ((vma->vm_flags) & VM_PFNMAP) {
				...
				if (bufEnd > vma->vm_end) {
					M4UMSG
					    ("error: page_num=%d,module=%s, va=0x%x, size=0x%x, vm_flag=0x%lu.\n",
				...
				M4UINFO
				    ("alloc_mva VM_PFNMAP module=%s, va=0x%x, size=0x%x, vm_flag=0x%lu.\n",
				     m4u_get_module_name(eModuleID), BufAddr, BufSize,
				     vma->vm_flags);
				up_read(&current->mm->mmap_sem);
			} else {

				ret = m4u_get_user_pages(eModuleID,
							 current,
							 current->mm,
							 BufAddr,
							 page_num,
							 write_mode,
							 /* m4u_get_write_mode_by_module(eModuleID), // 1 /* write */
							 0,	/* force */
				...
				if (ret < page_num) {
					...
					if (unlikely(fatal_signal_pending(current))) {
						...
						if (ret > 0) {
							M4UMSG
							    ("error: page_num=%d, get_user_pages return=%d,",
								page_num, ret);
							M4UMSG
							    ("module=%s, current_process:%s\n",
							     m4u_get_module_name(eModuleID),
							     current->comm);
							M4UMSG
							...
							M4UMSG
							    ("module=%s, current_process:%s\n",
							     ...
							     current->comm);
							M4UMSG
							    ("error hint: maybe the VA is deallocated, ");
							M4UMSG
		...
		for (i = 0; i < page_num; i++) {
			...
			if ((start_pa & 0x02) == 0) {
				...
				if (page_count(page) > 0) {
					...
					if (tmp) {
						/* M4UINFO("warning: put_unlock_page fail module=%s, va=0x%x,
						size=0x%x,mva=0x%x (page is unlocked before put page)\n", */
						/* m4u_get_module_name(eModuleID), BufAddr, BufSize, MVA); */
						/* M4UINFO("i=%d (%d)\n", i, page_num); */
						put_page_err = tmp;
					}
