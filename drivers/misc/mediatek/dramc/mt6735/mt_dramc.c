...
void dump_DMA_Reserved_AREA(void)
{
	...
	/*unsigned int *debug_p_src;*/
	   ...
	   pr_debug("=====DMA DUMP Reserved SRC AREA======\n");
	   /*  dump memory info to make sure 0x6a6a6a6a already write into memory */
	   for (i = 0; i < 8; i++) {
		pr_debug("[SRC] ");
		for (j = 0; j < 8; j++)
			pr_debug("0x%p=0x%x ", ((debug_p_src+i*8)+j), *((debug_p_src+i*8)+j));
		/* pr_debug("&0x%p=0x%x ", ((debug_p_src+i*8)+j), readl(IOMEM((debug_p_src+i*8)+j))); */
		pr_debug("\n");
	   }

	   pr_debug("=====DMA DUMP Reserved SRC AREA END======\n\n");

	pr_debug("=====DMA DUMP Reserved DST AREA======\n");
	for (i = 0; i < 8; i++) {
		pr_debug("[DST] ");
		for (j = 0; j < 8; j += 4) {
			pr_debug("val=0x%02X",
			       dfs_dummy_buffer[(i * 8) + (j + 3)]);
			pr_debug("%02X", dfs_dummy_buffer[(i * 8) + (j + 2)]);
			pr_debug("%02X", dfs_dummy_buffer[(i * 8) + (j + 1)]);
			pr_debug("%02X ", dfs_dummy_buffer[(i * 8) + j]);
		}
		/*pr_debug("&0x%p=0x%x ", ((debug_p_dst+i*8)+j), readl(IOMEM((debug_p_dst+i*8)+j)));*/
		pr_debug("\n");
	}
	pr_debug("=====DMA DUMP Reserved DST AREA END======\n\n");

	enable_clock(MT_CG_INFRA_GCE, "CQDMA");
	pr_debug("=====DMA DUMP REGS======\n");
	for (k = 0; k < 25; k++) {
		pr_debug("[CQDMA] addr:0x%p, value:%x\n",
		       (CQDMA_BASE_ADDR + k * 4),
...
#ifdef CONFIG_OF_RESERVED_MEM
reservedmem_of_init_fn dram_dummy_read_reserve_mem_of_init(struct reserved_mem *rmem,
