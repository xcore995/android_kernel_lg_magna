...
static void process_dbg_opt(const char *opt)
{
	...
	if (0 == strncmp(opt, "regr:", 5)) {
		...
		if (ret)
			pr_err("DISP/%s: errno %d\n", __func__, ret);
		ret = kstrtoul(p + 1, 16, (long unsigned int *)&val);
		if (ret)
			pr_err("DISP/%s: errno %d\n", __func__, ret);
		...
		p = (char *)opt + 11;
		ret = kstrtoul(p, 16, (long unsigned int *)&gRDMAUltraSetting);
		if (ret)
			pr_err("DISP/%s: errno %d\n", __func__, ret);
		...
		p = (char *)opt + 10;
		ret = kstrtoul(p, 16, (long unsigned int *)&gRDMAFIFOLen);
		if (ret)
			pr_err("DISP/%s: errno %d\n", __func__, ret);
		...
		p = (char *)opt + 7;
		ret = kstrtoul(p, 16, (long unsigned int *)&reg_pa);
		if (ret)
			pr_err("DISP/%s: errno %d\n", __func__, ret);
		...
		p = (char *)opt + 7;
		ret = kstrtoul(p, 10, (long unsigned int *)&met_on);
		if (ret)
			pr_err("DISP/%s: errno %d\n", __func__, ret);
		ret = kstrtoul(p + 1, 10, (long unsigned int *)&rdma0_mode);
		if (ret)
			pr_err("DISP/%s: errno %d\n", __func__, ret);
		ret = kstrtoul(p + 1, 10, (long unsigned int *)&rdma1_mode);
		if (ret)
			pr_err("DISP/%s: errno %d\n", __func__, ret);
		...
		p = (char *)opt + 10;
		ret = kstrtoul(p, 10, (long unsigned int *)&level);
		if (ret)
			pr_err("DISP/%s: errno %d\n", __func__, ret);
		...
		ret = kstrtoul(opt + 8, 10, (long unsigned int *)&aal_dbg_en);
		if (ret)
			pr_err("DISP/%s: errno %d\n", __func__, ret);
		...
		p = (char *)opt + 9;
		ret = kstrtoul(p, 10, (long unsigned int *)&module);
		if (ret)
			pr_err("DISP/%s: errno %d\n", __func__, ret);
		...
		p = (char *)opt + 10;
		ret = kstrtoul(p, 10, (long unsigned int *)&mutex_idx);
		if (ret)
			pr_err("DISP/%s: errno %d\n", __func__, ret);
