...
static void process_dbg_opt(const char *opt)
{
	...
	if (0 == strncmp(opt, "dsipattern", 10)) {
		...
		ret = kstrtoul(p, 16, (long unsigned int *)&pattern);
		if (ret)
		...
		ret = kstrtoul(p, 10, (long unsigned int *)&clk);
		if (ret)
			pr_err("DISP/%s: errno %d\n", __func__, ret);
		...
		ret = kstrtoul(p, 10, (long unsigned int *)&mode);
		if (ret)
			pr_err("DISP/%s: errno %d\n", __func__, ret);
		...
		gTriggerDispMode = 0;
		ret = kstrtoul(p, 10, (long unsigned int *)&gTriggerDispMode);
		if (ret)
			pr_err("DISP/%s: errno %d\n", __func__, ret);
		...
		ret = kstrtoul(p, 16, (long unsigned int *)&addr);
		if (ret)
			pr_err("DISP/%s: errno %d\n", __func__, ret);
		...
		ret = kstrtoul(p, 16, (long unsigned int *)&option);
		if (ret)
			pr_err("DISP/%s: errno %d\n", __func__, ret);
		...
		ret = kstrtoul(p, 10, (long unsigned int *)&maxlayer);
		if (ret)
			pr_err("DISP/%s: errno %d\n", __func__, ret);
		...
		ret = kstrtoul(p, 5, (long unsigned int *)&value);
		if (ret)
			pr_err("DISP/%s: errno %d\n", __func__, ret);
		...
		if (0 == strncmp(opt + 11, "on", 2)) {
			...
			ret = kstrtoul(p, 10, (long unsigned int *)&gCapturePriLayerDownX);
			if (ret)
				pr_err("DISP/%s: errno %d\n", __func__, ret);
			ret = kstrtoul(p + 1, 10, (long unsigned int *)&gCapturePriLayerDownY);
			if (ret)
				pr_err("DISP/%s: errno %d\n", __func__, ret);
			ret = kstrtoul(p + 1, 10, (long unsigned int *)&gCapturePriLayerNum);
			if (ret)
				pr_err("DISP/%s: errno %d\n", __func__, ret);
...
ssize_t simple_read_from_buffer1(void __user *to, size_t count, loff_t *ppos,
...
static ssize_t debug_read(struct file *file, char __user *ubuf, size_t count, loff_t *ppos)
{
	...
	if (*ppos != 0)
		return 0;
	n = mtkfb_get_debug_state(debug_buffer, debug_bufmax);

	ret += simple_read_from_buffer1(ubuf + ret, count, ppos, debug_buffer, n);

	n = primary_display_get_debug_state(debug_buffer, debug_bufmax);

	ret += simple_read_from_buffer1(ubuf + ret, count, ppos, debug_buffer, n);

	n = disp_sync_get_debug_info(debug_buffer, debug_bufmax);

	ret += simple_read_from_buffer1(ubuf + ret, count, ppos, debug_buffer, n);

	n = dprec_logger_get_result_string_all(debug_buffer, debug_bufmax);

	ret += simple_read_from_buffer1(ubuf + ret, count, ppos, debug_buffer, n);

	n = primary_display_check_path(debug_buffer, debug_bufmax);

	ret += simple_read_from_buffer1(ubuf + ret, count, ppos, debug_buffer, n);

	n = dprec_logger_get_buf(DPREC_LOGGER_ERROR, debug_buffer, debug_bufmax);

	ret += simple_read_from_buffer1(ubuf + ret, count, ppos, debug_buffer, n);

	n = dprec_logger_get_buf(DPREC_LOGGER_FENCE, debug_buffer, debug_bufmax);

	ret += simple_read_from_buffer1(ubuf + ret, count, ppos, debug_buffer, n);

	n = dprec_logger_get_buf(DPREC_LOGGER_DEBUG, debug_buffer, debug_bufmax);
