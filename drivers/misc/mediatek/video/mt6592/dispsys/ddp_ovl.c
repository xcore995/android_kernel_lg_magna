...
static enum OVL_INPUT_FORMAT ovl_fmt_convert(DpColorFormat fmt);
...
int OVLReset(void)
{
	...
	DISP_MSG("[DDP] OVLReset called %d\n", cnt++);
	...
	while (((DISP_REG_GET(DISP_REG_OVL_FLOW_CTRL_DBG) & 0x3ff) != 0x1) &&
		...
		delay_cnt++;
		if (delay_cnt > 10000) {
			DISP_ERR("[DDP] error, OVLReset() timeout!\n");
...
int OVLROI(unsigned int bgW, unsigned int bgH, unsigned int bgColor)
{
	if ((bgW > OVL_MAX_WIDTH) || (bgH > OVL_MAX_HEIGHT)) {
		DISP_ERR("DDP error, OVLROI bgW=%d, bgH=%d\n", bgW, bgH);
		return 0;
	}
...
int OVL3DConfig(unsigned int layer_id, unsigned int en_3d, unsigned int landscape, unsigned int r_first)
{
	ASSERT(layer_id <= 3);
	...
	unsigned char alpha)
{
	...
	int i, width;
	enum OVL_INPUT_FORMAT fmt=OVL_INPUT_FORMAT_UNKNOWN; //WBT TD#140373 , fmt referred w/o initialising
	if ((dst_w > OVL_MAX_WIDTH) || (dst_h > OVL_MAX_HEIGHT)) {
		DISP_ERR("DDP error, OVLLayerConfig layer=%d, src=%d, fmt=%d, addr=0x%x, x=%d, y=%d,",
			layer,	/* layer */
			source,	/* data source (0=memory) */
			fmt,
			addr,	/* addr */
			src_x,	/* x */
			src_y);	/* y */
		DISP_ERR(" pitch=%d, dst(%d, %d, %d, %d),keyEn=%d, key=%d, aen=%d, alpha=%d\n",
			src_pitch,	/* pitch, pixel number */
			dst_x,	/* x */
			dst_y,	/* y */
			dst_w,	/* width */
			dst_h,	/* height */
	...
	if ((layer == 3) && (isAEEEnabled == 1) && (addr != dal_fb_pa))
		DISP_MSG("[DDP], config AEE layer! addr=0x%x\n", addr);

	if (fmt == OVL_INPUT_FORMAT_ABGR8888 ||
	    fmt == OVL_INPUT_FORMAT_PABGR8888 ||
	    fmt == OVL_INPUT_FORMAT_xBGR8888 || fmt == OVL_INPUT_FORMAT_BGR888 || fmt == OVL_INPUT_FORMAT_BGR565) {
		rgb_swap = 1;
		fmt -= OVL_COLOR_BASE;
	} else {
		rgb_swap = 0;
	...
	case OVL_INPUT_FORMAT_VYUY:
		...
		break;
	default:
		DISP_ERR("DDP error, OVLLayerConfig unknown fmt , fmt=%d\n", fmt);
		ASSERT(0);	/* invalid input format */
	}

	if ((source == OVL_LAYER_SOURCE_SCL || source == OVL_LAYER_SOURCE_PQ) && (fmt != OVL_INPUT_FORMAT_YUV444)) {
		DISP_ERR("error: direct link to OVL only support YUV444!\n");
		ASSERT(0);	/* direct link support YUV444 only */
	}

	if ((source == OVL_LAYER_SOURCE_MEM && addr == 0)) {
		DISP_ERR("error: source from memory, but addr is 0!\n");
		ASSERT(0);	/* direct link support YUV444 only */
	}
...
*/
	...
	if ((width != 0) && (((start & 0x70) > 0x40) || ((end & 0x70) < 0x40))) {
		...
		DISP_MSG("OVL align 64byte:ROI(%d,%d %d,%d), (%d,%d), 0x%08x-->0x%08x, %d-->%d\n", src_x, src_y, dst_w,
			 dst_h, dst_x, dst_w, addr, address, dst_w, width);
	}
#endif
	switch (layer) {
	case 0:
		...
		if (TABLE_NO > mode) {	/* set up L0 YUV to RGB conversion matrix registers */
			...
			DISP_REG_SET_FIELD(L0_Y2R_PARA_YUV_A_0_FLD_C_CF_YA, DISP_REG_OVL_L0_Y2R_PARA_YUV_A_0,
					   coef[mode][3][0]);
			DISP_REG_SET_FIELD(L0_Y2R_PARA_YUV_A_0_FLD_C_CF_UA, DISP_REG_OVL_L0_Y2R_PARA_YUV_A_0,
					   coef[mode][3][1]);
			DISP_REG_SET_FIELD(L0_Y2R_PARA_YUV_A_1_FLD_C_CF_VA, DISP_REG_OVL_L0_Y2R_PARA_YUV_A_1,
					   coef[mode][3][2]);

			DISP_REG_SET_FIELD(L0_Y2R_PARA_RGB_A_0_FLD_C_CF_RA, DISP_REG_OVL_L0_Y2R_PARA_RGB_A_0,
					   coef[mode][4][0]);
			DISP_REG_SET_FIELD(L0_Y2R_PARA_RGB_A_0_FLD_C_CF_GA, DISP_REG_OVL_L0_Y2R_PARA_RGB_A_0,
					   coef[mode][4][1]);
			DISP_REG_SET_FIELD(L0_Y2R_PARA_RGB_A_1_FLD_C_CF_BA, DISP_REG_OVL_L0_Y2R_PARA_RGB_A_1,
					   coef[mode][4][2]);
	...
	case 1:
		...
		if (TABLE_NO > mode) {	/* set up L0 YUV to RGB conversion matrix registers */
			...
			DISP_REG_SET_FIELD(L1_Y2R_PARA_YUV_A_0_FLD_C_CF_YA, DISP_REG_OVL_L1_Y2R_PARA_YUV_A_0,
					   coef[mode][3][0]);
			DISP_REG_SET_FIELD(L1_Y2R_PARA_YUV_A_0_FLD_C_CF_UA, DISP_REG_OVL_L1_Y2R_PARA_YUV_A_0,
					   coef[mode][3][1]);
			DISP_REG_SET_FIELD(L1_Y2R_PARA_YUV_A_1_FLD_C_CF_VA, DISP_REG_OVL_L1_Y2R_PARA_YUV_A_1,
					   coef[mode][3][2]);

			DISP_REG_SET_FIELD(L1_Y2R_PARA_RGB_A_0_FLD_C_CF_RA, DISP_REG_OVL_L1_Y2R_PARA_RGB_A_0,
					   coef[mode][4][0]);
			DISP_REG_SET_FIELD(L1_Y2R_PARA_RGB_A_0_FLD_C_CF_GA, DISP_REG_OVL_L1_Y2R_PARA_RGB_A_0,
					   coef[mode][4][1]);
			DISP_REG_SET_FIELD(L1_Y2R_PARA_RGB_A_1_FLD_C_CF_BA, DISP_REG_OVL_L1_Y2R_PARA_RGB_A_1,
					   coef[mode][4][2]);
	...
	case 2:
		...
		if (TABLE_NO > mode) {	/* set up L0 YUV to RGB conversion matrix registers */
			...
			DISP_REG_SET_FIELD(L2_Y2R_PARA_YUV_A_0_FLD_C_CF_YA, DISP_REG_OVL_L2_Y2R_PARA_YUV_A_0,
					   coef[mode][3][0]);
			DISP_REG_SET_FIELD(L2_Y2R_PARA_YUV_A_0_FLD_C_CF_UA, DISP_REG_OVL_L2_Y2R_PARA_YUV_A_0,
					   coef[mode][3][1]);
			DISP_REG_SET_FIELD(L2_Y2R_PARA_YUV_A_1_FLD_C_CF_VA, DISP_REG_OVL_L2_Y2R_PARA_YUV_A_1,
					   coef[mode][3][2]);

			DISP_REG_SET_FIELD(L2_Y2R_PARA_RGB_A_0_FLD_C_CF_RA, DISP_REG_OVL_L2_Y2R_PARA_RGB_A_0,
					   coef[mode][4][0]);
			DISP_REG_SET_FIELD(L2_Y2R_PARA_RGB_A_0_FLD_C_CF_GA, DISP_REG_OVL_L2_Y2R_PARA_RGB_A_0,
					   coef[mode][4][1]);
			DISP_REG_SET_FIELD(L2_Y2R_PARA_RGB_A_1_FLD_C_CF_BA, DISP_REG_OVL_L2_Y2R_PARA_RGB_A_1,
					   coef[mode][4][2]);
	...
	case 3:
		...
		if (TABLE_NO > mode) {	/* set up L0 YUV to RGB conversion matrix registers */
			...
			DISP_REG_SET_FIELD(L3_Y2R_PARA_YUV_A_0_FLD_C_CF_YA, DISP_REG_OVL_L3_Y2R_PARA_YUV_A_0,
					   coef[mode][3][0]);
			DISP_REG_SET_FIELD(L3_Y2R_PARA_YUV_A_0_FLD_C_CF_UA, DISP_REG_OVL_L3_Y2R_PARA_YUV_A_0,
					   coef[mode][3][1]);
			DISP_REG_SET_FIELD(L3_Y2R_PARA_YUV_A_1_FLD_C_CF_VA, DISP_REG_OVL_L3_Y2R_PARA_YUV_A_1,
					   coef[mode][3][2]);

			DISP_REG_SET_FIELD(L3_Y2R_PARA_RGB_A_0_FLD_C_CF_RA, DISP_REG_OVL_L3_Y2R_PARA_RGB_A_0,
					   coef[mode][4][0]);
			DISP_REG_SET_FIELD(L3_Y2R_PARA_RGB_A_0_FLD_C_CF_GA, DISP_REG_OVL_L3_Y2R_PARA_RGB_A_0,
					   coef[mode][4][1]);
			DISP_REG_SET_FIELD(L3_Y2R_PARA_RGB_A_1_FLD_C_CF_BA, DISP_REG_OVL_L3_Y2R_PARA_RGB_A_1,
					   coef[mode][4][2]);
	...
	default:
		DISP_ERR("DDP error, OVLLayerConfig layer=%d\n", layer);
		ASSERT(0);	/* invalid layer index */
	}

	if (0) {		/* if(w==1080) */
		/* DISP_MSG("[DDP]set 1080p ultra\n"); */
...
static enum OVL_INPUT_FORMAT ovl_fmt_convert(DpColorFormat fmt)
{
	...
	/* DISP_MSG("BOOT ovl_fmt_convert 0 fmt=%d, ovl_fmt=%d\n", fmt, ovl_fmt); */
	switch (fmt) {
	case eYUY2:
	...
	case eXARGB8888:
		...
		break;
	case eXBGR8888:
		ovl_fmt = OVL_INPUT_FORMAT_xBGR8888;
		break;
	...
	/* DISP_MSG("BOOT ovl_fmt_convert 1 fmt=%d, ovl_fmt=%d\n", fmt, ovl_fmt); */

	return ovl_fmt;
}

unsigned int OVLGetLayer0Addr(void)
{
