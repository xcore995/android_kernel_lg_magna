...
/* TDSHPINDEX ioctl error handling */
...
static int disp_wait_intr(DISP_MODULE_ENUM module, unsigned int timeout_ms)
{
	...
	ret = wait_event_interruptible_timeout(g_disp_irq_done_queue,
					       g_disp_irq.irq_src & (1 << module), disp_ms2jiffies(timeout_ms));
...
/* disp_invoke_irq_callbacks(DISP_MODULE_COLOR, reg_val); */
	...
	case MT_DISP_MUTEX_IRQ_ID: /* can not do reg update done status after release mutex(for ECO requirement), */
		...
		if (reg_val & 0x0FF00) {	/* udpate timeout intr triggered */
			...
			for (mutexID = 0; mutexID < 4; mutexID++) {
				if ((DISP_REG_GET(DISP_REG_CONFIG_MUTEX_INTSTA) & (1 << (mutexID + 9))) ==
				    (1 << (mutexID + 9))) {
...
int disp_color_set_pq_param(void *arg)
{
	...
	if (copy_from_user(pq_param, (void *)arg, sizeof(DISP_PQ_PARAM))) {
		DISP_ERR("disp driver : DISP_IOCTL_SET_PQPARAM Copy from user failed\n");
		ReleaseUpdateMutex();
		return -EFAULT;
	}
...
/* cmq_status[value] = 1; */
...
#if !defined(CONFIG_MTK_AAL_SUPPORT)
		DISP_MSG("Invalid operation DISP_IOCTL_AAL_EVENTCTL since AAL is not turned on, in %s\n", __func__);
		return -EFAULT;
#else
		if (copy_from_user(&value, (void *)arg, sizeof(int))) {
	...
	case DISP_IOCTL_GET_AALSTATISTICS:
#if !defined(CONFIG_MTK_AAL_SUPPORT)
		DISP_MSG("Invalid operation DISP_IOCTL_GET_AALSTATISTICS since AAL is not turned on, in %s\n",
			 __func__);
		return -EFAULT;
#else
		...
		if (copy_to_user((void *)arg, (void *)(disp_get_hist_ptr()), sizeof(DISP_AAL_STATISTICS))) {
			DISP_MSG("disp driver : DISP_IOCTL_GET_AALSTATISTICS Copy to user failed\n");
			return -EFAULT;
		}
...
#if !defined(CONFIG_MTK_AAL_SUPPORT)
		DISP_MSG("Invalid operation : DISP_IOCTL_SET_AALPARAM since AAL is not turned on, in %s\n", __func__);
		return -EFAULT;
#else
/* disp_set_needupdate(DISP_MODULE_BLS , 0); */
		{
	...
	case DISP_IOCTL_GET_TDSHPINDEX:
		...
		if (tdshp_index_init == 0) {
			DISP_MSG("disp driver : DISP_IOCTL_GET_TDSHPINDEX TDSHPINDEX not init\n");
			return -EFAULT;
		}
		tdshp_index = get_TDSHP_index();
	...
	case DISP_IOCTL_SET_PQ_CAM_PARAM:
		...
		if (copy_from_user(pq_cam_param, (void *)arg, sizeof(DISP_PQ_PARAM))) {
			DISP_MSG("disp driver : DISP_IOCTL_SET_PQ_CAM_PARAM Copy from user failed\n");
			return -EFAULT;
		}
	...
	case DISP_IOCTL_SET_PQ_GAL_PARAM:
		...
		if (copy_from_user(pq_gal_param, (void *)arg, sizeof(DISP_PQ_PARAM))) {
			DISP_MSG("disp driver : DISP_IOCTL_SET_PQ_GAL_PARAM Copy from user failed\n");
			return -EFAULT;
		}
...
struct const dev_pm_ops disp_pm_ops = {
...
static const struct platform_driver disp_driver = {
	.probe = disp_probe,
	.remove = disp_remove,
