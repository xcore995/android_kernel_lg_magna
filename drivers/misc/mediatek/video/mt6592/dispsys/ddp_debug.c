...
static void process_dbg_opt(const char *opt)
{
	...
	if (0 == strncmp(opt, "regr:", 5)) {
		...
		char *p = (char *)opt + 9;
		kstrtoul(p, 10, &value);
		unsigned int module = (unsigned int)value;
		...
		char *p = (char *)opt + 9;
		kstrtoul(p, 10, &value);
		unsigned int arg = (unsigned int)value;
		if (arg == 0) {
			int i;
		...
		if (enable == 1) {
			...
			DISP_MSG("SMI_LARB_MON_REQ0=0x%x, SMI_LARB_MON_REQ1=0x%x, SMI_0=0x%x, SMI_600=0x%x,",
			...
			DISP_MSG(" SMI_604=0x%x, SMI_610=0x%x, SMI_614=0x%x,",
				*(volatile unsigned int *)0xf4010604,
				*(volatile unsigned int *)0xf4010610,
				*(volatile unsigned int *)0xf4010614);
			DISP_MSG(" color_h_cnt=%d, color_line_cnt=%d, ovl_add_con=0x%x, ovl_ctrl_flow=0x%x\n",
			...
			sprintf(dbg_buf + strlen(dbg_buf),
				"SMI_LARB_MON_REQ0=0x%x, SMI_LARB_MON_REQ1=0x%x, SMI_0=0x%x, SMI_600=0x%x,",
				*(volatile unsigned int *)0xf4010450,
				*(volatile unsigned int *)0xf4010454,
				*(volatile unsigned int *)0xf4010000,
				*(volatile unsigned int *)0xf4010600);
			sprintf(dbg_buf + strlen(dbg_buf),
				" SMI_604=0x%x, SMI_610=0x%x, SMI_614=0x%x,",
				*(volatile unsigned int *)0xf4010604,
				*(volatile unsigned int *)0xf4010610,
				*(volatile unsigned int *)0xf4010614);
			sprintf(dbg_buf + strlen(dbg_buf),
			...
			DISP_MSG
			("ddp debug set gUltraLevel = %d, DISP_REG_RDMA_MEM_GMC_SETTING_0=0x%x,",
			gUltraLevel, DISP_REG_GET(DISP_REG_RDMA_MEM_GMC_SETTING_0));
			DISP_MSG(" DISP_REG_RDMA_FIFO_CON=0x%x\n", DISP_REG_GET(DISP_REG_RDMA_FIFO_CON));
		} else if (enable == 16) {
			kstrtoul(p + 1, 16, &value);
			gOvlRdmaFifoCtrl = (unsigned int)value;
			...
			sprintf(buf, "base:\n config f4+0\n ovl 7\n rdma 8\n rdma1 12\n wdma 9\n bls a\n color b\n");
			sprintf(buf + strlen(buf), "dsi c\n dpi d\n mm_mutex e\n mm_cmdq f\n smi_larb0 10\n");
			sprintf(buf + strlen(buf), "smi_common 11\n");
		} else if (enable == 22) {
			disp_path_reset();
		...
		kstrtoul(p, 10, &value);
		sat_upper = (unsigned int)value;
		p++;
		kstrtoul(p, 10, &value);
		sat_lower = (unsigned int)value;
		p++;
		kstrtoul(p, 10, &value);
		hue_upper = (unsigned int)value;
		p++;
		kstrtoul(p, 10, &value);
		hue_lower = (unsigned int)value;
...
static int Ddp_Write_State(struct file *file, const char *buffer, unsigned long count, void *data)
{
	DISP_MSG("Ddp_Write_State\n");
	char desc[32];
	int len = 0;
	int num = 0;
...
int ddp_mem_test2(void)
{
	...
	if (pSrc == 0) {
		DISP_ERR("[DDP] error: dma_alloc_coherent error!  dma memory not available.\n");
		return 0;
	} else {
	...
	if (result)
		DISP_ERR("[DDP] error: DDPK_Bitblt_Config fail!, ret=%d\n", result);

	DISP_MSG("DDP, DDPK_Bitblt module setting:\n");
	...
	if (result)
		DISP_ERR("[DDP] error: DDPK_Bitblt() fail, result=%d\n", result);
	...
	/* result verify */
	{
		...
		for (t = 0; t < size; t++) {
			if (*((unsigned char *)pSrc + t) != *((unsigned char *)data_rgb888_64x64 + t)) {
				...
				DISP_MSG("t=%d, diff_cnt=%d, dst=0x%x, gold=0x%x\n",
					 t,
					 diff_cnt,
					 *((unsigned char *)pSrc + t), *((unsigned char *)data_rgb888_64x64 + t));
			}
		...
		if (diff_cnt == 0)
			...
			DISP_ERR("[DDP] error: ddp_mem_test src compare result: fail\n");
			DISP_ERR("detail, %d, %d, %%%d\n", diff_cnt, size, diff_cnt * 100 / size);
			result = -1;
		}
	}

	{
		...
		if (diff_cnt == 0)
			...
			DISP_ERR("[DDP] error: ddp_mem_test result: fail\n");
			DISP_ERR("detail, %d, %d, %%%d\n", diff_cnt, size, diff_cnt * 100 / size);
			result = -1;
		}
	}
...
int ddp_mem_test(void)
{
	...
	pSrc =
	    dma_alloc_coherent(NULL, DDP_TEST_WIDTH * DDP_TEST_HEIGHT * DDP_TEST_BPP, (dma_addr_t *) &pSrcPa,
			       GFP_KERNEL);
	if (pSrc == 0 || pSrcPa == 0) {
		DISP_ERR("dma_alloc_coherent error!  dma memory not available.\n");
		return 0;
	} else {
	...
	pDst =
	    dma_alloc_coherent(NULL, DDP_TEST_WIDTH * DDP_TEST_HEIGHT * DDP_TEST_BPP, (dma_addr_t *) &pDstPa,
			       GFP_KERNEL);
	if (pDst == 0 || pDstPa == 0) {
		DISP_ERR("dma_alloc_coherent error!  dma memory not available.\n");
		return 0;
...
char *ddp_get_fmt_name(DISP_MODULE_ENUM module, unsigned int fmt)
{
	...
	} else if (module == DISP_MODULE_RDMA || module == DISP_MODULE_RDMA1) {
		switch (fmt) {
		case 0:
			return "yuyv";
		case 1:
...
char *ddp_ovl_get_status(unsigned int status)
{
	...
	if (status & (1 << 21))
		DISP_MSG("trig,");
	if (status & (1 << 23))
		DISP_MSG("frame_hwrst_done,");
	if (status & (1 << 24))
		DISP_MSG("frame_swrst_done,");
	if (status & (1 << 25))
		DISP_MSG("frame_underrun,");
	if (status & (1 << 26))
		DISP_MSG("frame_done,");
	if (status & (1 << 27))
		DISP_MSG("ovl_running,");
	if (status & (1 << 28))
		DISP_MSG("ovl_start,");
	if (status & (1 << 29))
		DISP_MSG("ovl_clr,");
	...
	case 0x20:
		...
		break;
	case 0x40:
		DISP_MSG("s_wait_w_rst\n");
		break;
	case 0x80:
		DISP_MSG("s_w_rst\n");
		break;
	case 0x100:
		DISP_MSG("h_wait_w_rst\n");
		break;
	case 0x200:
		DISP_MSG("h_w_rst\n");
		break;
...
{
	DISP_ERR("error_state=");
	if ((reg & 0xf2) != 0) {
		if ((reg >> 7) & 0x1)
			DISP_MSG("contention_err, ");
		if ((reg >> 6) & 0x1)
			DISP_MSG("false_ctrl_err, ");
		if ((reg >> 5) & 0x1)
			DISP_MSG("lpdt_sync_err, ");
		if ((reg >> 4) & 0x1)
			DISP_MSG("esc_entry_err, ");
		if ((reg >> 1) & 0x1)
			DISP_MSG("buffer_underrun");
...
int ddp_dump_info(DISP_MODULE_ENUM module)
{
	...
	case DISP_MODULE_WDMA0:
		DISP_MSG
		    ("wdma0: en=%d, w=%d, h=%d, clip=(%d, %d, %d, %d), pitch=(%d,%d), addr=(0x%x,0x%x,0x%x), fmt=%s\n",
		     ...
		     (DISP_REG_GET(DISP_REG_WDMA_CLIP_SIZE) >> 16) & 0x3fff, DISP_REG_GET(DISP_REG_WDMA_DST_W_IN_BYTE),
	...
	case DISP_MODULE_RDMA1:
		...
		DISP_MSG("rdma%d: en=%d, mode=%s, smi_busy=%d, w=%d, h=%d, pitch=%d, addr=0x%x, fmt=%s, fifo_min=%d\n",
			 ...
			 ((DISP_REG_GET(DISP_REG_RDMA_GLOBAL_CON + DISP_INDEX_OFFSET * index) & 0x2) ==
			  0) ? "rgb888" : ddp_get_fmt_name(DISP_MODULE_RDMA,
							   (DISP_REG_GET
	...
	case DISP_MODULE_OVL:
		...
		(DISP_REG_GET(DISP_REG_OVL_SRC_CON) >> 1) & 0x1, (DISP_REG_GET(DISP_REG_OVL_SRC_CON) >> 2) & 0x1,
		(DISP_REG_GET(DISP_REG_OVL_SRC_CON) >> 3) & 0x1, DISP_REG_GET(DISP_REG_OVL_ROI_SIZE) & 0xfff,
	...
	case DISP_MODULE_MUTEX:
		...
		for (i = 0; i < 3; i++) {
			DISP_DBG("i=%d, mod=0x%x, en=%d\n",
				 i,
				 DISP_REG_GET(DISP_REG_CONFIG_MUTEX_MOD(i)), DISP_REG_GET(DISP_REG_CONFIG_MUTEX_EN(i)));
			...
			if ((DISP_REG_GET(DISP_REG_CONFIG_MUTEX_MOD(i)) != 0) &&
				...
				for (j = 3; j <= 12; j++) {
					if ((DISP_REG_GET(DISP_REG_CONFIG_MUTEX_MOD(i)) >> j) & 0x1)
						DISP_MSG("%s,", ddp_get_mutex_module_name(j));
				}
				DISP_MSG(")\n");
			}
		}
	...
	case DISP_MODULE_COLOR:
		DISP_MSG("color: w=%d, h=%d, h_cnt=%d, line_cnt=%d\n",
			 ...
			 DISP_REG_GET(DISP_REG_COLOR_H_CNT) & 0xffff, DISP_REG_GET(DISP_REG_COLOR_L_CNT) & 0xffff);
		break;
	...
	default:
		DISP_ERR("DDP error, dump_info unknow module=%d\n", module);
	}
	return 0;
}
