...
/* log */
/*  */
#define CMDQ_LOG(string, args...) pr_debug("[CMDQ]"string, ##args)
...
#define CMDQ_ERR(string, args...) pr_err("[CMDQ][ERR]"string, ##args)
...
do { \
	...
	pr_err("[CMDQ][AEE][%s]"string, tag, ##args); \
} while (0)

#define CMDQ_GET_CURRENT_TIME(value) do_gettimeofday(&(value))
...
extern "C" {
#endif
	int32_t cmdqRegisterCallback(int32_t index, CMDQ_TIMEOUT_PTR pTimeout, CMDQ_RESET_PTR pReset);
