...
{
	DISP_MSG("dispsys_bypass_bls, width=%d, height=%d, reg=0x%x\n", width, height, ((height << 16) + width));
...
int disp_path_get_mutex_(void)
{
	...
	while (((DISP_REG_GET(DISP_REG_CONFIG_MUTEX(gMutexID)) & DISP_INT_MUTEX_BIT_MASK) != DISP_INT_MUTEX_BIT_MASK)) {
		cnt++;
		if (cnt > 10000) {
			DISP_ERR("[DDP] disp_path_get_mutex() timeout!\n");
...
{
	DISP_DBG("[DDP] config_rdma(), idx=%d, mode=%d, in_fmt=%d, addr=0x%x, out_fmt=%d, pitch=%d,",
		pRdmaConfig->idx,
	...
	DISP_DBG(" x=%d, y=%d, byteSwap=%d, rgbSwap=%d\n",
...
int disp_path_config_wdma2(WDMA_CONFIG_STRUCT *pConfig)
{
	...
	WDMAConfig2(pConfig->idx, pConfig->inputFormat, pConfig->srcWidth, pConfig->srcHeight,
		    pConfig->clipX, pConfig->clipY, pConfig->clipWidth, pConfig->clipHeight,
		    pConfig->outputFormat, pConfig->dstAddress, pConfig->dstWidth, pConfig->dstPitch,
		    pConfig->dstPitchUV, pConfig->useSpecifiedAlpha, pConfig->alpha);
...
/* unsigned int reg_addr; */
	...
	DISP_DBG("[DDP] config_layer(), layer=%d, en=%d, src=%d, fmt=%d, addr=0x%x, x=%d, y=%d,",
		...
		pOvlConfig->src_x,	/* x */
		pOvlConfig->src_y);	/* y */
	DISP_DBG(" pitch=%d, dst(%d, %d, %d, %d),keyEn=%d, key=%d, aen=%d, alpha=%d\n",
...
int disp_path_config_mem_out_without_lcd(struct disp_path_config_mem_out_struct *pConfig)
{
	...
	DISP_DBG(" disp_path_config_mem_out(), enable = %d, outFormat=%d, dstAddr=0x%x, ROI(%d,%d,%d,%d)\n",
		 pConfig->enable,
		 pConfig->outFormat,
		 pConfig->dstAddr, pConfig->srcROI.x, pConfig->srcROI.y, pConfig->srcROI.width, pConfig->srcROI.height);
...
int disp_path_config_(struct disp_path_config_struct *pConfig, int mutexId)
{
	...
	DISP_DBG("[DDP] disp_path_config(), srcModule=%d, addr=0x%x, inFormat=%d, pitch=%d, bgROI(%d,%d,%d,%d),",
		pConfig->srcModule,
		pConfig->addr,
		pConfig->inFormat,
		pConfig->pitch,
	...
	DISP_DBG(" bgColor=%d, outFormat=%d, dstModule=%d, dstAddr=0x%x, dstPitch=%d, mutexId=%d\n",
		pConfig->bgColor,
		pConfig->outFormat,
		pConfig->dstModule,
		pConfig->dstAddr,
...

	{
		if (pConfig->dstModule == DISP_MODULE_WDMA0) {
			...
			/* ovl, rdma1, color, bls, ufoe(if enabled) */
			DISP_REG_SET(DISP_REG_CONFIG_MUTEX_MOD(mutexId),
				(1 << 3) | (1 << 10) | (1 << 7) | (1 << 9) | ((pConfig->enableUFOE ? 1 : 0) << 13));
		} else if (pConfig->srcModule == DISP_MODULE_RDMA) {
			/* rdma, color, bls, ufoe(if enabled) */
			DISP_REG_SET(DISP_REG_CONFIG_MUTEX_MOD(mutexId),
				(1 << 10) | (1 << 7) | (1 << 9) | ((pConfig->enableUFOE ? 1 : 0) << 13));
		...
		if (pConfig->dstModule == DISP_MODULE_WDMA0) {
			...
			if (DISP_IsDecoupleMode()) {
				...
				WDMAConfig2(0,
					    ...
					    pConfig->srcROI.height,
					    eUYVY,
					    decouple_addr, pConfig->srcROI.width, pConfig->srcROI.width * 2, 0, 1, 0);
				WDMAStart(0);
				/* Register WDMA intr */
				mem_out_done = 0;
				disp_register_irq(DISP_MODULE_WDMA0, _disp_path_wdma_callback);
				...
				RDMAConfig(0,
					   ...
					   pConfig->srcROI.width, pConfig->srcROI.height, 0, 0);
				RDMAStart(0);
			} else {
				RDMAConfig(0, RDMA_MODE_DIRECT_LINK,	/* /direct link mode */
					   eRGB888,	/* inputFormat */
