...
static long cmdq_proc_unlocked_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case DISP_IOCTL_LOCK_MUTEX:
		{
			wait_event_interruptible_timeout(gMutexWaitQueue,
							 (mutex_id = disp_lock_mutex()) != -1, msecs_to_jiffies(200));
	...
	case DISP_IOCTL_GET_TDSHPINDEX:
		...
		if (copy_to_user((void *)arg, tdshp_index, sizeof(DISPLAY_TDSHP_T))) {
			pr_warn("disp driver : DISP_IOCTL_GET_TDSHPINDEX Copy to user failed\n");
			return -EFAULT;
		}
	...
	case DISP_IOCTL_GET_PQPARAM:
		...
		if (copy_to_user((void *)arg, pq_param, sizeof(DISP_PQ_PARAM))) {
			pr_warn("disp driver : DISP_IOCTL_GET_PQPARAM Copy to user failed\n");
			return -EFAULT;
		}
		...
		for (index = 0; index < ENGINE_MUTEX_NUM; index += 1) {
			if ((1 << index) & pNode->u4LockedMutex) {
				...
				DISP_DBG("unlock index = %d ,mutex_used[ %d %d %d %d ]\n", index, mutex_used[0],
					 mutex_used[1], mutex_used[2], mutex_used[3]);
			}
		}
...
int32_t cmdqRegisterCallback(int32_t index, CMDQ_TIMEOUT_PTR pTimeoutFunc, CMDQ_RESET_PTR pResetFunc)
{
...
int cmdq_proc_status_print_seq(struct seq_file *m, void *v)
{
	...
	seq_printf(m, "RDMA0: count %d, owner %d, fail: %d, reset: %d\n", pEngine[tRDMA0].userCount,
		   pEngine[tRDMA0].currOwner, pEngine[tRDMA0].failCount, pEngine[tRDMA0].resetCount);
	...
	seq_printf(m, "TDSHP: count %d, owner %d, fail: %d, reset: %d\n", pEngine[tTDSHP].userCount,
		   pEngine[tTDSHP].currOwner, pEngine[tTDSHP].failCount, pEngine[tTDSHP].resetCount);
	seq_printf(m, "tWDMA1: count %d, owner %d, fail: %d, reset: %d\n", pEngine[tWDMA1].userCount,
		   pEngine[tWDMA1].currOwner, pEngine[tWDMA1].failCount, pEngine[tWDMA1].resetCount);
			...
			for (inner = 0; inner < CMDQ_MAX_TASK_COUNT; inner++) {
				...
				if (NULL != pTask) {
					seq_printf(m,
						   ...
						   inner, (uint32_t) pTask, pTask->MVABase, pTask->blockSize,
...
ssize_t cmdq_write_log_level_proc(struct file *pFile, const char __user *pBuffer, size_t count, loff_t *data)
{
	u_long flags = 0;
	int len = 0;
	int32_t value = 0;
...
ssize_t cmdq_write_debug_level_proc(struct file *pFile, const char __user *pBuffer, size_t count, loff_t *data)
{
	int len = 0;
	int32_t value = 0;
	char textBuf[10] = { 0 };
...
ssize_t cmdq_write_sw_timeout_proc(struct file *pFile, const char __user *pBuffer, size_t count, loff_t *data)
{
	int32_t len = 0;
	...
	sscanf_ret = sscanf(textBuf, "%u %u %u", &swTimeout, &predumpStartTime, &predumpDuration);
...
int32_t cmdqSuspendTask(void)
{
	...
	if ((0 != pEngine[tIMGI].userCount) ||
	    ...
	    (0 != pEngine[tTDSHP].userCount) || (0 != pEngine[tWROT].userCount) || (0 != pEngine[tWDMA1].userCount)) {
		status = -EFAULT;
	}
...
static TaskStruct *cmdq_acquire_task(cmdqCommandStruct *pCommandDesc)
{
	...
	CMDQ_MSG("-->TASK: ENG: 0x%08x, CMD: 0x%08x, size: %d\n", pCommandDesc->engineFlag,
		 (uint32_t) (pCommandDesc->pVABase), (pCommandDesc->blockSize));
	...
	if (list_empty(&gCmdqFreeTask) || (pCommandDesc->blockSize >= CMDQ_MAX_BLOCK_SIZE)) {
		spin_unlock_irqrestore(&gCmdqTaskLock, flags);

		pTask = kmalloc(sizeof(TaskStruct), GFP_KERNEL);
		if (NULL != pTask) {
			pVABase =
			    dma_alloc_coherent(NULL, pCommandDesc->blockSize + CMDQ_EXTRA_MARGIN, &MVABase, GFP_KERNEL);
			if (NULL == pVABase) {
				kfree(pTask);
				pTask = NULL;
	...
	if (NULL != pTask) {
		...
		if (false == copyCmdFromUserSpace) {
			...
			CMDQ_ERR("Source: 0x%08x, target: 0x%08x, size: %d\n", (uint32_t) pCommandDesc->pVABase,
				 (uint32_t) pTask->pVABase, pCommandDesc->blockSize);
...
void cmdq_dump_mmsys_config(void)
{
	...
	CMDQ_ERR
	    ("MMSYS_DL_VALID_0: 0x%08x, MMSYS_DL_VALID_1: 0x%08x, MMSYS_DL_READY0: 0x%08x, MMSYS_DL_READY1: 0x%08x\n",
	     DISP_REG_GET(0xF4000000 + 0x860), DISP_REG_GET(0xF4000000 + 0x864), DISP_REG_GET(0xF4000000 + 0x868),
...
static void cmdq_attach_error_task(const TaskStruct *pTask, int32_t thread)
{
	...
	if (NULL != pTask) {
		...
		CMDQ_ERR
		("Task: 0x%08x, Scenario: %d, State: %d, Priority: %d, Reorder: %d, Engine: 0x%08x, irgFlag: 0x%08x\n",
		(uint32_t) pTask, pTask->scenario, pTask->taskState, pTask->priority, pTask->reorder,
		pTask->engineFlag, pTask->irqFlag);
		CMDQ_ERR
		("VABase: 0x%08x, CMDEnd: 0x%08x, MVABase: 0x%08x, Size: %d, Last Inst: 0x%08x:0x%08x, 0x%08x:0x%08x\n",
	...
	if (NULL != pTask) {
		...
		/* CMDQ_MAX_ERROR_COUNT is 1 now, replace the old one instead of using gCmdqContext.errNum
		as the index init value */
...
void cmdq_attach_warning_task(TaskStruct *pTask,
			      uint32_t thread,
			      uint32_t taskCountInThread, uint32_t cookie, uint32_t irqFlag, struct timeval gotIRQ)
{
	uint32_t index = gCmdqContext.warningNum;
	do {
		CMDQ_ERR("warning task[%d][0x%p], THR[%d], taskCount[%d], CurrCookie[%d], irqFlag[%d], gotIRQ[%d:%d]",
			 index, pTask, thread, taskCountInThread, cookie, irqFlag, (uint32_t) (gotIRQ.tv_sec),
			 (uint32_t) (gotIRQ.tv_usec));
							    ...
							    uint32_t predump_duration_ms, uint32_t predump_retry_count)
{
	...
	waitQ =
	    wait_event_interruptible_timeout(gCmdWaitQueue[thread], (TASK_STATE_BUSY != pTask->taskState),
	...
	for (i = 0; i < predump_max_retry_count; i++) {
		...
		if (hasInterrupted) {
			waitQ =
			    wait_event_timeout(gCmdWaitQueue[thread], (TASK_STATE_BUSY != pTask->taskState),
					       msecs_to_jiffies(timeout_ms));
		} else {
			waitQ =
				 ...
				 uint32_t predump_duration_ms, uint32_t predump_retry_count)
{
	...
	waitQ =
	    cmdq_core_wait_task_done_with_interruptible_timeout(pTask, thread, predump_start_time_ms,
								predump_duration_ms, predump_retry_count);
	...
	if (TASK_STATE_DONE == pTask->taskState) {
		...
		do {
			...
			if (TASK_STATE_ERROR == pTask->taskState) {
				...
				/* except thread's taskCount, HW/SW cookie count, GCE/HW reset haved been fixed,
				and previous task are done */
			...
			if (throwAEE) {
				...
				case CMDQ_CODE_WFE:
					CMDQ_AEE("CMDQ", "%s SW timeout, INST:(0x%08x, 0x%08x), OP:WAIT EVENT:%s\n",
						 module, instA, instB, cmdq_core_get_event_name(instA & (~0xFF000000)));
					break;
				default:
...
void cmdqHandleIRQ(int32_t thread, const uint32_t value)
{
	...
	const uint32_t validIrqFlag =
	    (CMDQ_THR_IRQ_FALG_INVALID_INSTN | CMDQ_THR_IRQ_FALG_INSTN_TIMEOUT | CMDQ_THR_IRQ_FALG_EXEC_CMD);
...
void cmdq_debug_set_sw_timeout(const uint32_t sw_timeout_ms,
			       const uint32_t predump_start_time_ms, const uint32_t predump_duration_ms)
{
