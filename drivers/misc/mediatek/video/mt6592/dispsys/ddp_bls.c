...
#define BLS_DBG(string, args...) \
do { \
	if (bls_dbg_log) \
		pr_warn("[BLS]"string, ##args); \
} while (0)
#define BLS_MSG(string, args...) pr_warn("[BLS]"string, ##args)
	...
	{
	 {
	  ...
	  288, 292, 296,
	  300, 304, 308, 312, 316, 320, 324, 328, 332, 336, 340, 344, 348, 352, 356, 360, 364, 368, 372, 376, 380, 384,
	  388, 392, 396,
	  400, 404, 408, 412, 416, 420, 424, 428, 432, 436, 440, 444, 448, 452, 456, 460, 464, 468, 472, 476, 480, 484,
	  488, 492, 496,
	  500, 504, 508, 512, 516, 520, 524, 528, 532, 536, 540, 544, 548, 552, 556, 560, 564, 568, 572, 576, 580, 584,
	  588, 592, 596,
	  600, 604, 608, 612, 616, 620, 624, 628, 632, 636, 640, 644, 648, 652, 656, 660, 664, 668, 672, 676, 680, 684,
	  688, 692, 696,
	  700, 704, 708, 712, 716, 720, 724, 728, 732, 736, 740, 744, 748, 752, 756, 760, 764, 768, 772, 776, 780, 784,
	  788, 792, 796,
	  800, 804, 808, 812, 816, 820, 824, 828, 832, 836, 840, 844, 848, 852, 856, 860, 864, 868, 872, 876, 880, 884,
	  888, 892, 896,
	  900, 904, 908, 912, 916, 920, 924, 928, 932, 936, 940, 944, 948, 952, 956, 960, 964, 968, 972, 976, 980, 984,
	  988, 992, 996,
...
void disp_bls_update_gamma_lut(void)
{
	...
	for (i = 0; i < 256; i++) {
		CurVal =
		    (((g_gamma_lut.entry[0][i] & 0x3FF) << 20) | ((g_gamma_lut.
								   entry[1][i] & 0x3FF) << 10) | (g_gamma_lut.
												  entry[2][i] & 0x3FF));
	...
	CurVal =
	    (((g_gamma_lut.entry[0][256] & 0x3FF) << 20) | ((g_gamma_lut.entry[1][256] & 0x3FF) << 10) | (g_gamma_lut.
													  entry[2][256]
													  & 0x3FF));
...
void disp_bls_init(unsigned int srcWidth, unsigned int srcHeight)
{
	...
	if (cust_led_list) {
		...
		if ((strcmp(cust->name, "lcd-backlight") == 0) && (cust->mode == MT65XX_LED_MODE_CUST_BLS_PWM)) {
			...
			gPWMDiv &= 0x3FF;
			BLS_DBG("disp_bls_init : PWM config data (%d,%d)\n", config_data->clock_source,
				config_data->div);
		}
	}

	BLS_DBG("disp_bls_init : srcWidth = %d, srcHeight = %d\n", srcWidth, srcHeight);
	BLS_DBG("disp_bls_init : BLS_EN=0x%x, PWM_DUTY=%d, PWM_DUTY_RD=%d, CG=0x%x, %d, %d\n",
...
int disp_bls_config(void)
{
	...
	if (cust_led_list) {
		...
		if ((strcmp(cust->name, "lcd-backlight") == 0) && (cust->mode == MT65XX_LED_MODE_CUST_BLS_PWM)) {
			...
			gPWMDiv = (config_data->div == 0) ? PWM_DEFAULT_DIV_VALUE : config_data->div;
			gPWMDiv &= 0x3FF;
			BLS_DBG("disp_bls_config : PWM config data (%d,%d)\n", config_data->clock_source,
				config_data->div);
		}
	}
	...
	BLS_DBG("disp_bls_config : BLS_EN=0x%x, PWM_DUTY=%d, PWM_DUTY_RD=%d, CG=0x%x, %d, %d\n",
		DISP_REG_GET(DISP_REG_BLS_EN),
		DISP_REG_GET(DISP_REG_BLS_PWM_DUTY),
		DISP_REG_GET(DISP_REG_BLS_PWM_DUTY_RD),
		DISP_REG_GET(DISP_REG_CONFIG_MMSYS_CG_CON0),
...
#ifdef USE_DISP_BLS_MUTEX
	BLS_DBG("disp_bls_config : gBLSMutexID = %d\n", gBLSMutexID);
