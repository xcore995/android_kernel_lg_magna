...
#define DDP_ROT_LOG(fmt, arg...) {pr_warn("[ROT]"); pr_warn(fmt, ##arg); }
#else
#define DDP_ROT_LOG()
#endif
...
int ROTConfig(int rotateDegree,
	      ...
	      int srcWidth, int srcHeight, int srcPitch, struct DISP_REGION srcROI, DISP_COLOR_FORMAT *outFormat)
{
...
*/
unsigned disp_rot_get_bkgd(unsigned src_format, unsigned src_pitch, unsigned idx)
{				/* main frame or sub frame */
...
int disp_rot_src_offset(int pln, int boff_w, int boff_h)
{
	...
	/* address offset */
	tmp_0 = ((flip == 0) && (rotation == 1)) || ((flip == 0) && (rotation == 2)) || ((flip == 1) && (rotation == 2))
	    || ((flip == 1) && (rotation == 3));
	tmp_1 = ((flip == 0) && (rotation == 2)) || ((flip == 0) && (rotation == 3)) || ((flip == 1) && (rotation == 0))
	    || ((flip == 1) && (rotation == 3));
...
void disp_rot_reg_src(ROT_SRC_FORMAT src_format,
		      ...
		      int src_height,
		      int src_pitch,
		      const struct DISP_REGION *clipRect, ROT_VDO_MODE src_vdo_mode, ROT_VDO_FIELD src_vdo_field)
{
...
void disp_rot_reg_reset(void)
{
	...
	while ((DISP_REG_GET(DISP_REG_ROT_MON_STA_1) & 0xfff00) != 0x100) {
		delay_cnt++;
		if (delay_cnt > 10000) {
			pr_err("[DDP] error, disp_rot_reg_reset() timeout!\n");
