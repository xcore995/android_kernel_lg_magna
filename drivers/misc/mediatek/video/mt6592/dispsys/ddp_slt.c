...
int ddp_mutex_lock(void)
{
	...
	/* enable mutex 0,2,3's intr, other mutex will be used by MDP */
	DISP_REG_SET(DISP_REG_CONFIG_MUTEX_INTEN, 0x0d0d);

	while (((DISP_REG_GET(DISP_REG_CONFIG_MUTEX(gMutexID_slt)) & DISP_INT_MUTEX_BIT_MASK) !=
		DISP_INT_MUTEX_BIT_MASK)) {
		cnt++;
		if (cnt > 10000) {
			DISP_MSG("[DDP] disp_path_get_mutex() timeout!\n");
...
int ddp_mutex_unlock(int id)
{
	...
	while ((DISP_REG_CONFIG_MUTEX(id) & DISP_INT_MUTEX_BIT_MASK) != 0) {
		cnt++;
		if (cnt > 1000 * 1000 * 10) {
			DISP_MSG("error: disp_mutex_unlock timeout!\n");
			disp_dump_reg(DISP_MODULE_CONFIG);
			return -1;
		}
	}
	...
	while (DISP_REG_GET(DISP_REG_CONFIG_MUTEX_INTSTA) & (1 << id) != (1 << id)) {
		cnt++;
		if (cnt > 10000) {
			DISP_MSG("[DDP] reg update timeout!\n");
			disp_dump_reg(DISP_MODULE_CONFIG);
...
int ddp_bitblt_verify(unsigned int dst_addr, unsigned int golden_addr, struct REGION_SLT region)
{
	...
	for (t = 0; t < size; t++) {
		if (*((unsigned char *)dst_addr + t) != *((unsigned char *)golden_addr + t) &&
		    *((unsigned char *)dst_addr + t) != *((unsigned char *)data_rgb888_64x64_golden2 + t)) {
			diff_cnt++;
			DISP_MSG("n=%d, gold=0x%x, real=0x%x, gold2=0x%x\n", t,
				 *((unsigned char *)golden_addr + t),
				 *((unsigned char *)dst_addr + t), *((unsigned char *)data_rgb888_64x64_golden2 + t));
		}
	}
	if (diff_cnt == 0) {
		DISP_MSG("ddp_bitblt_verify, wdma1 output result: success\n");
		...
		if (gSltBitbltVerify == 1) {
			DISP_MSG("wdma1 output result: fail\n");
			DISP_MSG("detail: : fail, %d, %d, %%%d\n", diff_cnt, size, diff_cnt * 100 / size);
			/* dump reg info */
			disp_dump_reg(DISP_MODULE_CONFIG);
			disp_dump_reg(DISP_MODULE_MUTEX);
			disp_dump_reg(DISP_MODULE_OVL);
...
int m4u_ddp_test(unsigned int srcPa, unsigned int dst, unsigned int dstPa)
{
	...
	for (i = 0; i < 2; i++) {
		...
		WDMAConfig(0,
			   ...
			   region.width,
			   region.height,
			   0, 0, region.width, region.height, woutputfmt, (unsigned int)dstPa, region.width, 1, 0);
		if (woutputfmt == eYUV_420_3P) {
			WDMAConfigUV(0,
				     dstPa + region.width * region.height,
				     dstPa + region.width * region.height * 5 / 4, region.width);
		}
		WDMAStart(0);
		DISP_MSG
		    ("before release mutex dump-------------------------------------------------------------------\n");
		/* disp_dump_reg(DISP_MODULE_CONFIG); */
		/* disp_dump_reg(DISP_MODULE_MUTEX); */
		/* disp_dump_reg(DISP_MODULE_OVL); */
		/* disp_dump_reg(DISP_MODULE_WDMA0); */
...
int slt_test_stream1(void)
{
	...
	pDst = dma_alloc_coherent(NULL, 64 * 64 * 4, (dma_addr_t *) &pDstPa, GFP_KERNEL);
	if (pDst == 0 || pDstPa == 0) {
		DISP_MSG("dma_alloc_coherent error!  dma memory not available.\n");
		return -1;
	} else {
	...
	for (i = 0; i < 2; i++) {
		...
		for (layer = 0; layer < 4; layer++) {
			...
			OVLLayerConfig(layer,	/* layer */
				       OVL_LAYER_SOURCE_MEM,	/* data source (0=memory) */
				       ovl_in_fmt, (unsigned int)pSrcPa,	/* addr */
				       region.x,	/* x */
				       region.y,	/* y */
				       region.width * bpp,	/* pitch, pixel number */
				       region.x,	/* x */
		...
		WDMAConfig(0,
			   ...
			   region.width,
			   region.height,
			   0, 0, region.width, region.height, woutputfmt, (unsigned int)pDstPa, region.width, 1, 0);
		if (woutputfmt == eYUV_420_3P) {
			WDMAConfigUV(0,
				     (unsigned int)(pDstPa + region.width * region.height),
				     (unsigned int)(pDstPa + region.width * region.height * 5 / 4), region.width);
		}
		WDMAStart(0);
		/* DISP_MSG("before release mutex dump----------------------------------------------------------\n"); */
		/* disp_dump_reg(DISP_MODULE_CONFIG); */
		/* disp_dump_reg(DISP_MODULE_MUTEX); */
		/* disp_dump_reg(DISP_MODULE_OVL); */
		/* disp_dump_reg(DISP_MODULE_WDMA0); */
		...
		/* DISP_MSG("after release mutex dump-----------------------------------------------------------\n"); */
	...
	if (i == 2)
		DISP_MSG("result: fail, 2nd bitblt fail!!\n");
