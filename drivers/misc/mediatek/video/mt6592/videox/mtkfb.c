...
void mtkfb_show_sem_cnt(void)
{
	MTKFB_INFO("[FB driver: sem cnt = %d, %d, %d. fps = %d, vsync_cnt = %d\n", sem_overlay_buffer_cnt,
		   sem_early_suspend_cnt, sem_flipping_cnt, lcd_fps, vsync_cnt);
	MTKFB_INFO("[FB driver: sem cnt = %d, %d, %d\n", sem_overlay_buffer.count, sem_early_suspend.count,
		   sem_flipping.count);
}
...
static int mtkfb_set_overlay_layer(struct fb_info *info, struct fb_overlay_layer *layerInfo, unsigned int locked)
{
	...
	MMProfileLogEx(MTKFB_MMP_Events.SetOverlayLayer, MMProfileFlagStart,
		       (id << 24) | (enable << 16) | layerInfo->next_buff_idx, (unsigned int)layerInfo->src_phy_addr);
	...
	if (layerInfo->src_phy_addr != NULL) {
		...
		job->input[id].address =
		    disp_sync_query_buffer_mva(job->group_id, layerInfo->layer_id,
					       (unsigned int)layerInfo->next_buff_idx);
	}
#else
	job->input[id].address = (unsigned int)layerInfo->src_phy_addr;
#endif
...
job->input[id].tdshp = layerInfo->isTdshp;
	...
	*/
	...
	if (MTK_FB_FORMAT_ARGB8888 == layerInfo->src_fmt || MTK_FB_FORMAT_ABGR8888 == layerInfo->src_fmt) {
		job->input[id].alpha_enable = TRUE;
	...
	if (layerInfo->src_width != layerInfo->tgt_width || layerInfo->src_height != layerInfo->tgt_height) {
		MTKFB_WRAN("OVL cannot support clip:src(%d,%d), dst(%d,%d)\n", layerInfo->src_width,
			   layerInfo->src_height, layerInfo->tgt_width, layerInfo->tgt_height);
	}
...
static int mtkfb_get_overlay_layer_info(struct fb_overlay_layer_info *layerInfo)
{
	...
	MMProfileLogEx(MTKFB_MMP_Events.LayerInfo[layerInfo->layer_id], MMProfileFlagPulse,
		       ...
		       (layerInfo->hw_idx << 16) + (layerInfo->next_en << 8) + (layerInfo->curr_en << 4) +
		       layerInfo->hw_en);
	return 0;
}
...
*/
	...
	if (atomic_read(&capture_ui_layer_only)) {
		...
		}
		{
			unsigned int i;
			for (i = 0; i < h_yres; i++) {
				memcpy((void *)(pvbuf + i * w_xres * pixel_bpp), (void *)(fbv + i * w_fb * pixel_bpp),
				       w_xres * pixel_bpp);
			}
		}
		iounmap((void *)fbv);
	...
	case MTKFB_GET_DISPLAY_IF_INFORMATION:
		{
			...
			dispif_info[displayid].vsyncFPS = lcd_fps;
	...
	case MTKFB_SET_VIDEO_LAYERS:
		{
			...
			if (copy_from_user(&layerInfo, (void __user *)arg, sizeof(layerInfo))) {
				MTKFB_WRAN("[FB]: copy_from_user failed! line:%d\n", __LINE__);
				MMProfileLogMetaString(MTKFB_MMP_Events.SetVideoLayers, MMProfileFlagEnd,
				...
				is_ipoh_bootup = false;
				MMProfileLogStructure(MTKFB_MMP_Events.SetVideoLayers, MMProfileFlagEnd, layerInfo,
						      struct mmp_fb_overlay_layers);
			}
	...
	case MTKFB_SET_MULTIPLE_LAYERS:
		{
			...
			if (copy_from_user(&layerInfo, (void __user *)arg, sizeof(layerInfo))) {
				...
				for (i = 0; i < HW_OVERLAY_COUNT; ++i) {
					...
					if (is_early_suspended) {
						MTKFB_WRAN("in early suspend layer(0x%x),idx(%d)!\n",
							   layerId << 16 | layerInfo[i].layer_enable,
				...
				is_ipoh_bootup = false;
				MMProfileLogStructure(MTKFB_MMP_Events.SetMultipleLayers, MMProfileFlagEnd, layerInfo,
						      struct mmp_fb_overlay_layers);
			}
	...
	case MTKFB_SLT_AUTO_CAPTURE:
		{
			...
			if (copy_from_user(&config, argp, sizeof(config))) {
				...
				MTKFB_INFO("format bit 0x%x buf 0x%x width 0x%x ,height 0x%x\n", config.format,
					   (unsigned int)config.outputBuffer, config.wdma_width, config.wdma_height);
	...
	case DISP_IOCTL_CREATE_SESSION:
		{
			...
			MMProfileLogStructure(MTKFB_MMP_Events.CreateSession, MMProfileFlagStart, &config,
					      disp_session_config);
			...
			MMProfileLogEx(MTKFB_MMP_Events.CreateSession, MMProfileFlagEnd, config.session_id,
				       disp_config.session_id);
			return r;
		}
	...
	case DISP_IOCTL_DESTROY_SESSION:
		{
			...
			MMProfileLogStructure(MTKFB_MMP_Events.DestroySession, MMProfileFlagStart, &config,
					      disp_session_config);
			if (disp_destroy_session(&config) != DCP_STATUS_OK)
				r = -EFAULT;

			MMProfileLogEx(MTKFB_MMP_Events.DestroySession, MMProfileFlagEnd, config.session_id,
				       disp_config.session_id);
			return r;
		}
	...
	case DISP_IOCTL_TRIGGER_SESSION:
		{
			...
			MMProfileLogStructure(MTKFB_MMP_Events.TriggerSession, MMProfileFlagStart, &config,
					      disp_session_config);

/* #if defined(CONFIG_SINGLE_PANEL_OUTPUT) */
			/* FIXME: If WFD, ScreenRecord, don't do this */
			if (is_early_suspended
			    && config.session_id != MAKE_DISP_SESSION(DISP_SESSION_DECOUPLE_MODE, DISP_SESSION_MEMORY,
								      2)) {
				int i = 0;
				for (i = 0; i < HW_OVERLAY_COUNT; i++) {
...
/* #endif */
			...
			MMProfileLogEx(MTKFB_MMP_Events.TriggerSession, MMProfileFlagEnd, config.session_id,
				       disp_config.session_id);
			return r;
		}
	...
	case DISP_IOCTL_PREPARE_INPUT_BUFFER:
		{
			...
			MMProfileLogStructure(MTKFB_MMP_Events.PrepareInput, MMProfileFlagStart, &info,
					      disp_buffer_info);
			if (info.layer_en) {
	...
	case DISP_IOCTL_PREPARE_OUTPUT_BUFFER:
		{
			...
			MMProfileLogStructure(MTKFB_MMP_Events.PrepareOutput, MMProfileFlagStart, &info,
					      disp_buffer_info);
			if (info.layer_en) {
				if (disp_sync_prepare_buffer(&info) != SYNC_STATUS_OK) {
					info.fence_fd = DISP_INVALID_FENCE_FD;	/* invalid fd */
	...
	case DISP_IOCTL_SET_INPUT_BUFFER:
		{
			...
			MMProfileLogStructure(MTKFB_MMP_Events.SetInput, MMProfileFlagStart, &layerInfo,
	...
	case DISP_IOCTL_SET_OUTPUT_BUFFER:
		{
			...
			MMProfileLogStructure(MTKFB_MMP_Events.SetOutput, MMProfileFlagStart, &layerInfo,
	...
	case DISP_IOCTL_GET_SESSION_INFO:
		{
			...
			MMProfileLogStructure(MTKFB_MMP_Events.GetDispInfo, MMProfileFlagPulse, &info,
					      disp_session_info);
...
/* LGE_BSP_COMMON LGE_CHANGE_E 141030 jongwoo82.lee : AT%LCD */
	default:
		MTKFB_WRAN("mtkfb_ioctl Not support, info=0x%08x, cmd=0x%08x, arg=0x%08x, num=%d\n", (unsigned int)info,
			   (unsigned int)cmd, (unsigned int)arg, _IOC_NR(cmd));
		return -EINVAL;
	}
}
...
int m4u_reclaim_mva_callback_ovl(int moduleID, unsigned int va, unsigned int size, unsigned int mva)
{
	...
	for (id = 0; id < DDP_OVL_LAYER_MUN; id++) {
		...
		if (cached_layer_config[id].addr >= mva && cached_layer_config[id].addr < (mva + size)) {
...
static void mtkfb_fb_8888_to_8888(struct fb_info *fb_info)//NU
{
...
static void mtkfb_fb_8888_to_888(void* fb_va)//NU
{
...
static void mtkfb_fb_565_to_uyvy_on_meta_mode(void *fb_va)
{
	unsigned int xres = DISP_GetScreenWidth();
	unsigned int yres = DISP_GetScreenHeight();
	unsigned int x_virtual = ALIGN_TO(xres, disphal_get_fb_alignment());
	...
	unsigned char *d = (unsigned char *)(fb_va + DISP_GetFBRamSize() + DAL_GetLayerSize());
	unsigned short src_rgb565 = 0;
	unsigned short *s1 = (unsigned short *)(fb_va + fbsize * 2);
	unsigned short src_boot_mode = 0;
	int j = 0;
	int k = 0;
...
static int mtkfb_probe(struct device *dev)
{
	...
	if (!fbdev->layer_format) {
		MTKFB_WRAN("[mtkfb.c FB driver] vmalloc failed, %d\n", __LINE__);
		r = -ENOMEM;
		goto cleanup;
	}
	...
	fbdev->fb_size_in_byte = MTK_FB_SIZEV;
	{
		...
		if (fb_address_lk) {
			MTKFB_WRAN("[wwy]fbsize_copy=%d\n", fbsize_copy);
			memcpy((void *)fbdev->fb_va_base, (void *)(va_lk), fbsize_copy);
			iounmap((void *)va_lk);
		}
#ifdef CONFIG_MTK_GMO_RAM_OPTIMIZE
		MTKFB_WRAN("[MTKFB] max RAM = %x, actual RAM = %x\n", get_max_DRAM_size(), get_actual_DRAM_size());
		if (get_max_DRAM_size() < get_actual_DRAM_size()) {
			void *fb_va_base_lk;
			unsigned int fb_pa_base_lk =
			    get_phys_offset() + get_actual_DRAM_size() - (res->end - res->start + 1);

			MTKFB_WRAN("Boot logo address shift, copy framebuffer from 0x%08X to 0x%08X\n", fb_pa_base_lk,
				   fbdev->fb_pa_base);
...
void mtkfb_clear_lcm(void)
{
	...
	for (i = 0; i < DDP_OVL_LAYER_MUN; i++) {
		if (i == ASSERT_LAYER && isAEEEnabled) {
...
{
#if defined(CONFIG_MTK_HDMI_SUPPORT)
	MTKFB_WRAN("[FB Driver] enter hdmi_disc_disp_path\n");

	if (is_early_suspended) {
		MTKFB_WRAN("hdmi_disc_disp_path suspended %d\n", __LINE__);
		return 0;
	}
	...
	for (i = 0; i < HW_OVERLAY_COUNT; i++) {
		...
		MTKFB_WRAN("[FB Driver] layer%d release fences\n", i);
	}
#endif
	DISP_CHECK_RET(DISP_PanelEnable(FALSE));
	DISP_CHECK_RET(DISP_PowerEnable(FALSE));
	...
	MTKFB_WRAN("[FB Driver] hdmi_disc_disp_path\n");
#endif
...
{
#if defined(CONFIG_MTK_HDMI_SUPPORT)
	MTKFB_WRAN("[FB Driver] enter hdmi_conn_disp_path\n");
	...
	MTKFB_WRAN("[FB Driver] hdmi_conn_disp_path\n");
#endif
	return 0;
}
