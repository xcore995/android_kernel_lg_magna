...
/* hrtimer_try_to_cancel(&hrtimer_vsync_dpi); */
	...
	if (status.VSYNC && counter) {
		DISP_LOG_PRINT(ANDROID_LOG_ERROR, "DPI1",
...
#if ENABLE_DPI1_INTERRUPT
	if (request_irq(MT6589_DPI_IRQ_ID, _DPI1_InterruptHandler, IRQF_TRIGGER_LOW, "mtkdpi", NULL) < 0) {
...
DPI_STATUS DPI1_Init_PLL(HDMI_VIDEO_RESOLUTION resolution)
{
	...
	default:
		{
			DISP_DRV_WRAN("[DPI1] not supported format, %s, %d, format = %d\n", __func__, __LINE__,
				      resolution);
			break;
		}
	}
...
DPI_STATUS DPI1_ConfigVsync(DPI_POLARITY polarity, UINT32 pulseWidth, UINT32 backPorch, UINT32 frontPorch)
{
...
DPI_STATUS DPI1_ConfigHsync(DPI_POLARITY polarity, UINT32 pulseWidth, UINT32 backPorch, UINT32 frontPorch)
{
	...
	if (bpp == 32 && dpi_fb_bpp == 24) {
		...
		if (0 == strncmp(CONFIG_MTK_LCM_PHYSICAL_ROTATION, "180", 3)) {
			...
			for (i = 0; i < w * h; i++) {
				t = fbvt[i];
				*(unsigned int *)(pvbuf + (pix_count - i) * 4) =
				    0xff000000 | ((t & 0x001F) << 3) | ((t & 0x07E0) << 5) | ((t & 0xF800) << 8);
			}
		} else {
			for (i = 0; i < w * h; i++) {
				t = fbvt[i];
...
DPI_STATUS DPI1_OutputSetting(DPI_OUTPUT_BIT_NUM outBitNum, BOOL outBitSwap, DPI_OUTPUT_CHANNEL_SWAP outChSwap,
