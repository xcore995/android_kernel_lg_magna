...
{
	int z = 0;
	DISP_DRV_WRAN("sxk=>[PRINT_RECORD] type is %d\n", record_head->type);
	for (z = 0; z < record_head->ins_num; z++)
		DISP_DRV_WRAN("sxk=>[PRINT_RECORD]data is 0x%x\n", record_head->ins_array[z]);
}
#endif
static void record_list_init(void)
{
	DISP_DRV_WRAN("sxk=>call record_list_init!!\n");
...
static void print_from_head_to_tail(void)
{
	...
	DISP_DRV_WRAN("sxk=>print_from_head_to_tail:START\n");
	while (tmp != NULL) {
		DISP_DRV_WRAN("sxk=>record type is %d;record ins_num is %d\n", tmp->type, tmp->ins_num);
		for (z = 0; z < tmp->ins_num; z++)
			DISP_DRV_WRAN("sxk=>record conten is :0x%x\n", tmp->ins_array[z]);
		tmp = tmp->next;
	}
	DISP_DRV_WRAN("sxk=>print_from_head_to_tail:END\n");
}
...
static void print_from_head_to_tail_backup(void)
{
	...
	DISP_DRV_WRAN("sxk=>print_from_head_to_tail_BACKUP...........:START\n");
	while (tmp != NULL) {
		DISP_DRV_WRAN("sxk=>record type is %d;record ins_num is %d\n", tmp->type, tmp->ins_num);
		for (z = 0; z < tmp->ins_num; z++)
			DISP_DRV_WRAN("sxk=>record conten is :0x%x\n", tmp->ins_array[z]);
	...
	DISP_DRV_WRAN("sxk=>print_from_head_to_tail_BACKUP..................:END\n");
}
...
{
	CONFIG_RECORD *tmp = record_head;
	DISP_DRV_WRAN("sxk=>execute_cmd:START\n");
	while (tmp != NULL) {
		switch (tmp->type) {
		case RECORD_CMD:
	...
	DISP_DRV_WRAN("sxk=>execute_cmd:END\n");
...
static void fbconfig_reset_lcm_setting(void)
{
	...
	if (lcm_params->dsi.mode != CMD_MODE) {
		if (down_interruptible(&sem_early_suspend)) {
			DISP_DRV_WRAN("sxk=>can't get semaphore in fbconfig_reset_lcm_setting()\n");
			return;
		}
		...
		if (down_interruptible(&sem_early_suspend)) {
			DISP_DRV_WRAN("sxk=>can't get semaphore in execute_cmd()\n");
			return;
		}
		lcm_drv->init();
		up(&sem_early_suspend);
...
#if 1
		if (lcm_params->dsi.mode != CMD_MODE) {
			DISP_DRV_WRAN("sxk=>01will exec cmd!! in fbconfig_release()\n");
...
#endif
/*free the memory .....*/
		DISP_DRV_WRAN("sxk=>will free the memory\n");
		free_list_memory();
		return 0;
	}
}
