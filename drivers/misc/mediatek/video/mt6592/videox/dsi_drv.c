...
static volatile unsigned int teTryNum;
static volatile bool isTeSetting;
static volatile bool dsiTeEnable;
...
static bool isDSIStartinInterrupt;
...
static irqreturn_t _DSI_InterruptHandler(int irq, void *dev_id)
{
	...
	if (dsi_log_on)
		DISP_DRV_WRAN("DSI IRQ, value = 0x%x!!\n", AS_UINT32(&status));
...
/* long long ret; */
	if (EnableVSyncLog)
		DISP_DRV_WRAN("[DSI] VSync0\n");
	if (wait_dsi_vsync) {
		dsi_vsync = true;

		if (EnableVSyncLog)
			DISP_DRV_WRAN("[DSI] VSync1\n");
		wake_up_interruptible(&_vsync_wait_queue);
	}
/* ret = hrtimer_forward_now(timer, ktime_set(0, VSYNC_US_TO_NS(vsync_timer))); */
/* DISP_DRV_WRAN("hrtimer callback\n"); */
...
static void _WaitForEngineNotBusy(void)
{
	...
	if (lcm_params->dsi.mixmode_enable) {
		...
		if (dsi_log_on)
			pr_debug("DSI %s: dsi_wait_irq_count++=%d\n", __func__, irq_wait_cnt);
	}
#endif
	timeOut = 200;
	...
	if (lcm_params->dsi.mixmode_enable) {
		...
		if (irq_wait_cnt == 0) {
			...
			if (dsi_log_on)
				pr_debug("DSI %s: disable CMD_DONR_INT\n", __func__);
		}
	}
#endif
#else
	...
	while (_IsEngineBusy()) {
		mdelay(10);
		/*DISP_DRV_WRAN("xuecheng, dsi wait\n"); */
		if (--timeOut < 0) {
...
#endif /* CONFIG_MIXMODE_FOR_INCELL */
...
static void DSI_WaitBtaTE(void)
{
	...
	MMProfileLogEx(MTKFB_MMP_Events.DSICmd, MMProfileFlagStart, *(unsigned int *)(&DSI_CMDQ_REG->data[0]),
	...
	ret = wait_event_interruptible_timeout(_dsi_wait_bta_te, is_Wait_Te_Done(), WAIT_TIMEOUT);
	...
	if (0 == ret) {		/* Wait BTA TE Timeout */
		...
		MMProfileLogEx(MTKFB_MMP_Events.DSICmd, MMProfileFlagEnd, *(unsigned int *)(&DSI_CMDQ_REG->data[2]),
...
#if ENABLE_DSI_INTERRUPT
	ret = wait_event_interruptible_timeout(_dsi_wait_ext_te, is_Wait_Te_Done(), WAIT_TIMEOUT);
...
/* unsigned int fmod = 30;//Fmod = 30KHz by default */
	...
	DISP_DRV_WRAN
	    ("DSI_PHY_clk_adjusting: efuse register 0x10206180=0x%x, bit28_31=0x%x, bit24_27=0x%x, bit20_23=0x%x,",
	     m_hw_res3, temp1, temp2, temp3);
	DISP_DRV_WRAN("bit16_19=0x%x, bit12_15=0x%x\n", temp4, temp5);
	DISP_DRV_WRAN
	    ("before efuse adjust, DSI_CLK_REG = 0x%x, DSI_DAT0_REG=0x%x, DSI_DAT1_REG=0x%x,",
	...
	DISP_DRV_WRAN("DSI_DAT2_REG=0x%x, DSI_DAT3_REG=0x%x\n",
		INREG32(&DSI_PHY_REG->MIPITX_DSI0_DATA_LANE2), INREG32(&DSI_PHY_REG->MIPITX_DSI0_DATA_LANE3));
	...
	if (0 != data_Rate) {
		if (data_Rate > 1250) {
			DISP_DRV_WRAN("[dsi_drv.c error]Data Rate exceed limitation\n");
			ASSERT(0);
			...
			DISP_DRV_WRAN("[dsi_drv.c Error]: dataRate is too low,%d!!!\n", __LINE__);
			ASSERT(0);
		}
		/* PLL txdiv config */
		switch (txdiv) {
		...
		OUTREGBIT(MIPITX_DSI_PLL_CON2_REG, DSI_PHY_REG->MIPITX_DSI_PLL_CON2, RG_DSI0_MPPLL_SDM_PCW_H,
			  (pcw & 0x7F));
...
/* pmod = 433.33; */
		if (1 != lcm_params->dsi.ssc_disable) {
			...
			OUTREGBIT(MIPITX_DSI_PLL_CON1_REG, DSI_PHY_REG->MIPITX_DSI_PLL_CON1, RG_DSI0_MPPLL_SDM_SSC_PRD,
				  0x1B1);
			...
			DISP_DRV_WRAN("[dsi_drv.c] PLL config:data_rate=%d,txdiv=%d,pcw=%d,delta1=%d,pdelta1=0x%x\n",
				      data_Rate, txdiv, INREG32(&DSI_PHY_REG->MIPITX_DSI_PLL_CON2), delta1, pdelta1);
		}
	} else {
		OUTREGBIT(MIPITX_DSI_PLL_CON0_REG, DSI_PHY_REG->MIPITX_DSI_PLL_CON0, RG_DSI0_MPPLL_TXDIV0,
			  lcm_params->dsi.pll_div1);
		OUTREGBIT(MIPITX_DSI_PLL_CON0_REG, DSI_PHY_REG->MIPITX_DSI_PLL_CON0, RG_DSI0_MPPLL_TXDIV1,
			  lcm_params->dsi.pll_div2);

		OUTREGBIT(MIPITX_DSI_PLL_CON2_REG, DSI_PHY_REG->MIPITX_DSI_PLL_CON2, RG_DSI0_MPPLL_SDM_PCW_H,
			  ((lcm_params->dsi.fbk_div) << 2));
	...
	OUTREGBIT(MIPITX_DSI0_CLOCK_LANE_REG, DSI_PHY_REG->MIPITX_DSI0_CLOCK_LANE, RG_DSI0_LNTC_RT_CODE,
		  (temp1 == 0) ? 0x8 : temp1);
	...
	if (lcm_params->dsi.LANE_NUM > 0) {
		OUTREGBIT(MIPITX_DSI0_DATA_LANE0_REG, DSI_PHY_REG->MIPITX_DSI0_DATA_LANE0, RG_DSI0_LNT0_RT_CODE,
			  (temp2 == 0) ? 0x8 : temp2);
	...
	if (lcm_params->dsi.LANE_NUM > 1) {
		OUTREGBIT(MIPITX_DSI0_DATA_LANE1_REG, DSI_PHY_REG->MIPITX_DSI0_DATA_LANE1, RG_DSI0_LNT1_RT_CODE,
			  (temp3 == 0) ? 0x8 : temp3);
	...
	if (lcm_params->dsi.LANE_NUM > 2) {
		OUTREGBIT(MIPITX_DSI0_DATA_LANE2_REG, DSI_PHY_REG->MIPITX_DSI0_DATA_LANE2, RG_DSI0_LNT2_RT_CODE,
			  (temp4 == 0) ? 0x8 : temp4);
	...
	if (lcm_params->dsi.LANE_NUM > 3) {
		OUTREGBIT(MIPITX_DSI0_DATA_LANE3_REG, DSI_PHY_REG->MIPITX_DSI0_DATA_LANE3, RG_DSI0_LNT3_RT_CODE,
			  (temp5 == 0) ? 0x8 : temp5);
		OUTREGBIT(MIPITX_DSI0_DATA_LANE3_REG, DSI_PHY_REG->MIPITX_DSI0_DATA_LANE3, RG_DSI0_LNT3_LDOOUT_EN, 1);
	}
	DISP_DRV_WRAN
	    ("after efuse adjust, DSI_CLK_REG = 0x%x, DSI_DAT0_REG=0x%x, DSI_DAT1_REG=0x%x,",
	     INREG32(&DSI_PHY_REG->MIPITX_DSI0_CLOCK_LANE),
	     INREG32(&DSI_PHY_REG->MIPITX_DSI0_DATA_LANE0),
	     INREG32(&DSI_PHY_REG->MIPITX_DSI0_DATA_LANE1));
	DISP_DRV_WRAN(" DSI_DAT2_REG=0x%x, DSI_DAT3_REG=0x%x\n",
...
void DSI_PHY_TIMCONFIG(LCM_PARAMS *lcm_params)
{
	...
	if (0 != lcm_params->dsi.PLL_CLOCK) {
		...
		default:
			DISP_DRV_WRAN("div1 should be less than 4!!\n");
			div1 = 4;
			break;
		}
		...
		case 3:
			...
			break;
		default:
			DISP_DRV_WRAN("div2 should be less than 4!!\n");
			div2 = 4;
			break;
		}
		...
		default:
			DISP_DRV_WRAN("pre_div should be less than 4!!\n");
			pre_div = 4;
			break;
		}
		...
		default:
			DISP_DRV_WRAN("post_div should be less than 4!!\n");
			post_div = 4;
			break;
		}
		...
		default:
			DISP_DRV_WRAN("fbk_sel should be less than 4!!\n");
			fbk_sel = 4;
			break;
		}
...
#define NS_TO_CYCLE(n, c)	((n) / (c))
	...
	timcon0.HS_PRPR =
	    (lcm_params->dsi.HS_PRPR == 0) ? NS_TO_CYCLE((0x40 + 0x5 * ui), cycle_time) : lcm_params->dsi.HS_PRPR;
	/* HS_PRPR can't be 1. */
	if (timcon0.HS_PRPR == 0)
		timcon0.HS_PRPR = 1;

	timcon0.HS_ZERO =
	    (lcm_params->dsi.HS_ZERO == 0) ? NS_TO_CYCLE((0xC8 + 0x0a * ui), cycle_time) : lcm_params->dsi.HS_ZERO;
	...
	timcon1.DA_HS_EXIT =
	    (lcm_params->dsi.DA_HS_EXIT == 0) ? NS_TO_CYCLE((0x3c + 0x80 * ui),
							    cycle_time) : lcm_params->dsi.DA_HS_EXIT;

	timcon2.CLK_TRAIL =
	    ((lcm_params->dsi.CLK_TRAIL == 0) ? NS_TO_CYCLE(0x64, cycle_time) : lcm_params->dsi.CLK_TRAIL) + 0x0a;
	...
	timcon3.CLK_HS_PRPR =
	    (lcm_params->dsi.CLK_HS_PRPR == 0) ? NS_TO_CYCLE(0x40, cycle_time) : lcm_params->dsi.CLK_HS_PRPR;
	...
	timcon3.CLK_HS_POST =
	    (lcm_params->dsi.CLK_HS_POST == 0) ? NS_TO_CYCLE((0x3c + 0x80 * ui),
							     cycle_time) : lcm_params->dsi.CLK_HS_POST;
	...
	DISP_DRV_WRAN("%s, 0x%08x,0x%08x,0x%08x,0x%08x\n", __func__, INREG32(DSI_BASE + 0x110),
		      INREG32(DSI_BASE + 0x114), INREG32(DSI_BASE + 0x118), INREG32(DSI_BASE + 0x11c));
}
...
bool DSI_esd_check(void)
{
	...
	ret = wait_event_interruptible_timeout(_dsi_wait_vm_done_queue, !_IsEngineBusy(), WAIT_TIMEOUT);
	if (0 == ret) {
		DISP_DRV_WRAN(ANDROID_LOG_WARN, "DSI", " Wait for DSI engine read ready timeout!!!\n");
...
bool DSI_handle_int_TE(void)
{
	...
	if (DSI_REG->DSI_STATE_DBG3.TCON_STATE == DSI_VDO_VFP_STATE) {
		...
		if ((DSI_REG->DSI_STATE_DBG3.TCON_STATE == DSI_VDO_VFP_STATE)
			...
			/* Can't do int. TE check while INUSE FB number is not 0 because later
			disable/enable DPI will set INUSE FB to number 0. */
			...
			while (DSI_REG->DSI_INTSTA.RD_RDY == 0) {	/* polling RD_RDY */
				if (get_current_time_us() - dsi_current_time > _dsiContext.vfp_period_us) {
					DISP_DRV_WRAN(ANDROID_LOG_WARN, "DSI",
...
void DSI_handle_noncont_clk(void)
{
	...
	case DSI_VDO_VSA_VS_STATE:
		while (DSI_REG->DSI_STATE_DBG3.TCON_STATE != DSI_VDO_VSA_HS_STATE) {
			if (get_current_time_us() - dsi_current_time > _dsiContext.vsa_vs_period_us) {
				DISP_DRV_WRAN(ANDROID_LOG_WARN, "DSI", " Wait for %x state timeout %d (us)!!!\n",
	...
	case DSI_VDO_VBP_STATE:
		DISP_DRV_WRAN(ANDROID_LOG_WARN, "DSI", "Can't do clock switch in DSI_VDO_VBP_STATE !!!\n");
		break;
	...
	default:
		DISP_DRV_WRAN(ANDROID_LOG_ERROR, "DSI", "invalid state = %x\n", state);
		return;
	}
...
void DSI_set_cmdq_V2(unsigned cmd, unsigned char count, unsigned char *para_list, unsigned char force_update)
{
	...
	if (0 != DSI_REG->DSI_MODE_CTRL.MODE) {	/* not in cmd mode */
		...
		DISP_DRV_WRAN("set cmdq in VDO mode in set_cmdq_V2\n");
		if (cmd < 0xB0) {
			if (count > 1) {
				vm_cmdq.LONG_PKT = 1;
...
void DSI_set_cmdq_V3(LCM_setting_table_V3 *para_tbl, unsigned int size, unsigned char force_update)
{
	...
	do {
		...
		if (0 != DSI_REG->DSI_MODE_CTRL.MODE) {	/* not in cmd mode */
			DSI_VM_CMD_CON_REG vm_cmdq;
			OUTREG32(&vm_cmdq, AS_UINT32(&DSI_REG->DSI_VM_CMD_CON));
			DISP_DRV_WRAN("set cmdq in VDO mode\n");
			if (count > 1) {
				vm_cmdq.LONG_PKT = 1;
...
/* for (i = 0; i < queue_size; i++) */
/* DISP_DRV_WRAN("[DISP] - kernel - DSI_set_cmdq. DSI_CMDQ+%04x : 0x%08x\n", i*4, INREG32(DSI_BASE + 0x180 + i*4)); */
...
DSI_STATUS DSI_TXRX_Control(bool cksm_en,
			    ...
			    bool null_packet_en,
			    bool err_correction_en, bool dis_eotp_en, bool hstx_cklp_en, unsigned int max_return_size)
{
...
#define LINE_PERIOD_US				(8 * line_byte * _dsiContext.bit_time_ns / 1000)
	...
	line_byte = (lcm_params->dsi.horizontal_sync_active
		     ...
		     + lcm_params->dsi.horizontal_frontporch + lcm_params->dsi.horizontal_active_pixel) * dsiTmpBufBpp;

	if (lcm_params->dsi.mode == SYNC_EVENT_VDO_MODE || lcm_params->dsi.mode == BURST_VDO_MODE) {
		ASSERT((lcm_params->dsi.horizontal_backporch + lcm_params->dsi.horizontal_sync_active) * dsiTmpBufBpp >
		       9);
		horizontal_backporch_byte =
		    ((lcm_params->dsi.horizontal_backporch + lcm_params->dsi.horizontal_sync_active) * dsiTmpBufBpp -
		     10);
	} else {
...
/* horizontal_frontporch_byte -= ((300/dsi_cycle_time) * lcm_params->dsi.LANE_NUM); */
	...
	DISP_LOG_PRINT(ANDROID_LOG_INFO, "DSI", "[DISP] kernel - VSA : %d %d(us)\n", DSI_REG->DSI_VSA_NL,
		       (_dsiContext.vsa_vs_period_us + _dsiContext.vsa_hs_period_us + _dsiContext.vsa_ve_period_us));
	DISP_LOG_PRINT(ANDROID_LOG_INFO, "DSI", "[DISP] kernel - VBP : %d %d(us)\n", DSI_REG->DSI_VBP_NL,
		       _dsiContext.vbp_period_us);
...
UINT32 DSI_dcs_read_lcm_reg(UINT8 cmd)
{
	...
	do {
		...
		while (DSI_REG->DSI_STA.BUF_UNDERRUN || DSI_REG->DSI_STA.ESC_ENTRY_ERR || DSI_REG->DSI_STA.LPDT_SYNC_ERR
		       || DSI_REG->DSI_STA.CTRL_ERR || DSI_REG->DSI_STA.CONTENT_ERR) {
...
/* unsigned int DSI_DBG6_Status_bak = DSI_DBG6_Status; */
	...
	DISP_LOG_PRINT(ANDROID_LOG_INFO, "DSI", "DSI_STATE_DBG6=0x%08x, count=%d, means: [%s]\n", DSI_DBG6_Status,
		       count, DSI_DBG_STATUS_DESCRIPTION[count]);

	DISP_LOG_PRINT(ANDROID_LOG_INFO, "DSI", "---------- Start dump DSI registers ----------\n");

	for (i = 0; i < sizeof(DSI_REGS); i += 16) {
		DISP_LOG_PRINT(ANDROID_LOG_INFO, "DSI", "DSI+%04x : 0x%08x	0x%08x	0x%08x	0x%08x\n", i,
			       INREG32(DSI_BASE + i), INREG32(DSI_BASE + i + 0x4), INREG32(DSI_BASE + i + 0x8),
			       INREG32(DSI_BASE + i + 0xc));
	}

	for (i = 0; i < sizeof(DSI_CMDQ_REGS); i += 16) {
		DISP_LOG_PRINT(ANDROID_LOG_INFO, "DSI", "DSI_CMD+%04x : 0x%08x	0x%08x	0x%08x	0x%08x\n", i,
			       INREG32((DSI_BASE + 0x180 + i)), INREG32((DSI_BASE + 0x180 + i + 0x4)),
			       INREG32((DSI_BASE + 0x180 + i + 0x8)), INREG32((DSI_BASE + 0x180 + i + 0xc)));
	}

	for (i = 0; i < sizeof(DSI_PHY_REGS); i += 16) {
		DISP_LOG_PRINT(ANDROID_LOG_INFO, "DSI", "DSI_PHY+%04x : 0x%08x	  0x%08x  0x%08x  0x%08x\n", i,
			       INREG32((MIPI_CONFIG_BASE + i)), INREG32((MIPI_CONFIG_BASE + i + 0x4)),
			       INREG32((MIPI_CONFIG_BASE + i + 0x8)), INREG32((MIPI_CONFIG_BASE + i + 0xc)));
	}
	UFOE_DumpRegisters();
	return DSI_STATUS_OK;
...
DSI_STATUS DSI_Change_CLK(unsigned int clk)
{
	...
	for (lcm_params_for_clk_setting.dsi.pll_div2 = 15; lcm_params_for_clk_setting.dsi.pll_div2 > 0;
	     lcm_params_for_clk_setting.dsi.pll_div2--) {
		for (lcm_params_for_clk_setting.dsi.pll_div1 = 0; lcm_params_for_clk_setting.dsi.pll_div1 < 39;
...
DSI_STATUS DSI_Capture_Framebuffer(unsigned int pvbuf, unsigned int bpp, bool cmd_mode)
{
	...
	DISP_DRV_WRAN("enter DSI_Capture_FB!\n");
	...
	if (bpp == 32)
		...
		DISP_DRV_WRAN("DSI_Capture_FB, fb color format not support\n");

	DISP_DRV_WRAN("before alloc MVA: va = 0x%x, size = %d\n", pvbuf,
		      lcm_params->height * lcm_params->width * bpp / 8);
	ret = m4u_alloc_mva(DISP_WDMA, pvbuf, lcm_params->height * lcm_params->width * bpp / 8, 0, 0, &mva);
	if (ret != 0) {
		DISP_DRV_WRAN("m4u_alloc_mva() fail!\n");
		return DSI_STATUS_OK;
	}
	DISP_DRV_WRAN("addr=0x%x, format=%d\n", mva, mem_out.outFormat);

	m4u_dma_cache_maint(DISP_WDMA,
			    (const void *)pvbuf, lcm_params->height * lcm_params->width * bpp / 8, DMA_BIDIRECTIONAL);
...
DSI_STATUS DSI_WaitForDispFinish(void)
{
	...
	if (0 == ret) {
		DISP_DRV_WRAN(ANDROID_LOG_WARN, "DSI", " Wait for disp finished timeout\n");
		return DSI_STATUS_ERROR;
	}
	return DSI_STATUS_OK;
}
...
{
	pr_debug("DSI mode: new(%d), previous(%d)\n", status, dsi_incell_mix_mode_status);
...
void DSI_PHY_clk_change(LCM_PARAMS *lcm_params)
{
	...
	if (0 != data_Rate) {	/* if lcm_params->dsi.PLL_CLOCK=0, use other method */
		if (data_Rate > 1250) {
			pr_err("[dsi_drv.c error]Data Rate exceed limitation\n");
			ASSERT(0);
		} else if (data_Rate >= 500)
			txdiv = 1;
		else if (data_Rate >= 250)
		...
		*/
		...
		OUTREGBIT(MIPITX_DSI_PLL_CON2_REG, DSI_PHY_REG->MIPITX_DSI_PLL_CON2,
			RG_DSI0_MPPLL_SDM_PCW_8_15, ((256*(256*(data_Rate*txdiv%13)%13)/13) & 0xFF));
		OUTREGBIT(MIPITX_DSI_PLL_CON2_REG, DSI_PHY_REG->MIPITX_DSI_PLL_CON2,
			RG_DSI0_MPPLL_SDM_PCW_0_7, ((256*(256*(256*(data_Rate*txdiv%13)%13)%13)/13) & 0xFF));
...
DSI_STATUS fbconfig_DSI_set_CLK(unsigned int clk)
{
	...
	DISP_DRV_WRAN("sxk==>fbconfig_DSI_set_CLK:%d\n", clk);
	if (lcm_params->dsi.PLL_CLOCK != 0) {	/* PLL CLOCK FORMAT */
		...
		DISP_DRV_WRAN("dsi_set_clk==>fbconfig_DSI_set_CLK:will wait!!\n");

		_WaitForEngineNotBusy();	/* cmd mode */
		DISP_DRV_WRAN("dsi_set_clk==>will fbconfig_DSI_set_CLK:%d\n", clk);
		...
		DISP_DRV_WRAN("dsi_set_clk==>fbconfig_DSI_set_CLK:will wait!!\n");
		_WaitForEngineNotBusy();	/* cmd mode */
		DISP_DRV_WRAN("fbconfig_DSI_set_CLK:div1=>%d div2=>%d fbk_div=>%d\n", fb_lcm_params.dsi.pll_div1,
			      fb_lcm_params.dsi.pll_div2, fb_lcm_params.dsi.fbk_div);
		DSI_PHY_clk_setting(&fb_lcm_params);
		DSI_PHY_TIMCONFIG(&fb_lcm_params);
...
{
	LCM_PARAMS fb_lcm_params;
	DISP_DRV_WRAN("sxk==>fbconfig_DSI_set_SSC:%d\n", ssc);

	if ((ssc > 8) || (ssc < 1))
		return DSI_STATUS_ERROR;
	memcpy((void *)&fb_lcm_params, (void *)lcm_params, sizeof(LCM_PARAMS));
	fb_lcm_params.dsi.ssc_range = ssc;
	DISP_DRV_WRAN("sxk==>fbconfig_DSI_set_SSC:will wait!!\n");

	_WaitForEngineNotBusy();	/* cmd mode */
	DISP_DRV_WRAN("sxk==>will fbconfig_DSI_set_SSC:%d\n", ssc);
...
void fbconfig_DSI_set_timing(MIPI_TIMING timing)
{
	...
	switch (timing.type) {
	case SSC_EN:
		OUTREGBIT(MIPITX_DSI_PLL_CON1_REG, DSI_PHY_REG->MIPITX_DSI_PLL_CON1, RG_DSI0_MPPLL_SDM_SSC_EN,
			  timing.value);
		break;
	case LPX:
	...
	case VBP:
		...
		break;
	default:
		DISP_DRV_WRAN("fbconfig dsi set timing :no such type!!\n");
	}
	DSI_DumpRegisters();
}
...
int fbconfig_get_dsi_ssc(void)
{
	if (fbconfig_dsi_ssc == 0)
		return (lcm_params->dsi.ssc_range) ? lcm_params->dsi.ssc_range : 5;
	else
