...
void DPI_MIPI_clk_setting(unsigned int mipi_pll_clk_ref, unsigned int mipi_pll_clk_div1, unsigned int mipi_pll_clk_div2)
{
	...
	DISP_DRV_WRAN("[DPI] MIPIPLL Exit\n");
}
...
#if ENABLE_DPI_INTERRUPT
	if (request_irq(MT_DISP_DPI0_IRQ_ID, _DPI_InterruptHandler, IRQF_TRIGGER_LOW, "mtkdpi", NULL) < 0) {
...
DPI_STATUS DPI_Init_PLL(unsigned int mipi_pll_clk_ref, unsigned int mipi_pll_clk_div1, unsigned int mipi_pll_clk_div2)
{
	...
	DISP_LOG_PRINT(ANDROID_LOG_ERROR, "DPI",
		"DPI_Init_PLL, clk_ref %x clk_div %d clk_div2 %d !!!\n",
...
DPI_STATUS DPI_Capture_Framebuffer(unsigned int pvbuf, unsigned int bpp)
{
	...
	DISP_DRV_WRAN("enter DPI_Capture_FB!\n");
	...
	if (bpp == 32)
		...
		DISP_DRV_WRAN("DPI_Capture_FB, fb color format not support\n");
	...
	m4u_dma_cache_maint(DISP_WDMA,
			    (void *)pvbuf, lcm_params->height * lcm_params->width * bpp / 8, DMA_BIDIRECTIONAL);
