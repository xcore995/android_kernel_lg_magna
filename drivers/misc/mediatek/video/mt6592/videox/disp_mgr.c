	...
	for (i = 0; i < MAX_INPUT_CONFIG; i++) {
		...
		if (config->layer_enable) {
			if (config->next_buff_idx <= disp_sync_get_last_signaled(input->session_id, config->layer_id)) {
				MMProfileLogEx(MTKFB_MMP_Events.Debug, MMProfileFlagPulse, config->layer_id,
					       config->next_buff_idx);
				job->input[i].dirty = 0;
				continue;
			}
			...
			default:
				...
				AEE_WARNING("[DISP/Mgr]", "L%d of 0x%08x unknown format(0x%x)!\n", config->layer_id,
					    input->session_id, config->src_fmt);
				unknown_fmt = 1;
				break;
			}
			if (unknown_fmt) {
			...
			if (config->src_width != config->tgt_width || config->src_height != config->tgt_height) {
				XLOG_ERR("OVL cannot support clip:src(%d,%d), dst(%d,%d)\n", config->src_width,
					 config->src_height, config->tgt_width, config->tgt_height);
			}
		...
		unsigned int offset =
		    config->src_offset_y * config->src_pitch * layerpitch + config->src_offset_x * layerpitch;
		if (config->layer_enable && i == 0)
			update_frm_seq_info(input->session_id, config->src_phy_addr, offset, config->frm_sequence,
					    FRM_CONFIG);
