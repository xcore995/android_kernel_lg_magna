...
/* long long ret; */
	if (wait_lcd_vsync) {
		lcd_vsync = true;
		wake_up_interruptible(&_vsync_wait_queue);
/* DISP_DRV_WRAN("hrtimer Vsync, and wake up\n"); */
	}
...
#if ENABLE_LCD_INTERRUPT
	if (request_irq(MT6582_DISP_DBI_IRQ_ID, _LCD_InterruptHandler, IRQF_TRIGGER_LOW, "mtklcd", NULL) < 0) {
...
LCD_STATUS LCD_ConfigParallelIF(LCD_IF_ID id,
				...
				UINT32 writeHold,
				UINT32 writeWait,
				UINT32 readSetup, UINT32 readHold, UINT32 readLatency, UINT32 waitPeriod, UINT32 chw)
{
...
LCD_STATUS LCD_ConfigIfFormat(LCD_IF_FMT_COLOR_ORDER order,
			      ...
			      LCD_IF_FMT_PADDING padding, LCD_IF_FORMAT format, LCD_IF_WIDTH busWidth)
{
...
LCD_STATUS LCD_ConfigSerialIF(LCD_IF_ID id,
			      ...
			      UINT32 div2,
			      UINT32 hw_cs,
			      UINT32 css, UINT32 csh, UINT32 rd_1st, UINT32 rd_2nd, UINT32 wr_1st, UINT32 wr_2nd)
{
	LCD_REG_SCNF config;
	...
	DISP_DRV_WRAN("LCD_ConfigSerialIF 1\n");
	ASSERT(id >= LCD_IF_SERIAL_0 && id <= LCD_IF_SERIAL_1);

	DISP_DRV_WRAN("LCD_ConfigSerialIF 2\n");

	_WaitForEngineNotBusy();

	DISP_DRV_WRAN("LCD_ConfigSerialIF 3\n");
	...
	DISP_DRV_WRAN("LCD_ConfigSerialIF 4\n");
...
UINT32 LCD_DisableAllLayer(UINT32 vram_start, UINT32 vram_end)
{
	...
	for (id = 0; id < DDP_OVL_LAYER_MUN; id++) {
		...
		if (cached_layer_config[id].addr >= vram_start && cached_layer_config[id].addr < vram_end) {
...
LCD_STATUS LCD_TE_ConfigVHSyncMode(UINT32 hsDelayCnt, UINT32 vsWidthCnt, LCD_TE_VS_WIDTH_CNT_DIV vsWidthCntDiv)
{
/*    LCD_REG_TECON tecon = LCD_REG->TEARING_CFG;
...
LCD_STATUS LCD_Dump_Layer_Info(void)
{
	...
	DISP_DRV_WRAN("LCD_Dump_Layer_Info:\n");
	for (i = 0; i < 4; i++) {
		DISP_DRV_WRAN
		("layer=%d, en=%d, src=%d, fmt=%d, addr=0x%x, (%d, %d, %d, %d), pitch=%d, keyEn=%d, key=0x%x,",
		     cached_layer_config[i].layer, cached_layer_config[i].layer_en, cached_layer_config[i].source,
		     cached_layer_config[i].fmt, cached_layer_config[i].addr, cached_layer_config[i].dst_x,
		     cached_layer_config[i].dst_y, cached_layer_config[i].dst_w, cached_layer_config[i].dst_h,
		     cached_layer_config[i].src_pitch, cached_layer_config[i].keyEn, cached_layer_config[i].key);
		DISP_DRV_WRAN(" aen=%d, alpha=0x%x, isTdshp=%d, curr_buff_idx=%d, next_buff_idx=%d\n",
			cached_layer_config[i].aen, cached_layer_config[i].alpha, cached_layer_config[i].isTdshp,
			realtime_layer_config[i].buff_idx, cached_layer_config[i].buff_idx);
	}
...
LCD_STATUS LCD_Reset_WriteCycle(LCD_IF_ID id)
{
	...
	/* have modify lcd setting, so when fm turn off, we must decrease wst to default setting */
	if (wst_step_LCD > 0) {
...
int m4u_alloc_mva_stub(M4U_MODULE_ID_ENUM eModuleID, const unsigned int BufAddr, const unsigned int BufSize,
...
int m4u_dealloc_mva_stub(M4U_MODULE_ID_ENUM eModuleID, const unsigned int BufAddr, const unsigned int BufSize,
...
int m4u_insert_tlb_range_stub(M4U_MODULE_ID_ENUM eModuleID,
			      ...
			      const unsigned int MVAEnd, M4U_RANGE_PRIORITY_ENUM ePriority, unsigned int entryCount)
{
...
unsigned int LCD_Check_LCM(UINT32 color)
{
	...
	for (i = 0; i < 60; i += 3) {
		DISP_DRV_WRAN("read pixel = 0x%x,", (buffer[i] << 16) | (buffer[i + 1] << 8) | (buffer[i + 2]));
