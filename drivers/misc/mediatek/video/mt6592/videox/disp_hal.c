...
int disphal_init_ctrl_if(void)
{
	...
	case LCM_CTRL_SERIAL_DBI:
		...
		LCD_ConfigSerialIF(ctrl_if,
				   ...
				   dbi->serial.clock_base, dbi->serial.clock_div, dbi->serial.is_non_dbi_mode);
		...
		LCD_ConfigSerialIF(ctrl_if,
				   ...
				   dbi->serial.css,
				   dbi->serial.csh,
				   dbi->serial.rd_1st, dbi->serial.rd_2nd, dbi->serial.wr_1st, dbi->serial.wr_2nd);
#endif
		break;
	...
	LCD_CHECK_RET(LCD_ConfigIfFormat(dbi->data_format.color_order,
					 ...
					 dbi->data_format.format, to_lcd_if_width(dbi->data_format.width)));
	return 0;
}
...
void disphal_wait_displayDone(void)
{
	...
	if (wait_event_interruptible_timeout(disp_done_wq, !disp_running, HZ/10) == 0)
		pr_debug("[disphal] Wait disp finished timeout in early_suspend\n");
}
#endif
...
int disphal_update_screen(const LCM_DRIVER *lcm_drv, struct mutex *pLcmCmdMutex, UINT32 x, UINT32 y, UINT32 width,
			  UINT32 height)
{
	if (lcm_drv->update) {
		mutex_lock(pLcmCmdMutex);
...
int disphal_prepare_suspend(void)
{
	...
	if (clk_is_force_on(MT_CG_DISP0_SMI_LARB0) || clk_is_force_on(MT_CG_DISP0_SMI_COMMON)) {
		DISP_DRV_WRAN("[DDP] MT_CG_DISP0_SMI_LARB0 is forced on\n");
		clk_clr_force_on(MT_CG_DISP0_SMI_LARB0);
		clk_clr_force_on(MT_CG_DISP0_SMI_COMMON);
	}
	return 0;
...
const LCM_DRIVER *disphal_get_lcm_driver(const char *lcm_name, unsigned int *lcm_index)
{
	...
	DISP_DRV_WRAN("[LCM Auto Detect], we have %d lcm drivers built in\n", lcm_count);
	DISP_DRV_WRAN("[LCM Auto Detect], try to find driver for [%s]\n", (lcm_name == NULL) ? "unknown" : lcm_name);
	...
	if (lcm_count == 1) {
		...
		*lcm_index = 0;
		...
		for (i = 0; i < lcm_count; i++) {
			...
			DISP_DRV_WRAN("[LCM Auto Detect] [%d] - [%s]\t", i,
				      (lcm->name == NULL) ? "unknown" : lcm->name);
			lcm->set_util_funcs(&lcm_utils);
			memset((void *)&s_lcm_params, 0, sizeof(LCM_PARAMS));
			lcm->get_params(&s_lcm_params);
	...
	*va = (unsigned int)ioremap_nocache(res->start, res->end - res->start + 1);
	if (disp_use_mmu) {
		...
		/*ret = m4u_fill_linear_pagetable(*pa, res->end - res->start + 1);
		   if(ret)
		   {
		   DISP_DRV_WRAN("fill_linear_pagetable error, %d\n", ret);
		   }
		   *dma_pa = *pa; */
		ASSERT(dma_pa);
		DISP_DRV_WRAN("[DISPHAL] FB MVA is 0x%08X PA is 0x%08X\n", *dma_pa, *pa);
	} else {
		*dma_pa = *pa;
	}
	return 0;
...
static int disphal_fbconfig_DSI_set_get_misc(const char *name, void *parameter)
{
	...
	if (!strcmp(name, "reg_read")) {
		fbconfig_dsi_dcs_read_lcm_reg_v2(lcm_read->check_addr, lcm_read->check_type, lcm_read->check_buffer,
						 lcm_read->check_para_num);
