...
#define RGB888_To_RGB565(x) ((((x) & 0xF80000) >> 8) | (((x) & 0x00FC00) >> 5) | (((x) & 0x0000F8) >> 3))
...
static void DAL_UpdateScreen(BOOL enable, BOOL aen, UINT32 width, UINT32 height)
{
	...
	MTKFB_WRAN("[FB Driver] DAL_UpdateScreen(): id=%d,en=%d,aen=%d,fmt=%d,pitch=%d,w=%d,h=%d\n",
		   ASSERT_LAYER, enable, aen, DP_COLOR_GET_UNIQUE_ID(DAL_FORMAT), (int)(DAL_PITCH * DAL_BPP), width,
		   height);
...
int DAL_address_burst_align(void)
{
	...
	DISP_DRV_WRAN("dal address align 0x%x\n", align);
	return align;
}
...
DAL_STATUS DAL_Init(unsigned long layerVA, unsigned long layerPA)
{
	...
	DAL_CHECK_MFC_RET(MFC_Open_Ex(&mfc_handle, dal_fb_addr,
				      DAL_WIDTH, DAL_HEIGHT, DAL_PITCH, DAL_BPP, DAL_FG_COLOR, DAL_BG_COLOR));
...
DAL_STATUS DAL_Dynamic_Change_FB_Layer(unsigned int isAEEEnabled)
{
	...
	DISP_DRV_WRAN("[DDP] DAL_Dynamic_Change_FB_Layer, isAEEEnabled=%d\n", isAEEEnabled);

	if (DISP_DEFAULT_UI_LAYER_ID == DISP_CHANGED_UI_LAYER_ID) {
		DISP_DRV_WRAN("[DDP] DAL_Dynamic_Change_FB_Layer, no dynamic switch\n");
		return DAL_STATUS_OK;
	}
...
DAL_STATUS DAL_Clean(void)
{
	...
	MFC_CONTEXT *ctxt = (MFC_CONTEXT *) mfc_handle;
	MTKFB_WRAN("[MTKFB_DAL] DAL_Clean\n");
	if (NULL == mfc_handle)
		return DAL_STATUS_NOT_READY;
...
/* return DAL_STATUS_LCD_IN_SUSPEND; */
	...
	if (isAEEEnabled == 1) {
		...
		DISP_DRV_WRAN("[DDP]* isAEEEnabled from 1 to 0, %d\n", dal_clean_cnt++);
		isAEEEnabled = 0;
...
DAL_STATUS DAL_Printf(const char *fmt, ...)
{
	...
	uint i;
	DAL_STATUS ret = DAL_STATUS_OK;
	MTKFB_WRAN("%s", __func__);
	/* MTKFB_WRAN("[MTKFB_DAL] DAL_Printf mfc_handle=0x%08X, fmt=0x%08X\n", mfc_handle, fmt); */
	if (NULL == mfc_handle)
		return DAL_STATUS_NOT_READY;
	...
	DAL_LOCK();
	if (isAEEEnabled == 0) {
		DISP_DRV_WRAN("[DDP] isAEEEnabled from 0 to 1, ASSERT_LAYER=%d, dal_fb_pa %x\n",
...
DAL_STATUS DAL_Clean(void)
{
	MTKFB_WRAN("[MTKFB_DAL] DAL_Clean is not implemented\n");
	return DAL_STATUS_OK;
}
...
{
	NOT_REFERENCED(fmt);
	MTKFB_WRAN("[MTKFB_DAL] DAL_Printf is not implemented\n");
	return DAL_STATUS_OK;
}
