...
void init_dsi(BOOL isDsiPoweredOn)
{
	...
	if (1 == lcm_params->dsi.ufoe_enable)
		DSI_CHECK_RET(DSI_PS_Control
			      (lcm_params->dsi.PS, lcm_params->height,
			       (lcm_params->width + lcm_params->width % 4) / 2 * dsiTmpBufBpp));
	else
...
static DISP_STATUS dsi_init(UINT32 fbVA, UINT32 fbPA, BOOL isLcmInited)
{
	...
	if (clk_is_force_on(MT_CG_DISP0_SMI_LARB0) == 0 || clk_is_force_on(MT_CG_DISP0_SMI_COMMON) == 0)
		DISP_DRV_WRAN("[DDP] error, MT_CG_DISP0_SMI_LARB0 is not force on\n");
	else
		DISP_DRV_WRAN("[DDP] MT_CG_DISP0_LARB2_SMI is force on\n");
...
static UINT32 dsi_get_working_buffer_size(void)
{
	...
	if (lcm_params->dsi.mode != CMD_MODE)
		return lcm_params->width * lcm_params->height * dsiTmpBufBpp * lcm_params->dsi.intermediat_buffer_num;
