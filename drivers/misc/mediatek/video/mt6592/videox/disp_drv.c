...
static void _DISP_InitHWConfig(disp_job *job, OVL_CONFIG_STRUCT *pOVLConfig, WDMA_CONFIG_STRUCT *pWDMAConfig)
{
	int idx;
	if (job == NULL) {
		MTKFB_WRAN("[FB Driver] error: _DISP_InitHWConfig() invalid param!\n");
		return;
	}
	mutex_lock(&job->lock);
	...
	for (i = 0; i < DDP_OVL_LAYER_MUN; i++) {
		if (job->input[i].dirty) {
			...
			if (job->input[i].layer_id >= DDP_OVL_LAYER_MUN) {
				MTKFB_WRAN("[FB Driver] warning: still found invalid layer_id=0x%x(%d) in job input\n",
			...
			MMProfileLogStructure(MTKFB_MMP_Events.ConfigOVL, MMProfileFlagPulse, &ovl_config,
					      OVL_CONFIG_STRUCT);
			if (ovl_config.layer_en && ovl_config.addr == 0) {
				MMProfileLogEx(MTKFB_MMP_Events.Debug, MMProfileFlagPulse, ovl_config.layer,
					       ovl_config.buff_idx);
				AEE_WARNING("[DISP/Common]", "L%d,%d of 0x%08x invalid addr!\n", ovl_config.layer,
					    ovl_config.buff_idx, job->group_id);
			} else {
...
static int _DISP_DecoupleWorkerKThread(void *data)
{
	...
	while (1) {
		...
		while ((job = disp_acquire_job()) != NULL) {
			MMProfileLogEx(MTKFB_MMP_Events.TrigOverlayOut, MMProfileFlagStart, job->group_id,
				       (unsigned int)job);
...
extern bool lcd_test_mode;/* LGE_BSP_COMMON LGE_CHANGE_S 141031 jongwoo82.lee : AT%LCD */
...
void DISP_WaitMemWriteDoneIfNeeded(void)
{
	...
	if (DISP_IsDecoupleMode()) {
		if (wait_event_interruptible_timeout(write_mem_idle_wq, !write_mem_running, HZ / 10) == 0)
			MTKFB_WRAN("[FB Driver] wait write mem done timeout!\n");
	}
#endif
}
...
DISP_STATUS DISP_SwitchDisplayMode(struct fb_overlay_mode *pConfig)
{
	...
	if (!is_early_suspended) {
		MTKFB_WRAN("[FB Driver] Switch to display mode: %s\n",
	 ...
	 */
		if (disp_mode == DISP_DIRECT_LINK_MODE) {
			MTKFB_WRAN("[FB Driver] Switch display mode-0_1!\n");
			...
			MTKFB_WRAN("[FB Driver] Switch display mode-0_2!\n");
			...
			MTKFB_WRAN("[FB Driver] Switch display mode-0_3!\n");
			...
			MTKFB_WRAN("[FB Driver] Switch display mode-1_0!\n");
	 ...
	 */
		...
		MTKFB_WRAN("[FB Driver] Switch display mode-2_0!\n");
		...
		MTKFB_WRAN("[FB Driver] Switch display mode-2_1!\n");
	 ...
	 */
		...
		MTKFB_WRAN("[FB Driver] Switch display mode done!\n");
...
static void disp_dump_lcm_parameters(LCM_PARAMS *lcm_params)
{
	...
	MTKFB_WRAN("[mtkfb] LCM TYPE: %s\n", LCM_TYPE_NAME[lcm_params->type]);
	MTKFB_WRAN("[mtkfb] LCM INTERFACE: %s\n", LCM_CTRL_NAME[lcm_params->ctrl]);
	MTKFB_WRAN("[mtkfb] LCM resolution: %d x %d\n", lcm_params->width, lcm_params->height);
...
BOOL disp_get_lcm_name_boot(char *cmdline)
{
	...
	MTKFB_WRAN("[mtkfb] LCM is %sconnected\n", ((isLCMConnected) ? "" : "not "));
	p += 2;
	q = p;
	while (*q != ' ' && *q != '\0')
		q++;
...
BOOL DISP_SelectDeviceBoot(const char *lcm_name)
{
	...
	if (NULL == lcm_drv) {
		MTKFB_WRAN("%s, get_lcm_driver returns NULL\n", __func__);
		return FALSE;
	}
	isLCMFound = TRUE;
...
BOOL DISP_SelectDevice(const char *lcm_name)
{
	lcm_drv = disphal_get_lcm_driver(lcm_name, (unsigned int *)&u4IndexOfLCMList);
	if (NULL == lcm_drv) {
		MTKFB_WRAN("%s, disphal_get_lcm_driver() returns NULL\n", __func__);
		return FALSE;
	}
	isLCMFound = TRUE;
...
DISP_STATUS DISP_Init(UINT32 fbVA, UINT32 fbPA, BOOL isLcmInited)
{
	...
	r = (disp_if_drv->init) ? (disp_if_drv->init(fbVA, fbPA, isLcmInited)) : DISP_STATUS_NOT_IMPLEMENTED;
	msleep(50);
#ifndef MTKFB_FPGA_ONLY
	...
	case LCM_TYPE_DBI:
		{
			...
			MTKFB_WRAN("DISP Info: DBI, CMD Mode, HW Vsync enable\n");
			break;
		}
	case LCM_TYPE_DPI:
		{
			...
			dispif_info[MTKFB_DISPIF_PRIMARY_LCD].isHwVsyncAvailable = 1;
			MTKFB_WRAN("DISP Info: DPI, VDO Mode, HW Vsync enable\n");
			break;
		}
	case LCM_TYPE_DSI:
		{
			...
			if (lcm_params->dsi.mode == CMD_MODE) {
				dispif_info[MTKFB_DISPIF_PRIMARY_LCD].displayMode = DISPIF_MODE_COMMAND;
				dispif_info[MTKFB_DISPIF_PRIMARY_LCD].isHwVsyncAvailable = 1;
				MTKFB_WRAN("DISP Info: DSI, CMD Mode, HW Vsync enable\n");
			} else {
				dispif_info[MTKFB_DISPIF_PRIMARY_LCD].displayMode = DISPIF_MODE_VIDEO;
				dispif_info[MTKFB_DISPIF_PRIMARY_LCD].isHwVsyncAvailable = 1;
				MTKFB_WRAN("DISP Info: DSI, VDO Mode, HW Vsync enable\n");
			}
...

	{
		...
		if (lcm_drv) {
			...
			MTKFB_WRAN("DISP Info: LCM Panel Original Resolution(For DFO Only): %d x %d\n",
				   dispif_info[MTKFB_DISPIF_PRIMARY_LCD].lcmOriginalWidth,
				   dispif_info[MTKFB_DISPIF_PRIMARY_LCD].lcmOriginalHeight);
		} else {
			MTKFB_WRAN("DISP Info: Fatal Error!!, lcm_drv is null\n");
		}
	}
...
#if 0
	{
		...
		MTKFB_WRAN("DISP Info: from DFO, width/height=%d x %d\n", width, height);
	}
#endif
...
DISP_STATUS DISP_PowerEnable(BOOL enable)
{
	...
	if (down_interruptible(&sem_update_screen)) {
		MTKFB_WRAN("ERROR: Can't get sem_update_screen in DISP_PowerEnable()\n");
	...
	if ((!is_ipoh_bootup) ||
	    (is_ipoh_bootup
		 ...
		 || (lcm_params->type == LCM_TYPE_DSI && lcm_params->dsi.mode == CMD_MODE)))) {
		ret = (disp_if_drv->enable_power) ? (disp_if_drv->enable_power(enable)) : DISP_STATUS_NOT_IMPLEMENTED;
	}
...
DISP_STATUS DISP_PanelEnable(BOOL enable)
{
	...
	if (down_interruptible(&sem_update_screen)) {
		MTKFB_WRAN("ERROR: Can't get sem_update_screen in DISP_PanelEnable()\n");
		return DISP_STATUS_ERROR;
	}
...
DISP_STATUS DISP_SetBacklight(UINT32 level)
{
	...
	if (down_interruptible(&sem_update_screen)) {
		MTKFB_WRAN("ERROR: Can't get sem_update_screen in DISP_SetBacklight()\n");
		return DISP_STATUS_ERROR;
	}
...
DISP_STATUS DISP_SetBacklight_mode(UINT32 mode)
{
	...
	if (down_interruptible(&sem_update_screen)) {
		MTKFB_WRAN("ERROR: Can't get sem_update_screen in DISP_SetBacklight_mode()\n");
		return DISP_STATUS_ERROR;
	}
...
DISP_STATUS DISP_SetPWM(UINT32 divider)
{
	...
	if (down_interruptible(&sem_update_screen)) {
		MTKFB_WRAN("ERROR: Can't get sem_update_screen in DISP_SetPWM()\n");
		return DISP_STATUS_ERROR;
	}
...
DISP_STATUS DISP_UpdateScreen(UINT32 x, UINT32 y, UINT32 width, UINT32 height)
{
	...
	if (down_interruptible(&sem_update_screen)) {
		MTKFB_WRAN("ERROR: Can't get sem_update_screen in DISP_UpdateScreen()\n");
		return DISP_STATUS_ERROR;
	}
...
int DISP_RegisterExTriggerSource(DISP_EXTRA_CHECKUPDATE_PTR pCheckUpdateFunc, DISP_EXTRA_CONFIG_PTR pConfFunc)
{
	...
	if ((NULL == pCheckUpdateFunc) || (NULL == pConfFunc)) {
		MTKFB_WRAN("Warnning! [Func]%s register NULL function : %p,%p\n", __func__, pCheckUpdateFunc,
			   pConfFunc);
		return -1;
	}
...
void DISP_UnRegisterExTriggerSource(int u4ID)
{
	if (DISP_CB_MAXCNT < (u4ID + 1)) {
		MTKFB_WRAN("Warnning! [Func]%s unregister a never registered function : %d\n", __func__, u4ID);
		return;
	}
...
static void _DISP_DumpOverlay(void)
{
	...
	if (gCaptureOvlEnable) {
		...
		if (gCaptureOvlRawData) {
			...
			disphal_dma_map_kernel(paddr, Bitmap.data_size, (unsigned int *)&Bitmap.pData,
					       &Bitmap.data_size);
	...
	if (gCaptureLayerEnable && pLayer->layer_en) {
		if (gCaptureLayerRawData) {
			...
			case ePABGR8888:
				...
				break;
			default:
				MTKFB_WRAN("error: _DISP_DumpLayer(), unknow format=%d\n", pLayer->fmt);
			...
			Bitmap.down_sample_x = gCaptureLayerDownX;
			Bitmap.down_sample_y = gCaptureLayerDownY;
			disphal_dma_map_kernel(pLayer->addr, Bitmap.data_size, (unsigned int *)&Bitmap.pData,
					       &Bitmap.data_size);
		...
		for (i = 0; i < DDP_OVL_LAYER_MUN; i++) {
			...
			if (ovl_config.isDirty) {
				...
				if (ovl_config.layer_en) {
					disp_sync_inc_timeline(disp_config.session_id, ovl_config.layer,
							       ovl_config.buff_idx - 1);
				} else {
					disp_sync_inc_timeline(disp_config.session_id, ovl_config.layer,
							       ovl_config.buff_idx);
				}
...
static int _DISP_ConfigUpdateKThread(void *data)
{
	...
	while (1) {
		...
		if (!is_early_suspended) {
			...
			if (!DISP_IsDecoupleMode() && mutex_trylock(&SwitchModeMutex)) {
...
{
	MTKFB_WRAN("DISP_HalfVsync, enable half vsync, %d\n", half_vsync);
	half_vsync = enable;
}
...
{
	if (LCM_TYPE_DPI == lcm_params->type || (LCM_TYPE_DSI == lcm_params->type && lcm_params->dsi.mode != CMD_MODE)
...
void DISP_InitVSYNC(unsigned int vsync_interval)
{
	...
	config_update_task = kthread_create(_DISP_ConfigUpdateKThread, NULL, "disp_config_update_kthread");
	...
	if ((LCM_TYPE_DBI == lcm_params->type) ||
		...
		MTKFB_WRAN("[MTKFB] vsync timer_period=%d\n", vsync_interval);
...
void DISP_WaitVSYNC(void)
{
	...
	if (wait_event_interruptible_timeout(vsync_wq, vsync_wq_flag, HZ / 10 * 2) == 0)
		MTKFB_WRAN("[DISP] Wait VSync timeout. early_suspend=%d\n", is_early_suspended);
...
BOOL DISP_IsVideoMode(void)
{
	...
	if (lcm_params)
		...
		MTKFB_WRAN("WARNING!! DISP_IsVideoMode is called before display driver inited!\n");
		return 0;
	}
}
...
UINT32 DISP_GetScreenWidth(void)
{
	...
	if (lcm_params)
		...
		MTKFB_WRAN("WARNING!! get screen width before display driver inited!\n");
		return 0;
	}
}
...
UINT32 DISP_GetScreenHeight(void)
{
	...
	if (lcm_params)
		...
		MTKFB_WRAN("WARNING!! get screen height before display driver inited!\n");
		return 0;
	}
}
...
UINT32 DISP_GetPhysicalHeight(void)
{
	disp_drv_init_context();
	if (lcm_params) {
		MTKFB_WRAN("[wwy]lcm_parms->physical_height = %d\n", lcm_params->physical_height);
		return lcm_params->physical_height;
	} else {
		MTKFB_WRAN("WARNING!! get physical_height before display driver inited!\n");
		return 0;
	}
}
...
UINT32 DISP_GetPhysicalWidth(void)
{
	disp_drv_init_context();
	if (lcm_params) {
		MTKFB_WRAN("[wwy]lcm_parms->physical_width = %d\n", lcm_params->physical_width);
		return lcm_params->physical_width;
	} else {
		MTKFB_WRAN("WARNING!! get physical_width before display driver inited!\n");
		return 0;
	}
}
...
UINT32 DISP_GetVRamSizeBoot(char *cmdline)
{
	...
	if (disp_if_drv)
		...
		MTKFB_WRAN("%s, can't get lcm type, reserved memory size will be set as 0x800000\n", __func__);
		return 0x1400000;
	}
	/* Align vramSize to 1MB */
	/*  */
	...
	MTKFB_WRAN("DISP_GetVRamSizeBoot: %u bytes[%dMB]\n", vramSize, (vramSize >> 20));
...
UINT32 DISP_GetOutputBPPforDithering(void)
{
	...
	return (disp_if_drv->get_dithering_bpp) ? (disp_if_drv->get_dithering_bpp()) : DISP_STATUS_NOT_IMPLEMENTED;
}
...
DISP_STATUS DISP_Capture_Framebuffer(unsigned int pvbuf, unsigned int bpp, unsigned int is_early_suspended)
{
		...
		if (bpp == 32) {
			for (i = 0; i < ovl_buffer_size; i++) {
				*(unsigned int *)(pvbuf + i * 4) =
				    0xff000000 | fbv[i * 3] | (fbv[i * 3 + 1] << 8) | (fbv[i * 3 + 2] << 16);
			}
		}
		if (bpp == 16) {
			for (i = 0; i < ovl_buffer_size; i++) {
				*(unsigned short *)(pvbuf + i * 2) = ((fbv[i * 3 + 0] & 0xF8) >> 3) |
				    ((fbv[i * 3 + 1] & 0xFC) << 3) | ((fbv[i * 3 + 2] & 0xF8) << 8);
			}
		}
		iounmap(fbv);
		return DISP_STATUS_OK;
	...
	if (ret != 0) {
		MTKFB_WRAN("disphal_map_overlay_out_buffer fail!\n");
		return DISP_STATUS_OK;
	}
	disphal_init_overlay_to_memory();
	...
	if (bpp == 32)
		...
		MTKFB_WRAN("DSI_Capture_FB, fb color format not support\n");
		MemOutConfig.outFormat = eRGB888;
	}
...
unsigned int DISP_AutoTest(void)
{
	unsigned int ret = 0;
	if (down_interruptible(&sem_update_screen)) {
		MTKFB_WRAN("ERROR: Can't get sem_update_screen in DISP_Change_Update()\n");
		return DISP_STATUS_ERROR;
	}
...
BOOL DISP_EsdCheck(void)
{
	...
	if (down_interruptible(&sem_update_screen)) {
		MTKFB_WRAN("ERROR: Can't get sem_update_screen in DISP_EsdCheck()\n");
		return FALSE;
	}
...
BOOL DISP_EsdRecover(void)
{
	...
	if (down_interruptible(&sem_update_screen)) {
		MTKFB_WRAN("ERROR: Can't get sem_update_screen in DISP_EsdRecover()\n");
		return FALSE;
	}
...
BOOL fbconfig_dsi_vdo_prepare(void)
{
	if (!is_early_suspended) {
		...
		if (down_interruptible(&sem_update_screen)) {
			MTKFB_WRAN("ERROR: Can't get sem_update_screen in fbconfig_dsi_vdo_prepare()\n");
			return FALSE;
		}
		...
		MTKFB_WRAN("fbconfig=>02will set cmd mode in disp_drv.c!!\n");
...
{
	if (lcm_params) {
		MTKFB_WRAN("LCM Resolution will be changed, original: %dx%d, now: %dx%d\n", lcm_params->width,
			   lcm_params->height, width, height);
		if (width > lcm_params->width || height > lcm_params->height || width == 0 || height == 0) {
			MTKFB_WRAN("Invalid resolution: %dx%d\n", width, height);
			return DISP_STATUS_ERROR;
		}

		if (DISP_IsVideoMode()) {
			MTKFB_WRAN("Warning!!!Video Mode can't support multiple resolution!\n");
			return DISP_STATUS_ERROR;
		}
...
{
	MTKFB_WRAN("DISP_Auto_Capture_FB width %d height %d\n", wdma_width, wdma_height);
	...
	if (DISP_IsDecoupleMode()) {
		...
		ret =
		    disphal_map_overlay_out_buffer(pvbuf, DISP_GetScreenHeight() * DISP_GetScreenWidth() * bpp / 8,
						   &mva);
		if (ret != 0) {
			MTKFB_WRAN("disphal_map_overlay_out_buffer fail!\n");
			return DISP_STATUS_OK;
...
static BOOL fbconfig_disp_set_clk_prepare(unsigned int clk)
{
	if (!is_early_suspended) {
		...
		if (down_interruptible(&sem_update_screen)) {
			MTKFB_WRAN("ERROR: Can't get sem_update_screen in fbconfig_disp_set_clk_prepare()\n");
			return FALSE;
		}
		...
		if (fbconfig_if_drv->set_cmd_mode)
			fbconfig_if_drv->set_cmd_mode();
		MTKFB_WRAN("sxk==>in disp_drv 2124:%d\n", clk);
...
void fbconfig_disp_set_mipi_clk(unsigned int clk)
{
	if (lcm_params->dsi.mode != CMD_MODE) {
		if (down_interruptible(&sem_early_suspend)) {
			MTKFB_WRAN("sxk=>can't get semaphore in fbconfig_disp_set_mipi_clk()\n");
			return;
		}
...
static BOOL fbconfig_disp_set_ssc_prepare(unsigned int ssc)
{
	if (!is_early_suspended) {
		...
		if (down_interruptible(&sem_update_screen)) {
			MTKFB_WRAN("ERROR: Can't get sem_update_screen in fbconfig_disp_set_ssc_prepare()\n");
			return FALSE;
		}
		if (is_lcm_in_suspend_mode) {
			up(&sem_update_screen);
...
void fbconfig_disp_set_mipi_ssc(unsigned int ssc)
{
	if (lcm_params->dsi.mode != CMD_MODE) {
		if (down_interruptible(&sem_early_suspend)) {
			MTKFB_WRAN("sxk=>can't get semaphore in fbconfig_disp_set_mipi_ssc()\n");
			return;
		}
...
static int fbconfig_disp_get_esd_prepare(void)
{
	...
	if (!is_early_suspended) {
		...
		if (down_interruptible(&sem_update_screen)) {
			MTKFB_WRAN("ERROR: Can't get sem_update_screen in fbconfig_disp_get_esd_prepare()\n");
			ret = -2;
		}
...
int fbconfig_get_esd_check(void)
{
	...
	if (lcm_params->dsi.mode != CMD_MODE) {
		if (down_interruptible(&sem_early_suspend)) {
			MTKFB_WRAN("sxk=>can't get semaphore in fbconfig_get_esd_check()\n");
			ret = -2;
		}
...
BOOL fbconfig_rest_lcm_setting_prepare(void)
{
	if (!is_early_suspended) {
		...
		if (down_interruptible(&sem_update_screen)) {
			MTKFB_WRAN("ERROR: Can't get sem_update_screen in fbconfig_rest_lcm_setting_prepare()\n");
			return FALSE;
		}
