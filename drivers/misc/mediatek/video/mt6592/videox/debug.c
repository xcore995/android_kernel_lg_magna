...
static void dprec_logger_frame_seq_begin(DISP_SESSION_TYPE session_type, unsigned frm_sequence)
{
	...
	if (session_type > DISP_SESSION_MEMORY) {
		DISP_DRV_WRAN("seq_begin session_type(0x%x) error, seq(%d)", session_type, frm_sequence);
		return;
	}
...
static void dprec_logger_frame_seq_end(DISP_SESSION_TYPE session_type, unsigned frm_sequence)
{
	...
	if (session_type > DISP_SESSION_MEMORY) {
		DISP_DRV_WRAN("seq_end session_type(0x%x) , seq(%d)", session_type, frm_sequence);
		return;
	}
...
void update_frm_seq_info(unsigned int session_id, unsigned int addr, unsigned int addr_offset, unsigned int seq,
			 DISP_FRM_SEQ_STATE state)
{
	...
	if (device_type > DISP_SESSION_MEMORY) {
		DISP_DRV_WRAN("seq_end session_id(0x%x) , seq(%d)", session_id, seq);
		return;
	}
		...
		if (FRM_CONFIG == state) {
			...
			for (i = 0; i < FRM_UPDATE_SEQ_CACHE_NUM; i++) {
				if (frm_update_sequence[i].state == FRM_TRIGGER) {
					...
					MMProfileLogEx(MTKFB_MMP_Events.primary_seq_wdma0_efo, MMProfileFlagPulse,
			...
			for (i = 0; i < FRM_UPDATE_SEQ_CACHE_NUM; i++) {
				if (FRM_WDMA0_EOF == frm_update_sequence[i].state && device_type == DISP_SESSION_PRIMARY
				    && frm_update_sequence[i].mva == addr) {
					...
					MMProfileLogEx(MTKFB_MMP_Events.primary_seq_rdma0_sof, MMProfileFlagPulse,
						       frm_update_sequence[i].mva, frm_update_sequence[i].seq);
				}
			}
		} else if (FRM_RDMA0_EOF == state) {
			for (i = 0; i < FRM_UPDATE_SEQ_CACHE_NUM; i++) {
				if (FRM_RDMA0_SOF == frm_update_sequence[i].state && device_type == DISP_SESSION_PRIMARY
				    && frm_update_sequence[i].mva == addr) {
					frm_update_sequence[i].state = FRM_RDMA0_EOF;
					dprec_logger_frame_seq_end(DISP_SESSION_PRIMARY - 1,
								   frm_update_sequence[i].seq);
					MMProfileLogEx(MTKFB_MMP_Events.primary_seq_rdma0_eof, MMProfileFlagPulse,
						       frm_update_sequence[i].mva, frm_update_sequence[i].seq);
		...
		if (FRM_CONFIG == state) {
			...
			for (i = 0; i < FRM_UPDATE_SEQ_CACHE_NUM; i++) {
				if (frm_update_sequence_mem[i].state == FRM_TRIGGER) {
					MMProfileLogEx(MTKFB_MMP_Events.external_seq_wdma0_efo, MMProfileFlagPulse,
						       frm_update_sequence_mem[i].mva, frm_update_sequence_mem[i].seq);
...
static void dump_layer_info(void)
{
	unsigned int i;
	for (i = 0; i < 4; i++) {
		DISP_DRV_WRAN("LayerInfo in LCD driver, layer=%d,layer_en=%d, source=%d, fmt=%d, addr=0x%x,",
		...
		DISP_DRV_WRAN(" x=%d, y=%d, w=%d, h=%d, pitch=%d, keyEn=%d, key=%d, aen=%d, alpha=%d\n",
...
void DBG_OnLcdDone(void)
{
	...
	if (fps.trigger_lcd_count >= dbg_opt.log_fps_wnd_size) {
		...
		DISP_LOG_PRINT(ANDROID_LOG_INFO, "DBG",
			"MTKFB FPS: %ld.%02ld, Avg.update time: %ld.%02ldms (TE delay %ld.%02ldms LCD %ld.%02ldms)\n",
			f / 100, f % 100, update / 100, update % 100, te / 100, te % 100, lcd / 100, lcd % 100);
		reset_fps_logger();
	}
}
