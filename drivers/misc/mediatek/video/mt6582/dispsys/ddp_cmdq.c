...
static void cmdq_insert_to_thread_dispatch_queue(TaskStruct *pTask);
static void cmdq_handle_done_unlocked(int32_t thread, uint32_t value, const bool attachWarning,
			       struct timeval *pGotIRQ);
static void cmdq_handle_error_unlocked(int32_t thread, uint32_t value, const bool attachWarning,
				struct timeval *pGotIRQ);
...
static long cmdq_proc_unlocked_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case DISP_IOCTL_EXEC_COMMAND:
		...
		if (cmdqSubmitTask(cParams.scenario,
				   cParams.priority,
				   cParams.engineFlag, cParams.pFrameBaseSW, cParams.blockSize)) {
...
int cmdq_proc_status_print_seq(struct seq_file *m, void *v)
{
	...
	for (index = 0; index < CMDQ_MAX_FIXED_TASK; index++) {
		...
		seq_printf(m, "State %d, VABase: 0x%08x, MVABase: 0x%08x, Size: %d\n",
			   pTask->taskState, (uint32_t) pTask->pVABase, pTask->MVABase,
			   pTask->blockSize);
		seq_printf(m, "Scenario %d, Priority: %d, Flag: 0x%08x, VAEnd: 0x%08x\n",
			   pTask->scenario, pTask->priority, pTask->engineFlag,
			   (uint32_t) pTask->pCMDEnd);
		seq_printf(m, "Reorder: %d, Trigger %d:%d, IRQ: %d:%d, Wake Up: %d:%d\n",
			   pTask->reorder, (uint32_t) pTask->trigger.tv_sec,
			   (uint32_t) pTask->trigger.tv_usec, (uint32_t) pTask->gotIRQ.tv_sec,
			   (uint32_t) pTask->gotIRQ.tv_usec, (uint32_t) pTask->wakedUp.tv_sec,
...
ssize_t cmdq_write_log_level_proc(struct file *pFile,
				  const char __user *pBuffer, size_t count, loff_t *data)
{
	u_long flags = 0;
	int len = 0;
	int32_t value = 0;
	...
	sscanf_ret = sscanf(textBuf, "%d", &value);
...
ssize_t cmdq_write_debug_level_proc(struct file *pFile,
				    const char __user *pBuffer, size_t count, loff_t *data)
{
	int len = 0;
	int32_t value = 0;
...
ssize_t cmdq_write_sw_timeout_proc(struct file *pFile,
				   const char __user *pBuffer, size_t count, loff_t *data)
{
	int32_t len = 0;
	...
	sscanf_ret = sscanf(textBuf, "%d %d %d", &swTimeout, &predumpStartTime, &predumpDuration);
...
int32_t cmdqSuspendTask(void)
{
	...
	if ((0 != pEngine[tIMGI].userCount) ||
	    ...
	    (0 != pEngine[tWROT].userCount) || (0 != pEngine[tWDMA1].userCount)) {
...
static TaskStruct *cmdq_acquire_task(int32_t scenario,
				     int32_t priority,
				     uint32_t engineFlag, void *pCMDBlock, uint32_t blockSize)
{
	...
	if (list_empty(&gCmdqFreeTask) || (blockSize >= CMDQ_MAX_BLOCK_SIZE)) {
		spin_unlock_irqrestore(&gCmdqTaskLock, flags);
		...
		if (NULL != pTask) {
			pVABase =
			    dma_alloc_coherent(NULL, blockSize + CMDQ_EXTRA_MARGIN, &MVABase,
					       GFP_KERNEL);
			if (NULL == pVABase) {
				kfree(pTask);
				pTask = NULL;
	...
	if (NULL != pTask) {
		...
		if (false == copyCmdFromUserSpace) {
			...
			CMDQ_ERR("Source: 0x%08x, target: 0x%08x, size: %d\n", (uint32_t) pCMDBlock,
				 (uint32_t) pTask->pVABase, blockSize);
...
int32_t cmdq_acquire_thread(const TaskStruct *pTask)
{
	...
	do {
		...
		if (0 == waitQ) {
			...
			CMDQ_ERR
			    ("<--THREAD: engine: 0x%08x, waitQ: %d, pTask %p(sec:%d) acquires THR: %d, pid(%d:%d)\n",
			     pTask->engineFlag, waitQ, pTask, false, thread, current->tgid,
			     current->pid);
		} else {
			CMDQ_MSG
			    ("<--THREAD: engine: 0x%08x, waitQ: %d, pTask %p(sec:%d) acquires THR: %d, pid(%d:%d)\n",
...
int32_t cmdq_disable_clock(uint32_t engineFlag)
{
	...
	if (engineFlag & (0x1 << tWDMA1)) {
		pEngine[tWDMA1].userCount--;
		if ((pEngine[tWDMA1].userCount <= 0) && clock_is_on(MT_CG_DISP0_MDP_WDMA)) {
	...
	if (engineFlag & (0x1 << tWROT)) {
		pEngine[tWROT].userCount--;
		if ((pEngine[tWROT].userCount <= 0) && clock_is_on(MT_CG_DISP0_MDP_WROT)) {
	...
	if (engineFlag & (0x1 << tTDSHP)) {
		pEngine[tTDSHP].userCount--;
		if ((pEngine[tTDSHP].userCount <= 0) && clock_is_on(MT_CG_DISP0_MDP_TDSHP)) {
	...
	if (engineFlag & (0x1 << tSCL1)) {
		pEngine[tSCL1].userCount--;
		if ((pEngine[tSCL1].userCount <= 0) && clock_is_on(MT_CG_DISP0_MDP_RSZ1)) {
	...
	if (engineFlag & (0x1 << tSCL0)) {
		pEngine[tSCL0].userCount--;
		if ((pEngine[tSCL0].userCount <= 0) && clock_is_on(MT_CG_DISP0_MDP_RSZ0)) {
	...
	if (engineFlag & (0x1 << tRDMA0)) {
		...
		if ((pEngine[tRDMA0].userCount <= 0) && clock_is_on(MT_CG_DISP0_MDP_RDMA)) {
...
void cmdq_dump_mmsys_config(void)
{
	CMDQ_ERR("CAM_MDP_MOUT_EN: 0x%08x, MDP_RDMA_MOUT_EN: 0x%08x, MDP_RSZ0_MOUT_EN: 0x%08x\n",
		 DISP_REG_GET(0xF4000000 + 0x01c), DISP_REG_GET(0xF4000000 + 0x020),
		 DISP_REG_GET(0xF4000000 + 0x024));
	CMDQ_ERR("MDP_RSZ1_MOUT_EN: 0x%08x, MDP_TDSHP_MOUT_EN: 0x%08x, DISP_OVL_MOUT_EN: 0x%08x\n",
		 DISP_REG_GET(0xF4000000 + 0x028), DISP_REG_GET(0xF4000000 + 0x02c),
		 DISP_REG_GET(0xF4000000 + 0x030));
	CMDQ_ERR("MDP_RSZ0_SEL: 0x%08x, MDP_RSZ1_SEL: 0x%08x, MDP_TDSHP_SEL: 0x%08x\n",
		 DISP_REG_GET(0xF4000000 + 0x038), DISP_REG_GET(0xF4000000 + 0x03c),
		 DISP_REG_GET(0xF4000000 + 0x040));
	CMDQ_ERR("MDP_WROT_SEL: 0x%08x, MDP_WDMA_SEL: 0x%08x, DISP_OUT_SEL: 0x%08x\n",
		 DISP_REG_GET(0xF4000000 + 0x044), DISP_REG_GET(0xF4000000 + 0x048),
		 DISP_REG_GET(0xF4000000 + 0x04c));
	CMDQ_ERR
	    ("MMSYS_DL_VALID_0: 0x%08x, MMSYS_DL_VALID_1: 0x%08x, MMSYS_DL_READY0: 0x%08x, MMSYS_DL_READY1: 0x%08x\n",
	     DISP_REG_GET(0xF4000000 + 0x860), DISP_REG_GET(0xF4000000 + 0x864),
	     DISP_REG_GET(0xF4000000 + 0x868), DISP_REG_GET(0xF4000000 + 0x86c));
}
...
static void cmdq_attach_error_task(const TaskStruct *pTask, int32_t thread)
{
	...
	CMDQ_ERR
	    ("Curr Cookie: %d, Wait Cookie: %d, Next Cookie: %d, Task Count %d, IRQ: 0x%08x, IRQ_EN: 0x%08x\n",
	     value[4], pThread->waitCookie, pThread->nextCookie, pThread->taskCount, value[5],
	     value[6]);
	...
	if (NULL != pTask) {
		...
		CMDQ_ERR("Task: 0x%08x, Scenario: %d, State: %d, Priority: %d, Reorder: %d, Engine: 0x%08x,\n",
			(uint32_t) pTask, pTask->scenario, pTask->taskState, pTask->priority,
			pTask->reorder, pTask->engineFlag);
		CMDQ_ERR(" irgFlag: 0x%08x, VABase: 0x%08x, CMDEnd: 0x%08x, MVABase: 0x%08x, Size: %d,\n",
			pTask->irqFlag, (uint32_t) pTask->pVABase, (uint32_t) pTask->pCMDEnd, pTask->MVABase,
			pTask->blockSize);
		CMDQ_ERR(" Last Inst: 0x%08x:0x%08x, 0x%08x:0x%08x, Trigger: %d:%d, Got IRQ: %d:%d, Finish: %d:%d\n",
	...
	if (NULL != pTask) {
		...
		replace the old one instead of using gCmdqContext.errNum as the index init value */
		...
		if (engineFlag & (0x01 << tIMGI)) {
			...
			CMDQ_ERR
			    ("[CMDQ] CLK_CFG_0: 0x%08x, CLK_CFG_3: 0x%08x, ISP_CLK_CG: 0x%08x\n",
		...
		for (index = 0; index < CMDQ_MAX_TASK_COUNT; index++) {
			if (NULL != pThread->pCurTask[index]) {
				CMDQ_ERR("Slot %d, Task: 0x%08x, VABase: 0x%08x, MVABase 0x%08x, Size: %d,",
					index, (uint32_t) (pThread->pCurTask[index]),
	...
	while (index < gCmdqContext.warningNum && index < CMDQ_MAX_WARNING_COUNT) {
		CMDQ_ERR
		    ("warning task[%d][0x%p], THR[%d], taskCount[%d], CurrCookie[%d], irqFlag[%d], gotIRQ[%d:%d]",
		     index, gCmdqContext.warning[index].taskAddr,
		     gCmdqContext.warning[index].thread, gCmdqContext.warning[index].taskCount,
		     gCmdqContext.warning[index].cookie, gCmdqContext.warning[index].irqFlag,
...
void cmdq_attach_warning_task(TaskStruct *pTask,
			      uint32_t thread,
			      uint32_t taskCountInThread,
			      uint32_t cookie, uint32_t irqFlag, struct timeval gotIRQ)
{
...
void cmdq_remove_task_from_thread_unlocked(ThreadStruct *pThread, int32_t index,
					   TASK_STATE_ENUM targetTaskStaus)
{
	if ((NULL == pThread) || (CMDQ_MAX_TASK_COUNT < index) || (0 > index)) {
		CMDQ_ERR
		    ("remove task, invalid param. pThread[0x%p], task_slot[%d], targetTaskStaus[%d]\n",
	...
	if (TASK_STATE_BUSY != pThread->pCurTask[index]->taskState) {
		CMDQ_ERR
		    ("remove task, taskStatus err[%d]. pThread[0x%p], task_slot[%d], targetTaskStaus[%d]\n",
...
int32_t cmdq_core_wait_task_done_with_interruptible_timeout(TaskStruct *pTask,
							    int32_t thread,
							    ...
							    uint32_t predump_retry_count)
{
	...
	CMDQ_MSG
	    ("swTimeout: %d, predumpStartTime: %d, predumpDurationMS: %d, predumpRetryCount: %d\n",
	     swTimeout, predump_start_time_ms, predump_duration_ms, predump_retry_count);

	timeout_ms = predump_start_time_ms;
	waitQ =
	    wait_event_interruptible_timeout(gCmdWaitQueue[thread],
	...
	for (i = 0; i < predump_max_retry_count; i++) {
		if (0 < waitQ) {
			...
			CMDQ_ERR
			    ("not allow extend predump maxRetryCount[%d], pTask[0x%p], thr[%d], predump[%d : %d]\n",
			...
			hasInterrupted = true;
			CMDQ_ERR
			    ("extend predump maxRetryCount to %d, pTask[0x%p], thr[%d],  predump[%d : %d]\n",
			     predump_max_retry_count, pTask, thread, i, timeout_ms);
		}

		sprintf(predumpTag, "PREDUMP %3d|THR_%d_STATUS:0x%08x, ExecTHR:0x%x", i, thread,
		...
		if (hasInterrupted) {
			waitQ =
			    wait_event_timeout(gCmdWaitQueue[thread],
					       (TASK_STATE_BUSY != pTask->taskState),
					       msecs_to_jiffies(timeout_ms));
		} else {
			waitQ =
...
int32_t cmdq_core_wait_task_done(TaskStruct *pTask,
				 int32_t thread,
				 uint32_t predump_start_time_ms,
				 uint32_t predump_duration_ms, uint32_t predump_retry_count)
{
	int32_t status;
	ThreadStruct *pThread;
	u_long flags;
	...
	waitQ =
	    cmdq_core_wait_task_done_with_interruptible_timeout(pTask, thread,
								predump_start_time_ms,
								predump_duration_ms,
								predump_retry_count);
	...
	if (TASK_STATE_DONE == pTask->taskState) {
		...
		CMDQ_ERR
		    ("should not go here. waitQ[%d], thread[%d], taskCount[%d], pTask[0x%p], pTask->MVABase[0x%08x]\n",
		...
		do {
			...
			if (irqFlag &
			    (CMDQ_THR_IRQ_FALG_INSTN_TIMEOUT | CMDQ_THR_IRQ_FALG_INVALID_INSTN)) {
			...
			cmdq_attach_error_task(pTask, thread);
			throwAEE = true;
			cmdq_core_parse_error(pTask, thread, &module, &instA, &instB);

			status = -ETIMEDOUT;

			/* remove all task */
			CMDQ_ERR
			    ("WAIT: remove all tasks in thread %d because 0x%p sw timeout(taskState:%d)\n",
			     thread, pTask, pTask->taskState);
			cmdq_remove_all_task_in_thread_unlocked(thread);

			/* if sw timeout task is executing, */
			/* reset HW engine */
			if ((currPC >= pTask->MVABase)
			    && (currPC <= (pTask->MVABase + pTask->blockSize))) {
			...
			if (throwAEE) {
				...
				case CMDQ_CODE_WFE:
					CMDQ_AEE("CMDQ",
						 "%s SW timeout, INST:(0x%08x, 0x%08x), OP:WAIT EVENT:%s\n",
						 module, instA, instB,
						 cmdq_core_get_event_name(instA & (~0xFF000000)));
					break;
				default:
				if ( (cmdq_core_parse_op(op)) != NULL )//WBT Internal TD126350 //NULL pointer dereference check
					CMDQ_AEE("CMDQ",
						 "%s SW timeout, INST:(0x%08x, 0x%08x), OP:%s\n",
...
static int32_t cmdq_exec_task_sync(TaskStruct *pTask, int32_t thread)
{
	int32_t status;
	ThreadStruct *pThread;
	u_long flags;
	...
	if (0 < atomic_read(&gCmdqDebugProgressiveTimeout)) {
		...
		CMDQ_LOG("EXEC: progressive timeout: %d, oroginal: %d\n", start_predump_ms,
			 gCmdqContext.predumpStartTimeMS);
	}
#endif
	...
	if (pThread->taskCount <= 0) {
		...
		/* in order to prevent infinite INVALID_INSTN ISR, which is casued by resuming thread before */
		/* correct HW thread PC from invalid instn, */
		...
		DISP_REG_SET(DISP_REG_CMDQ_THRx_IRQ_FLAG_EN(thread),
			     CMDQ_THR_IRQ_FALG_EXEC_CMD | CMDQ_THR_IRQ_FALG_INSTN_TIMEOUT |
		...
		DISP_REG_SET(DISP_REG_CMDQ_THRx_END_ADDR(thread),
			     pTask->MVABase + pTask->blockSize);
		...
		(DISP_REG_GET(DISP_REG_CMDQ_THRx_PC(thread)) ==
			...
			minimum =
			    (DISP_REG_GET(DISP_REG_CMDQ_THRx_EXEC_CMDS_CNT(thread)) & 0x0FFFF) + 1;
			if (minimum >= 65536)
				minimum = 0;
			...
			for (index = (cookie % CMDQ_MAX_TASK_COUNT); loop > 0; loop--, index--) {
				...
				if (loop > 1) {
					...
					CMDQ_MSG
					    ("Set current(%d) order for the new task(loop <= 1)\n",
					     index);
	...
	status =
	    cmdq_core_wait_task_done(pTask, thread, start_predump_ms, predump_duration_ms,
				     predump_max_count);
	return status;
}
				...
				struct timeval *pGotIRQ)
{
	...
	if (pTask && TASK_STATE_BUSY == pTask->taskState) {
		...
		/* error task process */
		if (attachWarning) {
			cmdq_attach_warning_task(pTask, thread, taskCount, cookie, value,
						 (*pGotIRQ));
		}
		pTask->gotIRQ = (*pGotIRQ);
		pTask->irqFlag = value;
		cmdq_remove_task_from_thread_unlocked(pThread, (cookie % CMDQ_MAX_TASK_COUNT),
						      TASK_STATE_ERROR);
	}
...
void cmdqHandleIRQ(int32_t thread, const uint32_t value)
{
	...
	const uint32_t validIrqFlag =
	    (CMDQ_THR_IRQ_FALG_INVALID_INSTN | CMDQ_THR_IRQ_FALG_INSTN_TIMEOUT |
	...
	isAlreadySuspended =
	    (0 < (DISP_REG_GET(DISP_REG_CMDQ_THRx_SUSPEND(thread)) & 0x1)) ? true : false;
...
int32_t cmdqSubmitTask(int32_t scenario,
		       int32_t priority, uint32_t engineFlag, void *pCMDBlock, int32_t blockSize)
{
	...
	pTask = cmdq_acquire_task(scenario, priority, engineFlag, pCMDBlock, blockSize);
	if (NULL == pTask) {
...
void cmdq_debug_set_sw_timeout(const uint32_t sw_timeout_ms,
