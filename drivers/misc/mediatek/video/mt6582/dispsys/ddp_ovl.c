...
int OVLROI(unsigned int bgW, unsigned int bgH, unsigned int bgColor)
{
	if ((bgW > OVL_MAX_WIDTH) || (bgH > OVL_MAX_HEIGHT)) {
		DISP_ERR("error: OVLROI(), exceed OVL max size, w=%d, h=%d\n", bgW, bgH);
		return -1;
	}
		   ...
		   unsigned char alpha)
{
	...
	if ((dst_w > OVL_MAX_WIDTH) || (dst_h > OVL_MAX_HEIGHT)) {
		DISP_ERR("error: OVLLayerConfig(), exceed OVL max size, dst_w=%d, dst_h=%d\n", dst_w, dst_h);
		return -1;
	}
...
*/
	...
	if ((width != 0) && (((start & 0x70) > 0x40) || ((end & 0x70) < 0x40))) {
		...
		DISP_DBG("OVL align 64byte:ROI(%d,%d %d,%d), (%d,%d), 0x%08x-->0x%08x, %d-->%d\n",
