...
{
	DISP_DBG("dispsys_bypass_color, width=%d, height=%d\n", width, height);
	...
	*(volatile kal_uint32*)(DDP_REG_BASE_DISP_COLOR + 0xf00) = 0x00000001;
	...
	DISP_DBG("dispsys_bypass_color, 0x%x, 0x%x, 0x%x, 0x%x\n",
...
{
	DISP_DBG("dispsys_bypass_bls, width=%d, height=%d, reg=0x%x\n", width, height, ((height << 16) + width));
...
int disp_path_get_mutex_(void)
{
	...
	while (((DISP_REG_GET(DISP_REG_CONFIG_MUTEX(gMutexID)) & DISP_INT_MUTEX_BIT_MASK) != DISP_INT_MUTEX_BIT_MASK)) {
		cnt++;
		if (cnt > 10000) {
			DISP_DBG("[DDP] disp_path_get_mutex() timeout!\n");
...
int disp_path_release_mutex_(void)
{
...
int disp_path_ovl_reset(void)
{
	...
	if (((DISP_REG_GET(DISP_REG_OVL_FLOW_CTRL_DBG) & 0x3ff) != 0x1)
		...
		while (delay_cnt && (((DISP_REG_GET(DDP_REG_BASE_SMI_LARB0) & 0x1) == 0x1) ||
				     (((DISP_REG_GET(DDP_REG_BASE_SMI_COMMON + 0x404) >> 12) & 0x1)
				      != 0x1))) {
			delay_cnt--;
			udelay(100);
		}
...
static int disp_path_release_mutex1_(int mutexID);
	...
	if (engine & DDP_OVL_UPDATE_BIT) {	/* OVL */
		...
		if (DISP_REG_GET(DISP_REG_OVL_EN) == 0 ||
			...
			DISP_ERR("ovl abnormal, en=%d, clk=0x%x\n",
				 DISP_REG_GET(DISP_REG_OVL_EN), DISP_REG_GET(DISP_REG_CONFIG_MMSYS_CG_CON0));
		}
		/* ROI w, h >= 2 */
		...
		if ((DISP_IsVideoMode() == 0 || DISP_IsDecoupleMode()) &&
			...
			DISP_ERR("ovl abnormal, flow_ctrl=0x%x, add_con=0x%x\n",
				 DISP_REG_GET(DISP_REG_OVL_FLOW_CTRL_DBG), DISP_REG_GET(DISP_REG_OVL_ADDCON_DBG));
			disp_clock_check();
			ddp_dump_info(DISP_MODULE_CONFIG);
			ddp_dump_info(DISP_MODULE_MUTEX);
			ddp_dump_info(DISP_MODULE_OVL);
	...
	if (engine & DDP_RDMA_UPDATE_BIT) {	/* RDMA1 */
		if ((DISP_REG_GET(DISP_REG_RDMA_GLOBAL_CON) & 0x1) == 0 ||
			...
			DISP_ERR("rdma abnormal, en=%d, clk=0x%x\n",
				 DISP_REG_GET(DISP_REG_RDMA_GLOBAL_CON), DISP_REG_GET(DISP_REG_CONFIG_MMSYS_CG_CON0));
		}
	}
	...
	if (engine & DDP_WDMA0_UPDATE_BIT) {	/* WDMA0 */
		if (DISP_REG_GET(DISP_REG_WDMA_EN) == 0 ||
			...
			DISP_ERR("wdma0 abnormal, en=%d, clk=0x%x\n",
				 DISP_REG_GET(DISP_REG_WDMA_EN), DISP_REG_GET(DISP_REG_CONFIG_MMSYS_CG_CON0));
		}
	}
...
int disp_path_wait_reg_update(void)
{
	...
	ret = wait_event_interruptible_timeout(g_disp_mutex_wq, g_disp_mutex_reg_update, disp_ms2jiffies(1000));
...
{
	DISP_DBG("[DDP] config_rdma(), idx=%d, mode=%d, in_fmt=%d, addr=0x%x, out_fmt=%d, pitch=%d, x=%d, y=%d,",
	...
	RDMAConfig(pRdmaConfig->idx, pRdmaConfig->mode, pRdmaConfig->inputFormat,
		   ...
		   pRdmaConfig->width, pRdmaConfig->height, pRdmaConfig->isByteSwap, pRdmaConfig->isRGBSwap);
	return 0;
}
...
{
	DISP_DBG
	    ("disp_path_config_wdma2(), idx=%d, dstAddr=0x%08x, fmt=%d, clip(%d,%d %d,%d), dst(%d,%d,%d)\n",
...
#endif
		WDMAConfig2(pConfig->idx, pConfig->inputFormat, pConfig->srcWidth,
			    pConfig->srcHeight, pConfig->clipX, pConfig->clipY, pConfig->clipWidth,
			    pConfig->clipHeight, pConfig->outputFormat, pConfig->dstAddress,
			    pConfig->dstWidth, pConfig->dstPitch, pConfig->dstPitchUV,
...
int disp_path_switch_ovl_mode(struct disp_path_config_ovl_mode_t *pConfig)
{
	...
	if (DISP_IsDecoupleMode()) {
		...
		WDMAConfig(0,
			   ...
			   pConfig->roi.y,
			   pConfig->roi.width,
			   pConfig->roi.height, pConfig->format, pConfig->address, pConfig->roi.width, 1, 0);
		WDMAStart(0);
		...
		RDMAConfig(0, RDMA_MODE_MEMORY,	/* memory mode */
			   ...
			   pConfig->pitch,	/* pitch */
			   pConfig->roi.width, pConfig->roi.height, 0,	/* byte swap */
			   0);	/* is RGB swap */
		RDMAStart(0);
		/* disp_dump_reg(DISP_MODULE_WDMA0); */
	} else {
...
/* unsigned int reg_addr; */
	...
	DISP_DBG("[DDP] config_layer(), layer=%d, en=%d, src=%d, fmt=%d, addr=0x%x, x=%d, y=%d, pitch=%d,",
	...
	DISP_DBG(" dst(%d, %d, %d, %d),keyEn=%d, key=%d, aen=%d, alpha=%d\n",
		pOvlConfig->dst_x,	/* x */
		pOvlConfig->dst_y,	/* y */
	...
	if (pOvlConfig->layer_en != 0) {
		...
		/* when you clean up a layer, make sure the dirty is set. */
		if (pOvlConfig->isDirty != 0) {
			OVLLayerConfig(pOvlConfig->layer,	/* layer */
	...
	/* DISP_DBG("[DDP]disp_path_config_layer() done, addr=0x%x\n", pOvlConfig->addr); */
...
int disp_path_config_mem_out_without_lcd(struct disp_path_config_mem_out_struct *pConfig)
{
	...
	if (pConfig->enable == 1) {
		...
		WDMAConfig(0,
			   ...
			   0,
			   pConfig->srcROI.width,
			   pConfig->srcROI.height, pConfig->outFormat, pConfig->dstAddr, pConfig->srcROI.width, 1, 0);
		WDMAStart(0);
	...
	if (pConfig->enable == 1) {
		...
		if (pConfig->outFormat == eYUV_420_3P) {
			WDMAConfigUV(0,
				     ...
				     pConfig->dstAddr +
				     pConfig->srcROI.width * pConfig->srcROI.height * 5 / 4, pConfig->srcROI.width);
		}
		WDMAStart(0);
...
int disp_path_config_(struct disp_path_config_struct *pConfig, int mutexId)
{
	...
	DISP_DBG("[DDP] disp_path_config(), srcModule=%d, addr=0x%x, inFormat=%d, pitch=%d,",
		pConfig->srcModule, pConfig->addr, pConfig->inFormat, pConfig->pitch);
	DISP_DBG(" bgROI(%d,%d,%d,%d), bgColor=%d, outFormat=%d, dstModule=%d, dstAddr=0x%x, dstPitch=%d, mutexId=%d\n",
		pConfig->bgROI.x, pConfig->bgROI.y, pConfig->bgROI.width, pConfig->bgROI.height,
		pConfig->bgColor, pConfig->outFormat, pConfig->dstModule, pConfig->dstAddr, pConfig->dstPitch, mutexId);
	...
	case DISP_MODULE_DPI0:
		DISP_INF("DISI_MODULE_DPI0\n");
#if defined(CONFIG_MTK_HDMI_SUPPORT)
		if (DISP_IsDecoupleMode() || is_hdmi_active()) {
#else
		if (DISP_IsDecoupleMode()) {
#endif
			DISP_REG_SET(DISP_REG_CONFIG_DISP_OVL_MOUT_EN, 1 << 1);	/* OVL-->WDMA0 */
	...
	if (pConfig->srcModule != DISP_MODULE_RDMA1) {
		...
		if (pConfig->ovl_config.layer < 4) {
			...
			if (pConfig->ovl_config.layer_en != 0) {
				if (pConfig->ovl_config.addr == 0 ||
				    pConfig->ovl_config.dst_w == 0 || pConfig->ovl_config.dst_h == 0) {
					DISP_ERR
		...
		if (pConfig->dstModule == DISP_MODULE_WDMA0) {
			WDMAReset(0);
			if (pConfig->dstAddr == 0 || pConfig->srcROI.width == 0 || pConfig->srcROI.height == 0) {
			...
			if (DISP_IsDecoupleMode()) {
				DISP_INF("from de-couple\n");
				WDMAReset(0);
				if (decouple_addr == 0 || pConfig->srcROI.width == 0 || pConfig->srcROI.height == 0) {
					DISP_ERR
					    ("wdma parameter invalidate, addr=0x%x, w=%d, h=%d\n",
					     decouple_addr, pConfig->srcROI.width, pConfig->srcROI.height);
					return -1;
				}
				WDMAConfig(0,
					   WDMA_INPUT_FORMAT_ARGB,
					   ...
					   0,
					   pConfig->srcROI.width,
					   pConfig->srcROI.height, eRGB888, decouple_addr, pConfig->srcROI.width, 1, 0);
				WDMAStart(0);
				/* Register WDMA intr */
				mem_out_done = 0;
				disp_register_irq(DISP_MODULE_WDMA0, _disp_path_wdma_callback);
		...
		RDMAConfig(1, RDMA_MODE_MEMORY,	/* /direct link mode */
...
void reg_backup(unsigned int reg_addr)
{
	*(pRegBackup + reg_offset) = DISP_REG_GET(reg_addr);
	...
	DISP_INF("0x%08x(0x%08x), ", reg_addr, *(pRegBackup + reg_offset));
	if ((reg_offset + 1) % 8 == 0)
		DISP_INF("\n");
#endif
	reg_offset++;
...
#ifdef DDP_RECORD_REG_BACKUP_RESTORE
	DISP_INF("0x%08x(0x%08x), ", reg_addr, DISP_REG_GET(reg_addr));
	if ((reg_offset + 1) % 8 == 0)
		DISP_INF("\n");
#endif
	reg_offset++;
