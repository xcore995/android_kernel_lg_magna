...
static void process_dbg_opt(const char *opt)
{
	...
	if (0 == strncmp(opt, "regr:", 5)) {
		...
		char *p = (char *)opt + 5;
		kstrtoul(p, 16, &result);
		unsigned int addr = (unsigned int)result;
		kstrtoul(p + 1, 16, &result);
		...
		char *p = (char *)opt + 8;
		kstrtoul(p, 10, &result);
		unsigned int enable = (unsigned int)result;
		if (enable)
			dbg_log = 1;
		else
			dbg_log = 0;
		...
		char *p = (char *)opt + 10;
		kstrtoul(p, 10, &result);
		unsigned int level = (unsigned int)result;
		...
		char *p = (char *)opt + 9;
		kstrtoul(p, 10, &result);
		unsigned int module = (unsigned int)result;
		...
		char *p = (char *)opt + 9;
		kstrtoul(p, 10, &result);
		unsigned int arg = (unsigned int)result;
		if (arg == 0) {
			int i;
		...
		if (enable == 1) {
			...
			DISP_MSG("ddp debug set gUltraLevel = %d, DISP_REG_RDMA_MEM_GMC_SETTING_0=0x%x,",
				gUltraLevel, DISP_REG_GET(DISP_REG_RDMA_MEM_GMC_SETTING_0));
			DISP_MSG(" DISP_REG_RDMA_FIFO_CON=0x%x\n", DISP_REG_GET(DISP_REG_RDMA_FIFO_CON));
		} else if (enable == 21) {
			sprintf(buf, "base:\nconfig f4+0\novl 7\nrdma 8\nrdma1 12\nwdma 9\nbls a\ncolor b\ndsi c\n");
			sprintf(buf + strlen(buf), "dpi d\nmm_mutex e\nmm_cmdq f\nsmi_larb0 10\nsmi_common 11\n");
		} else if (enable == 22) {
		...
		kstrtoul(p, 10, &result);
		sat_upper = (unsigned int)result;
		p++;
		kstrtoul(p, 10, &result);
		sat_lower = (unsigned int)result;
		p++;
		kstrtoul(p, 10, &result);
		hue_upper = (unsigned int)result;
		p++;
		kstrtoul(p, 10, &result);
		hue_lower = (unsigned int)result;
...
int ddp_mem_test2(void)
{
	...
	if (pSrc == 0) {
		pr_debug("[DDP] error: dma_alloc_coherent error!  dma memory not available.\n");
		return 0;
	} else {
	...
	if (result)
		pr_debug("[DDP] error: DDPK_Bitblt_Config fail!, ret=%d\n", result);
	...
	if (result)
		pr_debug("[DDP] error: DDPK_Bitblt() fail, result=%d\n", result);
	...
	/* result verify */
	{
		...
		if (diff_cnt == 0)
			...
			pr_debug("[DDP] error: ddp_mem_test src compare result: fail\n");
...

	{
		...
		for (t = 0; t < size; t++) {
			if (*((unsigned char *)pDst + t) != *((unsigned char *)data_rgb888_64x64_golden + t)) {
				...
				pr_debug("t=%d, diff_cnt=%d, dst=0x%x, gold=0x%x\n",
				       t,
				       diff_cnt,
				       *((unsigned char *)pDst + t), *((unsigned char *)data_rgb888_64x64_golden + t));
			}
		...
		if (diff_cnt == 0)
			...
			pr_debug("[DDP] error: ddp_mem_test result: fail\n");
			pr_debug("detail, %d, %d, %%%d\n", diff_cnt, size, diff_cnt * 100 / size);
			result = -1;
		}
	}
...
int ddp_mem_test(void)
{
	...
	if (pSrc == 0 || pSrcPa == 0) {
		pr_debug("dma_alloc_coherent error!  dma memory not available.\n");
		return 0;
	} else {
	...
	if (0) {		/* compare source */
		...
		if (diff_cnt == 0)
			...
			pr_debug("ddp_mem_test src compare result: fail\n");
			pr_debug("detail, %d, %d, %%%d\n", diff_cnt, size, diff_cnt * 100 / size);
			result = -1;
		}
	}
	...
	if (1) {		/* compare dst */
		...
		if (diff_cnt == 0)
			...
			pr_debug("ddp_mem_test result: fail\n");
			pr_debug("detail, %d, %d, %%%d\n", diff_cnt, size, diff_cnt * 100 / size);
			result = -1;
		}
	}
	...
	case DISP_MODULE_MUTEX:
		...
		for (i = 0; i < 3; i++) {
			...
			if ((DISP_REG_GET(DISP_REG_CONFIG_MUTEX_MOD(i)) != 0) &&
				...
				DISP_MSG("mutex%d, mode=%s, module=(",
					 i,
