...
const char *cmdq_core_get_event_name(uint32_t event)
{
	...
	case 12:
		return "EOF_DISP_WDMA";
	case 13:
		return "EOF_DISP_RDMA";
	case 14:
		return "EOF_DISP_BLS";
	case 15:
		return "EOF_DISP_COLOR";
	case 16:
		return "EOF_DISP_OVL";
	case 17:
		return "EOF_MDP_TDSHP";
	case 18:
		return "EOF_MDP_RSZ1";
	case 19:
		return "EOF_MDP_RSZ0";
	...
	case 30:
		return "SOF_MDP_RDMA";
	case 31:
		return "SOF_DISP_WDMA";
	case 32:
		return "SOF_DISP_COLOR";
	case 33:
		return "SOF_MDP_TDSHP";
	case 34:
		return "SOF_DISP_BLS";
	case 35:
		return "SOF_DISP_RDMA";
	case 36:
		return "SOF_CAM_MDP";
	default:
		return "UNKOWN";
...
uint32_t *cmdq_core_dump_pc(const TaskStruct *pTask, int thread, const char *tag)
{
	...
	if (hwPC) {
		...
		case CMDQ_CODE_WFE:
			...
			CMDQ_LOG
			    ("[%s] Curr PC(VA): 0x%p, Inst: 0x%08x:0x%08x, WAIT for %s, token_value=0x%08x\n",
			     tag, hwPC, insts[2], insts[3], cmdq_core_get_event_name(argA), value);
			break;
		case CMDQ_CODE_POLL:
			CMDQ_LOG("[%s] Curr PC(VA): 0x%p, Inst: 0x%08x:0x%08x, POLL for %s\n",
				 tag, hwPC, insts[2], insts[3], cmdq_core_parse_subsys(argA, argB));
			break;
		default:
			CMDQ_LOG("[%s] Curr PC(VA): 0x%p, Inst: 0x%08x:0x%08x, OP: %s\n", tag, hwPC,
				 insts[2], insts[3], cmdq_core_parse_op(op));
			break;
		}
...
void cmdq_mdp_dump_rot(uint32_t base, const char *label)
{
	...
	CMDQ_ERR
	    ("[CMDQ] ROT_RST: 0x%08x, ROT_RST_STAT: 0x%08x, ROT_INIT: 0x%08x, [CMDQ] ROT_CROP_OFST: 0x%08x\n",
	...
	CMDQ_ERR
	    ("[CMDQ] ROT_BASE_ADDR_C: 0x%08x, ROT_OFST_ADDR_C: 0x%08x, ROT_STRIDE_C: 0x%08x, ROT_DITHER: 0x%08x\n",
