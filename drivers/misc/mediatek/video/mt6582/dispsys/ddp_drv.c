...
unsigned int irq_log = 0;/* must disable irq level log by default, else will block uart output */
...
int disp_lock_cmdq_thread(void)
{
	...
	DISP_INF("disp_lock_cmdq_thread()called\n");
	...
	DISP_INF("disp_lock_cmdq_thread(), i=%d\n", i);
...
static int disp_wait_intr(DISP_MODULE_ENUM module, unsigned int timeout_ms)
{
	...
	ret = wait_event_interruptible_timeout(g_disp_irq_done_queue,
					       g_disp_irq.irq_src & (1 << module), msecs_to_jiffies(timeout_ms));
...
static void print_mutex_timeout_info(void)
{
	unsigned int reg = 0;
		...
		if (reg_val & 0x0FF00) { /* udpate timeout intr triggered */
			...
			for (mutexID = 0; mutexID < 4; mutexID++) {
				if ((DISP_REG_GET(DISP_REG_CONFIG_MUTEX_INTSTA) &
				     (1 << (mutexID + 8))) == (1 << (mutexID + 8))) {
					print_mutex_timeout_info();
					cnt_mutex_timeout++;
					disp_check_clock_tree();
					/* reset mutex */
...
void DISP_REG_SET_FIELD(unsigned long field, unsigned long reg32, unsigned long val)
{
	...
	/* *(volatile unsigned int*)(reg32) =
		((*(volatile unsigned int*)(reg32) & ~(REG_FLD_MASK(field))) |  REG_FLD_VAL((field), (val))); */
...
int disp_color_set_pq_param(void *arg)
{
	...
	if (copy_from_user(pq_param, (void *)arg, sizeof(DISP_PQ_PARAM))) {
		DISP_INF("disp driver : DISP_IOCTL_SET_PQPARAM Copy from user failed\n");
		ReleaseUpdateMutex();
		return -EFAULT;
	}
...
/* unsigned long flags; */
	...
	case DISP_IOCTL_WRITE_REG:
		...
		*(volatile unsigned int *)wParams.reg =
		    (*(volatile unsigned int *)wParams.reg & ~wParams.mask) | (wParams.val & wParams.mask);
			...
			if (rTableParams.reg[count] > DISPSYS_REG_ADDR_MAX
				...
				DISP_ERR
				    ("reg read, addr invalid, addr min=0x%x, max=0x%x, addr=0x%x\n",
				     DISPSYS_REG_ADDR_MIN, DISPSYS_REG_ADDR_MAX, rTableParams.reg[count]);
				continue;
			}

			rTableParams.val[count] =
			    (*(volatile unsigned int *)rTableParams.reg[count]) & rTableParams.mask[count];
		}
	...
	case DISP_IOCTL_LOCK_THREAD:
		DISP_INF("DISP_IOCTL_LOCK_THREAD!\n");
...
/* cmq_status[value] = 1; */
...
#if !defined(CONFIG_MTK_AAL_SUPPORT)
		DISP_ERR("Invalid operation DISP_IOCTL_AAL_EVENTCTL since AAL is not turned on, in %s\n", __func__);
		return -EFAULT;
#else
		if (copy_from_user(&value, (void *)arg, sizeof(int))) {
			DISP_ERR("disp driver : DISP_IOCTL_AAL_EVENTCTL Copy from user failed\n");
			return -EFAULT;
		}
	...
	case DISP_IOCTL_GET_AALSTATISTICS:
		...
		if (disp_wait_hist_update(60)) {
			DISP_ERR("disp driver : DISP_IOCTL_GET_AALSTATISTICS wait time out\n");
			return -EFAULT;
		}
		...
		if (copy_to_user((void *)arg, (void *)(disp_get_hist_ptr()), sizeof(DISP_AAL_STATISTICS))) {
			DISP_ERR("disp driver : DISP_IOCTL_GET_AALSTATISTICS Copy to user failed\n");
			return -EFAULT;
		}
...
#if !defined(CONFIG_MTK_AAL_SUPPORT)
		DISP_ERR("Invalid operation : DISP_IOCTL_SET_AALPARAM since AAL is not turned on, in %s\n", __func__);
		return -EFAULT;
#else
/* disp_set_needupdate(DISP_MODULE_BLS , 0); */
		{
			...
			if (copy_from_user(aal_param, (void *)arg, sizeof(DISP_AAL_PARAM))) {
				DISP_ERR("disp driver : DISP_IOCTL_SET_AALPARAM Copy from user failed\n");
				return -EFAULT;
			}
	...
	case DISP_IOCTL_SET_PQINDEX:
		...
		if (copy_from_user(pq_index, (void *)arg, sizeof(DISPLAY_PQ_T))) {
			DISP_ERR("disp driver : DISP_IOCTL_SET_PQINDEX Copy from user failed\n");
			return -EFAULT;
		}
	...
	case DISP_IOCTL_GET_PQPARAM:
		...
		if (copy_to_user((void *)arg, pq_param, sizeof(DISP_PQ_PARAM))) {
			DISP_ERR("disp driver : DISP_IOCTL_GET_PQPARAM Copy to user failed\n");
			return -EFAULT;
		}
	...
	case DISP_IOCTL_SET_TDSHPINDEX:
		...
		if (copy_from_user(tdshp_index, (void *)arg, sizeof(DISPLAY_TDSHP_T))) {
			DISP_ERR("disp driver : DISP_IOCTL_SET_TDSHPINDEX Copy from user failed\n");
			return -EFAULT;
		}
	...
	case DISP_IOCTL_GET_TDSHPINDEX:
		...
		if (copy_to_user((void *)arg, tdshp_index, sizeof(DISPLAY_TDSHP_T))) {
			DISP_ERR("disp driver : DISP_IOCTL_GET_TDSHPINDEX Copy to user failed\n");
			return -EFAULT;
		}
	...
	case DISP_IOCTL_SET_GAMMALUT:
		...
		if (copy_from_user(gamma_index, (void *)arg, sizeof(DISPLAY_GAMMA_T))) {
			DISP_ERR("disp driver : DISP_IOCTL_SET_GAMMALUT Copy from user failed\n");
			return -EFAULT;
		}
	...
	case DISP_IOCTL_SET_CLKON:
		if (copy_from_user(&module, (void *)arg, sizeof(DISP_MODULE_ENUM))) {
			DISP_ERR("disp driver : DISP_IOCTL_SET_CLKON Copy from user failed\n");
			return -EFAULT;
		}
	...
	case DISP_IOCTL_SET_CLKOFF:
		if (copy_from_user(&module, (void *)arg, sizeof(DISP_MODULE_ENUM))) {
			DISP_ERR("disp driver : DISP_IOCTL_SET_CLKOFF Copy from user failed\n");
			return -EFAULT;
		}
	...
	case DISP_IOCTL_MUTEX_CONTROL:
		if (copy_from_user(&value, (void *)arg, sizeof(int))) {
			DISP_ERR("disp driver : DISP_IOCTL_MUTEX_CONTROL Copy from user failed\n");
			return -EFAULT;
		}
		...
		if (value == 1) {
			...
			DISP_ERR("disp driver : DISP_IOCTL_MUTEX_CONTROL invalid control\n");
			return -EFAULT;
		}
	...
	case DISP_IOCTL_GET_LCMINDEX:
		...
		if (copy_to_user((void *)arg, &lcmindex, sizeof(unsigned long))) {
			DISP_ERR("disp driver : DISP_IOCTL_GET_LCMINDEX Copy to user failed\n");
			return -EFAULT;
		}
	...
	case DISP_IOCTL_SET_PQ_CAM_PARAM:
		...
		if (copy_from_user(pq_cam_param, (void *)arg, sizeof(DISP_PQ_PARAM))) {
			DISP_ERR("disp driver : DISP_IOCTL_SET_PQ_CAM_PARAM Copy from user failed\n");
			return -EFAULT;
		}
	...
	case DISP_IOCTL_GET_PQ_CAM_PARAM:
		...
		if (copy_to_user((void *)arg, pq_cam_param, sizeof(DISP_PQ_PARAM))) {
			DISP_ERR("disp driver : DISP_IOCTL_GET_PQ_CAM_PARAM Copy to user failed\n");
			return -EFAULT;
		}
	...
	case DISP_IOCTL_SET_PQ_GAL_PARAM:
		...
		if (copy_from_user(pq_gal_param, (void *)arg, sizeof(DISP_PQ_PARAM))) {
			DISP_ERR("disp driver : DISP_IOCTL_SET_PQ_GAL_PARAM Copy from user failed\n");
			return -EFAULT;
		}
	...
	case DISP_IOCTL_GET_PQ_GAL_PARAM:
		...
		if (copy_to_user((void *)arg, pq_gal_param, sizeof(DISP_PQ_PARAM))) {
			DISP_ERR("disp driver : DISP_IOCTL_GET_PQ_GAL_PARAM Copy to user failed\n");
			return -EFAULT;
		}
...
static int disp_mmap(struct file *file, struct vm_area_struct *a_pstVMArea)
{
	...
	if (paStart < (DISPSYS_REG_ADDR_MIN - 0xE0000000)
	    || paEnd > (DISPSYS_REG_ADDR_MAX - 0xE0000000)) {
...
const struct dev_pm_ops disp_pm_ops = {
