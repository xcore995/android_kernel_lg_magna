			       ...
			       WDMA_CONFIG_STRUCT *pWDMAConfig)
{
	int idx;
	if (job == NULL) {
		DISP_DRV_ERR("[FB Driver] error: _DISP_InitHWConfig() invalid param!\n");
		return;
	}
	mutex_lock(&job->lock);
	...
	for (i = 0; i < DDP_OVL_LAYER_MUN; i++) {
		if (job->input[i].dirty) {
			...
			if (job->input[i].layer_id >= DDP_OVL_LAYER_MUN) {
				DISP_DRV_WARN("[FB Driver] still found invalid layer_id=0x%x(%d)\n",
	...
	while (1) {
		...
		while ((job = disp_acquire_job()) != NULL) {
			...
			if (is_early_suspended) {
				trigger_cnt++;
				DISP_DRV_WARN("In early_suspend, trigger OVL-WDMA to work cnt=%d\n",
					      trigger_cnt);
...
void DISP_WaitMemWriteDoneIfNeeded(void)
{
	...
	if (DISP_IsDecoupleMode()) {
		if (wait_event_interruptible_timeout(write_mem_idle_wq, !write_mem_running, HZ / 10)
		    == 0) {
			DISP_DRV_ERR("[FB Driver] wait write mem done timeout!\n");
		}
	}
#endif
}
...
DISP_STATUS DISP_SwitchDisplayMode(struct fb_overlay_mode *pConfig)
{
	...
	if (!is_early_suspended) {
		DISP_DRV_MSG("[FB Driver] Switch to display mode: %s\n",
		       (pConfig->mode == DISP_DECOUPLE_MODE) ? "de-couple" : "direct-link");
		MMProfileLogEx(MTKFB_MMP_Events.SwitchMode, MMProfileFlagStart, disp_mode,
			       pConfig->mode);
	/**
	 ...
	 */
		if (disp_mode == DISP_DIRECT_LINK_MODE) {
			DISP_DRV_MSG("[FB Driver] Switch display mode-0_1!\n");
			...
			DISP_DRV_MSG("[FB Driver] Switch display mode-0_2!\n");
			...
			DISP_DRV_MSG("[FB Driver] Switch display mode-0_3!\n");
			MMProfileLogEx(MTKFB_MMP_Events.SwitchMode, MMProfileFlagPulse, 0, 3);

		} else {
			wait_event_interruptible_timeout(write_mem_idle_wq, !write_mem_running,
							 HZ / 10);
			DISP_DRV_MSG("[FB Driver] Switch display mode-1_0!\n");
	 ...
	 */
		...
		DISP_DRV_MSG("[FB Driver] Switch display mode-2_0!\n");
		...
		DISP_DRV_MSG("[FB Driver] Switch display mode-2_1!\n");
	 ...
	 */
		...
		config.address =
		    (pConfig->mode == DISP_DECOUPLE_MODE) ? disp_request_buffer(buffer_queue) : 0;
		...
		DISP_DRV_MSG("[FB Driver] Switch display mode done!\n");
		MMProfileLogEx(MTKFB_MMP_Events.SwitchMode, MMProfileFlagEnd, disp_mode,
			       pConfig->mode);
	}
	ret = DISP_STATUS_OK;
#endif
...
static void disp_dump_lcm_parameters(LCM_PARAMS *lcm_params)
{
	...
	DISP_DRV_MSG("[mtkfb] LCM TYPE: %s\n", LCM_TYPE_NAME[lcm_params->type]);
	DISP_DRV_MSG("[mtkfb] LCM INTERFACE: %s\n", LCM_CTRL_NAME[lcm_params->ctrl]);
	DISP_DRV_MSG("[mtkfb] LCM resolution: %d x %d\n", lcm_params->width, lcm_params->height);
...
BOOL disp_get_lcm_name_boot(char *cmdline)
{
	...
	DISP_DRV_MSG("[mtkfb] LCM is %sconnected\n", ((isLCMConnected) ? "" : "not "));
	p += 2;
	q = p;
	while (*q != ' ' && *q != '\0')
		q++;
...
BOOL DISP_SelectDeviceBoot(const char *lcm_name)
{
	...
	if (NULL == lcm_drv) {
		DISP_DRV_ERR("%s, get_lcm_driver returns NULL\n", __func__);
		return FALSE;
	}
	isLCMFound = TRUE;
...
BOOL DISP_SelectDevice(const char *lcm_name)
{
	lcm_drv = disphal_get_lcm_driver(lcm_name, (unsigned int *)&u4IndexOfLCMList);
	if (NULL == lcm_drv) {
		DISP_DRV_ERR("%s, disphal_get_lcm_driver() returns NULL\n", __func__);
		return FALSE;
	}
	isLCMFound = TRUE;
...
DISP_STATUS DISP_Init(UINT32 fbVA, UINT32 fbPA, BOOL isLcmInited)
{
	...
	case LCM_TYPE_DBI:
		{
			...
			DISP_DRV_MSG("DISP Info: DBI, CMD Mode, HW Vsync enable\n");
			break;
		}
	case LCM_TYPE_DPI:
		{
			...
			dispif_info[MTKFB_DISPIF_PRIMARY_LCD].isHwVsyncAvailable = 1;
			DISP_DRV_MSG("DISP Info: DPI, VDO Mode, HW Vsync enable\n");
			break;
		}
	case LCM_TYPE_DSI:
		{
			...
			if (lcm_params->dsi.mode == CMD_MODE) {
				dispif_info[MTKFB_DISPIF_PRIMARY_LCD].displayMode =
				    DISPIF_MODE_COMMAND;
				dispif_info[MTKFB_DISPIF_PRIMARY_LCD].isHwVsyncAvailable = 1;
				DISP_DRV_MSG("DISP Info: DSI, CMD Mode, HW Vsync enable\n");
			} else {
				dispif_info[MTKFB_DISPIF_PRIMARY_LCD].displayMode =
				    DISPIF_MODE_VIDEO;
				dispif_info[MTKFB_DISPIF_PRIMARY_LCD].isHwVsyncAvailable = 1;
				DISP_DRV_MSG("DISP Info: DSI, VDO Mode, HW Vsync enable\n");
			}
...

	{
		...
		if (lcm_drv) {
			...
			DISP_DRV_ERR("DISP Info: Fatal Error!!, lcm_drv is null\n");
		}
	}
...
#if 0
	{
		...
		DISP_DRV_MSG("DISP Info: from DFO, width/height=%d x %d\n", width, height);
	}
#endif
...
DISP_STATUS DISP_PowerEnable(BOOL enable)
{
	...
	if (down_interruptible(&sem_update_screen)) {
		DISP_DRV_ERR("ERROR: Can't get sem_update_screen in DISP_PowerEnable()\n");
...
DISP_STATUS DISP_PanelEnable(BOOL enable)
{
	...
	if (down_interruptible(&sem_update_screen)) {
		DISP_DRV_ERR("ERROR: Can't get sem_update_screen in DISP_PanelEnable()\n");
		return DISP_STATUS_ERROR;
	}
...
DISP_STATUS DISP_SetBacklight(UINT32 level)
{
	...
	if (down_interruptible(&sem_update_screen)) {
		DISP_DRV_ERR("ERROR: Can't get sem_update_screen in DISP_SetBacklight()\n");
		return DISP_STATUS_ERROR;
	}
...
DISP_STATUS DISP_SetBacklight_mode(UINT32 mode)
{
	...
	if (down_interruptible(&sem_update_screen)) {
		DISP_DRV_ERR("ERROR: Can't get sem_update_screen in DISP_SetBacklight_mode()\n");
		return DISP_STATUS_ERROR;
	}
...
DISP_STATUS DISP_SetPWM(UINT32 divider)
{
	...
	if (down_interruptible(&sem_update_screen)) {
		DISP_DRV_ERR("ERROR: Can't get sem_update_screen in DISP_SetPWM()\n");
		return DISP_STATUS_ERROR;
	}
...
DISP_STATUS DISP_UpdateScreen(UINT32 x, UINT32 y, UINT32 width, UINT32 height)
{
	...
	if (down_interruptible(&sem_update_screen)) {
		DISP_DRV_MSG("ERROR: Can't get sem_update_screen in DISP_UpdateScreen()\n");
		return DISP_STATUS_ERROR;
	}
				 ...
				 DISP_EXTRA_CONFIG_PTR pConfFunc)
{
	...
	if ((NULL == pCheckUpdateFunc) || (NULL == pConfFunc)) {
		DISP_DRV_WARN("Warnning! [Func]%s register NULL function : %p,%p\n",
...
void DISP_UnRegisterExTriggerSource(int u4ID)
{
	if (DISP_CB_MAXCNT < (u4ID + 1)) {
		DISP_DRV_WARN("Warnning! [Func]%s unregister a never registered function : %d\n",
			      __func__, u4ID);
		return;
	}
	...
	if (gCaptureLayerEnable && pLayer->layer_en) {
		if (gCaptureLayerRawData) {
			...
			case ePABGR8888:
				...
				break;
			default:
				DISP_DRV_ERR("error: _DISP_DumpLayer(), unknow format=%d\n",
					     pLayer->fmt);
		...
		for (i = 0; i < DDP_OVL_LAYER_MUN; i++) {
			...
			if (ovl_config.isDirty) {
				...
				if (ovl_config.layer_en) {
					disp_sync_inc_timeline(disp_config.session_id,
							       ovl_config.layer,
...
static int _DISP_ConfigUpdateKThread(void *data)
{
	...
	while (1) {
		...
		if (down_interruptible(&sem_early_suspend)) {
			DISP_DRV_WARN("[FB Driver] can't get semaphore:%d\n", __LINE__);
			continue;
		}
...
{
	DISP_DRV_MSG("DISP_HalfVsync, enable half vsync, %d\n", half_vsync);
	half_vsync = enable;
}
...
static void _DISP_ScrUpdStartCallback(void *pParam)
{
	...
	unsigned int primary_session =
	    MAKE_DISP_SESSION(DISP_SESSION_DECOUPLE_MODE, DISP_SESSION_PRIMARY, 0);
...
void DISP_InitVSYNC(unsigned int vsync_interval)
{
	...
	if (IS_ERR(config_update_task)) {
		DISP_DRV_WARN("DISP_InitVSYNC(): Cannot create config update kthread\n");
		return;
	}
	...
	if (IS_ERR(clean_up_task))
		DISP_DRV_WARN("DISP_InitVSYNC(): Cannot create clean up kthread\n");
	...
	if (IS_ERR(decouple_worker_task))
		DISP_DRV_WARN("DISP_InitVSYNC(): Cannot create disp_worker kthread\n");
	...
	if ((LCM_TYPE_DBI == lcm_params->type) ||
		...
		DISP_DRV_MSG("[MTKFB] vsync timer_period=%d\n", vsync_interval);
...
void DISP_WaitVSYNC(void)
{
	...
	if (wait_event_interruptible_timeout(vsync_wq, vsync_wq_flag, HZ / 10 * 2) == 0)
		DISP_DRV_ERR("[DISP] Wait VSync timeout. early_suspend=%d\n", is_early_suspended);
...
DISP_STATUS DISP_PauseVsync(BOOL enable)
{
	if ((LCM_TYPE_DBI == lcm_params->type) ||
		...
		DISP_DRV_WARN("DISP_PauseVSYNC():unknown interface\n");
	}
	return DISP_STATUS_OK;
}
...
BOOL DISP_IsVideoMode(void)
{
	...
	if (lcm_params)
		...
		DISP_DRV_WARN
		    ("WARNING!! DISP_IsVideoMode is called before display driver inited!\n");
		return 0;
	}
}
...
UINT32 DISP_GetScreenWidth(void)
{
	...
	if (lcm_params)
		...
		DISP_DRV_WARN("WARNING!! get screen width before display driver inited!\n");
		return 0;
	}
}
...
UINT32 DISP_GetScreenHeight(void)
{
	...
	if (lcm_params)
		...
		DISP_DRV_WARN("WARNING!! get screen height before display driver inited!\n");
		return 0;
	}
}
...
UINT32 DISP_GetPhysicalHeight(void)
{
	disp_drv_init_context();
	if (lcm_params) {
		DISP_DRV_MSG("[wwy]lcm_parms->physical_height = %d\n", lcm_params->physical_height);
		return lcm_params->physical_height;
	} else {
		DISP_DRV_WARN("WARNING!! get physical_height before display driver inited!\n");
		return 0;
	}
}
...
UINT32 DISP_GetPhysicalWidth(void)
{
	disp_drv_init_context();
	if (lcm_params) {
		DISP_DRV_MSG("[wwy]lcm_parms->physical_width = %d\n", lcm_params->physical_width);
		return lcm_params->physical_width;
	} else {
		DISP_DRV_WARN("WARNING!! get physical_width before display driver inited!\n");
		return 0;
	}
}
...
DISP_STATUS DISP_SetScreenBpp(UINT32 bpp)
{
	...
	if (bpp != 16 && bpp != 24 && bpp != 32 && 1) {
		DISP_DRV_WARN("DISP_SetScreenBpp error, not support %d bpp\n", bpp);
		return DISP_STATUS_ERROR;
	}
...
UINT32 DISP_GetVRamSizeBoot(char *cmdline)
{
	...
	if (disp_if_drv)
		...
		DISP_DRV_WARN
		    ("%s, can't get lcm type, reserved memory size will be set as 0x800000\n",
		     __func__);
		return 0x1400000;
	}
	/* Align vramSize to 1MB */
	...
	DISP_DRV_MSG("DISP_GetVRamSizeBoot: %u bytes[%dMB]\n", vramSize, (vramSize >> 20));
				     ...
				     unsigned int is_early_suspended)
{
	...
	ret =
	    disphal_map_overlay_out_buffer(pvbuf,
					   ...
					   &mva);
	if (ret != 0) {
		DISP_DRV_ERR("disphal_map_overlay_out_buffer fail!\n");
		return DISP_STATUS_OK;
	}
	disphal_init_overlay_to_memory();
...
DISP_STATUS DISP_Change_Update(unsigned int speed)
{
	...
	if (down_interruptible(&sem_update_screen)) {
		DISP_DRV_WARN("ERROR: Can't get sem_update_screen in DISP_Change_Update()\n");
		return DISP_STATUS_ERROR;
	}
...
unsigned int DISP_AutoTest(void)
{
	unsigned int ret = 0;
	if (down_interruptible(&sem_update_screen)) {
		DISP_DRV_ERR("ERROR: Can't get sem_update_screen in DISP_Change_Update()\n");
		return DISP_STATUS_ERROR;
	}
...
BOOL DISP_EsdCheck(void)
{
	...
	if (down_interruptible(&sem_update_screen)) {
		DISP_DRV_ERR("ERROR: Can't get sem_update_screen in DISP_EsdCheck()\n");
		return FALSE;
	}
...
BOOL DISP_EsdRecover(void)
{
	...
	if (down_interruptible(&sem_update_screen)) {
		DISP_DRV_ERR("ERROR: Can't get sem_update_screen in DISP_EsdRecover()\n");
		return FALSE;
	}
...
BOOL fbconfig_dsi_vdo_prepare(void)
{
	if (!is_early_suspended) {
		...
		DISP_DRV_MSG("fbconfig=>02will set cmd mode in disp_drv.c!!\n");
...
{
	if (lcm_params) {
		DISP_DRV_MSG("LCM Resolution will be changed, original: %dx%d, now: %dx%d\n",
			     lcm_params->width, lcm_params->height, width, height);
		if (width > lcm_params->width || height > lcm_params->height || width == 0
		    || height == 0) {
			DISP_DRV_ERR("Invalid resolution: %dx%d\n", width, height);
			return DISP_STATUS_ERROR;
		}

		if (DISP_IsVideoMode()) {
			DISP_DRV_ERR("Warning!!!Video Mode can't support multiple resolution!\n");
			return DISP_STATUS_ERROR;
		}
...
{
	DISP_DRV_MSG("DISP_Auto_Capture_FB width %d height %d\n", wdma_width, wdma_height);
...
static BOOL fbconfig_disp_set_clk_prepare(unsigned int clk)
{
	if (!is_early_suspended) {
		...
		if (fbconfig_if_drv->set_cmd_mode)
			fbconfig_if_drv->set_cmd_mode();
		DISP_DRV_MSG("sxk==>in disp_drv 2124:%d\n", clk);
...
void fbconfig_disp_set_mipi_clk(unsigned int clk)
{
	if (lcm_params->dsi.mode != CMD_MODE) {
		if (down_interruptible(&sem_early_suspend)) {
			DISP_DRV_ERR("sxk=>can't get semaphore in fbconfig_disp_set_mipi_clk()\n");
			return;
		}
...
void fbconfig_disp_set_mipi_ssc(unsigned int ssc)
{
	if (lcm_params->dsi.mode != CMD_MODE) {
		if (down_interruptible(&sem_early_suspend)) {
			DISP_DRV_ERR("sxk=>can't get semaphore in fbconfig_disp_set_mipi_ssc()\n");
			return;
		}
...
int fbconfig_get_esd_check(void)
{
	...
	if (lcm_params->dsi.mode != CMD_MODE) {
		if (down_interruptible(&sem_early_suspend)) {
			DISP_DRV_ERR("sxk=>can't get semaphore in fbconfig_get_esd_check()\n");
			ret = -2;
		}
