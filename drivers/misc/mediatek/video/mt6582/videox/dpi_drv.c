...
{//need re-write
	...
	if(0 != pll_index){
		...
		DSI_PHY_REG_DPI->MIPITX_DSI_PLL_CON2.RG_DSI0_MPPLL_SDM_PCW_L =
			(pll_config[pll_index].SDM_PCW & 0xFFFFFF);
...
void DPI_MIPI_clk_setting(unsigned int mipi_pll_clk_ref, unsigned int mipi_pll_clk_div1,
			  unsigned int mipi_pll_clk_div2)
{
	...
	DISP_DRV_MSG("[DPI] MIPIPLL Initialed 222\n");
	...
	OUTREG32((MIPI_CONFIG_BASE + 0x000),
		 ((INREG32((MIPI_CONFIG_BASE + 0x000)) & 0xfffffbff) | 0x00000003));
	OUTREG32((MIPI_CONFIG_BASE + 0x050),
		 ((prediv << 1) | (txdiv0 << 3) | (txdiv1 << 5) | (posdiv << 7)));
	...
	OUTREG32((MIPI_CONFIG_BASE + 0x004),
		 INREG32(MIPI_CONFIG_BASE + 0x004) | 0x00000001 | (loopback_en << 1));
	if (lane0_en)
		OUTREG32((MIPI_CONFIG_BASE + 0x008),
			 INREG32(MIPI_CONFIG_BASE + 0x008) | 0x00000001 | (loopback_en << 1));
	if (lane1_en)
		OUTREG32((MIPI_CONFIG_BASE + 0x00C),
			 INREG32(MIPI_CONFIG_BASE + 0x00C) | 0x00000001 | (loopback_en << 1));
	if (lane2_en)
		OUTREG32((MIPI_CONFIG_BASE + 0x010),
			 INREG32(MIPI_CONFIG_BASE + 0x010) | 0x00000001 | (loopback_en << 1));
	if (lane3_en)
		OUTREG32((MIPI_CONFIG_BASE + 0x014),
			 INREG32(MIPI_CONFIG_BASE + 0x014) | 0x00000001 | (loopback_en << 1));
...
{
#if 0
	DISP_DRV_MSG("[DPI] DPI_Init for 6582 Test Har Code setting\n");
...
void DPI_mipi_switch(bool on)
{
	if (on) {
		...
		OUTREGBIT(MIPITX_DSI_SW_CTRL_CON0_REG, DSI_PHY_REG_DPI->MIPITX_DSI_SW_CTRL_CON0,
		...
		OUTREGBIT(MIPITX_DSI0_CLOCK_LANE_REG, DSI_PHY_REG_DPI->MIPITX_DSI0_CLOCK_LANE,
			  RG_DSI0_LNTC_LDOOUT_EN, 0);
		OUTREGBIT(MIPITX_DSI0_DATA_LANE0_REG, DSI_PHY_REG_DPI->MIPITX_DSI0_DATA_LANE0,
			  RG_DSI0_LNT0_LDOOUT_EN, 0);
		OUTREGBIT(MIPITX_DSI0_DATA_LANE1_REG, DSI_PHY_REG_DPI->MIPITX_DSI0_DATA_LANE1,
			  RG_DSI0_LNT1_LDOOUT_EN, 0);
		OUTREGBIT(MIPITX_DSI0_DATA_LANE2_REG, DSI_PHY_REG_DPI->MIPITX_DSI0_DATA_LANE2,
			  RG_DSI0_LNT2_LDOOUT_EN, 0);
		OUTREGBIT(MIPITX_DSI0_DATA_LANE3_REG, DSI_PHY_REG_DPI->MIPITX_DSI0_DATA_LANE3,
		...
		OUTREGBIT(MIPITX_DSI0_CON_REG, DSI_PHY_REG_DPI->MIPITX_DSI0_CON, RG_DSI0_LDOCORE_EN,
			  0);

		OUTREGBIT(MIPITX_DSI_BG_CON_REG, DSI_PHY_REG_DPI->MIPITX_DSI_BG_CON, RG_DSI_BG_CKEN,
			  0);
		OUTREGBIT(MIPITX_DSI_BG_CON_REG, DSI_PHY_REG_DPI->MIPITX_DSI_BG_CON,
			  RG_DSI_BG_CORE_EN, 0);
...
DPI_STATUS DPI_Capture_Framebuffer(unsigned int pvbuf, unsigned int bpp)
{
	...
	DISP_DRV_MSG("enter DPI_Capture_FB!\n");
	...
	if (bpp == 32)
		...
		DISP_DRV_WARN("DPI_Capture_FB, fb color format not support\n");
