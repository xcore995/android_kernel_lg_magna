...
static void dprec_logger_frame_seq_begin(DISP_SESSION_TYPE session_type, unsigned frm_sequence)
{
	...
	if (session_type > DISP_SESSION_MEMORY) {
		DISP_DRV_ERR("seq_begin session_type(0x%x) error, seq(%d)\n", session_type,
			     frm_sequence);
		return;
	}
	...
	if (dprec_met_info[session_type].begin_frm_seq != frm_sequence) {
		...
		event_trace_printk(tracing_mark_write_addr, "S|%d|%s|%d\n", current->tgid,
				   dprec_met_info[session_type].log_name, frm_sequence);
...
static void dprec_logger_frame_seq_end(DISP_SESSION_TYPE session_type, unsigned frm_sequence)
{
	...
	if (session_type > DISP_SESSION_MEMORY) {
		DISP_DRV_ERR("seq_end session_type(0x%x) , seq(%d)\n", session_type, frm_sequence);
		return;
	}
...
void update_frm_seq_info(unsigned int session_id, unsigned int addr, unsigned int addr_offset,
			 unsigned int seq, DISP_FRM_SEQ_STATE state)
{
	...
	if (device_type > DISP_SESSION_MEMORY) {
		DISP_DRV_ERR("seq_end session_id(0x%x) , seq(%d)\n", session_id, seq);
		return;
	}
	...
	if (device_type == DISP_SESSION_PRIMARY || device_type == DISP_SESSION_PRIMARY - 1) {
		if (FRM_CONFIG == state) {
			...
			MMProfileLogEx(MTKFB_MMP_Events.primary_seq_config, MMProfileFlagPulse,
				       addr, seq);
		} else if (FRM_TRIGGER == state) {
			frm_update_sequence[frm_update_cnt].state = FRM_TRIGGER;
			MMProfileLogEx(MTKFB_MMP_Events.primary_seq_trigger, MMProfileFlagPulse,
				       addr, seq);
			dprec_logger_frame_seq_begin(device_type,
			...
			for (i = 0; i < FRM_UPDATE_SEQ_CACHE_NUM; i++) {
				/* if((absabs(addr -frm_update_sequence[i].mva) <= frm_update_sequence[i].max_offset) &&
				...
				if (frm_update_sequence[i].state == FRM_TRIGGER) {
					...
					MMProfileLogEx(MTKFB_MMP_Events.primary_seq_wdma0_efo,
						       MMProfileFlagPulse,
			...
			for (i = 0; i < FRM_UPDATE_SEQ_CACHE_NUM; i++) {
				if (FRM_WDMA0_EOF == frm_update_sequence[i].state
				    && device_type == DISP_SESSION_PRIMARY
				    && frm_update_sequence[i].mva == addr) {
					frm_update_sequence[i].state = FRM_RDMA0_SOF;
					dprec_logger_frame_seq_end(device_type,
								   frm_update_sequence[i].seq);
					dprec_logger_frame_seq_begin(DISP_SESSION_PRIMARY - 1,
								     frm_update_sequence[i].seq);
					MMProfileLogEx(MTKFB_MMP_Events.primary_seq_rdma0_sof,
						       MMProfileFlagPulse,
						       frm_update_sequence[i].mva,
						       frm_update_sequence[i].seq);
				}
			...
			for (i = 0; i < FRM_UPDATE_SEQ_CACHE_NUM; i++) {
				if (FRM_RDMA0_SOF == frm_update_sequence[i].state
				    && device_type == DISP_SESSION_PRIMARY
				    && frm_update_sequence[i].mva == addr) {
					frm_update_sequence[i].state = FRM_RDMA0_EOF;
					dprec_logger_frame_seq_end(DISP_SESSION_PRIMARY - 1,
								   frm_update_sequence[i].seq);
					MMProfileLogEx(MTKFB_MMP_Events.primary_seq_rdma0_eof,
						       MMProfileFlagPulse,
						       frm_update_sequence[i].mva,
						       frm_update_sequence[i].seq);
		...
		if (FRM_CONFIG == state) {
			...
			MMProfileLogEx(MTKFB_MMP_Events.external_seq_config, MMProfileFlagPulse,
				       addr, seq);
		} else if (FRM_TRIGGER == state) {
			frm_update_sequence_mem[frm_update_cnt_mem].state = FRM_TRIGGER;
			MMProfileLogEx(MTKFB_MMP_Events.external_seq_trigger, MMProfileFlagPulse,
				       addr, seq);
			dprec_logger_frame_seq_begin(device_type,
						     frm_update_sequence_mem[frm_update_cnt_mem].
						     seq);
			...
			for (i = 0; i < FRM_UPDATE_SEQ_CACHE_NUM; i++) {
				/* if((absabs(addr -frm_update_sequence_mem[i].mva) <=
						frm_update_sequence_mem[i].max_offset) &&
					(frm_update_sequence_mem[i].state == FRM_TRIGGER)) */
				if (frm_update_sequence_mem[i].state == FRM_TRIGGER) {
					MMProfileLogEx(MTKFB_MMP_Events.external_seq_wdma0_efo,
						       MMProfileFlagPulse,
						       frm_update_sequence_mem[i].mva,
						       frm_update_sequence_mem[i].seq);
...
void init_mtkfb_mmp_events(void)
{
	if (MTKFB_MMP_Events.MTKFB == 0) {
		...
		MTKFB_MMP_Events.SetVideoLayers =
		    MMProfileRegisterEvent(MTKFB_MMP_Events.MTKFB, "SetVideoLayers");
		...
		MTKFB_MMP_Events.SessionMgr =
		    MMProfileRegisterEvent(MTKFB_MMP_Events.MTKFB, "SessionMgr");
		MTKFB_MMP_Events.GetDispInfo =
		    MMProfileRegisterEvent(MTKFB_MMP_Events.SessionMgr, "GetDispInfo");
		/* TODO: Prepare Input/Output is different */
		MTKFB_MMP_Events.PrepareInput =
		    MMProfileRegisterEvent(MTKFB_MMP_Events.SessionMgr, "PrepareBuffer");
		MTKFB_MMP_Events.PrepareOutput =
		    MMProfileRegisterEvent(MTKFB_MMP_Events.SessionMgr, "PrepareBuffer");
		MTKFB_MMP_Events.CreateSession =
		    MMProfileRegisterEvent(MTKFB_MMP_Events.SessionMgr, "CreateSession");
		MTKFB_MMP_Events.DestroySession =
		    MMProfileRegisterEvent(MTKFB_MMP_Events.SessionMgr, "DestroySession");
		MTKFB_MMP_Events.TriggerSession =
		    MMProfileRegisterEvent(MTKFB_MMP_Events.SessionMgr, "TriggerSession");
		MTKFB_MMP_Events.SetInput =
		    MMProfileRegisterEvent(MTKFB_MMP_Events.SessionMgr, "SetInput");
		MTKFB_MMP_Events.SetOutput =
		    MMProfileRegisterEvent(MTKFB_MMP_Events.SessionMgr, "SetOutput");

		MTKFB_MMP_Events.primary_seq_config =
		    MMProfileRegisterEvent(MTKFB_MMP_Events.SessionMgr, "primary_seq_config");
		MTKFB_MMP_Events.primary_seq_trigger =
		    MMProfileRegisterEvent(MTKFB_MMP_Events.SessionMgr, "primary_seq_trigger");
		MTKFB_MMP_Events.primary_seq_wdma0_efo =
		    MMProfileRegisterEvent(MTKFB_MMP_Events.SessionMgr, "primary_seq_wdma0_efo");
		MTKFB_MMP_Events.primary_seq_rdma0_sof =
		    MMProfileRegisterEvent(MTKFB_MMP_Events.SessionMgr, "primary_seq_rdma0_sof");
		MTKFB_MMP_Events.primary_seq_rdma0_eof =
		    MMProfileRegisterEvent(MTKFB_MMP_Events.SessionMgr, "primary_seq_rdma0_eof");
		MTKFB_MMP_Events.external_seq_config =
		    MMProfileRegisterEvent(MTKFB_MMP_Events.SessionMgr, "external_seq_config");
		MTKFB_MMP_Events.external_seq_trigger =
		    MMProfileRegisterEvent(MTKFB_MMP_Events.SessionMgr, "external_seq_trigger");
		MTKFB_MMP_Events.external_seq_wdma0_efo =
		    MMProfileRegisterEvent(MTKFB_MMP_Events.SessionMgr, "external_seq_wdma0_efo");
		...
		MTKFB_MMP_Events.acquire =
		    MMProfileRegisterEvent(MTKFB_MMP_Events.JobQue, "acquire");
		MTKFB_MMP_Events.release =
		    MMProfileRegisterEvent(MTKFB_MMP_Events.JobQue, "release");
		MTKFB_MMP_Events.query = MMProfileRegisterEvent(MTKFB_MMP_Events.JobQue, "query");
		MTKFB_MMP_Events.recycle =
		    MMProfileRegisterEvent(MTKFB_MMP_Events.JobQue, "recycle");

		MTKFB_MMP_Events.BufQue = MMProfileRegisterEvent(MTKFB_MMP_Events.MTKFB, "BufQue");
		MTKFB_MMP_Events.deque_buf =
		    MMProfileRegisterEvent(MTKFB_MMP_Events.BufQue, "deque");
		MTKFB_MMP_Events.enque_buf =
		    MMProfileRegisterEvent(MTKFB_MMP_Events.BufQue, "enque");
		MTKFB_MMP_Events.acquire_buf =
		    MMProfileRegisterEvent(MTKFB_MMP_Events.BufQue, "acquire");
		MTKFB_MMP_Events.release_buf =
		    MMProfileRegisterEvent(MTKFB_MMP_Events.BufQue, "release");
		MTKFB_MMP_Events.request_buf =
		    MMProfileRegisterEvent(MTKFB_MMP_Events.BufQue, "request");
		...
		MTKFB_MMP_Events.BypassOVL =
		    MMProfileRegisterEvent(MTKFB_MMP_Events.MTKFB, "BypassOVL");

		MTKFB_MMP_Events.UsingBufIdx[0][0] =
		    MMProfileRegisterEvent(MTKFB_MMP_Events.BypassOVL, "ovl_layer0");
		MTKFB_MMP_Events.UsingBufIdx[0][1] =
		    MMProfileRegisterEvent(MTKFB_MMP_Events.BypassOVL, "ovl_layer1");
		MTKFB_MMP_Events.UsingBufIdx[0][2] =
		    MMProfileRegisterEvent(MTKFB_MMP_Events.BypassOVL, "ovl_layer2");
		MTKFB_MMP_Events.UsingBufIdx[0][3] =
		    MMProfileRegisterEvent(MTKFB_MMP_Events.BypassOVL, "ovl_layer3");
		MTKFB_MMP_Events.UsingBufIdx[1][0] =
		    MMProfileRegisterEvent(MTKFB_MMP_Events.BypassOVL, "rdma_layer0");
		MTKFB_MMP_Events.UsingBufIdx[1][1] =
		    MMProfileRegisterEvent(MTKFB_MMP_Events.BypassOVL, "rdma_layer1");
		MTKFB_MMP_Events.UsingBufIdx[1][2] =
		    MMProfileRegisterEvent(MTKFB_MMP_Events.BypassOVL, "rdma_layer2");
		MTKFB_MMP_Events.UsingBufIdx[1][3] =
		    MMProfileRegisterEvent(MTKFB_MMP_Events.BypassOVL, "rdma_layer3");

		MTKFB_MMP_Events.MaxCleanIdx[0] =
		    MMProfileRegisterEvent(MTKFB_MMP_Events.BypassOVL, "MaxCleanIdx0");
		MTKFB_MMP_Events.MaxCleanIdx[1] =
		    MMProfileRegisterEvent(MTKFB_MMP_Events.BypassOVL, "MaxCleanIdx1");
		MTKFB_MMP_Events.MaxCleanIdx[2] =
		    MMProfileRegisterEvent(MTKFB_MMP_Events.BypassOVL, "MaxCleanIdx2");
		MTKFB_MMP_Events.MaxCleanIdx[3] =
		    MMProfileRegisterEvent(MTKFB_MMP_Events.BypassOVL, "MaxCleanIdx3");
		...
		MTKFB_MMP_Events.ConfigWDMA =
		    MMProfileRegisterEvent(MTKFB_MMP_Events.UpdateConfig, "ConfigWDMA");
		...
		MTKFB_MMP_Events.OverlayOutDone =
		    MMProfileRegisterEvent(MTKFB_MMP_Events.MTKFB, "OverlayOutDone");
		MTKFB_MMP_Events.SwitchMode =
		    MMProfileRegisterEvent(MTKFB_MMP_Events.MTKFB, "SwitchMode");
...
static void dump_layer_info(void)
{
	unsigned int i;
	for (i = 0; i < 4; i++) {
		DISP_DRV_INFO("LayerInfo in LCD driver,");
		DISP_DRV_INFO
		    ("layer=%d,layer_en=%d, source=%d, fmt=%d, addr=0x%x, x=%d, y=%d, w=%d, h=%d,",
		...
		DISP_DRV_INFO("pitch=%d, keyEn=%d, key=%d, aen=%d, alpha=%d\n",
			      cached_layer_config[i].src_pitch, cached_layer_config[i].keyEn,
			      cached_layer_config[i].key, cached_layer_config[i].aen,
...
static void process_dbg_opt(const char *opt)
{
	...
	if (0 == strncmp(opt, "hdmion", 6)) {
		...
		if (0 == strncmp(opt + 11, "on", 2)) {
			char *p = (char *)opt + 14;
			kstrtoul(p, 10, &value);
			gCaptureLayerDownX = value;
			kstrtoul(p + 1, 10, &value);
			gCaptureLayerDownY = value;
		...
		if (0 == strncmp(opt + 9, "on", 2)) {
			char *p = (char *)opt + 12;
			kstrtoul(p, 10, &value);
			gCaptureOvlDownX = value;
			kstrtoul(p + 1, 10, &value);
			gCaptureOvlDownY = value;
			gCaptureOvlEnable = 1;
		...
		if (0 == strncmp(opt + 8, "on", 2)) {
			char *p = (char *)opt + 11;
			kstrtoul(p, 10, &value);
			gCaptureFBDownX = value;
			kstrtoul(p + 1, 10, &value);
			gCaptureFBDownY = value;
			gCaptureFBEnable = 1;
			gCaptureFBRawData = 0;
