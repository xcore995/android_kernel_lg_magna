...
int fbconfig_get_esd_check_exec(void)
{
	...
	if (reg_read.check_buffer == NULL) {
		DISP_DRV_MSG("sxk=>esd_check :reg_read.check_buffer kmalloc fail!!\n");
		return -2;
	}
	array[0] = 0x00013700;
	array[0] = 0x3700 + (reg_read.check_para_num << 16);
	reg_read.check_para_num = reg_read.check_para_num + 6;
	DISP_DRV_MSG("sxk=>esd_check : arrar[0]=0x%x\n", array[0]);
...
{
	int z = 0;
	DISP_DRV_MSG("sxk=>[PRINT_RECORD] type is %d\n", record_head->type);
	for (z = 0; z < record_head->ins_num; z++)
		DISP_DRV_MSG("sxk=>[PRINT_RECORD]data is 0x%x\n", record_head->ins_array[z]);
}
#endif
static void record_list_init(void)
{
	DISP_DRV_MSG("sxk=>call record_list_init!!\n");
...
static void print_from_head_to_tail(void)
{
	...
	DISP_DRV_MSG("sxk=>print_from_head_to_tail:START\n");
	while (tmp != NULL) {
		DISP_DRV_MSG("sxk=>record type is %d;record ins_num is %d\n", tmp->type, tmp->ins_num);
		for (z = 0; z < tmp->ins_num; z++)
			DISP_DRV_MSG("sxk=>record conten is :0x%x\n", tmp->ins_array[z]);
		tmp = tmp->next;
	}
	DISP_DRV_MSG("sxk=>print_from_head_to_tail:END\n");
}
...
static void print_from_head_to_tail_backup(void)
{
	...
	DISP_DRV_MSG("sxk=>print_from_head_to_tail_BACKUP...........:START\n");
	while (tmp != NULL) {
		DISP_DRV_MSG("sxk=>record type is %d;record ins_num is %d\n", tmp->type, tmp->ins_num);
		for (z = 0; z < tmp->ins_num; z++)
			DISP_DRV_MSG("sxk=>record conten is :0x%x\n", tmp->ins_array[z]);
	...
	DISP_DRV_MSG("sxk=>print_from_head_to_tail_BACKUP..................:END\n");
}
...
{
	CONFIG_RECORD *tmp = record_head;
	DISP_DRV_MSG("sxk=>execute_cmd:START\n");
	while (tmp != NULL) {
		switch (tmp->type) {
		case RECORD_CMD:
		...
		case RECORD_PIN_SET:
			...
			break;
		default:
			DISP_DRV_MSG("sxk=>No such Type!!!!!\n");
		}
		tmp = tmp->next;
	}
	DISP_DRV_MSG("sxk=>execute_cmd:END\n");
...
{
	CONFIG_RECORD *tmp = backup_head;
	DISP_DRV_MSG("sxk=>fbconfig_apply_new_lcm_setting:START\n");
	while (tmp != NULL) {
		switch (tmp->type) {
		case RECORD_CMD:
			dsi_set_cmdq(tmp->ins_array, tmp->ins_num, 1);
	...
	DISP_DRV_MSG("sxk=>fbconfig_apply_new_lcm_setting:END\n");
	return;
}
...
static void fbconfig_reset_lcm_setting(void)
{
	...
	if (lcm_params->dsi.mode != CMD_MODE) {
		if (down_interruptible(&sem_early_suspend)) {
			DISP_DRV_MSG("sxk=>can't get semaphore in fbconfig_reset_lcm_setting()\n");
			return;
		}
		...
		if (down_interruptible(&sem_early_suspend)) {
			DISP_DRV_MSG("sxk=>can't get semaphore in execute_cmd()\n");
			return;
		}
		lcm_drv->init();
		up(&sem_early_suspend);
...
static long fbconfig_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case LCM_GET_DSI_CONTINU:
		{
			...
			DISP_DRV_MSG("fbconfig=>LCM_GET_DSI_CONTINU:%d\n", ret);
	...
	case LCM_TEST_DSI_CLK:
		{
			...
			DISP_DRV_MSG("fbconfig=>LCM_TEST_DSI_CLK:%d\n", ret);
	...
	case LCM_GET_DSI_CLK:
		{
			...
			DISP_DRV_MSG("fbconfig=>LCM_GET_DSI_CLK:%d\n", ret);
			return copy_to_user(argp, &ret, sizeof(ret)) ? -EFAULT : 0;
		}
	...
	case LCM_GET_DSI_SSC:
		{
			...
			DISP_DRV_MSG("fbconfig=>LCM_GET_DSI_SSC:%d\n", ret);
			return copy_to_user(argp, &ret, sizeof(ret)) ? -EFAULT : 0;
		}
	case LCM_GET_DSI_LANE_NUM:
		{
			...
			DISP_DRV_MSG("fbconfig=>LCM_GET_DSI_LANE_NUM:%d\n", ret);
			return copy_to_user(argp, &ret, sizeof(ret)) ? -EFAULT : 0;
		}
	case LCM_GET_DSI_TE:
		{
			...
			DISP_DRV_MSG("fbconfig=>LCM_GET_DSI_TE:%d\n", ret);
			return copy_to_user(argp, &ret, sizeof(ret)) ? -EFAULT : 0;
		}
	...
	case DRIVER_IC_CONFIG:
		{
			...
			if (record_list_initialed == 0) {
				...
				if (copy_from_user(record_tmp, (void __user *)arg, sizeof(CONFIG_RECORD))) {
					DISP_DRV_MSG("[DRIVER_IC_CONFIG]: copy_from_user failed! line:%d\n", __LINE__);
					return -EFAULT;
				}
#if 0				/* FBCONFIG_DEBUG */
				DISP_DRV_MSG("sxk=>will print before add to list\n");
	...
	case MIPI_SET_CLK:
		{
			...
			if (copy_from_user(&clk, (void __user *)argp, sizeof(clk))) {
				DISP_DRV_MSG("[MIPI_SET_CLK]: copy_from_user failed! line:%d\n", __LINE__);
				return -EFAULT;
			} else {
	...
	case MIPI_SET_SSC:
		{
			unsigned int ssc;
			DISP_DRV_MSG("sxk=>debug.c call set mipi ssc line:%d\n", __LINE__);
			if (copy_from_user(&ssc, (void __user *)argp, sizeof(ssc))) {
				DISP_DRV_MSG("[MIPI_SET_SSC]: copy_from_user failed! line:%d\n", __LINE__);
				return -EFAULT;
			} else {
				DISP_DRV_MSG("sxk=>debug.c call set mipi ssc line:%d\n", __LINE__);
	...
	case MIPI_SET_LANE:
		{
			...
			if (copy_from_user(&lane_num, (void __user *)argp, sizeof(lane_num))) {
				DISP_DRV_MSG("[MIPI_SET_LANE]: copy_from_user failed! line:%d\n", __LINE__);
				return -EFAULT;
			} else {
	...
	case TE_SET_ENABLE:
		{
			...
			if (copy_from_user(&enable, (void __user *)argp, sizeof(enable))) {
				DISP_DRV_MSG("[TE_SET_ENABLE]: copy_from_user failed! line:%d\n", __LINE__);
				return -EFAULT;
			} else {
	...
	case FB_LAYER_GET_SIZE:
		{
			...
			DISP_DRV_MSG("sxk==>global_layer_id is %d\n", global_layer_id);
	...
	case FB_LAYER_DUMP:
		{
			...
			if ((layer_size != 0) && (enable != 0)) {
				DISP_DRV_MSG("sxk==>FB_LAYER_DUMP==>layer_size is %d   mva is 0x%x\n", layer_size, mva);
				m4u_mva_map_kernel(mva, layer_size, 0, &kva, &mapped_size);
				DISP_DRV_MSG("sxk==> addr from user space is 0x%x\n", (unsigned int)argp);
				DISP_DRV_MSG("sxk==> kva is 0x%x   mmaped size is %d\n", kva, mapped_size);
	...
	case MIPI_SET_CC:
		{
			int enable;
			if (copy_from_user(&enable, (void __user *)argp, sizeof(enable))) {
				DISP_DRV_MSG("[MIPI_SET_CC]: copy_from_user failed! line:%d\n", __LINE__);
				return -EFAULT;
			} else {
	...
	case LCM_GET_ESD:
		{
			...
			if (copy_from_user(&esd_para, (void __user *)arg, sizeof(esd_para))) {
				DISP_DRV_MSG("[LCM_GET_ESD]: copy_from_user failed! line:%d\n", __LINE__);
				return -EFAULT;
			}
			...
			if ((ret != 0) && (reg_read.check_buffer != NULL)) {
				...
				for (i = 0; i < reg_read.check_para_num; i++)
					DISP_DRV_MSG("sxk=>%s, reg_read.check_buffer[%d]=0x%x\n",
	...
	case LCM_GET_ESD_RET:
		{
			ret =
			    (copy_to_user
			     (argp, (void *)reg_read.check_buffer,
			      sizeof(char) * (reg_read.check_para_num)) ? -EFAULT : 0);
			if (reg_read.check_buffer != NULL) {
				kfree(reg_read.check_buffer);
...
#if 1
		if (lcm_params->dsi.mode != CMD_MODE) {
			DISP_DRV_MSG("sxk=>01will exec cmd!! in fbconfig_release()\n");
...
#endif
/*free the memory .....*/
		DISP_DRV_MSG("sxk=>will free the memory\n");
		free_list_memory();
		return 0;
	}
}
