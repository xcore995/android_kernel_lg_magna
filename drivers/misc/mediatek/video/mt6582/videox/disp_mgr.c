...
DCP_STATUS disp_set_session_input(struct disp_session_input_config_t *input)
{
	...
	for (i = 0; i < MAX_INPUT_CONFIG; i++) {
		...
		if (config->layer_id >= MAX_INPUT_CONFIG) {
			...
			XLOG_DBG
			    ("disp_set_session_input: found invalid layer_id=%x(%d), mark un-dirty layer\n",
			     config->layer_id, config->layer_id);
			job->input[i].dirty = 0;
			continue;
		}
		if (config->layer_enable) {
			if (config->next_buff_idx <=
			    disp_sync_get_last_signaled(input->session_id, config->layer_id)) {
				MMProfileLogEx(MTKFB_MMP_Events.Debug, MMProfileFlagPulse,
					       config->layer_id, config->next_buff_idx);
			...
			default:
				MMProfileLogEx(MTKFB_MMP_Events.Debug, MMProfileFlagPulse,
					       config->layer_id, config->src_fmt);
			...
			if (config->src_phy_addr != NULL) {
				...
				job->input[i].address =
				    disp_sync_query_buffer_mva(job->group_id, config->layer_id,
							       (unsigned int)config->next_buff_idx);
			}
			...
			} else if (DISP_FORMAT_ARGB8888 == config->src_fmt
				   || DISP_FORMAT_ABGR8888 == config->src_fmt) {
				job->input[i].alpha_enable = 1;
			...
			if (config->src_width != config->tgt_width
			    || config->src_height != config->tgt_height) {
				XLOG_ERR("OVL cannot support clip:src(%d,%d), dst(%d,%d)\n",
					 config->src_width, config->src_height, config->tgt_width,
					 config->tgt_height);
			}
		...
		unsigned int offset =
		    config->src_offset_y * config->src_pitch * layerpitch +
		    config->src_offset_x * layerpitch;
		if (config->layer_enable && i == 0)
			update_frm_seq_info(input->session_id, config->src_phy_addr, offset,
					    config->frm_sequence, FRM_CONFIG);
		else if (config->layer_enable == 0 && i == 0)
			XLOG_ERR("layer 0 is not enabled\n");
		XLOG_DBG("L%d input:%d,%d\n", job->input[i].layer_id, job->input[i].layer_enable,
			 job->input[i].index);
	}
...
DCP_STATUS disp_set_session_output(struct disp_session_output_config_t *output)
{
	...
	default:
		...
		MMProfileLogEx(MTKFB_MMP_Events.Debug, MMProfileFlagPulse, output->session_id,
			       config->fmt);
	...
	if (config->pa != 0) {
		...
		job->output.address =
		    disp_sync_query_buffer_mva(job->group_id, MAX_INPUT_CONFIG, config->buff_idx);
	}
	...
	mutex_unlock(&job->lock);
	XLOG_DBG("L%d output:%d,0x%x\n", job->output.layer_id, job->output.index,
		 job->output.address);
	return ret;
}
	...
	if (!list_empty(&active_queue_head)) {
		list_for_each_entry_safe(pos, n, &active_queue_head, list) {
			if (gid == pos->group_id) {
				...
				MMProfileLogEx(MTKFB_MMP_Events.enque, MMProfileFlagPulse, gid,
					       (UINT) pos);
				break;
			}
		}
	...
	if (!list_empty(&todo_queue_head)) {
		list_for_each_entry(pos, &todo_queue_head, list) {
			if (pos->status == QUEUED) {
				...
				MMProfileLogEx(MTKFB_MMP_Events.acquire, MMProfileFlagPulse,
					       job->group_id, (UINT) job);
				break;
			}
		}
	}
	...
	if (!list_empty(&todo_queue_head)) {
		list_for_each_entry_safe(pos, n, &todo_queue_head, list) {
			if (pos->status == ACQUIRED) {
				...
				MMProfileLogEx(MTKFB_MMP_Events.release, MMProfileFlagPulse, cnt,
					       (UINT) pos);
			}
		}
	}
	...
	if (!list_empty(&done_queue_head)) {
		list_for_each_entry(pos, &done_queue_head, list) {
			if (pos->status == DONE) {
				...
				job = pos;
				XLOG_DBG("query:0x%p\n", pos);
				MMProfileLogEx(MTKFB_MMP_Events.query, MMProfileFlagPulse,
					       job->group_id, (UINT) job);
				break;
			}
		}
	...
	if (!list_empty(&done_queue_head)) {
		list_for_each_entry_safe(pos, n, &done_queue_head, list) {
			if (pos->status == FREE) {
				...
				MMProfileLogEx(MTKFB_MMP_Events.recycle, MMProfileFlagPulse,
					       pos->group_id, (UINT) pos);
				if (!all)
					break;
			}
		}
...
DCP_STATUS disp_buffer_queue_init_continous(disp_buffer_queue *que, UINT address, UINT size,
					    UINT cnt)
{
	DCP_STATUS ret = DCP_STATUS_NOT_IMPLEMENTED;
	UINT index, offset;
...
UINT disp_deque_buffer(disp_buffer_queue *que)
{
	...
	MMProfileLogEx(MTKFB_MMP_Events.deque_buf, MMProfileFlagPulse, que->read_slot,
		       que->write_slot);
...
void disp_enque_buffer(disp_buffer_queue *que)
{
	...
	spin_unlock_irqrestore(&que->lock, flag);
	MMProfileLogEx(MTKFB_MMP_Events.enque_buf, MMProfileFlagPulse, que->read_slot,
		       que->write_slot);
}
...
UINT disp_request_buffer(disp_buffer_queue *que)
{
	...
	slot = que->read_slot;
	spin_unlock_irqrestore(&que->lock, flag);
	MMProfileLogEx(MTKFB_MMP_Events.request_buf, MMProfileFlagPulse, que->read_slot,
		       que->write_slot);
	return que->buffer_queue[slot];
}
...
void disp_release_buffer(disp_buffer_queue *que)
{
	...
	spin_unlock_irqrestore(&que->lock, flag);
	MMProfileLogEx(MTKFB_MMP_Events.release_buf, MMProfileFlagPulse, que->read_slot,
		       que->write_slot);
}
