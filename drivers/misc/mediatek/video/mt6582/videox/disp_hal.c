...
int disphal_prepare_suspend(void)
{
	if (lcm_params->type == LCM_TYPE_DSI && lcm_params->dsi.mode != CMD_MODE) {
		...
		if (lcm_params->dsi.mixmode_enable) {
			...
			if (mode == DSI_INCELL_MIX_MODE) {
				DISP_DRV_MSG("[DDP] Enable CMD interrupt for wait function\n");
				DSI_EnableCMDDone();
			} else {
				DSI_SetMode(CMD_MODE);
			}
	...
	if (clk_is_force_on(MT_CG_DISP0_SMI_LARB0) || clk_is_force_on(MT_CG_DISP0_SMI_COMMON)) {
		DISP_DRV_MSG("[DDP] MT_CG_DISP0_SMI_LARB0 is forced on\n");
		clk_clr_force_on(MT_CG_DISP0_SMI_LARB0);
		clk_clr_force_on(MT_CG_DISP0_SMI_COMMON);
	}
	return 0;
...
const LCM_DRIVER *disphal_get_lcm_driver(const char *lcm_name, unsigned int *lcm_index)
{
	...
	DISP_DRV_MSG("[LCM Auto Detect], we have %d lcm drivers built in\n", lcm_count);
	DISP_DRV_MSG("[LCM Auto Detect], try to find driver for [%s]\n",
	...
	if (lcm_count == 1) {
		...
		*lcm_index = 0;
		...
		for (i = 0; i < lcm_count; i++) {
			...
			DISP_DRV_MSG("[LCM Auto Detect] [%d] - [%s]\t", i,
				     (lcm->name == NULL) ? "unknown" : lcm->name);
			lcm->set_util_funcs(&lcm_utils);
			...
			unsigned int *dma_pa)
{
	...
	*va = (unsigned int)ioremap_nocache(res->start, res->end - res->start + 1);
	if (disp_use_mmu) {
		...
		if (ret != 0)
			DISP_DRV_ERR("fill_linear_pagetable error, %d\n", ret);

		*dma_pa = *pa;
		ASSERT(dma_pa);
		DISP_DRV_MSG("[DISPHAL] FB MVA is 0x%08X PA is 0x%08X\n", *dma_pa, *pa);
	} else {
		*dma_pa = *pa;
	}
	return 0;
...
unsigned int disphal_check_lcm(UINT32 color)
{
	...
	if (LCM_TYPE_DBI == lcm_params->type) {	/* DBI */
		...
		DISP_DRV_ERR("DISP_AutoTest():unknown interface\n");
		ret = 0;
	}
	return ret;
}
