...
/*====================================================================================================================*/
#define FENCE_STEP_COUNTER	1 /* how many counters prior to current timeline real-time counter */
...
void disp_sync_signal_fence(UINT session, UINT layer)
{
	...
	if (NULL != info) {
		...
		if (num_fence > 0) {
			...
			MMProfileLogEx(MTKFB_MMP_Events.SignalSyncFence, MMProfileFlagEnd,
	...
	for (i = 0; i < MAX_SESSION_COUNT; i++) {
		...
		if (session_info->initialized && session_info->session != 0 && session_info->invalid) {
			if (DCP_STATUS_OK == disp_check_todo_job(session_info->session)) {
				XLOG_INFO
				    ("disp_sync_cleanup hold invalid session(%x) while todo job(s) exist.\n",
