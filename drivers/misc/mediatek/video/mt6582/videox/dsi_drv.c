...
static irqreturn_t _DSI_InterruptHandler(int irq, void *dev_id)
{
	...
	if (dsi_log_on)
		DISP_DRV_MSG("DSI IRQ, value = 0x%x!!\n", INREG32(0xF400C00C));
		...
		if (dsi_glitch_enable) {
			...
			if (!is_early_suspended && !wait_vm_done_irq) {
				if (1 == DSI_Detect_CLK_Glitch())
					DISP_DRV_ERR("VM Done detect glitch fail!!,%d\n", __LINE__);
...
/* long long ret; */
	if (EnableVSyncLog)
		DISP_DRV_MSG("[DSI] VSync0\n");
	if (wait_dsi_vsync) {
		dsi_vsync = true;

		if (EnableVSyncLog)
			DISP_DRV_MSG("[DSI] VSync1\n");
		wake_up_interruptible(&_vsync_wait_queue);
	}
...
static void _WaitForEngineNotBusy(void)
{
	...
	if (lcm_params->dsi.mixmode_enable) {
		...
		if (dsi_log_on)
			DISP_DRV_MSG("DSI %s: dsi_wait_irq_count++=%d\n", __func__, irq_wait_cnt);
	}
#endif
	timeOut = 200;
	...
	if (lcm_params->dsi.mixmode_enable) {
		...
		if (irq_wait_cnt == 0) {
			...
			if (dsi_log_on)
				DISP_DRV_MSG("DSI %s: disable CMD_DONR_INT\n", __func__);
		}
	}
#endif
#else
...
static inline int DSI_vidoe_TE_eint_setup(void)
{
	...
	DISP_DRV_MSG("DSI : DSI_vidoe_TE_eint_settup\n");
...
{
	DISP_DRV_MSG("<0>" "=============mt_pmic_pwm0_test===============\n");
	mt_set_gpio_mode(GPIO_INCELL_DISP_TE_PWM, GPIO_INCELL_DISP_TE_M_PWM);

	DISP_DRV_MSG("PWM: clk_div = %x, clk_src = %x, pwm_no = %x\n",
...
void DSI_set_fps_for_PWM(unsigned int fps)
{
	...
	DISP_DRV_MSG("DSI_set_fps_for_PWM fps (%d), widht (%d)\n", fps, width);
...
DSI_STATUS DSI_Init(BOOL isDsiPoweredOn)
{
	...
	if (request_irq(MT6582_DISP_DSI_IRQ_ID, _DSI_InterruptHandler, IRQF_TRIGGER_LOW, MTKFB_DRIVER, NULL) < 0) {
...
static enum hrtimer_restart _DSI_Single_Frame_ModeTimer_handler(struct hrtimer *timer)
{
	...
	if (!_IsEngineBusy()) {
		...
		DISP_DRV_MSG("DSI _IsEngineBusy\n");
	}
	return HRTIMER_NORESTART;
}
...
static enum hrtimer_restart _DSI_Send_Frame_without_TE_handler(struct hrtimer *timer)
{
	if (cnt_for_suspend_mode != 0) {
		if (!_IsEngineBusy()) {
			DISP_DRV_MSG("Send_Frame hrtimerCB: cnd (%d)\n", cnt_for_suspend_mode);
			DSI_clk_HS_mode(1);
			DSI_Start();
		} else {
			DISP_DRV_MSG("_DSI_Send_Frame_without_TE_handler _IsEngineBusy\n");
		}
	} else {
		DSI_SetMode(CMD_MODE);
...
{
	DISP_DRV_MSG("DSI mode: new(%d), previous(%d)\n", status, dsi_incell_mix_mode_status);
	...
	case DSI_INCELL_GPIO_TIMER_MODE:
		if (dsi_incell_mix_mode_status == DSI_INCELL_MIX_MODE) {
			...
			DISP_DRV_ERR("DSI_change_mode fail\n");
		}
	...
	case DSI_INCELL_MIX_MODE:
		if (dsi_incell_mix_mode_status == DSI_INCELL_GPIO_TIMER_MODE)
			...
			DISP_DRV_ERR("Chaning mode fail\n");
...
void DSI_MixMode_Start(void)
{
	...
	DISP_DRV_MSG("DSI FrameMode Start PWM fps (%d) clock (%d)\n",
...
DSI_STATUS DSI_PowerOn(void)
{
	if (!s_isDsiPowerOn) {
		...
		printf("[DISP] - uboot - DSI_PowerOn. 0x%8x,0x%8x,0x%8x\n", INREG32(0x14000110),
		       INREG32(0x14000114), INREG32(0x14000118));
...
/* spin_unlock_irq(&dsi_glitch_detect_lock); */
		...
		while (DSI_REG->DSI_INTSTA.RD_RDY == 0) {	/* /read clear */
			...
			if (((unsigned int)sched_clock() - (unsigned int)start_time) > 50000) {
				if (glitch_log_on)
					DISP_DRV_MSG
					    ("Test log 4:Polling DSI read ready timeout,%d us\n",
...
/* OUTREG32(&DSI_CMDQ_REG->data[0], 0x00290508); */
		...
		if (1 == read_timeout_ret) {
			read_timeout_ret = 0;
			DISP_DRV_MSG("iii detect timeout ID:%d\n", read_IC_ID);
			read_IC_ID = 0;
			continue;
		}
		MMProfileLogEx(MTKFB_MMP_Events.Debug, MMProfileFlagPulse, 0, 6);
		...
		if (((DSI_REG->DSI_TRIG_STA.TRIG2)) == 1) {
			...
			if (glitch_log_on) {
				...
				if (((read_data0.byte1 & 0x4) != 0)
					...
					DISP_DRV_ERR("111 ID:%d ECC err read_data0, %x,%x,%x,%x\n",
			...
			if (((read_data0.byte1 & 0x7) != 0) || ((read_data0.byte2 & 0x3) != 0)) {
				DISP_DRV_MSG("read_data0, %x,%x,%x,%x\n",
					     read_data0.byte0, read_data0.byte1, read_data0.byte2, read_data0.byte3);
				DISP_DRV_MSG("iii detect error ID:%d\n", read_IC_ID);
				read_IC_ID = 0;
				continue;
			} else {
				if (read_IC_ID == 0) {
...
/* unsigned int fmod = 30;//Fmod = 30KHz by default */
	...
	if (0 != data_Rate) {
		if (data_Rate > 1250) {
			DISP_DRV_ERR("[dsi_drv.c error]Data Rate exceed limitation\n");
			ASSERT(0);
			...
			DISP_DRV_ERR("[dsi_drv.c Error]: dataRate is too low,%d!!!\n", __LINE__);
			ASSERT(0);
		}
		/* PLL txdiv config */
		switch (txdiv) {
...
void DSI_PHY_TIMCONFIG(LCM_PARAMS *lcm_params)
{
	...
	if (0 != lcm_params->dsi.PLL_CLOCK) {
		...
		DISP_LOG_PRINT(ANDROID_LOG_INFO, "DSI",
			       "[DISP] - DSI_PHY_TIMCONFIG, Cycle Time %d(ns), Unit Interval %d(ns), lane#%d\n",
		...
		default:
			DISP_DRV_WARN("div1 should be less than 4!!\n");
			div1 = 4;
			break;
		}
		...
		case 3:
			...
			break;
		default:
			DISP_DRV_WARN("div2 should be less than 4!!\n");
			div2 = 4;
			break;
		}
		...
		default:
			DISP_DRV_WARN("pre_div should be less than 4!!\n");
			pre_div = 4;
			break;
		}
		...
		default:
			DISP_DRV_WARN("post_div should be less than 4!!\n");
			post_div = 4;
			break;
		}
		...
		default:
			DISP_DRV_WARN("fbk_sel should be less than 4!!\n");
			fbk_sel = 4;
			break;
		}
		...
		DISP_LOG_PRINT(ANDROID_LOG_INFO, "DSI",
			       "[DISP] - DSI_PHY_TIMCONFIG, Cycle Time %d(ns), Unit Interval %d(ns)",
			       cycle_time, ui);
		DISP_LOG_PRINT(ANDROID_LOG_INFO, "DSI",
...
#define NS_TO_CYCLE(n, c)	((n) / (c))
	...
	DISP_DRV_MSG("%s, 0x%08x,0x%08x,0x%08x,0x%08x\n",
		     __func__, INREG32(DSI_BASE + 0x110), INREG32(DSI_BASE + 0x114),
		     INREG32(DSI_BASE + 0x118), INREG32(DSI_BASE + 0x11c));
}
...
bool DSI_handle_int_TE(void)
{
	...
	if (DSI_REG->DSI_STATE_DBG3.TCON_STATE == DSI_VDO_VFP_STATE) {
		...
		if ((DSI_REG->DSI_STATE_DBG3.TCON_STATE == DSI_VDO_VFP_STATE)
			...
			/* Can't do int. TE check while INUSE FB number is not 0 because
			later disable/enable DPI will set INUSE FB to number 0. */
...
void DSI_handle_noncont_clk(void)
{
	...
	case DSI_VDO_VSA_VS_STATE:
		while (DSI_REG->DSI_STATE_DBG3.TCON_STATE != DSI_VDO_VSA_HS_STATE) {
			if (get_current_time_us() - dsi_current_time > _dsiContext.vsa_vs_period_us) {
				DISP_LOG_PRINT(ANDROID_LOG_WARN,
	...
	case DSI_VDO_VBP_STATE:
		DISP_LOG_PRINT(ANDROID_LOG_WARN, "DSI", "Can't do clock switch in DSI_VDO_VBP_STATE !!!\n");
		break;
...
void DSI_set_cmdq_V2(unsigned cmd, unsigned char count, unsigned char *para_list, unsigned char force_update)
{
	...
	if (0 != DSI_REG->DSI_MODE_CTRL.MODE) {	/* not in cmd mode */
		...
		DISP_DRV_MSG("set cmdq in VDO mode in set_cmdq_V2\n");
		if (cmd < 0xB0) {
			if (count > 1) {
				vm_cmdq.LONG_PKT = 1;
...
void DSI_set_cmdq_V3(LCM_setting_table_V3 *para_tbl, unsigned int size, unsigned char force_update)
{
	...
	do {
		...
		if (0 != DSI_REG->DSI_MODE_CTRL.MODE) {	/* not in cmd mode */
			DSI_VM_CMD_CON_REG vm_cmdq;
			OUTREG32(&vm_cmdq, AS_UINT32(&DSI_REG->DSI_VM_CMD_CON));
			DISP_DRV_MSG("set cmdq in VDO mode\n");
			if (count > 1) {
				vm_cmdq.LONG_PKT = 1;
...
/* _WaitForEngineNotBusy(); */
	if (!s_isDsiPowerOn) {
		DISP_DRV_ERR("[DSI Error0] DSI has been suspended, but still operate DSI HW\n");
		return;
	}
	if (((INREG32(&DSI_PHY_REG->MIPITX_DSI_PLL_CON0)) & 0x01) != 0x1) {
		DISP_DRV_ERR("[DSI Error1] MIPI TX has been OFF, but still operate DSI HW\n");
		return;
	}
	if (0 != DSI_REG->DSI_MODE_CTRL.MODE) {	/* not in cmd mode */
		DSI_VM_CMD_CON_REG vm_cmdq;
...
/* unsigned char packet_type; */
	...
	do {
		...
		while (DSI_REG->DSI_STA.BUF_UNDERRUN || DSI_REG->DSI_STA.ESC_ENTRY_ERR
		       || DSI_REG->DSI_STA.LPDT_SYNC_ERR || DSI_REG->DSI_STA.CTRL_ERR || DSI_REG->DSI_STA.CONTENT_ERR) {
...
UINT32 DSI_dcs_read_lcm_reg_v2(UINT8 cmd, UINT8 *buffer, UINT8 buffer_size)
{
	...
	do {
		...
		/* / 3: read data */
#if ENABLE_DSI_INTERRUPT
		ret = wait_event_interruptible_timeout(_dsi_dcs_read_wait_queue, waitRDDone, WAIT_TIMEOUT);
		if (0 == ret) {
			DISP_LOG_PRINT(ANDROID_LOG_WARN, "DSI", " Wait for DSI engine read ready timeout!!!\n");
...
/*you must read data before send RACK*/
		...
		if (dsi_log_on) {
			...
			DISP_DRV_MSG("read_data1, %x,%x,%x,%x\n", read_data1.byte0,
				     read_data1.byte1, read_data1.byte2, read_data1.byte3);
			DISP_DRV_MSG("read_data2, %x,%x,%x,%x\n", read_data2.byte0,
				     read_data2.byte1, read_data2.byte2, read_data2.byte3);
			DISP_DRV_MSG("read_data3, %x,%x,%x,%x\n", read_data3.byte0,
...
unsigned int DSI_Check_LCM(UINT32 color)
{
	...
	for (i = 0; i < 60; i += 3) {
		DISP_DRV_MSG("read pixel = 0x%x,", (buffer[i] << 16) | (buffer[i + 1] << 8) | (buffer[i + 2]));
...
/* unsigned int DSI_DBG6_Status_bak = DSI_DBG6_Status; */
	...
	for (i = 0; i < sizeof(DSI_REGS); i += 16) {
		DISP_LOG_PRINT(ANDROID_LOG_INFO, "DSI",
			       ...
			       INREG32(DSI_BASE + i), INREG32(DSI_BASE + i + 0x4),
			       INREG32(DSI_BASE + i + 0x8), INREG32(DSI_BASE + i + 0xc));
	}

	for (i = 0; i < sizeof(DSI_CMDQ_REGS); i += 16) {
		DISP_LOG_PRINT(ANDROID_LOG_INFO, "DSI",
			       "DSI_CMD+%04x : 0x%08x  0x%08x  0x%08x  0x%08x\n", i,
			       INREG32((DSI_BASE + 0x180 + i)),
			       INREG32((DSI_BASE + 0x180 + i + 0x4)),
	...
	for (i = 0; i < sizeof(DSI_PHY_REGS); i += 16) {
		DISP_LOG_PRINT(ANDROID_LOG_INFO, "DSI",
			       ...
			       INREG32((MIPI_CONFIG_BASE + i)),
			       INREG32((MIPI_CONFIG_BASE + i + 0x4)),
...
DSI_STATUS DSI_Change_CLK(unsigned int clk)
{
	...
	for (lcm_params_for_clk_setting.dsi.pll_div2 = 15;
	     lcm_params_for_clk_setting.dsi.pll_div2 > 0; lcm_params_for_clk_setting.dsi.pll_div2--) {
		for (lcm_params_for_clk_setting.dsi.pll_div1 = 0;
		     lcm_params_for_clk_setting.dsi.pll_div1 < 39; lcm_params_for_clk_setting.dsi.pll_div1++) {
...
DSI_STATUS fbconfig_DSI_set_CLK(unsigned int clk)
{
	...
	DISP_DRV_MSG("sxk==>fbconfig_DSI_set_CLK:%d\n", clk);

	if (clk > 1000)
		return DSI_STATUS_ERROR;
	memcpy((void *)&fb_lcm_params, (void *)lcm_params, sizeof(LCM_PARAMS));
	fb_lcm_params.dsi.PLL_CLOCK = clk;
	DISP_DRV_MSG("sxk==>fbconfig_DSI_set_CLK:will wait!!\n");

	_WaitForEngineNotBusy();	/* cmd mode */
	DISP_DRV_MSG("sxk==>will fbconfig_DSI_set_CLK:%d\n", clk);
...
void fbconfig_DSI_set_timing(MIPI_TIMING timing)
{
	...
	case VBP:
		...
		break;
	default:
		DISP_DRV_WARN("fbconfig dsi set timing :no such type!!\n");
	}
	DSI_DumpRegisters();
}
...
DSI_STATUS DSI_Capture_Framebuffer(unsigned int pvbuf, unsigned int bpp, bool cmd_mode)
{
	...
	DISP_DRV_MSG("enter DSI_Capture_FB!\n");
	...
	if (bpp == 32)
		...
		DISP_DRV_WARN("DSI_Capture_FB, fb color format not support\n");

	DISP_DRV_MSG("before alloc MVA: va = 0x%x, size = %d\n", pvbuf,
	...
	if (ret != 0) {
		DISP_DRV_ERR("m4u_alloc_mva() fail!\n");
		return DSI_STATUS_OK;
	}
	DISP_DRV_MSG("addr=0x%x, format=%d\n", mva, mem_out.outFormat);

	m4u_dma_cache_maint(DISP_WDMA, &pvbuf, lcm_params->height * lcm_params->width * bpp / 8, DMA_BIDIRECTIONAL);
...
{
	DISP_DRV_MSG("sxk==>set TE Enable %d\n", enable);
	dsiTeEnable = enable;
