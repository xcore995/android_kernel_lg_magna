...
static int mtkfb_capture_framebuffer(struct fb_info *info, unsigned int pvbuf)
{
    ...
    */
	...
	if (atomic_read(&capture_ui_layer_only)) {
		...
		unsigned int pixel_bpp = info->var.bits_per_pixel / 8;/* bpp is either 32 or 16, can't be other value */
	...
	case MTKFB_SET_VIDEO_LAYERS:
		{
			...
			if (copy_from_user(&layerInfo, (void __user *)arg, sizeof(layerInfo))) {
				MTKFB_WRAN("[FB]: copy_from_user failed! line:%d\n", __LINE__);
				MMProfileLogMetaString(MTKFB_MMP_Events.SetVideoLayers,
				...
				is_ipoh_bootup = false;
				MMProfileLogStructure(MTKFB_MMP_Events.SetVideoLayers,
	...
	case MTKFB_SET_MULTIPLE_LAYERS:
		{
			...
			if (copy_from_user(&layerInfo, (void __user *)arg, sizeof(layerInfo))) {
				...
				for (i = 0; i < HW_OVERLAY_COUNT; ++i) {
					...
					if (is_early_suspended) {
						MTKFB_WRAN
						    ("in early suspend layer(0x%x),idx(%d)!\n",
				...
				is_ipoh_bootup = false;
				MMProfileLogStructure(MTKFB_MMP_Events.SetMultipleLayers,
						      MMProfileFlagEnd, layerInfo, struct mmp_fb_overlay_layers);
			}
	...
	case MTKFB_SLT_AUTO_CAPTURE:
		{
			...
			if (copy_from_user(&config, argp, sizeof(config))) {
				...
				MTKFB_INFO("format bit 0x%x buf 0x%x width 0x%x ,height 0x%x\n",
					   config.format, (unsigned int)config.outputBuffer,
					   config.wdma_width, config.wdma_height);
	...
	case DISP_IOCTL_CREATE_SESSION:
		{
			...
			MMProfileLogStructure(MTKFB_MMP_Events.CreateSession, MMProfileFlagStart,
					      &config, disp_session_config);
			...
			MMProfileLogEx(MTKFB_MMP_Events.CreateSession, MMProfileFlagEnd,
				       config.session_id, disp_config.session_id);
			return r;
		}
	...
	case DISP_IOCTL_DESTROY_SESSION:
		{
			...
			MMProfileLogStructure(MTKFB_MMP_Events.DestroySession, MMProfileFlagStart,
					      &config, disp_session_config);
			if (disp_destroy_session(&config) != DCP_STATUS_OK)
				r = -EFAULT;

			MMProfileLogEx(MTKFB_MMP_Events.DestroySession, MMProfileFlagEnd,
				       config.session_id, disp_config.session_id);
			return r;
		}
	...
	case DISP_IOCTL_TRIGGER_SESSION:
		{
			...
			MMProfileLogStructure(MTKFB_MMP_Events.TriggerSession, MMProfileFlagStart,
					      &config, disp_session_config);

/* #if defined(CONFIG_SINGLE_PANEL_OUTPUT) */
			/* FIXME: If WFD, ScreenRecord, don't do this */
			if (is_early_suspended
			    && config.session_id != MAKE_DISP_SESSION(DISP_SESSION_DECOUPLE_MODE,
								      DISP_SESSION_MEMORY, 2)) {
				int i = 0;
				for (i = 0; i < HW_OVERLAY_COUNT; i++) {
...
/* #endif */
			...
			MMProfileLogEx(MTKFB_MMP_Events.TriggerSession, MMProfileFlagEnd,
				       config.session_id, disp_config.session_id);
			return r;
		}
	...
	case DISP_IOCTL_PREPARE_INPUT_BUFFER:
		{
			...
			MMProfileLogStructure(MTKFB_MMP_Events.PrepareInput, MMProfileFlagStart,
					      &info, disp_buffer_info);
			if (info.layer_en) {
	...
	case DISP_IOCTL_PREPARE_OUTPUT_BUFFER:
		{
			...
			MMProfileLogStructure(MTKFB_MMP_Events.PrepareOutput, MMProfileFlagStart,
					      &info, disp_buffer_info);
			if (info.layer_en) {
				if (disp_sync_prepare_buffer(&info) != SYNC_STATUS_OK) {
					info.fence_fd = DISP_INVALID_FENCE_FD;	/* invalid fd */
	...
	case DISP_IOCTL_SET_INPUT_BUFFER:
		{
			...
			MMProfileLogStructure(MTKFB_MMP_Events.SetInput, MMProfileFlagStart,
					      &layerInfo, disp_session_input_config);
	...
	case DISP_IOCTL_SET_OUTPUT_BUFFER:
		{
			...
			MMProfileLogStructure(MTKFB_MMP_Events.SetOutput, MMProfileFlagStart,
					      &layerInfo, disp_session_output_config);
	...
	case DISP_IOCTL_GET_SESSION_INFO:
		{
			...
			MMProfileLogStructure(MTKFB_MMP_Events.GetDispInfo, MMProfileFlagPulse,
					      &info, disp_session_info);
...
/* LGE_BSP_COMMON LGE_CHANGE_E 141030 jongwoo82.lee : AT%LCD */
	default:
		MTKFB_MSG("mtkfb_ioctl Not support, info=0x%08x, cmd=0x%08x, arg=0x%08x, num=%d\n",
...
static void mtkfb_fb_565_to_8888(struct fb_info *fb_info)
{
	...
	MTKFB_INFO
	    ("mtkfb_fb_565_to_8888 xres=%d yres=%d fbsize=0x%X x_virtual=%d s=0x%08X d=0x%08X\n",
	     xres, yres, fbsize, x_virtual, s, d);
	/* printf("[boot_logo_updater]normal\n"); */
	for (j = 0; j < yres; ++j) {
		for (k = 0; k < xres; ++k) {
...
static int mtkfb_probe(struct device *dev)
{
	...
	if (!fbdev->layer_format) {
		MTKFB_MSG("[mtkfb.c FB driver] vmalloc failed, %d\n", __LINE__);
		r = -ENOMEM;
		goto cleanup;
	}
	...
	fbdev->fb_size_in_byte = MTK_FB_SIZEV;
	{
		...
		if (get_max_DRAM_size() < get_actual_DRAM_size()) {
			...
			MTKFB_MSG
			    ("Boot logo address shift, copy framebuffer from 0x%08X to 0x%08X\n",
...
{
#if defined(CONFIG_MTK_HDMI_SUPPORT)
	MTKFB_MSG("[FB Driver] enter hdmi_disc_disp_path\n");

	if (is_early_suspended) {
		MTKFB_MSG("hdmi_disc_disp_path suspended %d\n", __LINE__);
		return 0;
	}
	...
	if (wait_event_interruptible_timeout(disp_done_wq, !disp_running, HZ / 10) == 0)
		MTKFB_MSG("[FB Driver] Wait disp finished timeout in early_suspend\n");
	...
	for (i = 0; i < HW_OVERLAY_COUNT; i++) {
		...
		MTKFB_MSG("[FB Driver] layer%d release fences\n", i);
	}
#endif
	DISP_CHECK_RET(DISP_PanelEnable(FALSE));
	DISP_CHECK_RET(DISP_PowerEnable(FALSE));
	...
	MTKFB_MSG("[FB Driver] hdmi_disc_disp_path\n");
#endif
...
{
#if defined(CONFIG_MTK_HDMI_SUPPORT)
	MTKFB_MSG("[FB Driver] enter hdmi_conn_disp_path\n");
	...
	MTKFB_MSG("[FB Driver] hdmi_conn_disp_path\n");
#endif
	return 0;
}
