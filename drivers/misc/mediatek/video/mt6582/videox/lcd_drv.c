...
static void _WaitForEngineNotBusy(void)
{
	...
	while (_IsEngineBusy()) {
		...
		wait_time++;
		if (wait_time > 2000) {
			DISP_DRV_MSG("[WARNING] Wait for LCD engine not busy timeout!!!\n");
			LCD_DumpRegisters();

			if (LCD_REG->STATUS.WAIT_SYNC) {
				DISP_DRV_MSG("reason is LCD can't wait TE signal!!!\n");
				LCD_TE_Enable(FALSE);
			}
			OUTREG16(&LCD_REG->START, 0);
			OUTREG16(&LCD_REG->START, 0x1);
			      ...
			      UINT32 rd_1st, UINT32 rd_2nd, UINT32 wr_1st, UINT32 wr_2nd)
{
	...
	DISP_DRV_MSG("LCD_ConfigSerialIF 1\n");
	ASSERT(id >= LCD_IF_SERIAL_0 && id <= LCD_IF_SERIAL_1);

	DISP_DRV_MSG("LCD_ConfigSerialIF 2\n");

	_WaitForEngineNotBusy();

	DISP_DRV_MSG("LCD_ConfigSerialIF 3\n");
	...
	DISP_DRV_MSG("LCD_ConfigSerialIF 4\n");
...
LCD_STATUS LCD_Dump_Layer_Info(void)
{
	...
	DISP_DRV_MSG("LCD_Dump_Layer_Info:\n");
	for (i = 0; i < 4; i++) {
		DISP_DRV_MSG("layer=%d, en=%d, src=%d, fmt=%d, addr=0x%x,",
			     cached_layer_config[i].layer,
			     cached_layer_config[i].layer_en,
			     cached_layer_config[i].source,
			     cached_layer_config[i].fmt, cached_layer_config[i].addr);
		DISP_DRV_MSG(" (%d, %d, %d, %d), pitch=%d, keyEn=%d, key=0x%x,",
		...
		DISP_DRV_MSG(" aen=%d, alpha=0x%x, isTdshp=%d, curr_buff_idx=%d, next_buff_idx=%d\n",
...
void LCD_DumpLayer(void)
{
	unsigned int i;
	for (i = 0; i < 4; i++) {
		DISP_DRV_MSG("LayerInfo in LCD driver, layer=%d,layer_en=%d, source=%d, fmt=%d, addr=0x%x,",
...
LCD_STATUS LCD_Capture_Layerbuffer(unsigned int layer_id, unsigned int pvbuf, unsigned int bpp)
{
	...
	LCD_TE_Enable(FALSE); LCD_SetOutputAlpha(0xff);
...
#endif
	LCD_CHECK_RET(LCD_StartTransfer(TRUE)); LCD_SetOutputMode(mode);
	/* capture end */
	_RestoreLCDRegisters();
#if defined(CONFIG_MTK_M4U_SUPPORT)
...
LCD_STATUS LCD_Capture_Videobuffer(unsigned int pvbuf, unsigned int bpp, unsigned int video_rotation)
{
	...
	unsigned int ppbuf = 0;
	UINT16 offset_x, offset_y, w, h;
	LCD_OUTPUT_MODE mode; if (pvbuf == 0 || bpp == 0) {
...
LCD_STATUS LCD_FM_Desense(LCD_IF_ID id, unsigned long freq)
{
	...
	/* this is 6573 E1, E2 will change */
	a = 13000 - freq * 10 - 20; b = 13000 - freq * 10 + 20; write_cycles = wst + c2wh + chw + 2;
...
int m4u_alloc_mva_stub(M4U_MODULE_ID_ENUM eModuleID,
			const unsigned int BufAddr, const unsigned int BufSize, unsigned int *pRetMVABuf)
{
#if 0
	if (!_m4u_lcdc_func.isInit) {
...
int m4u_dealloc_mva_stub(M4U_MODULE_ID_ENUM eModuleID,
...
BOOL LCD_esd_check(void)
{
	...
	LCD_TE_Enable(FALSE);
	/* Write to memory */
	LCD_CHECK_RET(LCD_SetOutputMode(LCD_OUTPUT_TO_MEM)); return lcd_esd_check;
#endif
	return TRUE;
}
