...
static void DAL_UpdateScreen(BOOL enable, BOOL aen, UINT32 width, UINT32 height)
{
	...
	DISP_DRV_INFO
	    ("[FB Driver] DAL_UpdateScreen(): id=%d,en=%d,aen=%d,fmt=%d,pitch=%d,w=%d,h=%d\n",
	     ASSERT_LAYER, enable, aen, DP_COLOR_GET_UNIQUE_ID(DAL_FORMAT),
	     (int)(DAL_PITCH * DAL_BPP), width, height);
...
int DAL_address_burst_align(void)
{
	...
	DISP_DRV_INFO("dal address align 0x%x\n", align);
	return align;
}
...
DAL_STATUS DAL_Dynamic_Change_FB_Layer(unsigned int isAEEEnabled)
{
	...
	DISP_DRV_MSG("[DDP] DAL_Dynamic_Change_FB_Layer, isAEEEnabled=%d\n", isAEEEnabled);

	if (DISP_DEFAULT_UI_LAYER_ID == DISP_CHANGED_UI_LAYER_ID) {
		DISP_DRV_MSG("[DDP] DAL_Dynamic_Change_FB_Layer, no dynamic switch\n");
		return DAL_STATUS_OK;
	}
...
DAL_STATUS DAL_Printf(const char *fmt, ...)
{
	...
	DAL_LOCK();
	if (isAEEEnabled == 0) {
		DISP_DRV_MSG("[DDP] isAEEEnabled from 0 to 1, ASSERT_LAYER=%d, dal_fb_pa %x\n",
...
DAL_STATUS DAL_Clean(void)
{
	DISP_DRV_MSG("[MTKFB_DAL] DAL_Clean is not implemented\n");
	return DAL_STATUS_OK;
}
...
{
	NOT_REFERENCED(fmt);
	DISP_DRV_MSG("[MTKFB_DAL] DAL_Printf is not implemented\n");
	return DAL_STATUS_OK;
}
