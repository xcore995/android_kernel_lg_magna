...
void kpd_slide_qwerty_init(void)
{
	...
	if (!power_op)
		kpd_print"Qwerty slide pin interface power on fail\n");
	else
	...
	if (!power_op)
		kpd_print("Qwerty slide pin interface power off fail\n");
	else
	...
	for (i = 0; i < KPD_NUM_MEMS; i++) {
		...
		for (j = 0; j < 16; j++) {
			...
			if (kpd_show_hw_keycode) {
				kpd_print("(%s) factory_mode HW keycode = %u\n",
...
void kpd_init_keymap_state(u16 keymap_state[])
{
	...
	kpd_print("init_keymap_state done: %x %x %x %x %x!\n", keymap_state[0],
...
void kpd_pmic_rstkey_hal(unsigned long pressed)
{
	...
	if (!kpd_sb_enable) {
		...
		input_sync(kpd_input_dev);
		if (kpd_show_hw_keycode) {
			kpd_print("(%s) HW keycode =%d using PMIC\n",
...
void kpd_pwrkey_handler_hal(unsigned long data)
{
	...
	if (kpd_show_hw_keycode)
		kpd_print("(%s) HW keycode = using EINT\n", pressed ? "pressed" : "released");
