...
static int do_umplock_create_locked( _lock_cmd_priv *lock_cmd)
{
	...
	#if 0 
	if ( lock_item->usage == 1 ) pr_debug( "UMPLOCK: C 0x%x GPU SURFACE\n", lock_item->secure_id );
	else if ( lock_item->usage == 2 ) pr_debug( "UMPLOCK: C 0x%x GPU TEXTURE\n", lock_item->secure_id );
	else pr_debug( "UMPLOCK: C 0x%x CPU\n", lock_item->secure_id );
	#endif
	...
	else if ( (i_index = umplock_find_item( lock_item->secure_id)) >= 0 )
	{
		...
		{
			pr_err( "UMPLOCK: whoops, item ran out of available reference slot\n" );
		}
	}
	else
	{
		...
		{
			pr_err( "UMPLOCK: whoops, ran out of available slots\n" );
		}
	}
...
static int do_umplock_process( _lock_cmd_priv *lock_cmd )
{
	...
	{	
		...
		#if 0
		if ( lock_item->usage == 1 ) pr_debug( "UMPLOCK:  P 0x%x GPU SURFACE\n", lock_item->secure_id );
		else if ( lock_item->usage == 2 ) pr_debug( "UMPLOCK:  P 0x%x GPU TEXTURE\n", lock_item->secure_id );
		else pr_debug( "UMPLOCK:  P 0x%x CPU\n", lock_item->secure_id );
		#endif
	}
	else
	{
		/*fail to find a item*/
		pr_err("UMPLOCK: IOCTL_UMPLOCK_PROCESS called with invalid parameter\n");
		mutex_unlock(&device.item_list_lock);
		return -EINVAL;
	}
	mutex_unlock(&device.item_list_lock);
...
static int do_umplock_release( _lock_cmd_priv *lock_cmd )
{
	...
	if ( ret >= 0 )
	{
		...
		#if 0
		if ( lock_item->usage == 1 ) pr_debug( "UMPLOCK:   R 0x%x GPU SURFACE\n", lock_item->secure_id );
		else if ( lock_item->usage == 2 ) pr_debug( "UMPLOCK:   R 0x%x GPU TEXTURE\n", lock_item->secure_id );
		else pr_debug( "UMPLOCK:   R 0x%x CPU\n", lock_item->secure_id );
		#endif
	...
	{
		/*fail to find item*/
		pr_err("UMPLOCK: IOCTL_UMPLOCK_RELEASE called with invalid parameter\n");
		mutex_unlock(&device.item_list_lock);
		return -EINVAL;
	}
	mutex_unlock(&device.item_list_lock);
...
static int do_umplock_zap( void )
{
	...
	pr_debug( "UMPLOCK: ZAP ALL ENTRIES!\n" );
...
static int do_umplock_dump( void )
{
	...
	for (i = 0; i < MAX_ITEMS; i++)
	{
		for (j = 0; j < MAX_PIDS; j++)
		{
			...
			{
				pr_debug("item[%d]->secure_id=%d\t reference[%d].ref_count=%d.pid=%d\n",
					i,
...
int do_umplock_client_add (_lock_cmd_priv *lock_cmd )
{
	...
	if( i==MAX_PIDS)
	{
		pr_err("Oops, Run out of cient slots\n ");
	}
	return 0;
}
...
static int umplock_driver_open( struct inode *inode, struct file *filp )
{
	...
	pr_debug( "UMPLOCK: OPEN SESSION (%i references)\n", atomic_read(&device.sessions) );
...
static int umplock_driver_release( struct inode *inode, struct file *filp )
{
	...
	pr_debug( "UMPLOCK: CLOSE SESSION (%i references)\n", atomic_read(&device.sessions) );
...
{
	pr_debug( "Inserting UMP lock device driver. Compiled: %s, time: %s\n", __DATE__, __TIME__ );

	if ( !umplock_constructor() )
	{
		pr_err( "UMP lock device driver init failed\n");
		return -ENOTTY;
	}

	pr_debug( "UMP lock device driver loaded\n" );
...
{
	pr_debug( "unloading UMP lock module\n" );
	umplock_destructor();
	pr_debug( "UMP lock module unloaded\n" );
}
