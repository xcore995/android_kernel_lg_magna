...
int iWriteCAM_CAL_DW9716(u16 a_u2Addr, u32 a_u4Bytes, u8 *puDataInBytes)
{
	int i4RetValue = 0;
	u32 u4Index = 0;
...
/* maximun read length is limited at "I2C_FIFO_SIZE" in I2c-mt65xx.c which is 8 bytes */
int iReadCAM_CAL_DW9716(u16 a_u2Addr, u32 ui4_length, u8 *a_puBuff)
{
	int i4RetValue = 0;
...
static int iWriteData(unsigned int ui4_offset, unsigned int ui4_length, unsigned char *pinputdata)
{
	...
	do {
		if (i4ResidueDataLength >= 6) {
			i4RetValue = iWriteCAM_CAL_DW9716((u16) u4CurrentOffset, 6, pBuff);
			if (i4RetValue != 0) {
			...
			i4RetValue =
			    iWriteCAM_CAL_DW9716((u16) u4CurrentOffset, i4ResidueDataLength, pBuff);
			if (i4RetValue != 0) {
				CAM_CALDB("[CAM_CAL] I2C iWriteData failed!!\n");
				return -1;
			}
...
int iReadDataFromDW9716(unsigned int ui4_offset, unsigned int ui4_length,
			     unsigned char *pinputdata)
{
	...
	CAM_CALDB("[dw9716_eeprom] ui4_offset=%x ui4_offset(80)=%x ui4_offset(8)=%x\n",
	...
	CAM_CALDB("[dw9716_eeprom] EEPROM_Address[0]=%x EEPROM_Address[1]=%x\n",
	...
	CAM_CALDB("[dw9716_eeprom] loop[0]=%d loop[1]=%d\n", (loop[0]), (loop[1]));
	...
	for (loopCount = 0; loopCount < Read_NUMofEEPROM; loopCount++) {
		do {
			if (16 <= loop[loopCount]) {
				CAM_CALDB("[dw9716_eeprom]1 loopCount=%d ", loopCount);
				...
				if (i4RetValue != 0) {
					CAM_CALDB
					    ("[dw9716_eeprom] I2C iReadData failed!!\n");
					return -1;
				}
				u4IncOffset += 16;
				loop[loopCount] -= 16;
				...
				CAM_CALDB("[dw9716_eeprom]2 loopCount=%d ", loopCount);
				CAM_CALDB("loop[loopCount]=%d puSendCmd[0]=%x ", loop[loopCount], puSendCmd[0]);
...
/* CAM_CALDB("[S24EEPORM] iReadData\n" ); */
	...
	if (ui4_offset + ui4_length >= 0x2000) {
		CAM_CALDB
		    ("[dw9716_eeprom] Read Error!! S-dw9716_eeprom not supprt address >= 0x2000!!\n");
		return -1;
	}
	...
	do {
		if (i4ResidueDataLength >= 8) {
			i4RetValue = iReadCAM_CAL_DW9716((u16) u4CurrentOffset, 8, pBuff);
			if (i4RetValue != 0) {
				CAM_CALDB("[dw9716_eeprom] I2C iReadData failed!!\n");
				return -1;
			}
			u4IncOffset += 8;
			...
			i4RetValue =
			    iReadCAM_CAL_DW9716((u16) u4CurrentOffset, i4ResidueDataLength, pBuff);
			if (i4RetValue != 0) {
				CAM_CALDB("[dw9716_eeprom] I2C iReadData failed!!\n");
				return -1;
			}
...
int DW9716_EEPROM_Ioctl(struct file *file, unsigned int a_u4Command, unsigned long a_u4Param)
...
#endif				/* COMMON_CAM_CAL_DRV */
{
	...
	if (_IOC_NONE != _IOC_DIR(a_u4Command)) {
		...
		if (NULL == pBuff) {
			CAM_CALDB("[dw9716_eeprom] ioctl allocate mem failed\n");
			return -ENOMEM;
		}
		...
		if (_IOC_WRITE & _IOC_DIR(a_u4Command)) {
			if (copy_from_user((u8 *) pBuff, (u8 *) a_u4Param,
				...
				kfree(pBuff);
				CAM_CALDB("[dw9716_eeprom] ioctl copy from user failed\n");
				return -EFAULT;
			}
		}
	}
	...
	CAM_CALDB("[dw9716_eeprom] init Working buffer address 0x%8x  command is 0x%8x\n",
	...
	case CAM_CALIOC_S_WRITE:
		CAM_CALDB("[dw9716_eeprom] Write CMD\n");
#ifdef CAM_CALGETDLT_DEBUG
	...
	case CAM_CALIOC_G_READ:
		CAM_CALDB("[dw9716_eeprom] Read CMD\n");
#ifdef CAM_CALGETDLT_DEBUG
		do_gettimeofday(&ktv1);
#endif
		CAM_CALDB("[dw9716_eeprom] offset %x\n", ptempbuf->u4Offset);
		CAM_CALDB("[dw9716_eeprom] length %d\n", ptempbuf->u4Length);
		CAM_CALDB("[dw9716_eeprom] Before read Working buffer address 0x%8x\n",
			  (u32) pWorkingBuff);
			  
		CAM_CALDB("POPO [dw9716_eeprom] EEPROM_Address[0] %d\n", EEPROM_Address[0]);
		CAM_CALDB("POPO [dw9716_eeprom] EEPROM_Address[1] %d\n", EEPROM_Address[1]);	  

		/* i4RetValue = iReadData((u16)ptempbuf->u4Offset, ptempbuf->u4Length, pWorkingBuff); */
		CAM_CALDB("[dw9716_eeprom] After read Working buffer data  0x%4x\n",
			  *pWorkingBuff);

		if ((ptempbuf->u4Offset & 0xFFFFFFFF) == 0xFFFD9716) {
			...
			*(u32 *) pWorkingBuff = (ptempbuf->u4Offset | 0x10000000);
			...
			CAM_CALDB("[dw9716_eeprom] Shading CheckSum MSB=> %x %x\n",
			...
			while (0 < readTryagain) {
				...
				/* i4RetValue =  iReadDataFromM24C08F((u16)ptempbuf->u4Offset,
					ptempbuf->u4Length, pWorkingBuff); */
				i4RetValue =
				    iReadDataFromDW9716(ptempbuf->u4Offset, ptempbuf->u4Length,
							     pWorkingBuff);
#if 0
				if (0 < test_retry) {
					CAM_CALDB
					    ("[dw9716_eeprom] Test error (%d) Read retry (%d)\n",
					     test_retry, i4RetValue);
					i4RetValue = -1;
					test_retry--;
				}
#endif
				CAM_CALDB("[dw9716_eeprom] error (%d) Read retry (%d)\n",
		...
		break;
	default:
		CAM_CALDB("[dw9716_eeprom] No CMD\n");
		i4RetValue = -EPERM;
		break;
	}
	...
	if (_IOC_READ & _IOC_DIR(a_u4Command)) {
		...
		CAM_CALDB("[dw9716_eeprom] to user length %d\n", ptempbuf->u4Length);
		CAM_CALDB("[dw9716_eeprom] to user  Working buffer address 0x%8x\n",
			  (u32) pWorkingBuff);
		if (copy_to_user
...
{
	CAM_CALDB("[dw9716_eeprom] CAM_CAL_Open\n");
	spin_lock(&g_CAM_CALLock);
	if (g_u4Opened) {
		spin_unlock(&g_CAM_CALLock);
		return -EBUSY;
...
static int RegisterCAM_CALCharDrv(void)
{
	...
	if (alloc_chrdev_region(&g_CAM_CALdevno, 0, 1, CAM_CAL_DRVNAME)) {
		CAM_CALDB("[dw9716_eeprom] Allocate device no failed\n");

		return -EAGAIN;
	}
#else
	if (register_chrdev_region(g_CAM_CALdevno, 1, CAM_CAL_DRVNAME)) {
		CAM_CALDB("[dw9716_eeprom] Register device no failed\n");
	...
	if (NULL == g_pCAM_CAL_CharDrv) {
		...
		CAM_CALDB("[dw9716_eeprom] Allocate mem for kobject failed\n");
	...
	if (cdev_add(g_pCAM_CAL_CharDrv, g_CAM_CALdevno, 1)) {
		CAM_CALDB("[dw9716_eeprom] Attatch file operation failed\n");
...
static int CAM_CAL_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	CAM_CALDB("[dw9716_eeprom] Attach I2C\n");
/* spin_lock_init(&g_CAM_CALLock); */

	/* get sensor i2c client */
	spin_lock(&g_CAM_CALLock);	/* for SMP */
	g_pstI2Cclient = client;
	g_pstI2Cclient->addr = dw9716_DEVICE_ID >> 1;
	spin_unlock(&g_CAM_CALLock);	/* for SMP */

	CAM_CALDB("[dw9716_eeprom] g_pstI2Cclient->addr = 0x%8x\n", g_pstI2Cclient->addr);
	/* Register char driver */
	i4RetValue = RegisterCAM_CALCharDrv();

	if (i4RetValue) {
		CAM_CALDB("[dw9716_eeprom] register char device failed!\n");
		return i4RetValue;
	}


	CAM_CALDB("[dw9716_eeprom] Attached!!\n");
	return 0;
}
...
static int __init CAM_CAL_i2C_init(void)
{
	...
	if (platform_driver_register(&g_stCAM_CAL_Driver)) {
		CAM_CALDB("failed to register dw9716_eeprom driver\n");
		return -ENODEV;
	}

	if (platform_device_register(&g_stCAM_CAL_Device)) {
		CAM_CALDB("failed to register dw9716_eeprom driver, 2nd time\n");
		return -ENODEV;
	}
