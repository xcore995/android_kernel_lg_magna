...
int iReadDataFromBRCB032GWZ3(unsigned int ui4_offset, unsigned int ui4_length,
	...
	for (loopCount = 0; loopCount < Read_NUMofEEPROM; loopCount++) {
		do {
			if (16 <= loop[loopCount]) {
				CAM_CALDB("[BRCB032GWZ_3_eeprom]1 loopCount=%d ", loopCount);
				...
				if (i4RetValue != 0) {
					CAM_CALDB
					    ("[BRCB032GWZ_3_eeprom] I2C iReadData failed!!\n");
					return -1;
				}
				u4IncOffset += 16;
				loop[loopCount] -= 16;
				...
				CAM_CALDB("[BRCB032GWZ_3_eeprom]2 loopCount=%d ", loopCount);
				CAM_CALDB("loop[loopCount]=%d puSendCmd[0]=%x ", loop[loopCount], puSendCmd[0]);
...
/* CAM_CALDB("[S24EEPORM] iReadData\n" ); */
	...
	if (ui4_offset + ui4_length >= 0x2000) {
		CAM_CALDB
		    ("[BRCB032GWZ_3_eeprom] Read Error!! S-BRCB032GWZ_3_eeprom not supprt address >= 0x2000!!\n");
		return -1;
	}
...
int BRCB032GW_3_EEPROM_Ioctl(struct file *file, unsigned int a_u4Command, unsigned long a_u4Param)
...
#endif				/* COMMON_CAM_CAL_DRV */
{
	...
	case CAM_CALIOC_G_READ:
		...
		do_gettimeofday(&ktv1);
#endif
		CAM_CALDB("[BRCB032GWZ_3_eeprom] offset %x\n", ptempbuf->u4Offset);
		...
		CAM_CALDB("[BRCB032GWZ_3_eeprom] After read Working buffer data  0x%4x\n",
			  *pWorkingBuff);
		CAM_CALDB("POPO [BRCB032GWZ_3_eeprom] EEPROM_Address[0] = %d\n", EEPROM_Address[0]);
		CAM_CALDB("POPO [BRCB032GWZ_3_eeprom] EEPROM_Address[1] = %d\n", EEPROM_Address[1]);		
		if ((ptempbuf->u4Offset & 0xFFFFFFFF) == 0xFFFBCB32) {
			...
			*(u32 *) pWorkingBuff = (ptempbuf->u4Offset | 0x10000000);
			...
			while (0 < readTryagain) {
				...
				/* i4RetValue =  iReadDataFromM24C08F((u16)ptempbuf->u4Offset,
					ptempbuf->u4Length, pWorkingBuff); */
				i4RetValue =
				    iReadDataFromBRCB032GWZ3(ptempbuf->u4Offset, ptempbuf->u4Length,
							     pWorkingBuff);
#if 0
				if (0 < test_retry) {
					CAM_CALDB
					    ("[BRCB032GWZ_3_eeprom] Test error (%d) Read retry (%d)\n",
