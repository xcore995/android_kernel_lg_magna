...
int iWriteEEPROM(u16 a_u2Addr, u32 a_u4Bytes, u8 *puDataInBytes)
{
	...
	if (a_u4Bytes + 2 > 8) {
		EEPROMDB
		    ("[BRCC064GWZ_3_eeprom] exceed I2c-mt65xx.c 8 bytes limitation (include address 2 Byte)\n");
		return -1;
	}
...
int iReadDataFromBRCC064GWZ_3(unsigned int ui4_offset, unsigned int ui4_length,
			      unsigned char *pinputdata)
{
	...
	/* unsigned char address_offset = ((SampleNum *SampleOffset) + EEPROM_Address_Offset
							+ ui4_length) / Boundary_Address; */
	...
	for (loopCount = 0; loopCount < Read_NUMofEEPROM; loopCount++) {
		do {
			if (16 <= loop[loopCount]) {
				EEPROMDB("[BRCC064GWZ_3_eeprom]1 loopCount=%d ", loopCount);
				EEPROMDB("loop[loopCount]=%d puSendCmd[0]=%x ", loop[loopCount], puSendCmd[0]);
				...
				EEPROMDB("[BRCC064GWZ_3_eeprom]2 loopCount=%d ", loopCount);
				EEPROMDB("loop[loopCount]=%d puSendCmd[0]=%x ", loop[loopCount], puSendCmd[0]);
				EEPROMDB("puSendCmd[1]=%x\n", puSendCmd[1]);
...
/* EEPROMDB("[S24EEPORM] iReadData\n" ); */
	...
	if (ui4_offset + ui4_length >= 0x2000) {
		EEPROMDB
		    ("[BRCC064GWZ_3_eeprom] Read Error!! S-24CS64A not supprt address >= 0x2000!!\n");
		return -1;
	}
...
int BRCC064GWZ_3_EEPROM_Ioctl(struct file *file, unsigned int a_u4Command, unsigned long a_u4Param)
...
#endif				/* COMMON_CAM_CAL_DRV */
{
	...
	case CAM_CALIOC_G_READ:
		...
		EEPROMDB("POPO [BRCC064GWZ_3_eeprom] EEPROM_Address[0] %d\n", EEPROM_Address[0]);
		EEPROMDB("POPO [BRCC064GWZ_3_eeprom] EEPROM_Address[1] %d\n", EEPROM_Address[1]);
		
		if ((ptempbuf->u4Offset & 0xFFFFFFFF) == 0xFFFBCC64) {
			...
			*(u32 *) pWorkingBuff = (ptempbuf->u4Offset | 0x10000000);
			...
			while (0 < readTryagain) {
				i4RetValue =
				    iReadDataFromBRCC064GWZ_3(ptempbuf->u4Offset,
