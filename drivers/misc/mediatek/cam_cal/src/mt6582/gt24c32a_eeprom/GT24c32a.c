...
/* for init.rc static struct i2c_board_info __initdata kd_cam_cal_dev={ I2C_BOARD_INFO("CAM_CAL_GT24C32A", 0xAA>>1)}; */
...
*
********************************************************************************/
/* for init.rc #define CAM_CAL_DRVNAME "CAM_CAL_GT24C32A" */
...
int iWriteCAM_CAL_GT24C32A(u16 a_u2Addr, u32 a_u4Bytes, u8 *puDataInBytes)
{
	int i4RetValue = 0;
	u32 u4Index = 0;
	...
	/* CAM_CALDB("[CAM_CAL] iWriteCAM_CAL_GT24C32A done!!\n"); */
	return 0;
}


/* maximun read length is limited at "I2C_FIFO_SIZE" in I2c-mt65xx.c which is 8 bytes */
int iReadCAM_CAL_GT24C32A(u16 a_u2Addr, u32 ui4_length, u8 *a_puBuff)
{
	int i4RetValue = 0;
	char puReadCmd[2] = { (char)(a_u2Addr >> 8), (char)(a_u2Addr & 0xFF) };

	/* CAM_CALDB("[CAM_CAL] iReadCAM_CAL_GT24C32A!!\n"); */
	...
	/* CAM_CALDB("[CAM_CAL] iReadCAM_CAL_GT24C32A done!!\n"); */
	return 0;
}


static int iWriteData_GT24C32A(unsigned int ui4_offset, unsigned int ui4_length, unsigned char *pinputdata)
{
	int i4RetValue = 0;
	...
	CAM_CALDB("[CAM_CAL] iWriteData_GT24C32A\n");
	...
	CAM_CALDB("[CAM_CAL] iWriteData_GT24C32A u4CurrentOffset is %d\n", u4CurrentOffset);

	do {
		if (i4ResidueDataLength >= 6) {
			i4RetValue = iWriteCAM_CAL_GT24C32A((u16) u4CurrentOffset, 6, pBuff);
			if (i4RetValue != 0) {
				CAM_CALDB("[CAM_CAL] I2C iWriteData_GT24C32A failed!!\n");
				return -1;
			}
			u4IncOffset += 6;
			...
			i4RetValue =
			    iWriteCAM_CAL_GT24C32A((u16) u4CurrentOffset, i4ResidueDataLength, pBuff);
			if (i4RetValue != 0) {
				CAM_CALDB("[CAM_CAL] I2C iWriteData_GT24C32A failed!!\n");
				return -1;
			}
	...
	CAM_CALDB("[CAM_CAL] iWriteData_GT24C32A done\n");
			     ...
			     unsigned char *pinputdata)
{
	...
	CAM_CALDB("[GT24C32A_eeprom] ui4_offset=%x ui4_offset(80)=%x ui4_offset(8)=%x\n",
	...
	CAM_CALDB("[GT24C32A_eeprom] EEPROM_Address[0]=%x EEPROM_Address[1]=%x\n",
	...
	CAM_CALDB("[GT24C32A_eeprom] loop[0]=%d loop[1]=%d\n", (loop[0]), (loop[1]));
	...
	for (loopCount = 0; loopCount < Read_NUMofEEPROM; loopCount++) {
		do {
			if (16 <= loop[loopCount]) {
				CAM_CALDB("[GT24C32A_eeprom]1 loopCount=%d ", loopCount);
				...
				if (i4RetValue != 0) {
					CAM_CALDB
					    ("[GT24C32A_eeprom] I2C iReadData_GT24C32A failed!!\n");
					return -1;
				}
                if(0) {
                    int i=0;
                    printk("[YONG] [EEPROM Data][%d] ",u4IncOffset);
                    for(i=0;i<16;i++)
                    {
                        printk("%d, ",pBuff[u4IncOffset+i]);
                    }
                    printk("\n");
                }
				...
				CAM_CALDB("[GT24C32A_eeprom]2 loopCount=%d ", loopCount);
				CAM_CALDB("loop[loopCount]=%d puSendCmd[0]=%x ", loop[loopCount], puSendCmd[0]);
...
/* int iReadData_GT24C32A(stCAM_CAL_INFO_STRUCT * st_pOutputBuffer) */
static int iReadData_GT24C32A(unsigned int ui4_offset, unsigned int ui4_length, unsigned char *pinputdata)
{
	int i4RetValue = 0;
	int i4ResidueDataLength;
	u32 u4IncOffset = 0;
	u32 u4CurrentOffset;
	u8 *pBuff;
/* CAM_CALDB("[S24EEPORM] iReadData_GT24C32A\n" ); */

	if (ui4_offset + ui4_length >= 0x2000) {
		CAM_CALDB
		    ("[GT24C32A_eeprom] Read Error!! S-GT24C32A_eeprom not supprt address >= 0x2000!!\n");
		return -1;
	}
	...
	do {
		if (i4ResidueDataLength >= 8) {
			i4RetValue = iReadCAM_CAL_GT24C32A((u16) u4CurrentOffset, 8, pBuff);
			if (i4RetValue != 0) {
				CAM_CALDB("[GT24C32A_eeprom] I2C iReadData_GT24C32A failed!!\n");
				return -1;
			}
			u4IncOffset += 8;
			...
			i4RetValue =
			    iReadCAM_CAL_GT24C32A((u16) u4CurrentOffset, i4ResidueDataLength, pBuff);
			if (i4RetValue != 0) {
				CAM_CALDB("[GT24C32A_eeprom] I2C iReadData_GT24C32A failed!!\n");
				return -1;
			}
...
/* CAM_CALDB("[S24EEPORM] iReadData_GT24C32A finial address is %d length is %d buffer address is 0x%x\n"
	,u4CurrentOffset, i4ResidueDataLength, pBuff); */
/* CAM_CALDB("[S24EEPORM] iReadData_GT24C32A done\n" ); */
	return 0;
}
...
#endif				/* COMMON_CAM_CAL_DRV */
{
	...
	if (_IOC_NONE != _IOC_DIR(a_u4Command)) {
		...
		if (NULL == pBuff) {
			CAM_CALDB("[GT24C32A_eeprom] ioctl allocate mem failed\n");
			return -ENOMEM;
		}
		...
		if (_IOC_WRITE & _IOC_DIR(a_u4Command)) {
			if (copy_from_user((u8 *) pBuff, (u8 *) a_u4Param,
				...
				kfree(pBuff);
				CAM_CALDB("[GT24C32A_eeprom] ioctl copy from user failed\n");
				return -EFAULT;
			}
		}
	}
	...
	CAM_CALDB("[GT24C32A_eeprom] init Working buffer address 0x%8x  command is 0x%8x\n",
	...
	case CAM_CALIOC_S_WRITE:
		CAM_CALDB("[GT24C32A_eeprom] Write CMD\n");
#ifdef CAM_CALGETDLT_DEBUG
		do_gettimeofday(&ktv1);
#endif
		i4RetValue = iWriteData_GT24C32A((u16) ptempbuf->u4Offset, ptempbuf->u4Length, pWorkingBuff);
#ifdef CAM_CALGETDLT_DEBUG
	...
	case CAM_CALIOC_G_READ:
		CAM_CALDB("[GT24C32A_eeprom] Read CMD\n");
#ifdef CAM_CALGETDLT_DEBUG
		do_gettimeofday(&ktv1);
#endif
		CAM_CALDB("[GT24C32A_eeprom] offset %x\n", ptempbuf->u4Offset);
		CAM_CALDB("[GT24C32A_eeprom] length %d\n", ptempbuf->u4Length);
		CAM_CALDB("[GT24C32A_eeprom] Before read Working buffer address 0x%8x\n",
			  (u32) pWorkingBuff);

		/* i4RetValue = iReadData_GT24C32A((u16)ptempbuf->u4Offset, ptempbuf->u4Length, pWorkingBuff); */
		CAM_CALDB("[GT24C32A_eeprom] After read Working buffer data  0x%4x\n",
			  *pWorkingBuff);
		CAM_CALDB("POPO [GT24C32A_eeprom] EEPROM_Address[0] = %d\n", EEPROM_Address[0]);
		CAM_CALDB("POPO [GT24C32A_eeprom] EEPROM_Address[1] = %d\n", EEPROM_Address[1]);		
		if ((ptempbuf->u4Offset & 0xFFFFFFFF) == 0xFFF24C32) {
			...
			*(u32 *) pWorkingBuff = (ptempbuf->u4Offset | 0x10000000);
			...
			CAM_CALDB("[GT24C32A_eeprom] Shading CheckSum MSB=> %x %x\n",
				  pWorkingBuff[0], pWorkingBuff[1]);
		} else {
			/* i4RetValue = iReadData_GT24C32A((u16)ptempbuf->u4Offset, ptempbuf->u4Length, pWorkingBuff); */

			/* i4RetValue =  iReadData_GT24C32AFromM24C08F((u16)ptempbuf->u4Offset, ptempbuf->u4Length,
				pWorkingBuff); */
			readTryagain = 3;
			test_retry = 2;
			while (0 < readTryagain) {
				...
				if (0 < test_retry) {
					CAM_CALDB
					    ("[GT24C32A_eeprom] Test error (%d) Read retry (%d)\n",
					     test_retry, i4RetValue);
					i4RetValue = -1;
					test_retry--;
				}
#endif
				CAM_CALDB("[GT24C32A_eeprom] error (%d) Read retry (%d)\n",
		...
		break;
	default:
		CAM_CALDB("[GT24C32A_eeprom] No CMD\n");
		i4RetValue = -EPERM;
		break;
	}
	...
	if (_IOC_READ & _IOC_DIR(a_u4Command)) {
		...
		CAM_CALDB("[GT24C32A_eeprom] to user length %d\n", ptempbuf->u4Length);
		CAM_CALDB("[GT24C32A_eeprom] to user  Working buffer address 0x%8x\n",
			  (u32) pWorkingBuff);
		if (copy_to_user
...
{
	CAM_CALDB("[GT24C32A_eeprom] CAM_CAL_Open\n");
	spin_lock(&g_CAM_CALLock);
	if (g_u4Opened) {
		spin_unlock(&g_CAM_CALLock);
		return -EBUSY;
...
#if 0
	if (TRUE != hwPowerOn(MT65XX_POWER_LDO_VCAMA, VOL_2800, "GT24C32A")) {
...
static int RegisterCAM_CALCharDrv(void)
{
	...
	if (alloc_chrdev_region(&g_CAM_CALdevno, 0, 1, CAM_CAL_DRVNAME)) {
		CAM_CALDB("[GT24C32A_eeprom] Allocate device no failed\n");

		return -EAGAIN;
	}
#else
	if (register_chrdev_region(g_CAM_CALdevno, 1, CAM_CAL_DRVNAME)) {
		CAM_CALDB("[GT24C32A_eeprom] Register device no failed\n");
	...
	if (NULL == g_pCAM_CAL_CharDrv) {
		...
		CAM_CALDB("[GT24C32A_eeprom] Allocate mem for kobject failed\n");
	...
	if (cdev_add(g_pCAM_CAL_CharDrv, g_CAM_CALdevno, 1)) {
		CAM_CALDB("[GT24C32A_eeprom] Attatch file operation failed\n");
...
static unsigned short force[] = {
	CAM_CAL_I2C_GROUP_ID, GT24C32A_DEVICE_ID, I2C_CLIENT_END, I2C_CLIENT_END };
#else
...
static int CAM_CAL_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	CAM_CALDB("[GT24C32A_eeprom] Attach I2C\n");
/* spin_lock_init(&g_CAM_CALLock); */
	...
	CAM_CALDB("[GT24C32A_eeprom] g_pstI2Cclient->addr = 0x%8x\n", g_pstI2Cclient->addr);
	/* Register char driver */
	i4RetValue = RegisterCAM_CALCharDrv();

	if (i4RetValue) {
		CAM_CALDB("[GT24C32A_eeprom] register char device failed!\n");
		return i4RetValue;
	}


	CAM_CALDB("[GT24C32A_eeprom] Attached!!\n");
	return 0;
}
...
static int __init CAM_CAL_i2C_init(void)
{
	...
	if (platform_driver_register(&g_stCAM_CAL_Driver)) {
		CAM_CALDB("failed to register GT24C32A_eeprom driver\n");
		return -ENODEV;
	}

	if (platform_device_register(&g_stCAM_CAL_Device)) {
		CAM_CALDB("failed to register GT24C32A_eeprom driver, 2nd time\n");
		return -ENODEV;
	}
