...
void mtktspmic_read_efuse(void)
{
    ...
    pr_debug("[mtktspmic_read_efuse] start\n");
#if 1
	/*
	   0x8	640	655
	   0x9	656	671
	...
	pr_debug("[mtktspmic_read_efuse]6328_efuse:\n"
                "efusevalue[0]=0x%x\n"
                "efusevalue[1]=0x%x\n"
                "efusevalue[2]=0x%x\n\n"
...
*/
    ...
    for(i=0x8;i<=0xa;i++)
    {
        ...
        pr_debug("5. 6328 delay 1 ms\n");


        //6. read data
        efusevalue[j] = upmu_get_reg_value(0x0C18);
        pr_debug("6328_efuse : efusevalue[%d]=0x%x\n",j, efusevalue[j]);
	...
	pr_debug("read_6328_efuse: g_o_vts        = %x\n", g_o_vts);
    pr_debug("read_6328_efuse: g_degc_cali    = %x\n", g_degc_cali);
    pr_debug("read_6328_efuse: g_adc_cali_en  = %x\n", g_adc_cali_en);
    pr_debug("read_6328_efuse: g_o_slope      = %x\n", g_o_slope);
    pr_debug("read_6328_efuse: g_o_slope_sign = %x\n", g_o_slope_sign);
    pr_debug("read_6328_efuse: g_id           = %x\n", g_id);


    pr_debug("[read_6328_efuse] Done\n");
...
void pmic_cali_prepare(void)
{
	...
	pr_debug("Power/PMIC_Thermal: g_o_vts        = 0x%x\n", g_o_vts);
    pr_debug("Power/PMIC_Thermal: g_degc_cali    = 0x%x\n", g_degc_cali);
    pr_debug("Power/PMIC_Thermal: g_adc_cali_en  = 0x%x\n", g_adc_cali_en);
    pr_debug("Power/PMIC_Thermal: g_o_slope      = 0x%x\n", g_o_slope);
    pr_debug("Power/PMIC_Thermal: g_o_slope_sign = 0x%x\n", g_o_slope_sign);
    pr_debug("Power/PMIC_Thermal: g_id           = 0x%x\n", g_id);
...
void pmic_cali_prepare2(void)
{
	...
	pr_debug("[Thermal calibration] SLOPE1=%d SLOPE2=%d INTERCEPT=%d, Vbe = %d\n",
...
static int mtktspmic_get_hw_temp(void)
{
	...
	//pr_debug("[mtktspmic_get_hw_temp]Raw=%d, T=%d, 0xeb0=%x,temp2=%d\n",temp, temp1,temp2,upmu_get_reg_value(0xeb0));
...
static ssize_t mtktspmic_write_log(struct file *file, const char __user *buffer, size_t count, loff_t *data)
{
	...
	{
		pr_debug("mtktspmic_write_log bad argument\n");
	}
	return -EINVAL;
...
/*
    ...
    if((pmic_data>>4&0x1)!=1 || (pmic_data>>2&0x1)!=0)
        pr_debug("[mtktspmic_init]: Warrning !!! Need to checking this !!!!!\n");
*/
	pmic_cali_prepare();
	pmic_cali_prepare2();
