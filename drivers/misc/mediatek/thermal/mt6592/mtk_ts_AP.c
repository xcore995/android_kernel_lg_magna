...
do {                                    \
	if (mtkts_AP_debug_log) {                \
		pr_debug("Power/AP_Thermal" fmt, ##args); \
	}                                   \
} while(0)
...
//	int i=0;
	...
	if (t_ret > 60000) /* abnormal high temp*/
		pr_debug("[Power/AP_Thermal] T_AP=%d\n", t_ret);
				 ...
				 unsigned long state)
{
	...
	{
		pr_debug("Power/AP_Thermal: reset, reset, reset!!!");
		pr_debug("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@");
		pr_debug("*****************************************");
		pr_debug("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@");
...
static ssize_t mtkts_AP_param_write(struct file *file, const char __user *buffer, size_t count, loff_t *data)
{
    ...
    if (sscanf(desc, "%s %d %s %d %s %d %s %d %d",pull_R, &valR, pull_V, &valV, overcrilow, &over_cri_low,NTC_TABLE,&ntc_table,&adc_channel) >= 8)
	{
        ...
        if (!strcmp(pull_R, "PUP_R")) {
			...
			pr_debug("[mtkts_AP_write] bad PUP_R argument\n");
            return -EINVAL;
        }
        ...
        if (!strcmp(pull_V, "PUP_VOLT")) {
			...
			pr_debug("[mtkts_AP_write] bad PUP_VOLT argument\n");
            return -EINVAL;
        }
        ...
        if (!strcmp(overcrilow, "OVER_CRITICAL_L")) {
			...
			pr_debug("[mtkts_AP_write] bad OVERCRIT_L argument\n");
            return -EINVAL;
        }
        ...
        if (!strcmp(NTC_TABLE, "NTC_TABLE")) {
			...
			pr_debug("[mtkts_AP_write] bad NTC_TABLE argument\n");
            return -EINVAL;
        }
