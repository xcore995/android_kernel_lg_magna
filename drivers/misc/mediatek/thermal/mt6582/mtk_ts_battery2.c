...
static int get_hw_battery2_temp(void)
{
	...
	while (i--)
	{
		ret_value = IMM_GetOneChannelValue(Channel, data, &ret_temp);
		ret += ret_temp;
		pr_debug("[thermal_auxadc_get_data(ADCIN %d)]: ret_temp=%d\n", Channel, ret_temp);
	}
				 ...
				 unsigned long state)
{
	...
	{
		pr_debug("Power/battery2_Thermal: reset, reset, reset!!!");
		pr_debug("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@");
		pr_debug("*****************************************");
		pr_debug("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@");
...
static ssize_t mtktsbattery2_param_write(struct file *file, const char *buffer, unsigned long count, void *data)
{
	...
	if (sscanf(desc, "%s %d %s %d %s %d %s %d",pull_R, &valR, pull_V, &valV, overcrilow, &over_cri_low,NTC_TABLE,&ntc_table ) == 8)
	{
        ...
        if (!strcmp(pull_R, "PUP_R")) {
			...
			pr_debug("[mtktsbattery2_write] bad PUP_R argument\n");
            return -EINVAL;
        }
        ...
        if (!strcmp(pull_V, "PUP_VOLT")) {
			...
			pr_debug("[mtktsbattery2_write] bad PUP_VOLT argument\n");
            return -EINVAL;
        }
        ...
        if (!strcmp(overcrilow, "OVER_CRITICAL_L")) {
			...
			pr_debug("[mtktsbattery2_write] bad OVERCRIT_L argument\n");
            return -EINVAL;
        }
        ...
        if (!strcmp(NTC_TABLE, "NTC_TABLE")) {
			...
			pr_debug("[mtktsbattery2_write] bad NTC_TABLE argument\n");
            return -EINVAL;
        }
	...
	{
		pr_debug("[mtktsbattery2_write] bad argument\n");
	}
