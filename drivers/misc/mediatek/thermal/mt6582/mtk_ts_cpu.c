...
#if 0 //debug
{
	...
	if (cunt == 0)
		pr_debug("[Power/CPU_Thermal]0 hw_protect temp=%d, %d\n", temp1, __LINE__);
	...
	if (cunt == 0)
		pr_debug("[Power/CPU_Thermal]1 hw_protect temp=%d, %d\n", temp1, __LINE__);
	while (temp1 <= 3000 &&  cunt < 100) {
		cunt++;
		pr_debug("[Power/CPU_Thermal]1 hw_protect temp=%d, %d\n", temp1, __LINE__);
		temp1 = DRV_Reg32(TEMPMSR1) & 0x0fff;
	...
	if (cunt == 0)
		pr_debug("[Power/CPU_Thermal]2 hw_protect temp=%d, %d\n", temp1, __LINE__);
	while (temp1 <= 3000 &&  cunt < 100) {
		cunt++;
		pr_debug("[Power/CPU_Thermal]2 hw_protect temp=%d, %d\n", temp1, __LINE__);
		temp1 = DRV_Reg32(TEMPMSR2) & 0x0fff;
...
static void set_thermal_ctrl_trigger_SPM(int temperature)
{
	...
	//mtktscpu_dprintk("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@\n");
	pr_warn("[Set_thermal_ctrl_trigger_SPM]: temperature=%d\n", temperature);
	//mtktscpu_dprintk("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@\n");
...
int mtk_cpufreq_register(struct mtk_cpu_power_info *freqs, int num)
{
	...
	for (i=0; i<num; i++)
	{
		...
		pr_warn("[Power/CPU_Thermal] freqs[%d].cpufreq_khz=%u, .cpufreq_ncpu=%u, .cpufreq_power=%u\n",
...
{
	int i=0;
	pr_debug("[Power/CPU_Thermal] setup gpu power table\n");
	...
	for (i=0; i<num; i++)
	{
		...
		pr_debug("[Power/CPU_Thermal] freqs[%d].cpufreq_khz=%u, .cpufreq_power=%u\n",
...
static void thermal_cal_prepare(void)
{
	...
	{
		pr_warn("This sample is not Thermal calibrated\n");
		g_adc_ge_t = 512;
		g_adc_oe_t = 512;
		g_degc_cali = 40;
		g_o_slope = 0;
	...
	/*pr_debug("[Power/CPU_Thermal] [Thermal calibration] g_adc_ge_t = 0x%x, g_adc_oe_t = 0x%x,
		g_degc_cali = 0x%x, g_adc_cali_en_t = 0x%x, g_o_slope = 0x%x, g_o_slope_sign = 0x%x,
		g_id = 0x%x\n",		g_adc_ge_t, g_adc_oe_t, g_degc_cali, g_adc_cali_en_t,
		g_o_slope, g_o_slope_sign, g_id);*/
	/*pr_debug("[Power/CPU_Thermal] [Thermal calibration] g_o_vtsmcu1 = 0x%x, g_o_vtsmcu2 = 0x%x,
...
static void thermal_cal_prepare_2(kal_uint32 ret)
{
	...
	/*pr_debug("[Power/CPU_Thermal] [Thermal calibration] g_ge = %d, g_oe = %d,
		g_gain = %d, g_x_roomt1 = %d, g_x_roomt2 = %d, g_x_roomtabb = %d\n",*/
							...
							unsigned long *t)
{
	...
	if((curr_temp > (trip_temp[0] - 15000)) || (curr_temp<-30000))
		pr_warn("[Power/CPU_Thermal] CPU T=%d\n", curr_temp);
						...
						struct thermal_cooling_device *cdev)
{
	...
	if (mtk_thermal_zone_bind_cooling_device(thermal, table_val, cdev)) {
		pr_debug("[mtktscpu_bind] error binding cooling dev\n");
		return -EINVAL;
	} else {
		pr_debug("[mtktscpu_bind] binding OK, %d\n", table_val);
	}
                           ...
                           struct thermal_cooling_device *cdev)
{
	...
	if (thermal_zone_unbind_cooling_device(thermal, table_val, cdev)) {
		pr_debug("[mtktscpu_unbind] error unbinding cooling dev\n");
	return -EINVAL;
	} else {
		pr_debug("[mtktscpu_unbind] unbinding OK\n");
	}
				       ...
				       loff_t *data)
{
	...
	{
		pr_debug("[Power/CPU_Thermal][mtktscpu_write_dtm_setting] input %d %d %d %d %d %d %d %d %d\n",
	...
	pr_debug("[Power/CPU_Thermal][mtktscpu_write_dtm_setting] applied %d %d %d %d %d %d %d %d\n",
		i_id, FIRST_STEP_TOTAL_POWER_BUDGETS[i_id], PACKAGE_THETA_JA_RISES[i_id],
		PACKAGE_THETA_JA_FALLS[i_id], MINIMUM_BUDGET_CHANGES[i_id], MINIMUM_CPU_POWERS[i_id],
		MAXIMUM_CPU_POWERS[i_id], MINIMUM_GPU_POWERS[i_id], MAXIMUM_GPU_POWERS[i_id]);
        }
        else
        {
...
/*		
		...
		if(i==0)
		{
			pr_debug("Power/CPU_Thermal: [mtktscpu_write] setting error");
		}
		else if(i==num_trip)
			set_high_low_threshold(trip_temp[i-1], 10000);
		else
