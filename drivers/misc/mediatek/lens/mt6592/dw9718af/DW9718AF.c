...
//LGE_UPDATE_S [seongjo.kim@lge.com] 2015/06/18, Check Flag Bit before sending DAC code
#if 0 // MTK Original
	if (i4RetValue != 2) {
#else
	if (i4RetValue != 1) {
#endif
//LGE_UPDATE_E [seongjo.kim@lge.com] 2015/06/18, Check Flag Bit before sending DAC code
...
//LGE_UPDATE_S [seongjo.kim@lge.com] 2015/06/18, Check Flag Bit before sending DAC code
static int s4DW9718AF_ReadReg_FlagBit(unsigned short * a_pu2Result)
{
    *a_pu2Result = (read_data(0x10)&0x01);

    //DW9718AFDB("[DW9718AF][CHECK_BUSY]  s4DW9718AF_ReadReg %d \n",  *a_pu2Result);
    return 0;
}

static int s4DW9718AF_CheckFlagBit()
{
    unsigned short move_Flag;
    int check_count = 0;
    while (1)
    {
        s4DW9718AF_ReadReg_FlagBit(&move_Flag);
        if (move_Flag == 0)
        {
            //DW9718AFDB("[DW9718AF] CHECK_BUSY [%d] ***NOT SLEEP(BREAK)**** check_count(%d)\n", move_Flag, check_count);
            break;
        }
        else
        {
            DW9718AFDB("[DW9718AF] CHECK_BUSY [%d] ***SLEEP**** check_count(%d)\n", move_Flag, check_count);
            msleep(10);
        }
        check_count++;
        if (check_count > 3)
        {
            DW9718AFDB("[DW9718AF] CHECK_BUSY [%d] check_count(%d) (break by check_count)\n", move_Flag, check_count);
            break;
        }
    }
    return 0;
...
inline static int moveDW9718AF(unsigned long a_u4Position)
{
    ...
    //DW9718AFDB("[DW9718AF] move [curr] %d [target] %d\n", g_u4CurrPosition, g_u4TargetPosition);
            ...
            g_i4MotorStatus = 0;
            spin_unlock(&g_DW9718AF_SpinLock);	
            s4DW9718AF_CheckFlagBit(); //LGE_UPDATE [seongjo.kim@lge.com] 2015/06/18, Check Flag Bit before sending DAC code
