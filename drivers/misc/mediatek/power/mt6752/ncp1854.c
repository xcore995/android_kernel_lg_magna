...
kal_uint32 ncp1854_read_interface (kal_uint8 RegNum, kal_uint8 *val, kal_uint8 MASK, kal_uint8 SHIFT)
{
	...
	battery_log(BAT_LOG_CRTI, "[ncp1854_read_interface] Reg[%x]=0x%x\n", RegNum, ncp1854_reg);

    ncp1854_reg &= (MASK << SHIFT);
    *val = (ncp1854_reg >> SHIFT);
	battery_log(BAT_LOG_CRTI, "[ncp1854_read_interface] Val=0x%x\n", *val);
...
kal_uint32 ncp1854_config_interface (kal_uint8 RegNum, kal_uint8 val, kal_uint8 MASK, kal_uint8 SHIFT)
{
    ...
    ret = ncp1854_read_byte(RegNum, &ncp1854_reg);
    /*battery_log(BAT_LOG_CRTI, "[ncp1854_config_interface] Reg[%x]=0x%x\n", RegNum, ncp1854_reg));*/

    ncp1854_reg &= ~(MASK << SHIFT);
    ncp1854_reg |= (val << SHIFT);

    ret = ncp1854_write_byte(RegNum, ncp1854_reg);
    /*battery_log(BAT_LOG_CRTI, "[ncp18546_config_interface] Write Reg[%x]=0x%x\n", RegNum, ncp1854_reg));*/

    // Check
    //ncp1854_read_byte(RegNum, &ncp1854_reg);
    /*battery_log(BAT_LOG_CRTI, "[ncp1854_config_interface] Check Reg[%x]=0x%x\n", RegNum, ncp1854_reg));*/
...
{
    ncp1854_read_byte(i, &ncp1854_reg[i]);
	battery_log(BAT_LOG_CRTI, "[ncp1854_read_register] Reg[0x%X]=0x%X\n", i, ncp1854_reg[i]);
}
...
static ssize_t store_ncp1854_access(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
    ...
    if(buf != NULL && size != 0)
    {
		battery_log(BAT_LOG_CRTI, "[store_ncp1854_access] buf is %s and size is %zu\n", buf, size);
        reg_address = simple_strtoul(buf,&pvalue,16);

        if(size > 3)
        {
            reg_value = simple_strtoul((pvalue+1),NULL,16);
			battery_log(BAT_LOG_CRTI, "[store_ncp1854_access] write ncp1854 reg 0x%x with value 0x%x !\n", reg_address, reg_value);
        ...
        else
        {
			...
			battery_log(BAT_LOG_CRTI, "[store_ncp1854_access] read ncp1854 reg 0x%x with value 0x%x !\n", reg_address, g_reg_value_ncp1854);
			battery_log(BAT_LOG_CRTI, "[store_ncp1854_access] Please use \"cat ncp1854_access\" to get value\r\n");
        }
    }
    return size;
}
