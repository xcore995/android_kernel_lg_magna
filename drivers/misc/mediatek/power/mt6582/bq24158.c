...
kal_uint32 bq24158_config_interface (kal_uint8 RegNum, kal_uint8 val, kal_uint8 MASK, kal_uint8 SHIFT)
{
	...
	/*battery_log(BAT_LOG_CRTI, "[bq24158_config_interface] Check Reg[%x]=0x%x\n", RegNum, bq24158_reg);*/
...
{
    int i=0;
	battery_log(BAT_LOG_CRTI, "[bq24158] ");
    for (i=0;i<bq24158_REG_NUM;i++)
    {
        bq24158_read_byte(i, &bq24158_reg[i]);
		battery_log(BAT_LOG_CRTI, "[0x%x]=0x%x ", i, bq24158_reg[i]);
    }
...
void bq24158_hw_init(void)
{
    if(g_enable_high_vbat_spec == 1)
    {
        ...
        {
			battery_log(BAT_LOG_CRTI, "[bq24158_hw_init] (0x06,0x70) because 0x1020\n");
            bq24158_config_interface_liao(0x06,0x70); // set ISAFE
        }
        else
        {
			battery_log(BAT_LOG_CRTI, "[bq24158_hw_init] (0x06,0x77)\n");
            bq24158_config_interface_liao(0x06,0x77); // set ISAFE and HW CV point (4.34)
        }
    }
    else
    {
		battery_log(BAT_LOG_CRTI, "[bq24158_hw_init] (0x06,0x70) \n");
        bq24158_config_interface_liao(0x06,0x70); // set ISAFE
    }
}
#endif
