...
static int max8971_write_reg(struct i2c_client *client, u8 reg, u8 value)
{
	...
	while (i--) {
        ...
        if (ret < 0)
				battery_log(BAT_LOG_CRTI, "***** max8971_write_reg error *****\n");
		else
		break;
	}
...
static int max8971_read_reg(struct i2c_client *client, u8 reg)
{
	...
	while(i--){
	 ...
	 if (ret < 0)
				battery_log(BAT_LOG_CRTI, "***** max8971_read_reg error *****\n");
		else
		break;
...
{
    u8  reg_val= 0;
	battery_log(BAT_LOG_CRTI, "[Max8971] set charger called\n");
...
//	 D("  __set_charger\n");
	...
	battery_log(BAT_LOG_CRTI,  "[LGE_MAX8971] MAX8971_REG_PROTCMD[0x%x] = 0x%x\n", MAX8971_REG_PROTCMD, reg_val);
	...
	if (enable) {
		...
		battery_log(BAT_LOG_CRTI,  "[LGE_MAX8971] MAX8971_REG_FCHGCRNT[0x%x] = 0x%x\n", MAX8971_REG_FCHGCRNT, reg_val);
       ...
       // Set input current limit and charger restart threshold
		...
		battery_log(BAT_LOG_CRTI,  "[LGE_MAX8971] MAX8971_REG_DCCRNT[0x%x] = 0x%x\n", MAX8971_REG_DCCRNT, reg_val);

	if( MST_test_mode == 1){
		battery_log(BAT_LOG_CRTI, "[Max8971] MST_test_mode set chgcv 4.1V");
		...
		battery_log(BAT_LOG_CRTI, "[LGE_MAX8971] MAX8971_REG_TOPOFF[0x%x] = 0x%x\n", MAX8971_REG_TOPOFF, reg_val);
	}else{
        // Set topoff condition
        reg_val = ((chip->pdata->topofftime<<MAX8971_TOPOFFTIME_SHIFT) |
                   (chip->pdata->topofftshld<<MAX8971_TOPOFFTSHLD_SHIFT) |
		...
		battery_log(BAT_LOG_CRTI,  "[LGE_MAX8971] MAX8971_REG_TEMPREG[0x%x] = 0x%x\n", MAX8971_REG_TEMPREG, reg_val);
...
//        max8971_write_reg(chip->client, MAX8971_REG_CHGCNTL1, reg_val);
	...
	battery_log(BAT_LOG_CRTI, "[max8971]!!!!!!!!!!!!!!!! max8971 reg value debug start!!!!!!!!!!!!!!!!!!!!!!!!\n");
	reg_val=0;
	reg_val = max8971_read_reg(chip->client, MAX8971_REG_CHG_STAT);
	battery_log(BAT_LOG_CRTI, "[max8971] max8971_charger MAX8971_REG_CHG_STAT[0x%x] = 0x%x\n",
		MAX8971_REG_CHG_STAT, reg_val);
	reg_val = max8971_read_reg(chip->client, MAX8971_REG_DETAILS1);
	battery_log(BAT_LOG_CRTI, "[max8971] max8971_charger MAX8971_REG_DETAILS1[0x%x] = 0x%x\n",
		MAX8971_REG_DETAILS1, reg_val);
	reg_val = max8971_read_reg(chip->client, MAX8971_REG_DETAILS2);
	battery_log(BAT_LOG_CRTI, "[max8971] max8971_charger MAX8971_REG_DETAILS2[0x%x] = 0x%x\n",
		MAX8971_REG_DETAILS2, reg_val);
	reg_val = max8971_read_reg(chip->client, MAX8971_REG_CHGCNTL1);
	battery_log(BAT_LOG_CRTI, "[max8971] max8971_charger MAX8971_REG_CHGCNTL1 = 0x%x\n", reg_val);
	reg_val = max8971_read_reg(chip->client, MAX8971_REG_FCHGCRNT);
	battery_log(BAT_LOG_CRTI, "[max8971] max8971_charger MAX8971_REG_FCHGCRNT = 0x%x\n", reg_val);
	reg_val = max8971_read_reg(chip->client, MAX8971_REG_DCCRNT);
	battery_log(BAT_LOG_CRTI, "[max8971] max8971_charger MAX8971_REG_DCCRNT = 0x%x\n", reg_val);
	reg_val = max8971_read_reg(chip->client, MAX8971_REG_TOPOFF);
	battery_log(BAT_LOG_CRTI, "[max8971] max8971_charger MAX8971_REG_TOPOFF = 0x%x\n", reg_val);
	reg_val = max8971_read_reg(chip->client, MAX8971_REG_TEMPREG);
	battery_log(BAT_LOG_CRTI, "[max8971] max8971_charger MAX8971_REG_TEMPREG = 0x%x\n", reg_val);
	reg_val = max8971_read_reg(chip->client, MAX8971_REG_PROTCMD);
	battery_log(BAT_LOG_CRTI, "[max8971] max8971_charger MAX8971_REG_PROTCMD = 0x%x\n", reg_val);
	battery_log(BAT_LOG_CRTI, "[max8971]!!!!!!!!!!!!!!!! max8971 reg value debug end!!!!!!!!!!!!!!!!!!!!!!!!\n");
...
int max8971_check_eoc_status(void)
{
	...
	battery_log(BAT_LOG_CRTI, "[LGE_MAX8971] eoc state!!! MAX8971_REG_DETAILS2[0x%x] = 0x%x\n", MAX8971_REG_DETAILS2, eoc_status);

    if( eoc_status == MAX8971_CHG_DTLS_DONE )
    {
		battery_log(BAT_LOG_CRTI, "[LGE_MAX8971] max8971_check_eoc_status = %d\n", eoc_status);
        return 1;
    }
...
int max8971_start_charging(unsigned mA)
{
	...
	battery_log(BAT_LOG_CRTI, "[LGE_MAX8971] max8971_start_charging (%d)mA\n", mA);
...
//    max8971_write_reg(max8971_chg->client, MAX8971_REG_FCHGCRNT, reg_val);
    ...
    {
		battery_log(BAT_LOG_CRTI,  "[LGE_MAX8971]  600mA ");
        reg_val = (0<<MAX8971_DCMON_DIS_SHIFT) | (0<<MAX8971_USB_SUS_SHIFT);
        max8971_write_reg(max8971_chg->client, MAX8971_REG_CHGCNTL1, reg_val);
        dev_info(&max8971_chg->client->dev, "[LGE_MAX8971] MAX8971_REG_CHGCNTL1[0x%x] = 0x%x\n", MAX8971_REG_CHGCNTL1,reg_val);
    }
...
//	charging_ic_status = POWER_SUPPLY_TYPE_USB ;
	...
	battery_log(BAT_LOG_CRTI, "[max8971_start_chargin] can not find current\n");
   ...
   // reg_val = max8971_chg->pdata->int_mask=0xF3 ;
	...
	battery_log(BAT_LOG_CRTI, "[max8971] max8971_chg->pdata->chgcc: 0x%x\n", max8971_chg->pdata->chgcc);
...
//                   (max8971_chg->pdata->fchgtime<<MAX8971_FCHGTIME_SHIFT));
  //      max8971_write_reg(max8971_chg->client, MAX8971_REG_FCHGCRNT, reg_val);
		/*battery_log(BAT_LOG_CRTI, "[max8971] max8971_chg->pdata->dcilmt: %d\n",i);*/
		battery_log(BAT_LOG_CRTI,   "[LGE_MAX8971] MAX8971_REG_CHGCNTL1[0x%x] = 0x%x\n", MAX8971_REG_DCCRNT, reg_val);
        udelay(200);
    }
    reg_val = ((max8971_chg->pdata->chgrstrt<<MAX8971_CHGRSTRT_SHIFT) |
...
static int max8971_charger_detail_irq(int irq, void *data, u8 *val)
{
	...
	battery_log(BAT_LOG_CRTI, "[max8971] max8971_charger_detail_irq called\n");
    switch (irq)
    {
...
//    D("  max8971_charger_handler\n");
	battery_log(BAT_LOG_CRTI, "[max8971] max8971_charger_handler called ");
...
//    max8971_write_reg(max8971_chg->client,MAX8971_REG_CHGINT_MASK , reg_val);
    ...
    if(max8971_chg != NULL )
    {
	battery_log(BAT_LOG_CRTI, " max8971_chg pointer is not NULL\n");
     	max8971_chg->chg_online = 0;
    	__set_charger(max8971_chg, 0);
    }
    else
	battery_log(BAT_LOG_CRTI, " max8971_chg pointer is NULL\n");
...
//	charging_ic_status = POWER_SUPPLY_TYPE_USB ;
    ...
    for (i=250; i<=450; i+=50)
    {
        ...
        reg_val = ((FCHG_CURRENT(450)<<MAX8971_CHGCC_SHIFT) |
                   (max8971_chg->pdata->fchgtime<<MAX8971_FCHGTIME_SHIFT));
        max8971_write_reg(max8971_chg->client, MAX8971_REG_FCHGCRNT, reg_val);
	battery_log(BAT_LOG_CRTI, "[max8971] max8971_chg->pdata->chgcc: %d\n", i);
        udelay(200);
    }
...
int max8971_start_Factory_charging(void)
{
	...
	battery_log(BAT_LOG_CRTI, "[max8971] max8971_start_Factory_charging\n");
#ifdef _TEST_
    charging_ic_status = POWER_SUPPLY_TYPE_FACTORY;
#endif /* _TEST_ */
                                        ...
                                        union power_supply_propval *val)
{
	...
	int ret = 0;
    	int chg_dtls_val;
	battery_log(BAT_LOG_CRTI, " max8971_charger_get_property: psp= %d\n", psp);
...
{
		battery_log(BAT_LOG_CRTI, " charging_ic_status: %d\n", charging_ic_status);
	...
	switch (muic_mode)
	{
		...
		case MUIC_AP_USB :
			if(get_unlimited_temp() ==  UNLIMITED_TEMP_VAL){
				MST_test_mode = 1;
				battery_log(BAT_LOG_CRTI, "%s: AP_USB.\n", __func__);
		...
		case MUIC_CP_USB :
			battery_log(BAT_LOG_CRTI, "%s: CP_USB.\n", __func__);
			max8971_start_Factory_charging();
			break;

		case MUIC_CP_UART :
			battery_log(BAT_LOG_CRTI, "%s: CP_UART.\n", __func__);
			max8971_start_Factory_charging();
			break;

		case MUIC_MHL :
			battery_log(BAT_LOG_CRTI, "%s: MHL.\n", __func__);
		...
		case MUIC_NONE :
			MST_test_mode=0;
			charger_source=0;
			battery_log(BAT_LOG_CRTI, "%s: NONE.\n", __func__);
			max8971_stop_charging();
			break;

		default :
			battery_log(BAT_LOG_CRTI, "%s: No charging.\n", __func__);
			break;
	}
			...
			const struct i2c_device_id *id)
{
	...
	battery_log(BAT_LOG_CRTI, " max8971_probe called\n");
	...
	battery_log(BAT_LOG_CRTI, "GPIO_SCL[88] Mode:%d\nGPIO_SDA[89] Mode:%d\n"
...
*/
    /* LGE_CHANGE_E [wonhui.lee@lge.com] 2011-07-29, Add for AT%CHARGE*/
		...
		battery_log(BAT_LOG_CRTI, " MAX8971_REG_CHGINT : %d\n", ret);
	chip->chg_online = 0;
...
{
	int ret;
	battery_log(BAT_LOG_CRTI, "max8971_init called\n");
	ret = i2c_register_board_info(2, &max8971_i2c_data, 1);
	if(ret)
    {
		battery_log(BAT_LOG_CRTI, "failed to i2c register driver. (%d)\n", ret);
        return ret;
    } else {
		battery_log(BAT_LOG_CRTI, "success to i2c register driver. (%d)\n", ret);
    }
