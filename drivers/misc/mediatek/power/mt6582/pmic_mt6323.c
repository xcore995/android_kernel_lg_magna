...
{
	battery_log(BAT_LOG_CRTI, "enable_pll is not ready.\n");
}
void pmic_disable_pll(int id, char *mod_name)
{
	battery_log(BAT_LOG_CRTI, "disable_pll is not ready.\n");
}
//----------------------------------------------------------------------
...
kal_uint32 upmu_get_reg_value(kal_uint32 reg)
{
	...
	/*battery_log(BAT_LOG_CRTI, "[upmu_get_reg_value]\n");*/
...
void upmu_set_reg_value(kal_uint32 reg, kal_uint32 reg_val)
{
	...
	/*battery_log(BAT_LOG_CRTI, "[upmu_set_reg_value]\n");*/
...
void PMIC_IMM_PollingAuxadcChannel(void)
{
	...
	/*battery_log(BAT_LOG_CRTI, "[PMIC_IMM_PollingAuxadcChannel] before:%d ",upmu_get_rg_adc_deci_gdly());*/
	...
	/*battery_log(BAT_LOG_CRTI, "[PMIC_IMM_PollingAuxadcChannel] after:%d ",upmu_get_rg_adc_deci_gdly());*/
}
	...
	do
	{
	    ...
	    switch(dwChannel){
			...
			if( (count++) > count_time_out)
			{
				...
				battery_log(BAT_LOG_CRTI,  "[upmu_get_rg_adc_out_ch6] 0x%x\n", ret_data);
	            break;
	        case 2:
...
int pmic_get_buck_current(int avg_times)
{
	...
	battery_log(BAT_LOG_CRTI,  "[pmic_get_buck_current] raw_data=%d, val=%d, avg_times=%d\n",
...
{
	battery_log(BAT_LOG_CRTI,  "[upmu_interrupt_chrdet_int_en] val=%d.\r\n", val);
...
{
	battery_log(BAT_LOG_CRTI,  "[wake_up_pmic]\r\n");
...
void spkl_ab_int_handler(void)
{
	...
	battery_log(BAT_LOG_CRTI,  "[spkl_ab_int_handler]....\n");
...
void spkl_d_int_handler(void)
{
	...
	battery_log(BAT_LOG_CRTI,  "[spkl_d_int_handler]....\n");
...
void bat_l_int_handler(void)
{
	...
	battery_log(BAT_LOG_CRTI,  "[bat_l_int_handler]....\n");
...
void bat_h_int_handler(void)
{
	...
	battery_log(BAT_LOG_CRTI,  "[bat_h_int_handler]....\n");
...
void watchdog_int_handler(void)
{
	...
	battery_log(BAT_LOG_CRTI,  "[watchdog_int_handler]....\n");
...
void pwrkey_int_handler(void)
{
	...
	battery_log(BAT_LOG_CRTI,  "[pwrkey_int_handler]....\n");

		if(upmu_get_pwrkey_deb()==1)
    {
		battery_log(BAT_LOG_CRTI,  "[pwrkey_int_handler] Release pwrkey\n");
#if defined (CONFIG_MTK_KERNEL_POWER_OFF_CHARGING)
		if(g_boot_mode == KERNEL_POWER_OFF_CHARGING_BOOT && timer_pre != 0)
		{
				...
				battery_log(BAT_LOG_CRTI,
					"[pmic_thread_kthread] timer_pos = %ld, timer_pre = %ld,\
					imer_pos-timer_pre = %ld, long_pwrkey_press = %d\r\n",
					timer_pos, timer_pre, timer_pos-timer_pre, long_pwrkey_press);
				if(long_pwrkey_press)   //500ms
				{
					battery_log(BAT_LOG_CRTI,
						"[pmic_thread_kthread] Power Key Pressed during kernel power \
						off charging,reboot OS\r\n");
					arch_reset(0, NULL);
				}
		}
#endif
    ...
    {
		battery_log(BAT_LOG_CRTI,  "[pwrkey_int_handler] Press pwrkey\n");
#if defined (CONFIG_MTK_KERNEL_POWER_OFF_CHARGING)
...
void thr_l_int_handler(void)
{
	...
	battery_log(BAT_LOG_CRTI,  "[thr_l_int_handler]....\n");
...
void thr_h_int_handler(void)
{
	...
	battery_log(BAT_LOG_CRTI,  "[thr_h_int_handler]....\n");
...
void vbaton_undet_int_handler(void)
{
	...
	battery_log(BAT_LOG_CRTI,  "[vbaton_undet_int_handler]....\n");
...
void bvalid_det_int_handler(void)
{
	...
	battery_log(BAT_LOG_CRTI,  "[bvalid_det_int_handler]....\n");
...
void chrdet_int_handler(void)
{
	...
	battery_log(BAT_LOG_CRTI,  "[chrdet_int_handler]....\n");
...
void vbat_ov_int_handler(void)
{
	...
	battery_log(BAT_LOG_CRTI,  "[vbat_ov_int_handler]....\n");
...
void ldo_oc_int_handler(void)
{
	...
	battery_log(BAT_LOG_CRTI,  "[ldo_oc_int_handler]....\n");

	battery_log(BAT_LOG_CRTI,  "[PMIC] Reg[0x%x]=0x%x\n", OCSTATUS0, upmu_get_reg_value(OCSTATUS0));
...
void fchr_key_int_handler(void)
{
	...
	battery_log(BAT_LOG_CRTI,  "[fchr_key_int_handler]....\n");

    if(upmu_get_fchrkey_deb()==1)
    {
		battery_log(BAT_LOG_CRTI,  "[fchr_key_int_handler] Release fchrkey\n");
        kpd_pmic_rstkey_handler(0x0);
        upmu_set_rg_fchrkey_int_sel(0);
    }
    else
    {
		battery_log(BAT_LOG_CRTI,  "[fchr_key_int_handler] Press fchrkey\n");
...
void accdet_int_handler(void)
{
	...
	battery_log(BAT_LOG_CRTI,  "[accdet_int_handler]....\n");

    ret = accdet_irq_handler();
    if(0 == ret){
		battery_log(BAT_LOG_CRTI,  "[accdet_int_handler] don't finished\n");
    }
...
void audio_int_handler(void)
{
	...
	battery_log(BAT_LOG_CRTI,  "[audio_int_handler]....\n");
...
void rtc_int_handler(void)
{
	...
	battery_log(BAT_LOG_CRTI,  "[rtc_int_handler]....\n");
...
void vproc_oc_int_handler(void)
{
	...
	battery_log(BAT_LOG_CRTI,  "[vproc_oc_int_handler]....\n");
...
void vsys_oc_int_handler(void)
{
	...
	battery_log(BAT_LOG_CRTI,  "[vsys_oc_int_handler]....\n");
...
void vpa_oc_int_handler(void)
{
	...
	battery_log(BAT_LOG_CRTI,  "[vpa_oc_int_handler]....\n");
...
static int pmic_thread_kthread(void *x)
{
	...
	battery_log(BAT_LOG_CRTI,  "[pmic_thread_kthread] enter\n");

    /* Run on a process content */
    while (1) {
        mutex_lock(&pmic_mutex);

		battery_log(BAT_LOG_CRTI,  "[pmic_thread_kthread] running\n");
        ...
        ret=pmic_read_interface(INT_STATUS0,(&int_status_val_0),0xFFFF,0x0);
		battery_log(BAT_LOG_CRTI,  "[INT] after ,int_status_val_0=0x%x\n", int_status_val_0);

        ret=pmic_read_interface(INT_STATUS1,(&int_status_val_1),0xFFFF,0x0);
		battery_log(BAT_LOG_CRTI,  "[INT] after ,int_status_val_1=0x%x\n", int_status_val_1);
...
{
	battery_log(BAT_LOG_CRTI,  "[mt6323_pmic_eint_irq] receive interrupt\n");
...
{
	battery_log(BAT_LOG_CRTI,  "[PMIC_EINT_SETTING] start: CUST_EINT_MT6323_PMIC_NUM=%d\n",
	...
	/*battery_log(BAT_LOG_CRTI, "[CUST_EINT] GPIO37=GPIO_MODE_01 for EINT3 usage\n");*/
	...
	battery_log(BAT_LOG_CRTI,  "[CUST_EINT] CUST_EINT_MT6323_PMIC_NUM=%d\n",
		CUST_EINT_MT6323_PMIC_NUM);
	battery_log(BAT_LOG_CRTI,  "[CUST_EINT] CUST_EINT_MT6323_PMIC_DEBOUNCE_CN=%d\n",
		CUST_EINT_MT6323_PMIC_DEBOUNCE_CN);
	/*battery_log(BAT_LOG_CRTI, "[CUST_EINT] CUST_EINT_MT6323_PMIC_POLARITY=%d\n",
	CUST_EINT_MT6323_PMIC_POLARITY);*/
	/*battery_log(BAT_LOG_CRTI, "[CUST_EINT] CUST_EINT_MT6323_PMIC_SENSITIVE=%d\n",
	CUST_EINT_MT6323_PMIC_SENSITIVE);*/
	battery_log(BAT_LOG_CRTI,  "[CUST_EINT] CUST_EINT_MT6323_PMIC_TYPE=%d\n",
	CUST_EINT_MT6323_PMIC_TYPE);
	battery_log(BAT_LOG_CRTI,  "[CUST_EINT] CUST_EINT_MT6323_PMIC_DEBOUNCE_EN=%d\n",
...
U32 pmic_read_interface (U32 RegNum, U32 *val, U32 MASK, U32 SHIFT)
{
    ...
    if(return_value!=0)
    {
		battery_log(BAT_LOG_CRTI,  "[pmic_read_interface] Reg[%x]= pmic_wrap read data fail\n", RegNum);
    ...
    *val = (pmic6323_reg >> SHIFT);
    ...
    mutex_unlock(&pmic_access_mutex);
#else
	battery_log(BAT_LOG_CRTI,  "[pmic_read_interface] Can not access HW PMIC\n");
#endif
...
U32 pmic_config_interface (U32 RegNum, U32 val, U32 MASK, U32 SHIFT)
{
    ...
    if(return_value!=0)
    {
		battery_log(BAT_LOG_CRTI,  "[pmic_config_interface] Reg[%x]= pmic_wrap read data fail\n", RegNum);
        mutex_unlock(&pmic_access_mutex);
        return return_value;
    }
    ...
    if(return_value!=0)
    {
		battery_log(BAT_LOG_CRTI,  "[pmic_config_interface] Reg[%x]= pmic_wrap write data fail\n", RegNum);
        mutex_unlock(&pmic_access_mutex);
        return return_value;
    }
    //pr_debug( "[pmic_config_interface] Reg[%x]=0x%x\n", RegNum, pmic6323_reg);
    ...
    mutex_unlock(&pmic_access_mutex);
#else
	battery_log(BAT_LOG_CRTI,  "[pmic_config_interface] Can not access HW PMIC\n");
#endif
...
U32 pmic_read_interface_nolock (U32 RegNum, U32 *val, U32 MASK, U32 SHIFT)
{
    ...
    if(return_value!=0)
    {
		battery_log(BAT_LOG_CRTI,  "[pmic_read_interface_nolock] Reg[%x]= pmic_wrap read data fail\n",
			RegNum);
        return return_value;
    }
...
#else
	battery_log(BAT_LOG_CRTI,  "[pmic_read_interface_nolock] Can not access HW PMIC\n");
#endif
...
U32 pmic_config_interface_nolock (U32 RegNum, U32 val, U32 MASK, U32 SHIFT)
{
    ...
    if(return_value!=0)
    {
		battery_log(BAT_LOG_CRTI,  "[pmic_config_interface_nolock] Reg[%x]= pmic_wrap read data fail\n",
			RegNum);
        return return_value;
    }
...
#else
	battery_log(BAT_LOG_CRTI,  "[pmic_config_interface_nolock] Can not access HW PMIC\n");
#endif
...
{
	battery_log(BAT_LOG_CRTI,  "[show_pmic_access] 0x%x\n", g_reg_value);
...
static ssize_t store_pmic_access(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
    ...
    U32 reg_value = 0;
    U32 reg_address = 0;
	battery_log(BAT_LOG_CRTI,  "[store_pmic_access]\n");
    if(buf != NULL && size != 0)
    {
		battery_log(BAT_LOG_CRTI,  "[store_pmic_access] buf is %s and size is %zu\n", buf, size);
        reg_address = simple_strtoul(buf,&pvalue,16);

        if(size > 5)
        {
            reg_value = simple_strtoul((pvalue+1),NULL,16);
			battery_log(BAT_LOG_CRTI,  "[store_pmic_access] write PMU reg 0x%x with value 0x%x !\n",
        ...
        else
        {
			...
			battery_log(BAT_LOG_CRTI,  "[store_pmic_access] read PMU reg 0x%x with value 0x%x !\n",
				reg_address, g_reg_value);
			battery_log(BAT_LOG_CRTI,  "[store_pmic_access] Please use \"cat pmic_access\" \
				to get value\r\n");
        }
    }
    return size;
...
{
	battery_log(BAT_LOG_CRTI,  "[dct_pmic_VPROC_enable] %d\n", dctEnable);
...
{
	battery_log(BAT_LOG_CRTI,  "[dct_pmic_VSYS_enable] %d\n", dctEnable);
...
{
	battery_log(BAT_LOG_CRTI,  "[dct_pmic_VPA_enable] %d\n", dctEnable);
...
{
	battery_log(BAT_LOG_CRTI,  "[dct_pmic_VIO28_enable] %d\n", dctEnable);
...
{
	battery_log(BAT_LOG_CRTI,  "[dct_pmic_VUSB_enable] %d\n", dctEnable);
...
{
	battery_log(BAT_LOG_CRTI,  "[dct_pmic_VMC_enable] %d\n", dctEnable);
...
{
	battery_log(BAT_LOG_CRTI,  "[dct_pmic_VMCH_enable] %d\n", dctEnable);
...
{
	battery_log(BAT_LOG_CRTI,  "[dct_pmic_VEMC_3V3_enable] %d\n", dctEnable);
...
{
	battery_log(BAT_LOG_CRTI,  "[dct_pmic_VGP1_enable] %d\n", dctEnable);
...
{
	battery_log(BAT_LOG_CRTI,  "[dct_pmic_VGP2_enable] %d\n", dctEnable);
...
{
	battery_log(BAT_LOG_CRTI,  "[dct_pmic_VGP3_enable] %d\n", dctEnable);
...
{
	battery_log(BAT_LOG_CRTI,  "[dct_pmic_VCN18_enable] %d\n", dctEnable);
...
{
	battery_log(BAT_LOG_CRTI,  "[dct_pmic_VSIM1_enable] %d\n", dctEnable);
...
{
	battery_log(BAT_LOG_CRTI,  "[dct_pmic_VSIM2_enable] %d\n", dctEnable);
...
{
	battery_log(BAT_LOG_CRTI,  "[dct_pmic_VRTC_enable] %d\n", dctEnable);
...
{
	battery_log(BAT_LOG_CRTI,  "[dct_pmic_VCAM_AF_enable] %d\n", dctEnable);
...
{
	battery_log(BAT_LOG_CRTI,  "[dct_pmic_VIBR_enable] %d\n", dctEnable);
...
{
	battery_log(BAT_LOG_CRTI,  "[dct_pmic_VM_enable] %d\n", dctEnable);
...
{
	battery_log(BAT_LOG_CRTI,  "[dct_pmic_VRF18_enable] %d\n", dctEnable);
...
{
	battery_log(BAT_LOG_CRTI,  "[dct_pmic_VIO18_enable] %d\n", dctEnable);
...
{
	battery_log(BAT_LOG_CRTI,  "[dct_pmic_VCAMD_enable] %d\n", dctEnable);
...
{
	battery_log(BAT_LOG_CRTI,  "[dct_pmic_VCAM_IO_enable] %d\n", dctEnable);
...
{
	battery_log(BAT_LOG_CRTI,  "[dct_pmic_VTCXO_enable] %d\n", dctEnable);
...
{
	battery_log(BAT_LOG_CRTI,  "[dct_pmic_VA_enable] %d\n", dctEnable);
...
{
	battery_log(BAT_LOG_CRTI,  "[dct_pmic_VCAMA_enable] %d\n", dctEnable);
...
{
	battery_log(BAT_LOG_CRTI,  "[dct_pmic_VCN28_enable_bt] %d\n", dctEnable);
...
{
	battery_log(BAT_LOG_CRTI,  "[dct_pmic_VCN28_enable_wifi] %d\n", dctEnable);
...
{
	battery_log(BAT_LOG_CRTI,  "[dct_pmic_VCN28_enable] %d\n", dctEnable);
...
{
	battery_log(BAT_LOG_CRTI,  "****[dct_pmic_VMC_sel] value=%d\n", volt);
    ...
    if(volt == VOL_DEFAULT)     {upmu_set_rg_vmc_vosel(1);}
		...
		battery_log(BAT_LOG_CRTI,  "Error Setting %d. DO nothing.\r\n", volt);
    }
}

void dct_pmic_VMCH_sel(kal_uint32 volt)
{
	battery_log(BAT_LOG_CRTI,  "****[dct_pmic_VMCH_sel] value=%d\n", volt);
...
{
	battery_log(BAT_LOG_CRTI,  "****[dct_pmic_VEMC_3V3_sel] value=%d\n", volt);
...
{
	battery_log(BAT_LOG_CRTI,  "****[dct_pmic_VGP1_sel] value=%d\n", volt);
...
{
	battery_log(BAT_LOG_CRTI,  "****[dct_pmic_VGP2_sel] value=%d\n", volt);
...
{
	battery_log(BAT_LOG_CRTI,  "****[dct_pmic_VGP3_sel] value=%d\n", volt);
...
{
	battery_log(BAT_LOG_CRTI,  "****[dct_pmic_VCAM_AF_sel] value=%d\n", volt);
...
{
	battery_log(BAT_LOG_CRTI,  "****[dct_pmic_VSIM1_sel] value=%d\n", volt);
...
{
	battery_log(BAT_LOG_CRTI,  "****[dct_pmic_VSIM2_sel] value=%d\n", volt);
...
{
	battery_log(BAT_LOG_CRTI,  "****[dct_pmic_VIBR_sel] value=%d\n", volt);
...
{
	battery_log(BAT_LOG_CRTI,  "****[dct_pmic_VM_sel] value=%d\n", volt);
...
{
	battery_log(BAT_LOG_CRTI,  "****[dct_pmic_VCAMD_sel] value=%d\n", volt);
...
{
	battery_log(BAT_LOG_CRTI,  "****[dct_pmic_VCAMA_sel] value=%d\n", volt);
...
{
	battery_log(BAT_LOG_CRTI,  "****[dct_pmic_VCN33_sel] value=%d\n", volt);
...
void pmic_ldo_enable(MT65XX_POWER powerId, kal_bool powerEnable)
{
    ...
    {
		battery_log(BAT_LOG_CRTI,  "[pmic_ldo_enable] UnKnown powerId (%d)\n", powerId);
    }
...
void pmic_ldo_vol_sel(MT65XX_POWER powerId, MT65XX_POWER_VOLTAGE powerVolt)
{
	...
	if (powerId == MT6323_POWER_LDO_VIO28)
		battery_log(BAT_LOG_CRTI,  "[dct_pmic_VIO28_enable] No vlotage can setting!\n");
	else if (powerId == MT6323_POWER_LDO_VUSB)
		battery_log(BAT_LOG_CRTI,  "[dct_pmic_VUSB_enable] No vlotage can setting!\n");
	else if (powerId == MT6323_POWER_LDO_VMC)
		dct_pmic_VMC_sel(powerVolt);
	else if (powerId == MT6323_POWER_LDO_VMCH)
		dct_pmic_VMCH_sel(powerVolt);
	else if (powerId == MT6323_POWER_LDO_VEMC_3V3)
		dct_pmic_VEMC_3V3_sel(powerVolt);
	else if (powerId == MT6323_POWER_LDO_VGP1)
		dct_pmic_VGP1_sel(powerVolt);
	else if (powerId == MT6323_POWER_LDO_VGP2)
		dct_pmic_VGP2_sel(powerVolt);
	else if (powerId == MT6323_POWER_LDO_VGP3)
		dct_pmic_VGP3_sel(powerVolt);
	else if (powerId == MT6323_POWER_LDO_VCN_1V8)
		battery_log(BAT_LOG_CRTI,  "[dct_pmic_VCN_1V8_enable] No vlotage can setting!\n");
	else if (powerId == MT6323_POWER_LDO_VSIM1)
		dct_pmic_VSIM1_sel(powerVolt);
	else if (powerId == MT6323_POWER_LDO_VSIM2)
		dct_pmic_VSIM2_sel(powerVolt);
	else if (powerId == MT6323_POWER_LDO_VRTC)
		battery_log(BAT_LOG_CRTI,  "[dct_pmic_VRTC_enable] No vlotage can setting!\n");
	else if (powerId == MT6323_POWER_LDO_VCAM_AF)
		dct_pmic_VCAM_AF_sel(powerVolt);
	else if (powerId == MT6323_POWER_LDO_VIBR)
		dct_pmic_VIBR_sel(powerVolt);
	else if (powerId == MT6323_POWER_LDO_VM)
		dct_pmic_VM_sel(powerVolt);
	else if (powerId == MT6323_POWER_LDO_VRF18)
		battery_log(BAT_LOG_CRTI,  "[dct_pmic_VRF18_enable] No vlotage can setting!\n");
	else if (powerId == MT6323_POWER_LDO_VIO18)
		battery_log(BAT_LOG_CRTI,  "[dct_pmic_VIO18_enable] No vlotage can setting!\n");
	else if (powerId == MT6323_POWER_LDO_VCAMD)
		dct_pmic_VCAMD_sel(powerVolt);
	else if (powerId == MT6323_POWER_LDO_VCAM_IO)
		battery_log(BAT_LOG_CRTI,  "[dct_pmic_VCAM_IO_enable] No vlotage can setting!\n");
	else if (powerId == MT6323_POWER_LDO_VTCXO)
		battery_log(BAT_LOG_CRTI,  "[dct_pmic_VTCXO_enable] No vlotage can setting!\n");
	else if (powerId == MT6323_POWER_LDO_VA)
		battery_log(BAT_LOG_CRTI,  "[dct_pmic_VA_enable] No vlotage can setting!\n");
	else if (powerId == MT6323_POWER_LDO_VCAMA)
		dct_pmic_VCAMA_sel(powerVolt);
	else if (powerId == MT6323_POWER_LDO_VCN33_BT)
		dct_pmic_VCN33_sel(powerVolt);
	else if (powerId == MT6323_POWER_LDO_VCN33_WIFI)
		dct_pmic_VCN33_sel(powerVolt);
	else if (powerId == MT6323_POWER_LDO_VCN28)
		battery_log(BAT_LOG_CRTI,  "[dct_pmic_VCN28_enable] No vlotage can setting!\n");
    else
		battery_log(BAT_LOG_CRTI,  "[pmic_ldo_ldo_vol_sel] UnKnown powerId (%d)\n", powerId);
...
void dump_ldo_status_read_debug(void)
{
	...
	battery_log(BAT_LOG_CRTI, "********** ldo status dump [1:ON,0:OFF]**********\n");

	battery_log(BAT_LOG_CRTI, "VRTC    =%d, ",  (((val_0)&(0x0008))>>3));
	battery_log(BAT_LOG_CRTI, "VA      =%d, ",  (((val_0)&(0x0010))>>4));
	battery_log(BAT_LOG_CRTI, "VCAMA   =%d, ",  (((val_0)&(0x0020))>>5));
	battery_log(BAT_LOG_CRTI, "VCAMD   =%d\n",  (((val_0)&(0x0040))>>6));

	battery_log(BAT_LOG_CRTI, "VCAM_AF =%d, ",  (((val_0)&(0x0080))>>7));
	battery_log(BAT_LOG_CRTI, "VCAM_IO =%d, ",  (((val_0)&(0x0100))>>8));
	battery_log(BAT_LOG_CRTI, "VCN28   =%d, ",  (((val_0)&(0x0200))>>9));
	battery_log(BAT_LOG_CRTI, "VCN33   =%d\n",  (((val_0)&(0x0400))>>10));

	battery_log(BAT_LOG_CRTI, "VCN_1V8 =%d, ",  (((val_0)&(0x0800))>>11));
	battery_log(BAT_LOG_CRTI, "VEMC_3V3=%d, ",  (((val_0)&(0x1000))>>12));
	battery_log(BAT_LOG_CRTI, "VGP1    =%d, ",  (((val_0)&(0x2000))>>13));
	battery_log(BAT_LOG_CRTI, "VGP2    =%d\n",  (((val_0)&(0x4000))>>14));

	battery_log(BAT_LOG_CRTI, "VGP3    =%d, ",  (((val_0)&(0x8000))>>15));
	battery_log(BAT_LOG_CRTI, "VIBR    =%d, ",  (((val_1)&(0x0001))>>0));
	battery_log(BAT_LOG_CRTI, "VIO18   =%d, ",  (((val_1)&(0x0002))>>1));
	battery_log(BAT_LOG_CRTI, "VIO28   =%d\n",  (((val_1)&(0x0004))>>2));

	battery_log(BAT_LOG_CRTI, "VM      =%d, ",  (((val_1)&(0x0008))>>3));
	battery_log(BAT_LOG_CRTI, "VMC     =%d, ",  (((val_1)&(0x0010))>>4));
	battery_log(BAT_LOG_CRTI, "VMCH    =%d, ",  (((val_1)&(0x0020))>>5));
	battery_log(BAT_LOG_CRTI, "VRF18   =%d\n",  (((val_1)&(0x0040))>>6));

	battery_log(BAT_LOG_CRTI, "VSIM1   =%d, ",  (((val_1)&(0x0080))>>7));
	battery_log(BAT_LOG_CRTI, "VSIM2   =%d, ",  (((val_1)&(0x0100))>>8));
	battery_log(BAT_LOG_CRTI, "VTCXO   =%d, ",  (((val_1)&(0x0200))>>9));
	battery_log(BAT_LOG_CRTI, "VUSB    =%d\n",  (((val_1)&(0x0400))>>10));
}
...
void pmic_debug_init(void)
{
    ...
    if (!mt_pmic_dir) {
		battery_log(BAT_LOG_CRTI,  "fail to mkdir /proc/mt_pmic\n");
        return;
    }
	...
	battery_log(BAT_LOG_CRTI,  "proc_create pmic_debug_proc_fops\n");
#endif
}
...
void PMIC_INIT_SETTING_V1(void)
{
    ...
    {
		battery_log(BAT_LOG_CRTI,  "[Kernel_PMIC_INIT_SETTING_V1] PMIC Chip = %x\n", chip_version);
		battery_log(BAT_LOG_CRTI,  "[Kernel_PMIC_INIT_SETTING_V1] 20130604_0.85v\n");
...
ret = pmic_config_interface(0x778,0x1,0x1,15); // [15:15]: RG_VREF18_ENB_MD;
    ...
    {
		battery_log(BAT_LOG_CRTI,  "[Kernel_PMIC_INIT_SETTING_V1] PMIC Chip = %x\n", chip_version);
		battery_log(BAT_LOG_CRTI,  "[Kernel_PMIC_INIT_SETTING_V1] 20130328_0.85v\n");
...
ret = pmic_config_interface(0x778,0x1,0x1,15); // [15:15]: RG_VREF18_ENB_MD;
    ...
    {
		battery_log(BAT_LOG_CRTI,  "[Kernel_PMIC_INIT_SETTING_V1] Unknown PMIC Chip (%x)\n", chip_version);
    }
...
void pmic_setting_depends_rtc(void)
{
    ...
    if( crystal_exist_status() )
    {
		...
		battery_log(BAT_LOG_CRTI,  "[pmic_setting_depends_rtc] With 32K. Reg[0x%x]=0x%x\n",
    ...
    else
    {
		...
		battery_log(BAT_LOG_CRTI,  "[pmic_setting_depends_rtc] Without 32K. Reg[0x%x]=0x%x\n",
            ANALDO_CON1, upmu_get_reg_value(ANALDO_CON1));
    }
#else
	battery_log(BAT_LOG_CRTI,  "[pmic_setting_depends_rtc] no define CONFIG_MTK_RTC\n");
#endif
}
...
static int pmic_mt6323_probe(struct platform_device *dev)
{
	...
	battery_log(BAT_LOG_CRTI,  "******** MT6323 pmic driver probe!! ********\n");


    //get PMIC CID
    ret_val=upmu_get_cid();
	battery_log(BAT_LOG_CRTI,  "MT6323 PMIC CID=0x%x\n", ret_val);
	...
	battery_log(BAT_LOG_CRTI,  "[PMIC_INIT_SETTING_V1] Done\n");
    PMIC_CUSTOM_SETTING_V1();
	battery_log(BAT_LOG_CRTI,  "[PMIC_CUSTOM_SETTING_V1] Done\n");
    ...
    //PMIC Interrupt Service
    PMIC_EINT_SETTING();
	battery_log(BAT_LOG_CRTI,  "[PMIC_EINT_SETTING] Done\n");

    kthread_run(pmic_thread_kthread, NULL, "pmic_thread_kthread");
	battery_log(BAT_LOG_CRTI,  "[pmic_thread_kthread] Done\n");
    ...
    pmic_debug_init();
	battery_log(BAT_LOG_CRTI,  "[PMIC] pmic_debug_init : done.\n");
	...
	battery_log(BAT_LOG_CRTI,  "[PMIC] VTCXO control by SRCLKEM due to CONFIG_MTK_ENABLE_MD5, Reg[0x402]=0x%x\n",
...
{
	battery_log(BAT_LOG_CRTI,  "******** MT6323 pmic driver remove!! ********\n");
...
{
	battery_log(BAT_LOG_CRTI,  "******** MT6323 pmic driver shutdown!! ********\n");
}

static int pmic_mt6323_suspend(struct platform_device *dev, pm_message_t state)
{
	battery_log(BAT_LOG_CRTI,  "******** MT6323 pmic driver suspend!! ********\n");
...
{
	battery_log(BAT_LOG_CRTI,  "******** MT6323 pmic driver resume!! ********\n");
...
static int mt_pmic_probe(struct platform_device *dev)
{
	...
	battery_log(BAT_LOG_CRTI,  "******** mt_pmic_probe!! ********\n");
...
{
	battery_log(BAT_LOG_CRTI,  "******** MT6323 pmic driver early suspend!! ********\n");
	upmu_set_rg_vref18_enb(0);
	//upmu_set_rg_adc_deci_gdly(0);
	upmu_set_rg_clksq_en_aux(1);
	upmu_set_rg_aud26m_div4_ck_pdn(0);
...
{
	battery_log(BAT_LOG_CRTI,  "******** MT6323 pmic driver early resume!! ********\n");
	upmu_set_rg_vref18_enb(0);
	//upmu_set_rg_adc_deci_gdly(0);
	upmu_set_rg_clksq_en_aux(1);
	upmu_set_rg_aud26m_div4_ck_pdn(0);
...
static int __init pmic_mt6323_init(void)
{
    ...
    if (ret) {
		battery_log(BAT_LOG_CRTI,  "****[pmic_mt6323_init] Unable to device register(%d)\n", ret);
        return ret;
    }
    ret = platform_driver_register(&pmic_mt6323_driver);
    if (ret) {
		battery_log(BAT_LOG_CRTI,  "****[pmic_mt6323_init] Unable to register driver (%d)\n", ret);
        return ret;
    }
	...
	battery_log(BAT_LOG_CRTI,  "****[pmic_mt6323_init] Initialization : DONE !!\n");
