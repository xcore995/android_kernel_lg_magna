...
kal_uint32 bq24296_read_interface (kal_uint8 RegNum, kal_uint8 *val, kal_uint8 MASK, kal_uint8 SHIFT)
{
	...
	battery_log(BAT_LOG_CRTI, "[bq24296_read_interface] Reg[%x]=0x%x\n", RegNum, bq24296_reg);

    bq24296_reg &= (MASK << SHIFT);
    *val = (bq24296_reg >> SHIFT);
	battery_log(BAT_LOG_CRTI, "[bq24296_read_interface] Val=0x%x\n", *val);
...
kal_uint32 bq24296_config_interface (kal_uint8 RegNum, kal_uint8 val, kal_uint8 MASK, kal_uint8 SHIFT)
{
    ...
    ret = bq24296_read_byte(RegNum, &bq24296_reg);
	/*battery_log(BAT_LOG_CRTI, "[bq24296_config_interface] Reg[%x]=0x%x\n", RegNum, bq24296_reg);*/

    bq24296_reg &= ~(MASK << SHIFT);
    bq24296_reg |= (val << SHIFT);

    ret = bq24296_write_byte(RegNum, bq24296_reg);
	/*battery_log(BAT_LOG_CRTI, "[bq24296_config_interface] Write Reg[%x]=0x%x\n", RegNum, bq24296_reg);*/

    // Check
    //bq24296_read_byte(RegNum, &bq24296_reg);
	/*battery_log(BAT_LOG_CRTI, "[bq24296_config_interface] Check Reg[%x]=0x%x\n", RegNum, bq24296_reg);*/
...
void bq24296_dump_register(void)
{
    ...
    for (i=0;i<bq24296_REG_NUM;i++)
    {
		...
		battery_log(BAT_LOG_CRTI, "[bq24296_dump_register] Reg[0x%X]=0x%X\n", i, bq24296_reg[i]);
    }
}
...
static ssize_t store_bq24296_access(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
    ...
    if(buf != NULL && size != 0)
    {
		battery_log(BAT_LOG_CRTI, "[store_bq24296_access] buf is %s and size is %d \n", buf, size);
        reg_address = simple_strtoul(buf,&pvalue,16);

        if(size > 3)
        {
            reg_value = simple_strtoul((pvalue+1),NULL,16);
			battery_log(BAT_LOG_CRTI, "[store_bq24296_access] write bq24296 reg 0x%x with value 0x%x !\n",
        ...
        else
        {
			...
			battery_log(BAT_LOG_CRTI, "[store_bq24296_access] read bq24296 reg 0x%x with value 0x%x !\n",
				reg_address, g_reg_value_bq24296);
			battery_log(BAT_LOG_CRTI, "[store_bq24296_access] Please use \"cat bq24296_access\" to get value\r\n");
        }
    }
    return size;
}
