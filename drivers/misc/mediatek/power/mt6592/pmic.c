...
{
	PMICLOG("enable_pll is not ready.\n");
}
void pmic_disable_pll(int id, char *mod_name)
{
	PMICLOG("disable_pll is not ready.\n");
}
//----------------------------------------------------------------------
...
void PMIC_IMM_PollingAuxadcChannel(void)
{
	...
	/*PMICLOG("[PMIC_IMM_PollingAuxadcChannel] before:%d ", upmu_get_rg_adc_deci_gdly());*/
	...
	/*PMICLOG("[PMIC_IMM_PollingAuxadcChannel] after:%d ", upmu_get_rg_adc_deci_gdly());*/
}
...
int PMIC_IMM_GetOneChannelValue(int dwChannel, int deCount, int trimd)
{
    ...
    */
	...
	do
	{
	    ...
	    switch(dwChannel){
			...
			if( (count++) > count_time_out)
			{
				...
				PMICLOG("[upmu_get_rg_adc_out_ch6] 0x%x\n", ret_data);
	            break;
	        case 2:
    ...
    {
        //debug
		PMICLOG("[AUXADC] adc_result_temp=%d, adc_result=%d, r_val_temp=%d.\n",
...
int pmic_get_buck_current(int avg_times)
{
	...
	PMICLOG("[pmic_get_buck_current] before meter, Reg[0x%x]=0x%x, Reg[0x%x]=0x%x, Reg[0x%x]=0x%x\n",
	...
	PMICLOG("[pmic_get_buck_current] raw_data=%d, val=%d, avg_times=%d\n",
        raw_data, val, avg_times);

    upmu_set_rg_smps_testmode_b(0x0);
	PMICLOG("[pmic_get_buck_current] after meter, Reg[0x%x]=0x%x, Reg[0x%x]=0x%x, Reg[0x%x]=0x%x\n",
        0x200, upmu_get_reg_value(0x200),
        0x758, upmu_get_reg_value(0x758),
        0x76E, upmu_get_reg_value(0x76E)
        );
...
void fchr_key_int_handler(void)
{
    ...
    {
		PMICLOG("[fchr_key_int_handler] Release fchrkey\n");
        kpd_pmic_rstkey_handler(0x0);
        upmu_set_rg_fchrkey_int_sel(0);
    }
    else
    {
		PMICLOG("[fchr_key_int_handler] Press fchrkey\n");
...
static int pmic_thread_kthread(void *x)
{
	...
	PMICLOG("[pmic_thread_kthread] enter\n");

    /* Run on a process content */
    while (1) {
        mutex_lock(&pmic_mutex);

		/* PMICLOG("[pmic_thread_kthread] running\n"); */

        //--------------------------------------------------------------------------------
        ret=pmic_read_interface(INT_STATUS0,(&int_status_val_0),0xFFFF,0x0);
		/* PMICLOG("[INT] int_status_val_0=0x%x\n", int_status_val_0); */
		...
		/* PMICLOG("[INT] int_status_val_1=0x%x\n", int_status_val_1); */
        ...
        ret=pmic_read_interface(INT_STATUS0,(&int_status_val_0),0xFFFF,0x0);
		/* PMICLOG("[INT] after ,int_status_val_0=0x%x\n", int_status_val_0); */

        ret=pmic_read_interface(INT_STATUS1,(&int_status_val_1),0xFFFF,0x0);
		/* PMICLOG("[INT] after ,int_status_val_1=0x%x\n", int_status_val_1); */
...
{
	PMICLOG("[PMIC_EINT_SETTING] start: CUST_EINT_PMIC_NUM=%d\n", CUST_EINT_MT6322_PMIC_NUM);
	...
	PMICLOG("[CUST_EINT] CUST_EINT_PMIC_NUM=%d\n", CUST_EINT_MT6322_PMIC_NUM);
...
{
	PMICLOG("[dct_pmic_VPROC_enable] %d\n", dctEnable);
...
{
	PMICLOG("[dct_pmic_VSYS_enable] %d\n", dctEnable);
...
{
	PMICLOG("[dct_pmic_VUSB_enable] %d\n", dctEnable);
...
{
	PMICLOG("[dct_pmic_VEMC_3V3_enable] %d\n", dctEnable);
...
{
	PMICLOG("[dct_pmic_VRF18_enable] %d\n", dctEnable);
...
{
	PMICLOG("[dct_pmic_VIO18_enable] %d\n", dctEnable);
...
{
	PMICLOG("[dct_pmic_VTCXO_enable] %d\n", dctEnable);
...
{
	PMICLOG("[dct_pmic_VCN28_enable_bt] %d\n", dctEnable);
...
{
	PMICLOG("[dct_pmic_VCN28_enable_wifi] %d\n", dctEnable);
...
{
	/* PMICLOG("****[dct_pmic_VEMC_3V3_sel] value=%d\n", volt); */
...
{
	/* PMICLOG("****[dct_pmic_VM_sel] value=%d\n", volt); */
...
void pmic_ldo_vol_sel(MT65XX_POWER powerId, MT65XX_POWER_VOLTAGE powerVolt)
{
	...
	if (powerId == MT6323_POWER_LDO_VIO28)
		PMICLOG("[dct_pmic_VIO28_enable] No vlotage can setting!\n");
	else if (powerId == MT6323_POWER_LDO_VUSB)
		PMICLOG("[dct_pmic_VUSB_enable] No vlotage can setting!\n");
		...
		PMICLOG("[dct_pmic_VCN_1V8_enable] No vlotage can setting!\n");
		...
		PMICLOG("[dct_pmic_VRTC_enable] No vlotage can setting!\n");
		...
		PMICLOG("[dct_pmic_VRF18_enable] No vlotage can setting!\n");
	else if (powerId == MT6323_POWER_LDO_VIO18)
		PMICLOG("[dct_pmic_VIO18_enable] No vlotage can setting!\n");
	else if (powerId == MT6323_POWER_LDO_VCAMD)
		dct_pmic_VCAMD_sel(powerVolt);
	else if (powerId == MT6323_POWER_LDO_VCAM_IO)
		PMICLOG("[dct_pmic_VCAM_IO_enable] No vlotage can setting!\n");
	else if (powerId == MT6323_POWER_LDO_VTCXO)
		PMICLOG("[dct_pmic_VTCXO_enable] No vlotage can setting!\n");
	else if (powerId == MT6323_POWER_LDO_VA)
		PMICLOG("[dct_pmic_VA_enable] No vlotage can setting!\n");
		...
		PMICLOG("[dct_pmic_VCN28_enable] No vlotage can setting!\n");
	else if (powerId == MT6322_POWER_LDO_VIO28)
		PMICLOG("[dct_pmic_VIO28_enable] No vlotage can setting!\n");
	else if (powerId == MT6322_POWER_LDO_VUSB)
		PMICLOG("[dct_pmic_VUSB_enable] No vlotage can setting!\n");
	else if (powerId == MT6322_POWER_LDO_VMC)
		dct_pmic_VMC_sel(powerVolt);
	else if (powerId == MT6322_POWER_LDO_VMCH)
		dct_pmic_VMCH_sel(powerVolt);
	else if (powerId == MT6322_POWER_LDO_VEMC_3V3)
		dct_pmic_VEMC_3V3_sel(powerVolt);
	else if (powerId == MT6322_POWER_LDO_VGP1)
		dct_pmic_VGP1_sel(powerVolt);
	else if (powerId == MT6322_POWER_LDO_VGP2)
		dct_pmic_VGP2_sel(powerVolt);
	else if (powerId == MT6322_POWER_LDO_VGP3)
		dct_pmic_VGP3_sel(powerVolt);
	else if (powerId == MT6322_POWER_LDO_VCN_1V8)
		PMICLOG("[dct_pmic_VCN_1V8_enable] No vlotage can setting!\n");
	else if (powerId == MT6322_POWER_LDO_VSIM1)
		dct_pmic_VSIM1_sel(powerVolt);
	else if (powerId == MT6322_POWER_LDO_VSIM2)
		dct_pmic_VSIM2_sel(powerVolt);
	else if (powerId == MT6322_POWER_LDO_VRTC)
		PMICLOG("[dct_pmic_VRTC_enable] No vlotage can setting!\n");
	else if (powerId == MT6322_POWER_LDO_VCAM_AF)
		dct_pmic_VCAM_AF_sel(powerVolt);
	else if (powerId == MT6322_POWER_LDO_VIBR)
		dct_pmic_VIBR_sel(powerVolt);
	else if (powerId == MT6322_POWER_LDO_VM)
		dct_pmic_VM_sel(powerVolt);
	else if (powerId == MT6322_POWER_LDO_VRF18)
		PMICLOG("[dct_pmic_VRF18_enable] No vlotage can setting!\n");
	else if (powerId == MT6322_POWER_LDO_VIO18)
		PMICLOG("[dct_pmic_VIO18_enable] No vlotage can setting!\n");
	else if (powerId == MT6322_POWER_LDO_VCAMD)
		dct_pmic_VCAMD_sel(powerVolt);
	else if (powerId == MT6322_POWER_LDO_VCAM_IO)
		PMICLOG("[dct_pmic_VCAM_IO_enable] No vlotage can setting!\n");
	else if (powerId == MT6322_POWER_LDO_VTCXO)
		PMICLOG("[dct_pmic_VTCXO_enable] No vlotage can setting!\n");
	else if (powerId == MT6322_POWER_LDO_VA)
		PMICLOG("[dct_pmic_VA_enable] No vlotage can setting!\n");
	else if (powerId == MT6322_POWER_LDO_VCAMA)
		dct_pmic_VCAMA_sel(powerVolt);
	else if (powerId == MT6322_POWER_LDO_VCN33_BT)
		dct_pmic_VCN33_sel(powerVolt);
	else if (powerId == MT6322_POWER_LDO_VCN33_WIFI)
		dct_pmic_VCN33_sel(powerVolt);
	else if (powerId == MT6322_POWER_LDO_VCN28)
		PMICLOG("[dct_pmic_VCN28_enable] No vlotage can setting!\n");
    else
...
void dump_ldo_status_read_debug(void)
{
	...
	PMICLOG("********** ldo status dump [1:ON,0:OFF]**********\n");

	PMICLOG("VRTC    =%d, ",  (((val_0)&(0x0008))>>3));
	PMICLOG("VA      =%d, ",  (((val_0)&(0x0010))>>4));
	PMICLOG("VCAMA   =%d, ",  (((val_0)&(0x0020))>>5));
	PMICLOG("VCAMD   =%d\n",  (((val_0)&(0x0040))>>6));

	PMICLOG("VCAM_AF =%d, ",  (((val_0)&(0x0080))>>7));
	PMICLOG("VCAM_IO =%d, ",  (((val_0)&(0x0100))>>8));
	PMICLOG("VCN28   =%d, ",  (((val_0)&(0x0200))>>9));
	PMICLOG("VCN33   =%d\n",  (((val_0)&(0x0400))>>10));

	PMICLOG("VCN_1V8 =%d, ",  (((val_0)&(0x0800))>>11));
	PMICLOG("VEMC_3V3=%d, ",  (((val_0)&(0x1000))>>12));
	PMICLOG("VGP1    =%d, ",  (((val_0)&(0x2000))>>13));
	PMICLOG("VGP2    =%d\n",  (((val_0)&(0x4000))>>14));

	PMICLOG("VGP3    =%d, ",  (((val_0)&(0x8000))>>15));
	PMICLOG("VIBR    =%d, ",  (((val_1)&(0x0001))>>0));
	PMICLOG("VIO18   =%d, ",  (((val_1)&(0x0002))>>1));
	PMICLOG("VIO28   =%d\n",  (((val_1)&(0x0004))>>2));

	PMICLOG("VM      =%d, ",  (((val_1)&(0x0008))>>3));
	PMICLOG("VMC     =%d, ",  (((val_1)&(0x0010))>>4));
	PMICLOG("VMCH    =%d, ",  (((val_1)&(0x0020))>>5));
	PMICLOG("VRF18   =%d\n",  (((val_1)&(0x0040))>>6));

	PMICLOG("VSIM1   =%d, ",  (((val_1)&(0x0080))>>7));
	PMICLOG("VSIM2   =%d, ",  (((val_1)&(0x0100))>>8));
	PMICLOG("VTCXO   =%d, ",  (((val_1)&(0x0200))>>9));
	PMICLOG("VUSB    =%d\n",  (((val_1)&(0x0400))>>10));
}
...
void ext_buck_init_for_platform(void)
{
    ...
    if( (g_vproc_vsel_gpio_number!=0) && (g_vproc_en_gpio_number!=0) )
    {
		...
		PMICLOG("[ext_buck_init_for_platform] turn off internal VPROC.\n");
    }
    else
    {
        #ifdef CONFIG_IS_VCORE_USE_6333VCORE
			PMICLOG("[ext_buck_init_for_platform] CONFIG_IS_VCORE_USE_6333VCORE=y, nothing for internal VPROC.\n");
            //add setting id needed
        #else
			PMICLOG("[ext_buck_init_for_platform] CONFIG_IS_VCORE_USE_6333VCORE=n, nothing for internal VPROC.\n");
            //add setting id needed
        #endif
    }
}
...
void ext_buck_init(void)
{
	...
	PMICLOG("[low bat protect] Reg[0x%x]=0x%x, Reg[0x%x]=0x%x, Reg[0x%x]=0x%x\n",
        AUXADC_CON6, upmu_get_reg_value(AUXADC_CON6),
        AUXADC_CON5, upmu_get_reg_value(AUXADC_CON5),
        INT_CON0, upmu_get_reg_value(INT_CON0)
        );
...
void ext_buck_pre_init(void)
{
    ...
    if(g_vproc_vsel_gpio_number!=0)
    {
        if(g_vproc_en_gpio_number!=0)
        {
			...
			PMICLOG("[ext_buck_pre_init] Reg[0x220]=0x%x\n", upmu_get_reg_value(0x220));

            ext_buck_vproc_en(1);
			PMICLOG("[ext_buck_pre_init] ext_buck_vproc_en(1).\n");
        }
        else
        {
			PMICLOG("[ext_buck_pre_init] g_vproc_en_gpio_number==0\n");
        }
    }
    else
    {
		PMICLOG("[ext_buck_pre_init] g_vproc_vsel_gpio_number==0\n");
    }
#else
    if(g_vproc_vsel_gpio_number!=0)
    {
        ext_buck_vproc_vsel(1);
		PMICLOG("[ext_buck_pre_init] ext_buck_vproc_vsel(1)\n");
    }
...
void PMIC_INIT_SETTING_V1(void)
{
    ...
    {
		PMICLOG("[Kernel_PMIC_INIT_SETTING_V1] PMIC Chip = %x\n", chip_version);
		PMICLOG("[Kernel_PMIC_INIT_SETTING_V1] 20130604_0.85v\n");
...
ret = pmic_config_interface(0x778,0x1,0x1,15); // [15:15]: RG_VREF18_ENB_MD;
    ...
    {
		PMICLOG("[Kernel_PMIC_INIT_SETTING_V1] PMIC Chip = %x\n", chip_version);
		PMICLOG("[Kernel_PMIC_INIT_SETTING_V1] 20130328_0.85v\n");
...
ret = pmic_config_interface(0x778,0x1,0x1,15); // [15:15]: RG_VREF18_ENB_MD;
    ...
    {
		PMICLOG("[Kernel_PMIC_INIT_SETTING_V1] Unknown PMIC Chip (%x)\n", chip_version);
    }
...
void PMIC_INIT_SETTING_V1_hpm(void)
{
	...
	PMICLOG("[PMIC_INIT_SETTING_V1_hpm] PMIC Chip = %x\n", chip_version);
	PMICLOG("[PMIC_INIT_SETTING_V1_hpm] 20130902\n");
...
void pmic_setting_depends_rtc(void)
{
    ...
    if( crystal_exist_status() )
    {
		...
		PMICLOG("[pmic_setting_depends_rtc] With 32K. Reg[0x%x]=0x%x\n",
    ...
    else
    {
		...
		PMICLOG("[pmic_setting_depends_rtc] Without 32K. Reg[0x%x]=0x%x\n",
            ANALDO_CON1, upmu_get_reg_value(ANALDO_CON1));
    }
#else
	PMICLOG("[pmic_setting_depends_rtc] no define CONFIG_MTK_RTC\n");
#endif
}

void test_ldo_id(void)
{
	PMICLOG("MT6323_POWER_LDO_VIO28     =%d\n", MT6323_POWER_LDO_VIO28);
	PMICLOG("MT6323_POWER_LDO_VUSB      =%d\n", MT6323_POWER_LDO_VUSB);
	PMICLOG("MT6323_POWER_LDO_VMC       =%d\n", MT6323_POWER_LDO_VMC);
	PMICLOG("MT6323_POWER_LDO_VMCH      =%d\n", MT6323_POWER_LDO_VMCH);
	PMICLOG("MT6323_POWER_LDO_VEMC_3V3  =%d\n", MT6323_POWER_LDO_VEMC_3V3);
	PMICLOG("MT6323_POWER_LDO_VGP1      =%d\n", MT6323_POWER_LDO_VGP1);
	PMICLOG("MT6323_POWER_LDO_VGP2      =%d\n", MT6323_POWER_LDO_VGP2);
	PMICLOG("MT6323_POWER_LDO_VGP3      =%d\n", MT6323_POWER_LDO_VGP3);
	PMICLOG("MT6323_POWER_LDO_VCN_1V8   =%d\n", MT6323_POWER_LDO_VCN_1V8);
	PMICLOG("MT6323_POWER_LDO_VSIM1     =%d\n", MT6323_POWER_LDO_VSIM1);
	PMICLOG("MT6323_POWER_LDO_VSIM2     =%d\n", MT6323_POWER_LDO_VSIM2);
	PMICLOG("MT6323_POWER_LDO_VRTC      =%d\n", MT6323_POWER_LDO_VRTC);
	PMICLOG("MT6323_POWER_LDO_VCAM_AF   =%d\n", MT6323_POWER_LDO_VCAM_AF);
	PMICLOG("MT6323_POWER_LDO_VIBR      =%d\n", MT6323_POWER_LDO_VIBR);
	PMICLOG("MT6323_POWER_LDO_VM        =%d\n", MT6323_POWER_LDO_VM);
	PMICLOG("MT6323_POWER_LDO_VRF18     =%d\n", MT6323_POWER_LDO_VRF18);
	PMICLOG("MT6323_POWER_LDO_VIO18     =%d\n", MT6323_POWER_LDO_VIO18);
	PMICLOG("MT6323_POWER_LDO_VCAMD     =%d\n", MT6323_POWER_LDO_VCAMD);
	PMICLOG("MT6323_POWER_LDO_VCAM_IO   =%d\n", MT6323_POWER_LDO_VCAM_IO);
	PMICLOG("MT6323_POWER_LDO_VTCXO     =%d\n", MT6323_POWER_LDO_VTCXO);
	PMICLOG("MT6323_POWER_LDO_VA        =%d\n", MT6323_POWER_LDO_VA);
	PMICLOG("MT6323_POWER_LDO_VCAMA     =%d\n", MT6323_POWER_LDO_VCAMA);
	PMICLOG("MT6323_POWER_LDO_VCN33_BT  =%d\n", MT6323_POWER_LDO_VCN33_BT);
	PMICLOG("MT6323_POWER_LDO_VCN33_WIFI=%d\n", MT6323_POWER_LDO_VCN33_WIFI);
	PMICLOG("MT6323_POWER_LDO_VCN28     =%d\n", MT6323_POWER_LDO_VCN28);

	PMICLOG("MT6322_POWER_LDO_VIO28     =%d\n", MT6322_POWER_LDO_VIO28);
	PMICLOG("MT6322_POWER_LDO_VUSB      =%d\n", MT6322_POWER_LDO_VUSB);
	PMICLOG("MT6322_POWER_LDO_VMC       =%d\n", MT6322_POWER_LDO_VMC);
	PMICLOG("MT6322_POWER_LDO_VMCH      =%d\n", MT6322_POWER_LDO_VMCH);
	PMICLOG("MT6322_POWER_LDO_VEMC_3V3  =%d\n", MT6322_POWER_LDO_VEMC_3V3);
	PMICLOG("MT6322_POWER_LDO_VGP1      =%d\n", MT6322_POWER_LDO_VGP1);
	PMICLOG("MT6322_POWER_LDO_VGP2      =%d\n", MT6322_POWER_LDO_VGP2);
	PMICLOG("MT6322_POWER_LDO_VGP3      =%d\n", MT6322_POWER_LDO_VGP3);
	PMICLOG("MT6322_POWER_LDO_VCN_1V8   =%d\n", MT6322_POWER_LDO_VCN_1V8);
	PMICLOG("MT6322_POWER_LDO_VSIM1     =%d\n", MT6322_POWER_LDO_VSIM1);
	PMICLOG("MT6322_POWER_LDO_VSIM2     =%d\n", MT6322_POWER_LDO_VSIM2);
	PMICLOG("MT6322_POWER_LDO_VRTC      =%d\n", MT6322_POWER_LDO_VRTC);
	PMICLOG("MT6322_POWER_LDO_VCAM_AF   =%d\n", MT6322_POWER_LDO_VCAM_AF);
	PMICLOG("MT6322_POWER_LDO_VIBR      =%d\n", MT6322_POWER_LDO_VIBR);
	PMICLOG("MT6322_POWER_LDO_VM        =%d\n", MT6322_POWER_LDO_VM);
	PMICLOG("MT6322_POWER_LDO_VRF18     =%d\n", MT6322_POWER_LDO_VRF18);
	PMICLOG("MT6322_POWER_LDO_VIO18     =%d\n", MT6322_POWER_LDO_VIO18);
	PMICLOG("MT6322_POWER_LDO_VCAMD     =%d\n", MT6322_POWER_LDO_VCAMD);
	PMICLOG("MT6322_POWER_LDO_VCAM_IO   =%d\n", MT6322_POWER_LDO_VCAM_IO);
	PMICLOG("MT6322_POWER_LDO_VTCXO     =%d\n", MT6322_POWER_LDO_VTCXO);
	PMICLOG("MT6322_POWER_LDO_VA        =%d\n", MT6322_POWER_LDO_VA);
	PMICLOG("MT6322_POWER_LDO_VCAMA     =%d\n", MT6322_POWER_LDO_VCAMA);
	PMICLOG("MT6322_POWER_LDO_VCN33_BT  =%d\n", MT6322_POWER_LDO_VCN33_BT);
	PMICLOG("MT6322_POWER_LDO_VCN33_WIFI=%d\n", MT6322_POWER_LDO_VCN33_WIFI);
	PMICLOG("MT6322_POWER_LDO_VCN28     =%d\n", MT6322_POWER_LDO_VCN28);

	PMICLOG("MT6323_POWER_LDO_DEFAULT   =%d\n", MT6323_POWER_LDO_DEFAULT);
	PMICLOG("MT6322_POWER_LDO_DEFAULT   =%d\n", MT6322_POWER_LDO_DEFAULT);
	PMICLOG("MT65XX_POWER_COUNT_END     =%d\n", MT65XX_POWER_COUNT_END);
}
...
static int pmic_mt_probe(struct platform_device *dev)
{
	...
	PMICLOG("[PMIC_INIT_SETTING_V1_hpm] Done\n");
    ...
    if (IS_ERR(pmic_thread_handle))
    {
		...
		PMICLOG("[pmic_thread_kthread] creation fails\n");
    }
    else
    {
        wake_up_process(pmic_thread_handle);
	...
	PMICLOG("[PMIC] VTCXO control by SRCLKEM due to MTK_ENABLE_MD5, Reg[0x402]=0x%x\n",
...
static int pmic_mt_resume(struct platform_device *dev)
{
	...
	PMICLOG("[low bat protect] Reg[0x%x]=0x%x, Reg[0x%x]=0x%x, Reg[0x%x]=0x%x, g_low_battery_level=%d\n",
        AUXADC_CON6, upmu_get_reg_value(AUXADC_CON6),
        AUXADC_CON5, upmu_get_reg_value(AUXADC_CON5),
        INT_CON0, upmu_get_reg_value(INT_CON0),
        g_low_battery_level
