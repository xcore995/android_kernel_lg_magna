...
static ssize_t sd_single_read(struct file *filp, char __user *buf,
				size_t count, loff_t addr, Region part)
{
	...
	if (fo->act_filp->f_op->unlocked_ioctl) {
		result = fo->act_filp->f_op->unlocked_ioctl(fo->act_filp, 1,
				(unsigned long)&cmd);
	} else if (fo->act_filp->f_op->compat_ioctl) {
		result = fo->act_filp->f_op->compat_ioctl(fo->act_filp, 1,
				(unsigned long)&cmd);
	}
	if (result == 0)
		result = count;
...
static ssize_t sd_read(struct file *filp, char __user *buf, size_t count,
			loff_t *pos, Region part)
{
	struct file_obj *fo = filp->private_data;
	...
	if (addr % ALIE_LEN != 0 || (addr + count) % ALIE_LEN != 0) {
		...
		if (!pbuf) {
			pr_err("DumChar: malloc buff fail\n");
			return -ENOMEM;
		}
		pbuf2 = pbuf;
		curr_fs = get_fs();
		...
		if (total_retlen == (endaddr - startaddr)) {
			...
			if (n != 0)
				pr_debug(KERN_ERR "read fail in DumChar_sd_read\n");
			total_retlen = count - n;
		} else {
			pr_debug(KERN_ERR "read fail DumChar_sd_read!\n");
		}
#if defined(PrintBuff)
...
static ssize_t sd_write(struct file *filp, const char __user *buf,
				size_t count, loff_t *pos, Region part)
{
	struct file_obj *fo = filp->private_data;
	struct dumchar_dev *dev = dumchar_devices + fo->index;
	size_t total_retlen = 0;
...
#endif
static ssize_t dumchar_read(struct file *filp, char __user *buf, size_t count,
				loff_t *f_pos)
{
	struct file_obj *fo = filp->private_data;
	struct dumchar_dev *dev = dumchar_devices + fo->index;
	...
	if (fo->act_filp == (struct file *)0xffffffff) {
		pr_err("[dumchar_read]Forbidded access %s with dumchar(/dev/%s),the %s part managed by filesystem\n",
				dev->dumname, dev->dumname, dev->dumname);
		pr_err("[dumchar_read] show_stack*************************************\n");
...
ssize_t dumchar_write(struct file *filp, const char __user *buf, size_t count,
			loff_t *f_pos)
{
	struct file_obj *fo = filp->private_data;
	struct dumchar_dev *dev = dumchar_devices + fo->index;
...
#endif
	if (fo->act_filp == (struct file *)0xffffffff) {
		pr_err("[dumchar_write]Forbidded to access %s with dumchar(/dev/%s),%s part managed by filesystem!\n",
				dev->dumname, dev->dumname, dev->dumname);
		pr_err("[dumchar_write] show_stack*************************************\n");
		show_stack(NULL, NULL);
		return -EINVAL;
	}
#ifdef CONFIG_MTK_COMBO_NAND_SUPPORT
...
#endif
	if (fo->act_filp == (struct file *)0xffffffff) {
		pr_err("[dumchar_ioctl]Forbidded to access %s with dumchar(/dev/%s),%s part managed by filesystem!\n",
				dev->dumname, dev->dumname, dev->dumname);
		pr_err("[dumchar_loctl] show_stack*************************************\n");
		show_stack(NULL, NULL);
		return -EINVAL;
	}
	if (dev->type != NAND && dev->type != EMMC) {
...
loff_t dumchar_llseek(struct file *filp, loff_t off, int whence)
{
	...
	loff_t newpos;
	if (fo->act_filp == (struct file *)0xffffffff) {
		pr_err("[dumchar_llseek]Forbidded to access %s with dumchar(/dev/%s),%s part managed by filesystem\n",
				dev->dumname, dev->dumname, dev->dumname);
		pr_err("[dumchar_llseek] show_stack*************************************\n");
		show_stack(NULL, NULL);
		return -EINVAL;
	}
	if (dev->type != NAND && dev->type != EMMC) {
...
int dumchar_open(struct inode *inode, struct file *filp)
{
	...
	if (!fo) {
		pr_err("DumChar: kmalloc file_obj fail!\n");
		return -ENOMEM;
	}
	...
	if (!strcmp(dev->dumname, "usrdata") || !strcmp(dev->dumname, "cache") ||
		...
		pr_err("[dumchar_open]Forbidded to access %s with dumchar(/dev/%s),%s part managed by filesystem\n",
				dev->dumname, dev->dumname, dev->dumname);
		pr_err("[dumchar_open] show_stack*************************************\n");
		show_stack(NULL, NULL);
...
int dumchar_fsync(struct file *filp, loff_t start, loff_t end, int datasync)
{
...
#endif
	if (fo->act_filp == (struct file *)0xffffffff) {
		pr_err("[dumchar_fsync]Forbidded to access %s with dumchar(/dev/%s),%s part managed by filesystem!\n",
				dev->dumname, dev->dumname, dev->dumname);
		pr_err("[dumchar_fsync] show_stack*************************************\n");
		show_stack(NULL, NULL);
		return -EINVAL;
	}
	...
	if (dev->type == EMMC) {
		...
		case EMMC_PART_USER:
			...
			break;
		case EMMC_PART_BOOT1:
			pr_err("Dumchar: fsync not support on eMMC BOOT region\n");
			result = -EINVAL;
			break;
		default:
			pr_err("DumChar: Wrong EMMC Region\n");
		...
		pr_err("Dumchar: fsync not support on Nand\n");
		result = -EINVAL;
	}
	return result;
}
...
int dumchar_release(struct inode *inode, struct file *filp)
{
	...
	if (!strcmp(dev->dumname, "usrdata") || !strcmp(dev->dumname, "cache") ||
	    !strcmp(dev->dumname, "android") || !strcmp(dev->dumname, "fat")) {
		pr_debug("[dumchar_release]Forbidded access %s with dumchar(/dev/%s),%s part managed by filesystem\n",
				dev->dumname, dev->dumname, dev->dumname);
	} else {
		filp_close(fo->act_filp, NULL);
	}
...
int dumchar_probe(struct platform_device *dev)
{
	...
	pr_debug("[Dumchar_probe]There are 3 addr in eMMC Project: Linear Address, Logical Address,Physical Address\n");
	pr_debug("[Dumchar_probe]Linear Address: Used in scatter file, uboot, preloader,flash tool etc.\n");
	pr_debug("[Dumchar_probe]Linear Address: MBR linear address fixed in eMMCComo.mk,that is same for all chips\n");
	...
	pr_debug("[Dumchar]MBR address %llx\n", MBR_START_ADDRESS_BYTE);
#endif
...
static int dumchar_proc_show(struct seq_file *m, void *v)
{
	...
	if (dumchar_devices[0].type == NAND) {
		seq_puts(m, "Part_Name:Partition name you should open;\nSize:size of partition\n");
		seq_puts(m, "StartAddr:Index (MTD);\nType:Type of partition(MTD=1,EMMC=2);\n");
		seq_puts(m, "MapTo:actual device you operate\n");
	} else {
		seq_puts(m, "Part_Name:Partition name you should open;\nSize:size of partition\n");
		seq_puts(m, "StartAddr:Start Address of partition;\nType:Type of partition(MTD=1,EMMC=2)\n");
		seq_puts(m, "MapTo:actual device you operate\n");
	}
	return 0;
}
