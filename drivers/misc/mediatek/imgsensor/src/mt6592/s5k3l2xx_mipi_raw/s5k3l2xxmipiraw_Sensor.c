...
kal_uint16 S5K3L2XX_get_framelength(void)
{
    ...
    if(s5k3l2xx.sensorMode == SENSOR_MODE_PREVIEW)
        ...
        Framelength = s5k3l2xx_video_frame_length; //[LGE_UPDATE][yonghwan.lym@lge.com][2015-04-14] frame length bug fix
...
kal_uint32 S5K3L2XX_get_active_height(void)
{
    kal_uint32 active_height = s5k3l2xx_sensor_output_preview_height;

    if(s5k3l2xx.sensorMode == SENSOR_MODE_PREVIEW)
        active_height = s5k3l2xx_sensor_output_preview_height;
    else if(s5k3l2xx.sensorMode == SENSOR_MODE_CAPTURE)
        active_height = s5k3l2xx_sensor_output_capture_height;
    else if(s5k3l2xx.sensorMode == SENSOR_MODE_VIDEO)
        active_height = s5k3l2xx_sensor_output_video_height;

    return active_height;
}


kal_uint32 S5K3L2XX_get_line_pixel(void)
{
    kal_uint32 line_pixel = s5k3l2xx_sensor_output_preview_width;

    if(s5k3l2xx.sensorMode == SENSOR_MODE_PREVIEW)
        line_pixel = s5k3l2xx_sensor_output_preview_width;
    else if(s5k3l2xx.sensorMode == SENSOR_MODE_CAPTURE)
        line_pixel = s5k3l2xx_sensor_output_capture_width;
    else if(s5k3l2xx.sensorMode == SENSOR_MODE_VIDEO)
        line_pixel = s5k3l2xx_sensor_output_video_width;
...
void S5K3L2XXInitSetting(void)
{
    ...
    S5K3L2XX_write_cmos_sensor1(0xFCFC,0xD000);
    S5K3L2XX_write_cmos_sensor1(0x6010,0x0001);
    Sleep(5);
    ...
    S5K3L2XX_write_cmos_sensor1(0x6214,0x7970);
    S5K3L2XX_write_cmos_sensor1(0x6218,0x7150);
    S5K3L2XX_write_cmos_sensor1(0x6004,0x0001);
    ...
    S5K3L2XX_write_cmos_sensor1(0x6F12,0x0040);
    S5K3L2XX_write_cmos_sensor1(0x6028,0xD000);
    S5K3L2XX_write_cmos_sensor1(0x6004,0x0000);
...
void S5K3L2XXCaptureSetting(void)
{
    ...
    S5K3L2XX_write_cmos_sensor(0x6F12,0x01);
#ifdef FAST_MODE
 //LGE_UPDATE_S [youmi.jun@lge.com] 2015/04/25, modify fast mode
    S5K3L2XX_write_cmos_sensor1(0x602A,0x0202);
    S5K3L2XX_write_cmos_sensor1(0x6F12,S5K3L2XX_capture_shutter);
    gain = S5K3L2XX_capture_gain/2;
    S5K3L2XX_write_cmos_sensor1(0x602A,0x0204);
    S5K3L2XX_write_cmos_sensor1(0x6F12,gain);
 //LGE_UPDATE_E [youmi.jun@lge.com] 2015/04/25, modify fast mode
    S5K3L2XX_write_cmos_sensor(0x0104,0x00);
#else
    for(read_count = 0; read_count <= 4; read_count++)
    {
...
UINT32 S5K3L2XXGetInfo(MSDK_SCENARIO_ID_ENUM ScenarioId, MSDK_SENSOR_INFO_STRUCT *pSensorInfo, MSDK_SENSOR_CONFIG_STRUCT *pSensorConfigData)
{
...
#ifdef FAST_MODE
    pSensorInfo->CaptureDelayFrame = 2; //LGE_UPDATE [youmi.jun@lge.com] 2015/04/25, modify fast mode (0->2)
#else
    ...
    #if 1 //ACUTE_porting AE_blinking
...
UINT32 S5K3L2XXFeatureControl(MSDK_SENSOR_FEATURE_ENUM FeatureId, UINT8 *pFeaturePara, UINT32 *pFeatureParaLen)
{
    ...
    switch (FeatureId)
    {
        ...
        case SENSOR_FEATURE_GET_SENSOR_VIEWANGLE:
            {
                ...
                S5K3L2XXDB("SENSOR_FEATURE_GET_SENSOR_VIEWANGLE\n");
                *pHorFOV = (0x3E)<<16 | (0x3E); // 16:9 = 62, 4:3 = 62 // (62)<<16 | (62)
        ...
        case SENSOR_FEATURE_GET_SENSOR_INFO_FOR_ACUTE:
            S5K3L2XXDB("SENSOR_FEATURE_GET_SENSOR_INFO_FOR_ACUTE\n");
            {
                S5K3L2XXMIPIGetDefaultFramerateByScenario(S5K3L2XXCurrentScenarioId, &(pSensorInfoDataForAcute->max_fps));
                pSensorInfoDataForAcute->max_fps = pSensorInfoDataForAcute->max_fps/10; //300 -> 30
                pSensorInfoDataForAcute->pclk = S5K3L2XX_get_pixelclock();
                pSensorInfoDataForAcute->line_length = S5K3L2XX_get_linelength();
                pSensorInfoDataForAcute->frame_length = S5K3L2XX_get_framelength();
                pSensorInfoDataForAcute->line_pixel = S5K3L2XX_get_line_pixel();
                pSensorInfoDataForAcute->active_height = S5K3L2XX_get_active_height();
