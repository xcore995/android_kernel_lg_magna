		     ...
		     u16 i2cId)
{
	...
	*((char *)buf + 1) = *(a_pSendData + 1);
	...
	msgs[0].ext_flag =
	    ((g_pstI2Cclient->
	      ext_flag) & I2C_MASK_FLAG) | (I2C_RS_FLAG) | I2C_WR_FLAG | I2C_DMA_FLAG;
...
#if 0  //[LGE_UPDATE][yonghwan.lym@lge.com][2016-01-15] Acute Porting - Flash Capture Lock Up
...
static int kdSetSensorGpio(int *pBuf)
{
	...
	if (pSensorgpio->SensroInterfaceType == SENSORIF_PARALLEL) {
		...
		PK_DBG("GPIO153~GPIO156 0x%x, 0x%x\n", SENSOR_RD32((void *)(GPIO_BASE + 0x7E0)),
		       SENSOR_RD32((void *)(GPIO_BASE + 0x7E0)));
		...
		if (pSensorgpio->GpioEnable) {
			...
			if (pSensorgpio->mSensorDev == DUAL_CAMERA_MAIN_SENSOR) {
				...
				PK_DBG("GPIO147~GPIO156 0x%x, 0x%x, 0x%x\n",
				       SENSOR_RD32((void *)(GPIO_BASE + 0x7d0)),
				       SENSOR_RD32((void *)(GPIO_BASE + 0x7E0)),
				       SENSOR_RD32((void *)(GPIO_BASE + 0x7f0)));
			} else {
			    ...
			    unsigned int a_u4Command, unsigned long a_u4Param)
{
	...
	if (_IOC_NONE != _IOC_DIR(a_u4Command)) {
		...
		pBuff =
		    kmalloc(2 *
			    (sizeof(MSDK_SCENARIO_ID_ENUM) + sizeof(ACDK_SENSOR_INFO_STRUCT) +
			     sizeof(ACDK_SENSOR_CONFIG_STRUCT)), GFP_KERNEL);
		if (NULL == pBuff) {
