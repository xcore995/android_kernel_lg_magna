...
int iBurstReadRegI2C_RS(u8 *a_pSendData , u16 a_sizeSendData, u8 * a_pRecvData, u16 a_sizeRecvData, u16 i2cId)
{
	u8 *buf = NULL;
    int i4RetValue = 0;
    u32 phyAddr = 0;
...
// [LGE_UPDATE_E] [kipo.yun@lge.com] [2014-10-25] AT command added for main/sub camera to prevent mixed items
                    err = ERROR_NONE;
                    g_SensorIDInfo[(g_invokeSocketIdx[i]-1)] = sensorID;
                }
                if(ERROR_NONE != err)
                {
