...

{
	...
	SENSORDB("sym GC0329Preview\n");
	#if 0
	if(mt_set_gpio_mode(138,0)){SENSORDB(" set flashlight gpio mode failed!! \n");}
	if(mt_set_gpio_dir(138,1)){SENSORDB("[CAMERA LENS] set gpio dir failed!! \n");}
			 if(mt_set_gpio_out(138,1)){SENSORDB("[CAMERA LENS] set gpio failed!! \n");}  //sym modify
			 msleep(3);
#endif
...
SENSORDB("sym GC0329Capture\n");
#if 0
	if(mt_set_gpio_mode(138,0)){SENSORDB(" set flashlight gpio mode failed!! \n");}
	if(mt_set_gpio_dir(138,1)){SENSORDB("[CAMERA LENS] set gpio dir failed!! \n");}
			 if(mt_set_gpio_out(138,0)){SENSORDB("[CAMERA LENS] set gpio failed!! \n");}  //sym modify
			 msleep(3);
			 #endif
    image_window->GrabStartX = IMAGE_SENSOR_VGA_GRAB_PIXELS;
    image_window->GrabStartY = IMAGE_SENSOR_VGA_GRAB_LINES;
