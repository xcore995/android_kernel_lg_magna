...
static int cldma_gpd_rx_refill(struct md_cd_queue *queue)
{
	...
	while (1) {
		...
		new_skb = ccci_alloc_skb(queue->tr_ring->pkt_size, !is_net_queue);
		if (likely(new_skb)) {
	...
	*rxbytes = 0;
	...
	while (1) {
		...
		*rxbytes += skb_bytes;
		...
		req = queue->rx_refill;
		if (!req->skb) {
			new_skb = ccci_alloc_skb(queue->tr_ring->pkt_size, blocking);
			if (likely(new_skb)) {
				rgpd = (struct cldma_rgpd *)req->gpd;
				req->data_buffer_ptr_saved =
...
static int cldma_net_rx_push_thread(void *arg)
{
	...
	while (1) {
		if (skb_queue_empty(&queue->skb_list)) {
			count = 0;
			ret = wait_event_interruptible(queue->rx_wq, !skb_queue_empty(&queue->skb_list));
			if (ret == -ERESTARTSYS)
				continue;	/* FIXME */
		}
...
#endif
		skb = skb_dequeue(&queue->skb_list);
		ccci_h = (struct ccci_header *)skb->data;
		/* check wakeup source */
		if (atomic_cmpxchg(&md->wakeup_src, 1, 0) == 1)
			CCCI_INF_MSG(md->index, TAG, "CLDMA_MD wakeup source:(%d/%d)\n", queue->index, ccci_h->channel);
...
again:
	result = rx_bytes = 0;
	count += queue->tr_ring->handle_rx_done(queue, queue->budget, !IS_NET_QUE(md, queue->index),
											&result, &rx_bytes);
	rx_total += rx_bytes;
...
static void cldma_rx_ring_init(struct ccci_modem *md, struct cldma_ring *ring)
{
	...
	if (ring->type == RING_GPD) {
		for (i = 0; i < ring->length; i++) {
			...
			item->skb = ccci_alloc_skb(ring->pkt_size, 1);
