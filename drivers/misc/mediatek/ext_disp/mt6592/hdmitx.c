...
int hdmi_log_on = 1;
int hdmi_bufferdump_on = 7;
unsigned long hdmi_reschange = HDMI_VIDEO_RESOLUTION_NUM;
...
#endif
int dp_mutex_src = -1, dp_mutex_dst = -1;
...
#ifdef MTK_SMARTBOOK_SUPPORT
static struct task_struct *hdmi_status_update_task;
...
#define HDMI_BUFFER_LOG(fmt, arg...) \
	do { \
		if (enable_hdmi_buffer_log) { \
			HDMI_LOG("[hdmi_buffer] "); HDMI_LOG(fmt, ##arg); \
		} \
	} while (0)
#else
bool enable_hdmi_buffer_log = 0;
...
static void hdmi_ion_deinit(void)
{
	if (ion_client) {
...
static unsigned int hdmi_get_timeline_counter_inc(void)
...
unsigned int hdmi_timeline_inc(void)
{
	...
	if (inc < 0 || inc > 5) {
		if (hdmi_bufferdump_on > 0) {
			MMProfileLogEx(HDMI_MMP_Events.ErrorInfo, MMProfileFlagPulse, Timeline_Err,
				       inc);
		}
...
static void hdmi_buffer_init(int num)
{
	...
	hdmi_buffer_available = (long int *)vmalloc(hdmi_buffer_num * sizeof(int));
	hdmi_buffer_queue = (long int *)vmalloc(hdmi_buffer_num * sizeof(int));
...
static int hdmi_dump_buffer_queue(void)
	return 0;
			...
			if (!list_empty(&HDMI_Buffer_List)) {
				...
				if ((pBuffList == NULL) || (pBuffList->buf_state != insert_new)
					...
					if ((pBuffList != NULL)
						...
						HDMI_LOG
						    (" buffer config error to configed %p, state %d, idx %d\n",
						     pBuffList, pBuffList->buf_state,
						     pBuffList->idx);
					}
				} else {
					spin_lock_bh(&hdmi_lock);
					...
					if (hdmi_bufferdump_on > 0) {
						MMProfileLogEx(HDMI_MMP_Events.BufferUsed,
							       MMProfileFlagPulse, pBuffList,
							       buf_configed);
					}
				}
...
static int hdmi_status_update_kthread(void *data)
{
	...
	for (;;) {
		wait_event_interruptible(hdmi_status_update_wq,
					 atomic_read(&hdmi_status_update_event));
		...
		if (p->is_clock_on == true) {	/*remove the first head here */
			if (!list_empty(&HDMI_Buffer_List)) {
				...
				if (remove_buffer_cnt > 1) {
					if (hdmi_bufferdump_on > 0) {
						MMProfileLogEx(HDMI_MMP_Events.ErrorInfo,
							       MMProfileFlagPulse, Buff_Dup_Err1,
							       remove_buffer_cnt);
					}

					HDMI_LOG("[hdmi] %s, %d remove two buffer one time", __func__,
					       __LINE__);
...
*/
...

	{
		...
		if (!hdmi_status_update_task) {
			hdmi_status_update_task =
			    kthread_create(hdmi_status_update_kthread, NULL,
					   "hdmi_status_update_kthread");
...
static void hdmi_dpi_config_update(void)
{
	...
	DPI_CHECK_RET(HDMI_DPI(_FBSetPitch) (DPI_FB_0, p->hdmi_width * 3));
...
int hdmi_free_hdmi_buffer(void)
{
	...
	if (hdmi_mva_r) {
		...
		m4u_dealloc_mva(DISP_RDMA1, hdmi_va, hdmi_va_size, hdmi_mva_r);
		hdmi_mva_r = 0;
	}
...
int hdmi_rdma_address_config(bool enable, hdmi_video_buffer_info buffer_info)
{
	if (enable) {
		...
		if ((hdmi_abs(buffer_info.src_height - p->hdmi_height) > 32)
			...
			HDMI_LOG
			    ("info: fmt %d, h %d, w %d, x_o %d, y_o %d, pitch %d hdmi_h %d -w %d\n",
	...
	case MTK_HDMI_VIDEO_CONFIG:
		{
			...
			if (hdmi_bufferdump_on > 0) {
				MMProfileLogEx(HDMI_MMP_Events.HDMIState, MMProfileFlagStart,
					       ResChange, arg);
			}
	...
	case MTK_HDMI_POST_VIDEO_BUFFER:
		{
			...
			if ((p->is_enabled == false) || (p->is_clock_on == false)
			    || IS_HDMI_NOT_ON()) {
				MMProfileLogEx(HDMI_MMP_Events.ErrorInfo, MMProfileFlagPulse,
					       insert_Buffer_Err1, insert_Buffer_Err1);
				RETIF(!p->is_enabled, 0);
			...
			if (copy_from_user
				...
				MMProfileLogEx(HDMI_MMP_Events.ErrorInfo, MMProfileFlagPulse,
					       insert_Buffer_Err2, insert_Buffer_Err2);
			...
			if (hdmi_bufferdump_on > 0)
				MMProfileLogEx(HDMI_MMP_Events.BufferPost, MMProfileFlagStart,
			...
			if (p->is_clock_on) {
				...
				if (pBuffList) {
					...
					if (pBuffList->hnd != 0) {
						if (video_buffer_info.src_phy_addr != NULL) {
							...
							HDMI_LOG
							    ("ion disable, bufflist %p, vir %p, phy %p\n",
							     pBuffList,
						...
						pBuffList->buffer_info.src_phy_addr =
						    pBuffList->mva;
						pBuffList->buffer_info.src_base_addr =
						    pBuffList->va;
					}
				} else {
					spin_unlock_bh(&hdmi_lock);
				...
				if (capture_screen == true) {
					memcpy(capture_addr, video_buffer_info.src_base_addr,
					       Bitmap.data_size);
			...
			if (hdmi_bufferdump_on > 0)
				MMProfileLogEx(HDMI_MMP_Events.BufferInsert, MMProfileFlagPulse,
					       pBuffList, video_buffer_info.next_buff_idx);


			/*up(&hdmi_update_mutex); */
			if (hdmi_bufferdump_on > 0) {
				if (pBuffList)
					MMProfileLogEx(HDMI_MMP_Events.BufferPost, MMProfileFlagEnd,
						       pBuffList,
						       pBuffList->buffer_info.src_phy_addr);
				else
					MMProfileLogEx(HDMI_MMP_Events.BufferPost, MMProfileFlagEnd,
	...
	case MTK_HDMI_GET_DEV_INFO:
		{
			...
			if (hdmi_bufferdump_on > 0)
				MMProfileLogEx(HDMI_MMP_Events.GetDevInfo, MMProfileFlagStart,
					       p->is_enabled, p->is_clock_on);
	...
	case MTK_HDMI_PREPARE_BUFFER:
		{
			...
			if (hdmi_bufferdump_on > 0)
				MMProfileLogEx(HDMI_MMP_Events.FenceCreate, MMProfileFlagStart, 0,
					       0);
			...
			if (hdmi_bufferdump_on > 0)
				MMProfileLogEx(HDMI_MMP_Events.FenceCreate, MMProfileFlagEnd,
					       hdmi_buffer.fence_fd, pBuffList);
#endif
			break;
		}
...
struct const file_operations hdmi_fops = {
	.owner = THIS_MODULE,
