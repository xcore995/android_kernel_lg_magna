...
static int mjc_open(struct inode *pInode, struct file *pFile)
{
	...
	MJCDBG("Mapping register: 0x%x, PA:0x%x, Size:0x%x, Irq ID:%d\n", gi4Register,
		   MJC_REG_PA, MJC_REG_REGION, MJC_TOP_IRQ_ID);
	...
	case MJC_READ_REG:
		{
			...
			if (rReadReg.reg > (MJC_REG_PA + MJC_REG_REGION)
				...
				MJCMSG
					("[ERROR] MJC_READ_REG, addr invalid, addr min=0x%x, max=0x%x, addr=0x%lx\n",
	...
	case MJC_WRITE_REG:
		{
			...
			if (rWriteReg.reg > (MJC_REG_PA + MJC_REG_REGION)
				...
				MJCMSG
					("[ERROR] MJC_WRITE_REG, addr invalid, addr min=0x%x, max=0x%x, addr=0x%lx\n",
			...
			*(volatile unsigned int *)i4Add =
				(*(volatile unsigned int *)i4Add & ~rWriteReg.mask) | (rWriteReg.
										   val & rWriteReg.
										   mask);
		}
		break;
	...
	case MJC_WRITE_REG_TBL:
		{
			...
			for (cnt = 0; cnt < MJC_FORCE_REG_NUM; cnt++) {
				...
				*(volatile unsigned int *)i4Add =
					(*(volatile unsigned int *)i4Add & ~gfWriteReg[cnt].
					 mask) | (gfWriteReg[cnt].val & gfWriteReg[cnt].mask);
			}
		}
		break;
	...
	case MJC_SOURCE_CLK:
		{
			...
			if (sizeof(MJC_IOCTL_SRC_CLK_T) != rSrcClk.u4StructSize) {
				MJCMSG
				("[ERROR] mjc_ioctl() MJC_SOURCE_CLK context size mismatch (user:%d, kernel:%d)\n",
	...
	case MJC_REG_INFO:
		{
			...
			if (sizeof(MJC_IOCTL_REG_INFO_T) != rRegInfo.u4StructSize) {
				MJCMSG
				("[ERROR] mjc_ioctl() MJC_REG_INFO context size mismatch (user:%d, kernel:%d)\n",
				rRegInfo.u4StructSize, sizeof(MJC_IOCTL_REG_INFO_T));
			   return -1;
			}

			rRegInfo.ulRegPAddress = MJC_REG_PA;
			rRegInfo.ulRegPSize = MJC_REG_REGION;
...
static int mjc_probe(struct platform_device *pDev)
{
	...
	/* Gary todo: */
	if (request_irq
		(MJC_TOP_IRQ_ID, (irq_handler_t) mjc_intr_dlr, IRQF_TRIGGER_LOW, MJC_DEVNAME,
		 NULL) < 0) {
