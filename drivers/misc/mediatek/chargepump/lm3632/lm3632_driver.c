...
unsigned char bright_arr[] = {	/* array index max 100, value under 255 */
	...
	146, 148, 152, 156, 160, 164, 168, 172, 178, 182, 186, 190, 194, 200, 204, 208,
	212, 218, 222, 228, 232, 238, 242, 248, 255	/* 100 */
};
#endif
...
int chargepump_set_backlight_level(unsigned int level)
{
	...
	if (level == 0) {
		...
		bright_per =
		    (level -
		     (unsigned int)LM3632_MIN_VALUE_SETTINGS) * (unsigned int)100 /
		    (unsigned int)(255 - LM3632_MIN_VALUE_SETTINGS);
#else
...
static int __init lm3632_init(void)
{
	...
	if (strcmp(cus_hw->name, lm3632_backlight_driver.driver.name)) {
		CPD_ERR("lm3632 mismatch device_name(%s,%s)\n", cus_hw->name,
			lm3632_backlight_driver.driver.name);
		return -1;
	}
