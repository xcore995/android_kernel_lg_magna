...
static kal_int32 g_adc_ge_efuse = 0;
static kal_int32 g_adc_oe_efuse = 0;
#endif
...
/*static*/ int IMM_auxadc_GetOneChannelValue(int dwChannel, int data[4], int *rawdata)
{
   ...
   //Set SPL_EN
   if( 12 == dwChannel )    /* 12 == AUXADC channel of USB_ID */
       *(volatile unsigned short *)AUXADC_SPL_EN = *(volatile unsigned short *)AUXADC_SPL_EN | (1 << dwChannel);
       //AUXADC_DRV_SetBits16((volatile u16 *)AUXADC_SPL_EN, (1 << dwChannel));
   ...
   if ((dwChannel == 0) || (dwChannel == 1) || (dwChannel >= 12))
   {
      temp_vol = (unsigned int)(((( channel[dwChannel]-g_adc_oe_efuse+512)) *(1500) / (4096+g_adc_ge_efuse-512))); // unit mV
      printk("[kernel_auxadc] adc_raw = %d, temp_vol = %d, g_adc_ge_efuse=%d, g_adc_oe_efuse=%d\n",channel[dwChannel],temp_vol,g_adc_ge_efuse,g_adc_oe_efuse);

      /* value = XX.XXX : the voltage calculate down to three places of decimals */
      data[0] = ((temp_vol) / 1000);
      data[1] = ((temp_vol) % 1000);
      printk("[kernel_auxadc] voltage = %d.%d\n",data[0],data[1]);
   }
   else
   {
      data[0] = (channel[dwChannel] * 10 * 150 / AUXADC_PRECISE / 1000);
      data[1] = ((channel[dwChannel] * 10 * 150 / AUXADC_PRECISE) % 1000);
   }
#else
...
static int IMM_auxadc_GetOneChannelValue_Cali(int Channel, int*voltage)
{
     ...
     if (g_adc_cali_en) {
        kal_int32 adc_gain = 1*1000 + g_adc_ge;
        //printk("IMM_auxadc_GetOneChannelValue_Cali, rawvalue=%d, oe=%d(0x%x), gain=%d(0x%x)\n", rawvalue, g_adc_oe, g_adc_oe, adc_gain, adc_gain);
        temp_vol = (u_int64_t)(rawvalue - g_adc_oe)*1500000/AUXADC_PRECISE;
        *voltage = (kal_int32)temp_vol*1000/adc_gain;
     }
     else {
...
{
#if defined (TARGET_MT6732_C90)
    kal_uint32 adc_cali_en = 0;
    kal_int32 temp = 0;

    g_adc_ge_efuse = 512;
    g_adc_oe_efuse = 512;

    adc_cali_en = ((DRV_Reg32(AUXADC_CALI_EN_A) & AUXADC_CALI_EN_A_MASK) << (AUXADC_CALI_EN_A_SHIFT));

    if (adc_cali_en) { /* do calibration */
        g_adc_cali_en = 1;

        g_adc_ge_efuse = (DRV_Reg32(AUXADC_GE_A) >> AUXADC_GE_A_SHIFT) & (AUXADC_GE_A_MASK);
        g_adc_ge = (g_adc_ge_efuse-512)*1000/4096;
        g_adc_oe_efuse = (DRV_Reg32(AUXADC_OE_A) >> AUXADC_OE_A_SHIFT) & (AUXADC_OE_A_MASK);
        g_adc_oe = g_adc_oe_efuse - 512;

    }
    printk("[kernel_auxadc]CALI_EN=%d, g_adc_ge_efuse=%d,    g_adc_oe_efuse=%d\n",adc_cali_en,g_adc_ge_efuse,   g_adc_oe_efuse);
#else
