...
static int mt_fh_hal_sync_ncpo_to_FHCTL_DDS(enum FH_PLL_ID pll_id)
{
	if (pll_id == MT658X_FH_ARM_PLL) {	/* FHCTL0 */
		...
		fh_write32(REG_FHCTL2_DDS,
			   ((DRV_Reg32(DDRPHY_BASE + 0x624) >> 11) & 0x1FFFFF) | (1U << 31));
	} else if (pll_id == MT658X_FH_MSDC_PLL) {	/* FHCTL3 */
...
static int mt_fh_hal_dvfs(enum FH_PLL_ID pll_id, unsigned int dds_value)
{
	...
	if (pll_id == MT658X_FH_ARM_PLL) {	/* FHCTL0 */
		...
		fh_write32((DDRPHY_BASE + 0x624),
			   ...
			   ((DRV_Reg32(DDRPHY_BASE + 0x624)) & 0x7FF));
...
{				/* mmpll dfs mode */
	...
	if (g_fh_pll[MT658X_FH_MM_PLL].fh_status == FH_FH_ENABLE_SSC) {	/* only when SSC is enable */
		/* Turn off MEMPLL hopping */
	...
	if (g_fh_pll[MT658X_FH_MM_PLL].fh_status == FH_FH_ENABLE_SSC) {
		...
		fh_write32(REG_FHCTL4_UPDNLMT,
			   (PERCENT_TO_DDSLMT
			    ((fh_read32(REG_FHCTL4_DDS) & 0x1FFFFF),
			     mt_ssc_mmpll_setting[2].lowbnd) << 16));
...
{				/* vencpll dfs mode */
	...
	if (g_fh_pll[MT658X_FH_VENC_PLL].fh_status == FH_FH_ENABLE_SSC) {	/* only when SSC is enable */
		/* Turn off MEMPLL hopping */
	...
	if (g_fh_pll[MT658X_FH_VENC_PLL].fh_status == FH_FH_ENABLE_SSC) {
		...
		fh_write32(REG_FHCTL5_UPDNLMT,
			   (PERCENT_TO_DDSLMT
			    ((fh_read32(REG_FHCTL5_DDS) & 0x1FFFFF),
			     mt_ssc_vencpll_setting[2].lowbnd) << 16));
			...
			fh_write32(REG_FHCTL2_UPDNLMT,
				   (PERCENT_TO_DDSLMT
				    ((fh_read32(REG_FHCTL2_DDS) & 0x1FFFFF),
				     mt_ssc_mempll_setting[5].lowbnd) << 16));
		} else {	/* LPDDR -8% */
...
static int freqhopping_dvfs_proc_read(struct seq_file *m, void *v)
{
	...
	for (i = 0; i < MT_FHPLL_MAX; i++) {
		seq_printf(m, "FHCTL%d:   CFG:0x%08x    DVFS:0x%08x\r\n", i,
			   ...
			   DRV_Reg32(REG_FHCTL0_DVFS + (i * 0x14)));
	}
...
/* struct          freqhopping_ioctl fh_ctl; */
	...
	temp = sscanf(kbuf, "%d %d %d %d %d", &p1, &p2, &p3, &p4, &p5);
...
static int freqhopping_dumpregs_proc_read(struct seq_file *m, void *v)
{
	...
	seq_printf(m,
		   ...
		   DRV_Reg32(MMPLL_CON0), DRV_Reg32(VENCPLL_CON0));

	seq_printf(m,
		   "\r\nPLL_CON1 :\r\nARM:0x%08x MAIN:0x%08x MSDC:0x%08x MM:0x%08x VENC:0x%08x\r\n",
		   DRV_Reg32(ARMPLL_CON1), DRV_Reg32(MAINPLL_CON1), DRV_Reg32(MSDCPLL_CON1),
		   DRV_Reg32(MMPLL_CON1), DRV_Reg32(VENCPLL_CON1));
	...
	seq_printf(m,
		   ...
		   DRV_Reg32(DDRPHY_BASE + 0x634), DRV_Reg32(DDRPHY_BASE + 0x638),
		   DRV_Reg32(DDRPHY_BASE + 0x63C), DRV_Reg32(DDRPHY_BASE + 0x640));
