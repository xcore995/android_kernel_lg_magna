...
#if !defined(CONFIG_MTK_LEGACY) && defined(CONFIG_ARCH_MT6735)
extern int dpidle_enter(int cpu);
extern int soidle_enter(int cpu);
extern int slidle_enter(int cpu);
extern int rgidle_enter(int cpu);
extern int mt_idle_select(int cpu);
extern void mt_cpuidle_framework_init(void);

extern void faudintbus_pll2sq(void);
extern void faudintbus_sq2pll(void);
...
enum cg_clk_id {  /* FIXME: temporaraily copy from mt_clkmgr.h. The following is CODA name */
...
#endif /* !defined(CONFIG_MTK_LEGACY) && defined(CONFIG_ARCH_MT6735) */
#endif
