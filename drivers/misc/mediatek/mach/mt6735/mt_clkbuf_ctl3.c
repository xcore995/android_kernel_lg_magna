...
static void clk_buf_Send_BSI_CW(CLK_BUF_SWCTRL_STATUS_T *status)
{
	...
	clk_buf_spm_cfg[BSI_D2_SR] &= ~(1 << clk_buf_bit_ctrl);
/*
	clk_buf_dbg("BSI_EN_SR=0x%x, BSI_CLK_SR=0x%x, BSI_D0_SR=0x%x, BSI_D1_SR=0x%x, BSI_D2_SR=0x%x\n",
				clk_buf_spm_cfg[BSI_EN_SR],
				clk_buf_spm_cfg[BSI_CLK_SR],
				clk_buf_spm_cfg[BSI_D0_SR],
				clk_buf_spm_cfg[BSI_D1_SR],
				clk_buf_spm_cfg[BSI_D2_SR]);
*/
...
bool clk_buf_ctrl(enum clk_buf_id id, bool onoff)
{
	...
	if (node) {
		...
		clk_buf_err("%s can NOT find compatible node\n", __func__);
		BUG();
	}
#endif
...
#if 0
	/* for bring-up */
	clk_buf_dbg("clk_buf_ctrl is disabled for bring-up\n");
	return false;
#endif
