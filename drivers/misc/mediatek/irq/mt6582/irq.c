...
static void fiq_isr(struct fiq_glue_handler *h, void *regs, void *svc_sp)
{
    ...
    volatile unsigned int spmISR = 0;
#endif
...
#ifdef CONFIG_TRUSTONIC_TEE_SUPPORT
    spmISR = spm_read(SPM_SLEEP_ISR_STATUS);
    if (spmISR &  SPM_ACTIVE_STATUS /*32*/) {
      spm_write(SPM_PCM_SW_INT_CLEAR, PCM_SW_INT_ALL);
      (irqs_to_fiq[5].handler)(irqs_to_fiq[5].arg, regs, svc_sp);
    }
    else
    {
      (irqs_to_fiq[7].handler)(irqs_to_fiq[7].arg, regs, svc_sp);
    }
#else
    if (irq >= NR_IRQS) {
        return;
