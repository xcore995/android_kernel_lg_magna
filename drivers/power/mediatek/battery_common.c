...
#endif
#ifdef CONFIG_LGE_PM_HIDDEN_MENU_TIMER_DISABLE
				...
				enum power_supply_property psp, union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_SAFETY_TIMER :
		 if(BMT_status.is_hm_timer_enabled)
		    val->intval =1;
		 else
		    val->intval =0;
		break;
				...
				const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_SAFETY_TIMER :
			if(val->intval)
			BMT_status.is_hm_timer_enabled =1;
			else
			 BMT_status.is_hm_timer_enabled =0;
			break;
#endif
	default:
		rc = -EINVAL;
...
static void battery_update(struct battery_data *bat_data)
{
	...
	if (adjust_power != -1) {
        ...
        battery_log(BAT_LOG_FULL, "battery_update.\n");
...
void mt_battery_GetBatteryData(void)
{
	...
	battery_log(BAT_LOG_CRTI,
			    "AvgVbat=(%d),bat_vol=(%d),AvgI=(%d),I=(%d),VChr=(%d),AvgT=(%d),T=(%d),pre_SOC=(%d),SOC=(%d),ZCV=(%d), type=(%d)\n",
			    BMT_status.bat_vol, bat_vol, BMT_status.ICharging, ICharging,
			    BMT_status.charger_vol, BMT_status.temperature, temperature,
			    previous_SOC, BMT_status.SOC, BMT_status.ZCV, BMT_status.charger_type);
}
...
#ifdef CONFIG_LGE_PM_HIDDEN_MENU_TIMER_DISABLE
	else if(!BMT_status.is_hm_timer_enabled){
		battery_log(BAT_LOG_CRTI, "[LGE_PM] Disable safety timer \n");
...
#ifdef CONFIG_LGE_PM_HIDDEN_MENU_TIMER_DISABLE
	BMT_status.is_hm_timer_enabled = KAL_TRUE;
#endif
