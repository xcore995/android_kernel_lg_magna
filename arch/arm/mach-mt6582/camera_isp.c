...
#define LOG_DBG(format, args...)    pr_info(MyTag "[%s]" format, __FUNCTION__,  ##args)
#define LOG_INF(format, args...)    pr_info(MyTag "[%s]" format, __FUNCTION__,  ##args)
...
static MINT32 ISP_open(struct inode *pInode, struct file *pFile)
{
	...
	if (g_log_def_constraint < 200) {
		set_detect_count(200);
		LOG_DBG("default constraint lines: %d, update to 200", g_log_def_constraint);
	} else {
		LOG_DBG("keep default constraint lines: %d", g_log_def_constraint);
	}
	#endif
