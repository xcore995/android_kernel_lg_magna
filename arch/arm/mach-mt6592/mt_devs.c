...
u64 trusty_reserved_mem_start = 0x0;	/* trusty_reserved_mem_start is inited in mt_fixup */
u64 trusty_reserved_mem_sz = 0x0;	/* trusty_reserved_mem_sz is inited in mt_fixup */
#endif
...
static int __init parse_tag_masp_data_fixup(const struct tag *tags)
{
	int i;
	
	g_rom_info_sbc_attr = tags->u.masp_data.rom_info_sbc_attr;
	g_rom_info_sdl_attr = tags->u.masp_data.rom_info_sdl_attr;
	g_hw_sbcen = tags->u.masp_data.hw_sbcen;
	g_lock_state = tags->u.masp_data.lock_state;
	
	for (i = 0; i < NUM_RID; i++)
		g_random_id[i] = tags->u.masp_data.rid[i];
	for (i = 0; i < NUM_CRYPTO_SEED; i++)
		g_crypto_seed[i] = tags->u.masp_data.crypto_seed[i];
	for (i = 0; i < NUM_SBC_PUBK_HASH; i++)
		g_sbc_pubk_hash[i] = tags->u.masp_data.sbc_pubk_hash[i];

	printk("tag_masp_data0, rid %x %x %x %x\n",g_random_id[0] ,g_random_id[1], g_random_id[2], g_random_id[3]);
...
void mt_fixup(struct tag *tags, char **cmdline, struct meminfo *mi)
{
    ...
    for (; tags->hdr.size; tags = tag_next(tags)) {
		...
		if (mem_desc && mem_desc->size) {
			...
			trusty_reserved_mem_start = mem_desc->start;
			trusty_reserved_mem_sz = mem_desc->size;
		}
	}
#endif
        ...
        }else if (tags->hdr.tag == ATAG_MASP_DATA){
            parse_tag_masp_data_fixup(tags);
        }
...
/* Laser sensor                                                        */
/*=======================================================================*/
#ifdef CONFIG_MTK_LASER_SENSOR_SUPPORT
...
/* add backlight device as charge pump*/
#if defined (CONFIG_BACKLIGHT_LM3639) || defined (CONFIG_BACKLIGHT_LM3632)
...
{
#if defined(CONFIG_TRUSTY)
	return kernel_mem_sz + RESERVED_MEM_MODEM + trusty_reserved_mem_sz;
#else
...
#if defined(CONFIG_TRUSTY)
	memblock_reserve(trusty_reserved_mem_start, trusty_reserved_mem_sz);
#endif
    /* 
