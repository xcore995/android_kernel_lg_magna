...
#define MyTag "ISP"
#define LOG_VRB(format, args...)    pr_debug(MyTag "[%s]" format, __FUNCTION__, ##args)
#ifdef ISP_DEBUG
#define LOG_DBG(format, args...)    pr_debug(MyTag "[%s]" format, __FUNCTION__,  ##args)
#else
#define LOG_DBG(format, args...)
#endif
#define LOG_INF(format, args...)    pr_debug(MyTag "[%s]" format, __FUNCTION__,  ##args)
#define LOG_NOTICE(format, args...) pr_notice(MyTag "[%s]" format, __FUNCTION__,  ##args)
#define LOG_WRN(format, args...)    pr_warn(MyTag "[%s]" format, __FUNCTION__,  ##args)
#define LOG_ERR(format, args...)    pr_err(MyTag "[%s]" format, __FUNCTION__,  ##args)
#define LOG_AST(format, args...)    pr_alert(MyTag "[%s]" format, __FUNCTION__,  ##args)
...
static MINT32 ISP_DumpReg(MVOID)
{
    ...
    {
        LOG_ERR("0x%08X %08X ", ISP_ADDR + i, ISP_RD32((void *)(ISP_ADDR + i)));
    }
    ...
    //CDP
    LOG_ERR("0x%08X %08X ", ISP_ADDR + 0xCC0, ISP_RD32((void *)(ISP_ADDR + 0xCC0)));
    LOG_ERR("0x%08X %08X ", ISP_ADDR + 0xD40, ISP_RD32((void *)(ISP_ADDR + 0xD40)));
    ...
    LOG_ERR("0x%08X %08X ", ISP_ADDR_CAMINF, ISP_RD32((void *)ISP_ADDR_CAMINF));
    LOG_ERR("0x%08X %08X ", ISP_ADDR + 0x150, ISP_RD32((void *)(ISP_ADDR + 0x150)));
...
static MINT32 ISP_SOF_Buf_Get(unsigned long long sec,unsigned long usec)
{
    ...
    if(g_IspInfo.DebugMask & ISP_DBG_BUF_CTRL)
    {
        LOG_DBG("[rtbc]dropCnt(%d)",(unsigned int)g_pstRTBuf->dropCnt);
    }
    ...
    if(IrqStatus[ISP_IRQ_TYPE_INT] & ISP_IRQ_INT_STATUS_TG1_ERR_ST)
    {
        ...
        if(senifIntStA & 0x01)
        {
            ...
            if(regVal & 0x01)
            {
                ...
                LOG_ERR("sensor overrun,4024(0x%08x),8018(0x%08x)",IrqStatus[ISP_IRQ_TYPE_INT],senifIntStA);

                LOG_ERR("8014(0x%08x),8018(0x%08x)",ISP_RD32((void *)(ISP_ADDR + 0x4014)),ISP_RD32((void *)(ISP_ADDR + 0x4018)));
                LOG_ERR("4004(0x%08x),4008(0x%08x)",ISP_RD32((void *)(ISP_ADDR + 0x4)),ISP_RD32((void *)(ISP_ADDR + 0x8)));
                LOG_ERR("400C(0x%08x),4414(0x%08x)",ISP_RD32((void *)(ISP_ADDR + 0xC)),ISP_RD32((void *)(ISP_ADDR + 0x414)));
                LOG_ERR("4020(0x%08x),4024(0x%08x)",ISP_RD32((void *)(ISP_ADDR + 0x20)),ISP_RD32((void *)(ISP_ADDR + 0x24)));   // 0x4024 is read clear
                LOG_ERR("43a8(0x%08x)",ISP_RD32((void *)(ISP_ADDR + 0x3a8)));
                LOG_ERR("43b0(0x%08x)",ISP_RD32((void *)(ISP_ADDR + 0x3b0)));
                LOG_ERR("43b8(0x%08x)",ISP_RD32((void *)(ISP_ADDR + 0x3b8)));
                LOG_ERR("43c4(0x%08x)",ISP_RD32((void *)(ISP_ADDR + 0x3c4)));
                LOG_ERR("43c8(0x%08x)",ISP_RD32((void *)(ISP_ADDR + 0x3c8)));
                LOG_ERR("43cc(0x%08x)",ISP_RD32((void *)(ISP_ADDR + 0x3cc)));
                LOG_ERR("43d0(0x%08x)",ISP_RD32((void *)(ISP_ADDR + 0x3d0)));
                LOG_ERR("43d4(0x%08x)",ISP_RD32((void *)(ISP_ADDR + 0x3d4)));
                LOG_ERR("4040(0x%08x)",ISP_RD32((void *)(ISP_ADDR + 0x040)));
                LOG_ERR("4044(0x%08x)",ISP_RD32((void *)(ISP_ADDR + 0x044)));
...
static MINT32 mmap_kmem(struct file *filp, struct vm_area_struct *vma)
{
    ...
    LOG_INF("Vma->vm_pgoff(0x%x),Vma->vm_start(0x%x),Vma->vm_end(0x%x),length(0x%x)",\
			(unsigned int)vma->vm_pgoff,(unsigned int)vma->vm_start,(unsigned int)vma->vm_end,length);
...
static MINT32 ISP_mmap(struct file *pFile, struct vm_area_struct *pVma)
{
    ...
    else
    {
		...
		LOG_INF("pVma->vm_pgoff(0x%x),phy(0x%x),pVmapVma->vm_start(0x%x),pVma->vm_end(0x%x),length(0x%x)",\
			(unsigned int)pVma->vm_pgoff,(unsigned int)pVma->vm_pgoff<<PAGE_SHIFT,(unsigned int)pVma->vm_start,(unsigned int)pVma->vm_end,length);
